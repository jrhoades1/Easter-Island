"""I's leftover n=4 remaining remaining-after-090-076 remaining-after-430-076 exactly 2 share 076 020.

Cycle 333 text-search lock. Uses already-vendored A–V and the
...
Leftover n=4 remaining remaining-after-090-076 = N=11 leftover
4-grams that do not contain 090 076. Cycle 328 LOSE unique most
frequent 2-gram (5-way tie at K=2, G=430 076 largest-id). Cycle
329 HOLD: exactly 2 of the N=11 contain contiguous 430 076,
matching leftovers 430 076 001 076 and 430 076 049 400.
N_remaining after peeling G=9. Cycle 330 LOSE: 2-gram 430 076
is not I-only (N_I=30, N_off_I=16 including T at Ta4[19]/Ta8[2],
extra I=26). Cycle 331 HOLD: the two leftover 4-grams that
contain 430 076 are all I-only (430 076 001 076 N_I=2 N_off_I=0
at Ia10[103]/Ia13[156]; 430 076 049 400 N_I=2 N_off_I=0 at
Ia13[170]/Ia14[63]; extra I of the 4-grams=0, not hapax). Nested
cycle 330 2-gram leak stays nested. Extra-I peel of those
4-grams is closed (extra I=0). Cycle 332 LOSE unique most
frequent 2-gram among remaining-after-430-076 (N=9 leftover
4-grams that do not contain 090 076 and do not contain
contiguous 430 076). unique_max false, 4-way tie at K=2:
076 020 / 076 010 / 020 010 / 010 079. G=076 020 largest-id
labeling only. Matching leftovers of labeled G: 076 020 010 050,
053 076 020 010. N_remaining after peeling G=7. This cycle's
N=9 population is leftover n=4 remaining remaining-after-090-076
remaining-after-430-076. This cycle does not re-lock unique_max.

Already locked (do not re-lock): cycle 332, 331, 330, 329, 328,
222, leftover n=4 remaining I 090 076 peels (288–327), leftover
extra 090 076 peels, cycles 220–221, or cycle 223. Do not claim
2-gram 076 020 I-only or 4-gram I-only this cycle.

This cycle is leftover-inventory, not I-only of G. HOLD iff
exactly K=2 of the N=9 remaining-after-430-076 leftover 4-grams
contain contiguous 2-gram 076 020, those two are 076 020 010 050
and 053 076 020 010, and N_remaining after peeling G=7. LOSE if
K≠2, the matching set differs, or N_remaining is not 7. Record
N=9, K, G=076 020, both matching 4-grams, N_remaining after
peeling G (=7). Nested cycle 332 unique_max false stays nested.

Analog: cycle 329 leftover n=4 remaining remaining-after-090-076
exactly 2 share 430 076 HOLD after cycle 328 unique-max LOSE.

Measured: N=9, K=2, G=076 020, matching leftovers
076 020 010 050 and 053 076 020 010, N_remaining=7.
unique_max stays false (nested cycle 332). Claim that can
lose:
...
exception present, grandparent remaining=16, exactly 5 contain
090 076, N=11 remaining-after-090-076, exactly 2 contain
430 076, N=9 remaining-after-430-076, exactly K=2 contain
076 020, the matching set is those two 4-grams, and
N_remaining after peeling G=7. The claim is true. Do not
retune.
...
from tests.test_mamari_i_leftover_n4_remaining_after_090_076_remaining_after_430_076_next_2gram_scoreboard import (
    ...
    leftover_remaining_after_090_076_remaining_after_430_076_with_g,
    leftover_remaining_after_090_076_remaining_after_430_076_without_g,
)
...
STANDING_N_REMAINING_GRANDPARENT = 16
STANDING_K_GRANDPARENT = 5
STANDING_N_REMAINING_PARENT = 11
STANDING_K_PARENT = 2
STANDING_N = 9
GRAM2 = ("076", "020")
STANDING_G = GRAM2
STANDING_N2 = 2
STANDING_K = 2
STANDING_N_WITHOUT_G = 7
STANDING_N_REMAINING = 7
STANDING_N_DISTINCT = 23
STANDING_N_TIED_AT_K = 4
...
STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_EXACTLY_2_SHARE_076_020 = (
    True
)
STANDING_RESULT = (
...
def leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[str, ...], ...]:
    """Tokens of remaining-after-430-076 leftovers that contain G."""
    return tuple(
        gram
        for gram, _n, _f, _s in leftover_remaining_after_090_076_remaining_after_430_076_with_g(
...
def i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020(
    ...
    """True iff exactly K=2 of N=9 remaining-after-430-076 contain G and N_remaining=7."""
    if not leftover_n4_family_counts_hold(leftovers):
        return False
    remaining = leftover_remaining_n4(leftovers)
    if len(remaining) != expected_remaining_grandparent:
        return False
    if not i_leftover_n4_remaining_exactly_5_contain_090_076(leftovers):
        return False
    if len(leftover_remaining_with_g(remaining, peeled_090_076)) != expected_k_grandparent:
        return False
    remaining_after_090 = leftover_remaining_without_g(remaining, peeled_090_076)
    if len(remaining_after_090) != expected_remaining_parent:
        return False
    if not i_leftover_n4_remaining_after_090_076_exactly_2_share_430_076(leftovers):
        return False
    if len(leftover_remaining_with_g(remaining_after_090, peeled_430_076)) != expected_k_parent:
        return False
    ...
    if len(remaining_after) != expected_n:
        return False
    matching = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
    ...
    if not matching_set_is_labeled_g_pair(matching, expected_matching):
        return False
    without = leftover_remaining_after_090_076_remaining_after_430_076_without_g(
        remaining_after, expected_g
    )
    return len(without) == expected_n_remaining


STANDING_REMAINING_GRANDPARENT = leftover_remaining_n4(STANDING_LEFTOVER)
STANDING_REMAINING_PARENT = leftover_remaining_after_090_076(STANDING_LEFTOVER)
STANDING_REMAINING = leftover_remaining_after_090_076_remaining_after_430_076(
    STANDING_LEFTOVER
)
STANDING_WITH_ROWS = leftover_remaining_after_090_076_remaining_after_430_076_with_g(
    STANDING_REMAINING
)
STANDING_WITHOUT_ROWS = leftover_remaining_after_090_076_remaining_after_430_076_without_g(
    STANDING_REMAINING
)
STANDING_MATCHING_TOKENS = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
    STANDING_REMAINING
)
...
class TestILeftoverN4RemainingAfter090076RemainingAfter430076Exactly2076020Helpers(
    unittest.TestCase
):
    """Helpers on remaining-after-430-076 exactly 2 share 076 020. No CV, no LLM."""

    def test_exactly_2_can_fail_on_k_or_matching_set_or_n_remaining(self):
        """N=9 holds at K=2 matching pair N_remaining=7; empty, K≠2, and swapped set lose."""
        ...
        self.assertEqual(len(remaining), STANDING_N_REMAINING_GRANDPARENT)
        self.assertEqual(len(remaining), CYCLE222_N_REMAINING)
        remaining_parent = leftover_remaining_after_090_076(leftover)
        self.assertEqual(len(remaining_parent), STANDING_N_REMAINING_PARENT)
        self.assertEqual(len(remaining_parent), CYCLE329_N)
        remaining_after = leftover_remaining_after_090_076_remaining_after_430_076(
            leftover
        )
        self.assertEqual(remaining_after, STANDING_REMAINING)
        self.assertEqual(len(remaining_after), STANDING_N)
        self.assertEqual(len(remaining_after), 9)
        self.assertEqual(GRAM2, ("076", "020"))
        self.assertEqual(LOCKED_G_090_076, ("090", "076"))
        self.assertEqual(LOCKED_G_430_076, ("430", "076"))
        self.assertEqual(
            contiguous_2grams(("076", "020", "010", "050")),
            (("076", "020"), ("020", "010"), ("010", "050")),
        )
        self.assertEqual(
            contiguous_2grams(("053", "076", "020", "010")),
            (("053", "076"), ("076", "020"), ("020", "010")),
        )
        self.assertTrue(leftover_is_remaining(("076", "020", "010", "050")))
        self.assertTrue(leftover_is_remaining(("053", "076", "020", "010")))
        ...
        self.assertFalse(
            is_contiguous_substring(LOCKED_G_090_076, ("076", "020", "010", "050"))
        )
        self.assertFalse(
            is_contiguous_substring(LOCKED_G_430_076, ("076", "020", "010", "050"))
        )
        self.assertTrue(
        ...
        self.assertTrue(is_contiguous_substring(GRAM2, ("076", "020", "010", "050")))
        self.assertTrue(is_contiguous_substring(GRAM2, ("053", "076", "020", "010")))
        ...
        with_gram = leftover_remaining_after_090_076_remaining_after_430_076_with_g(
            remaining_after
        )
        without_gram = leftover_remaining_after_090_076_remaining_after_430_076_without_g(
            remaining_after
        )
        matching = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
            remaining_after
        ...
        extra_after = leftover_remaining_after_090_076_remaining_after_430_076(extra_g)
        self.assertEqual(len(extra_after), STANDING_N)
        extra_matching = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
            extra_after
        )
        ...
        dropped = tuple(
            row for row in leftover if row[0] != ("076", "020", "010", "050")
        )
        self.assertFalse(
            i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020(
                dropped
        ...
        swapped_after = leftover_remaining_after_090_076_remaining_after_430_076(swapped)
        self.assertEqual(len(swapped_after), STANDING_N)
        swapped_matching = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
            swapped_after
        )
        ...
        self.assertEqual(STANDING_N_REMAINING, 7)
    ...
    def test_nested_cycle332_unique_max_stays_false(self):
        """Cycle 332 unique-max LOSE stays nested; exactly-K is this claim."""
        provider = MockProvider()
        leftover = leftover_n4_rows()
        remaining_after = leftover_remaining_after_090_076_remaining_after_430_076(
            leftover
        ...
        gram, count, unique = select_g(remaining_after)
        self.assertEqual(gram, CYCLE332_G)
        self.assertEqual(gram, STANDING_G)
        self.assertEqual(count, CYCLE332_K)
        self.assertEqual(count, STANDING_K)
        self.assertFalse(unique)
        ...
        self.assertEqual(CYCLE332_TIED, STANDING_TIED_2GRAMS)
        self.assertEqual(CYCLE332_MATCHING, STANDING_MATCHING_LEFTOVERS)
        self.assertEqual(CYCLE332_GRAM2, GRAM2)
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076Exactly2076020Scoreboard(
    unittest.TestCase
):
    """Cited-fixture remaining-after-430-076 exactly 2 share 076 020. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining_grandparent = leftover_remaining_n4(self.leftover)
        self.remaining_parent = leftover_remaining_after_090_076(self.leftover)
        self.remaining = leftover_remaining_after_090_076_remaining_after_430_076(
            self.leftover
        )
        self.with_gram = leftover_remaining_after_090_076_remaining_after_430_076_with_g(
            self.remaining
        )
        self.without_gram = leftover_remaining_after_090_076_remaining_after_430_076_without_g(
            self.remaining
        )
        self.matching = leftover_remaining_after_090_076_remaining_after_430_076_matching_set(
            self.remaining
        )
        self.claim_holds = (
            i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020(
    ...
    def test_nested_leftover_n4_family_and_cycles_222_328_329_332(self):
        """Nested leftover n=4 27/7/4/1, 222 16/5/11, 328/329 11/2/9, 332 9/2/7 stay."""
        ...
        self.assertEqual(len(self.remaining_grandparent), 16)
        self.assertTrue(
        ...
        self.assertEqual(
            tuple(
                ...
                for gram, _n, _f, _s in leftover_remaining_with_g(
                    self.remaining_parent, LOCKED_G_430_076
                )
            ),
            CYCLE329_MATCHING,
        )
        self.assertEqual(len(self.remaining_parent), 11)
        ...
        self.assertEqual(STANDING_K_GRANDPARENT + STANDING_N_REMAINING_PARENT, 16)
        self.assertFalse(CYCLE332_UNIQUE)
        self.assertFalse(CYCLE332_UNIQUE_MAX_CLAIM)
        self.assertEqual(CYCLE332_G, STANDING_G)
    ...
    def test_remaining_after_9_exactly_2_share_076_020_holds(self):
        """9 remaining-after-430-076; exactly 2 share 076 020. Claim is true."""
        self.assertEqual(len(self.remaining), STANDING_N)
        self.assertEqual(len(self.remaining), 9)
        ...
        self.assertEqual(STANDING_N_WITHOUT_G, 7)
        self.assertEqual(STANDING_N_REMAINING, 7)
        ...
        self.assertEqual(STANDING_N_DISTINCT, 23)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_per_remaining_after_only_the_expected_two_for_labeled_g(self):
        """Labeled G=076 020 matches two leftovers; peeled 090 076 / 430 076 do not count."""
    ...
    def test_leftover_sites_on_i(self):
        """Remaining-after-430-076 leftover I sites; Ib unpublished."""
        self.assertEqual(self.remaining, STANDING_REMAINING)
    ...
    def test_existing_332_331_330_329_328_223_222_and_220_scoreboards_still_compute(self):
        """Cycle 332 unique-max lose, 331 2/0, 330 30/16, 329 K=2, 328 unique-max lose stay."""
        leftover = leftover_n4_rows()
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-333 remaining-after-430-076 exactly-2 lock."""
        lock = self.survey[
            "i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly2_076_020"
        ]
        self.assertEqual(lock["cycle"], 333)
        ...
        self.assertEqual(lock["N_remaining_grandparent"], 16)
        self.assertEqual(lock["K_grandparent"], STANDING_K_GRANDPARENT)
        self.assertEqual(lock["K_grandparent"], 5)
        ...
        self.assertTrue(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020"]
        )
        self.assertEqual(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020"],
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_EXACTLY_2_SHARE_076_020,
        ...
        self.assertEqual(lock["nested_cycle332_N"], 9)
        self.assertEqual(lock["nested_cycle332_N_remaining"], 7)
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076Exactly2076020ImageSnapshot(
    unittest.TestCase
):
    """Cycle 333 does not touch clustering. 83/62 / Hamming 6 stays."""
