"""I's cycle-237 leftover extra remaining-after-700 forward-530 cluster lock.

Cycle 238 text-search lock. Uses already-vendored A–V and the
...
stem as an exact-K claim on remaining-after-700. Do not lock
I-only of 090 076 530. Do not lock unique-max
remaining-after-700. Do not lock the other five tied stems
(280/087/011/005/000). Do not lock previous 4-grams of
090 076 700. Off-I T sites are not this cycle. I-only of
...
Leftover extra remaining-after-700 = leftover extra I 090 076
sites with a next token whose next token is none of 070, 071,
013, 001, or 700. Nested-check leftover extra N_leftover==56,
...
G=001 K=3, leftover extra remaining-after-001 N_remaining4==33
exactly 2 share 700 (do not retune cycles 225–235).
Nested-check cycle 236 090 076 700 I-only 2/0 and cycle 237
two forward 4-grams 1/0 each (do not retune). Nested-check
N_remaining5==31 (33−2). Ia2[174] has next token 000 and is
remaining-after-700; it is not no-next.

Hypothesis K=2: leftover extra remaining-after-700 includes
exactly 2 sites that share next stem 530 (forward 3-gram
090 076 530). Measured: K=2 at Ia12[42], Ia14[9]; next
4-grams 090 076 530 090 / 090 076 530 499. Cycle 234 listed
530×2 among the 7-way tie; re-measured, not assumed. Claim
that can lose:
...
235 (remaining-after-001 exactly 2 share 700). This can lose
if remaining-after-700 does not contain exactly 2 530
next-stem sites, or if N_remaining5 ≠ 31. Nested cycle 237
2/0 hapax, cycle 236 2/0, cycle 235 K=2 / G=700, cycle 234
7-way tie at 2, cycle 233 3/0 hapax, cycle 232 3/0, cycle
223 69/3, cycle 195 6/0, and cycle 171 43/0 stay. Do not
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_001_fwd700_scoreboard import (
    ...
    leftover_extra_remaining_after_001_without_700,
...
STANDING_UNIQUE_MAX_REMAINING_AFTER_700_IS_NOT_THIS_CYCLE = True
STANDING_REMAINING5_SITES = (
    (SIDE_IA, "Ia1", 2),
    (SIDE_IA, "Ia1", 15),
    (SIDE_IA, "Ia1", 27),
    (SIDE_IA, "Ia1", 59),
...
STANDING_CYCLE234_530_SITES = CYCLE234_FREQUENCY[1][2]
STANDING_MATCHING_EQUALS_CYCLE234_530_SITES = True
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530"
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_700_EXACTLY_2_SHARE_530 = True
STANDING_RESULT = "i_leftover_extra_090_076_remaining_after_700_fwd530"
...
STANDING_I_ONLY_OF_090_076_530_IS_NOT_THIS_CYCLE = True
STANDING_PREVIOUS_4GRAMS_OF_090_076_700_ARE_NOT_THIS_CYCLE = True
...
def leftover_extra_remaining_after_700(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_700,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra with-next sites whose next token is none of 070/071/013/001/700."""
...
def leftover_extra_remaining_after_700_next_stems(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_700,
) -> tuple[str, ...]:
    """Next stems of leftover extra remaining-after-700 sites."""
    locked_set = set(locked)
    return tuple(
        nxt
...
def leftover_extra_remaining_after_700_with_530(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-700 sites whose next token is 530."""
    remaining5 = set(leftover_extra_remaining_after_700(sites, next_stems))
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
        if nxt == stem and site in remaining5
    )


def leftover_extra_remaining_after_700_without_530(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_700,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-700 sites whose next token is not 530."""
    locked_set = set(locked)
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_extra_remaining_after_700_nested_counts_hold(
    n_leftover: int,
    n_with_next: int,
    n_no_next: int,
    n_share_070: int,
    ...
    n_share_700: int,
    n_remaining5: int,
    expected_share_700: int = STANDING_N_SHARE_700,
    expected_remaining5: int = STANDING_N_REMAINING5,
) -> bool:
    """Nested leftover extra chain through remaining-after-700 N_remaining5==31."""
    return leftover_extra_remaining_after_001_nested_counts_hold(
        n_leftover,
        n_with_next,
        n_no_next,
        n_share_070,
        ...
        n_share_001,
        n_remaining4,
    ) and n_share_700 == expected_share_700 and n_remaining5 == expected_remaining5 and (
        n_remaining5 == n_remaining4 - n_share_700
    )


def matching_equals_cycle234_530_sites(
    matching_sites: tuple[tuple[str, str, int], ...],
    cycle234_sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE234_530_SITES,
) -> bool:
    """True iff remaining-after-700 090 076 530 sites equal cycle 234's 530 pair."""
    return matching_sites == cycle234_sites


def matching_leftover_extra_remaining_after_700_fwd530_local_4gram_rows(
    leftover_sites: tuple[tuple[str, str, int], ...] = STANDING_MATCHING_SITES,
    next_4grams: tuple[tuple[str, ...], ...] = STANDING_MATCHING_NEXT_4GRAMS,
) -> list[dict]:
    """Survey-shaped matching leftover extra remaining-after-700 530 next-4-gram rows."""
    rows = []
...
def i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(
    k: int,
...
class TestILeftoverExtra090076RemainingAfter700Fwd530Helpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-700 forward 530. No CV, no LLM."""

    def test_forward_530_requires_remaining_after_700_next_stem(self):
        """Next stem 530 is remaining-after-700; locked 070/071/013/001/700 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_FORWARD, ("090", "076", "530"))
        ...
        planted_stems = ("530", "700", "070", "071", "013", "001", None, "280")
        rem5 = leftover_extra_remaining_after_700(planted_sites, planted_stems)
        self.assertEqual(rem5, (planted_sites[0], planted_sites[7]))
        self.assertEqual(
            leftover_extra_remaining_after_700_with_530(planted_sites, planted_stems),
            (planted_sites[0],),
        )
        self.assertEqual(
            leftover_extra_remaining_after_700_without_530(planted_sites, planted_stems),
            (planted_sites[7],),
        )
        self.assertNotIn(planted_sites[1], rem5)
        self.assertNotIn(planted_sites[2], rem5)
        self.assertNotIn(planted_sites[3], rem5)
        self.assertNotIn(planted_sites[4], rem5)
        self.assertNotIn(planted_sites[5], rem5)
        self.assertNotIn(planted_sites[6], rem5)
    ...
    def test_exactly_2_can_fail(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(2))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(0))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(1))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(3))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(6))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(31))
        planted = STANDING_MATCHING_SITES + ((SIDE_IA, "Ia1", 2),)
        planted_stems = ("530",) * 3
        self.assertEqual(
            leftover_extra_remaining_after_700_with_530(planted, planted_stems),
            planted,
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(len(planted))
        )
        self.assertEqual(
            STANDING_CLAIM,
            "i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530",
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_700_EXACTLY_2_SHARE_530
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_700_EXACTLY_2_SHARE_530,
            HYPOTHESIS_K == STANDING_K,
        )
        self.assertEqual(STANDING_N_SHARE_700 + STANDING_N_REMAINING5, STANDING_N_REMAINING4)
        self.assertEqual(2 + 31, 33)
        self.assertEqual(STANDING_K + STANDING_N_WITHOUT, STANDING_N_REMAINING5)
        self.assertEqual(2 + 29, 31)
        self.assertEqual(provider.get_call_history(), [])

    def test_cycle234_530_site_agreement_and_tie_minus_700_can_fail(self):
        """Matching sites must equal cycle 234's Ia12[42]/Ia14[9]; unique-max stays unlocked."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle234_530_sites(STANDING_MATCHING_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_530_SITES)
        self.assertEqual(STANDING_MATCHING_SITES, STANDING_CYCLE234_530_SITES)
        planted = STANDING_MATCHING_SITES[:-1]
        self.assertFalse(matching_equals_cycle234_530_sites(planted))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(len(planted))
        )
        swapped = ((SIDE_IA, "Ia2", 159), (SIDE_IA, "Ia13", 143))
        self.assertFalse(matching_equals_cycle234_530_sites(swapped))
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(len(swapped))
        )
        self.assertEqual(swapped, CYCLE235_MATCHING_SITES)
        ...
        self.assertTrue(STANDING_UNIQUE_MAX_REMAINING_AFTER_700_IS_NOT_THIS_CYCLE)
        self.assertEqual(CYCLE234_N_TIED_AT_K, 7)
        self.assertEqual(CYCLE234_TIED_STEMS, STANDING_TIED_STEMS)
        self.assertEqual(
            tuple(stem for stem in CYCLE234_TIED_STEMS if stem != "700"),
            STANDING_TIED_STEMS_MINUS_700,
        )
        self.assertEqual(len(STANDING_TIED_STEMS_MINUS_700), STANDING_N_TIED_MINUS_700)
        self.assertEqual(STANDING_N_TIED_MINUS_700, 6)
        self.assertTrue(STANDING_OTHER_TIED_STEMS_NOT_LOCKED)
        self.assertTrue(STANDING_I_ONLY_OF_090_076_530_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_PREVIOUS_4GRAMS_OF_090_076_700_ARE_NOT_THIS_CYCLE)
...
class TestMamariILeftoverExtra090076RemainingAfter700Fwd530Scoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-700 forward-530 lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining5_stems = leftover_extra_remaining_after_700_next_stems(
            self.leftover_sites,
            self.next_stems,
        )
        self.matching = leftover_extra_remaining_after_700_with_530(
            self.leftover_sites,
            self.next_stems,
        )
        self.without = leftover_extra_remaining_after_700_without_530(
            self.leftover_sites,
            self.next_stems,
        )
        ...
        self.equals_cycle234_530 = matching_equals_cycle234_530_sites(self.matching)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(
            self.k,
        )

    def test_tokens_and_nested_leftover_extra_through_700_not_retuned(self):
        """2-gram and leftover extra 56/55/1 through remaining-after-001 33/2/700 stay."""
        self.assertEqual(GRAM2, ("090", "076"))
    ...
    def test_counts_2_of_31_and_hypothesis_k_2_holds(self):
        """N_remaining5=31, 7-way-tie-minus-700 stays, K=2 share 530. Claim holds."""
        ...
        if (
            ...
            self.fail(
                ...
                "remaining-after-001 33/2/700 drifted"
            )
        self.assertTrue(
        ...
        self.assertEqual(self.n_remaining5, CYCLE235_N_WITHOUT)
        self.assertEqual(self.n_remaining5, self.n_remaining4 - self.n_share_700)
        self.assertEqual(33 - 2, 31)
        if self.n_remaining5 != 31:
            self.fail("measured N_remaining5 drifted from 31")
        if self.n_remaining5 != self.n_remaining4 - self.n_share_700:
            self.fail("leftover extra remaining-after-700 filter disagrees with nested 33−2")
        self.assertEqual(self.remaining5, STANDING_REMAINING5_SITES)
        self.assertEqual(len(self.remaining5), len(self.remaining5_stems))
        self.assertEqual(
            self.remaining5,
            leftover_extra_remaining_after_001_without_700(
                self.leftover_sites,
        ...
        self.assertEqual(
            ...
            leftover_extra_remaining_after_001_without_g(
                self.leftover_sites,
                self.next_stems,
                LOCKED_FORWARD_STEM_700,
            ),
        )
        self.assertNotIn(STANDING_NO_NEXT_SITES[0], self.remaining5)
        self.assertIn(STANDING_IA2_174, self.remaining5)
        self.assertEqual(STANDING_IA2_174_NEXT_STEM, "000")
        for site in self.share_070:
            self.assertNotIn(site, self.remaining5)
        ...
        self.assertEqual(STANDING_N_TIED_MINUS_700, 6)
        if self.tied_minus_700 != STANDING_TIED_STEMS_MINUS_700:
            self.fail("nested cycle 234 7-way-tie-minus-700 drifted")
        ...
        self.assertEqual(self.k + self.n_without, self.n_remaining5)
        self.assertEqual(2 + 29, 31)
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530(self.k)
        )
        self.assertEqual(
            self.claim_holds,
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_700_EXACTLY_2_SHARE_530,
        ...
        self.assertTrue(self.equals_cycle234_530)
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_530_SITES)
        self.assertTrue(matching_equals_cycle234_530_sites(self.matching))
        if len(self.matching) != 2 or not self.equals_cycle234_530:
            self.fail("leftover extra remaining-after-700 530 set drifted from cycle-234 pair")
    ...
    def test_matching_leftover_extra_remaining_after_700_sites_share_530(self):
        """Two leftover extra remaining-after-700 sites are 090 076 530."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_next_4grams, STANDING_MATCHING_NEXT_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia12", 42), ("090", "076", "530", "090")),
            ((SIDE_IA, "Ia14", 9), ("090", "076", "530", "499")),
        )
        for (site, nxt), (want_site, want_nxt) in zip(
            ...
            self.assertEqual(stems[index + STANDING_N2], "530")
            self.assertEqual(site_next_stem(stems, index, GRAM2), "530")
            ...
            self.assertIn(site, STANDING_REMAINING5_SITES)
        ...
        self.assertEqual(self.matching, STANDING_CYCLE234_530_SITES)
        self.assertTrue(matching_equals_cycle234_530_sites(self.matching))
        ...
        other_tied = tuple(
            stem for stem in STANDING_TIED_STEMS_MINUS_700 if stem != "530"
        )
        self.assertEqual(other_tied, CYCLE235_OTHER_TIED_STEMS[1:])
        self.assertEqual(other_tied, ("280", "087", "011", "005", "000"))
        for stem in other_tied:
            ...
            for site in other:
                self.assertNotIn(site, self.matching)
                self.assertIn(site, self.remaining5)
    ...
    def test_existing_237_236_235_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 237 2/0 hapax, 236 2/0, 235 K=2/G=700, 234 7-way tie, 233 3/0, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
        ...
        self.assertEqual(
            tuple(stem for stem in tied if stem != "700"),
            STANDING_TIED_STEMS_MINUS_700,
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-238 leftover extra remaining-after-700 fwd530 lock."""
        lock = self.survey["i_leftover_extra_090_076_remaining_after_700_fwd530"]
        self.assertEqual(lock["cycle"], 238)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_700_sites"]),
            STANDING_MATCHING_SITES,
        )
        self.assertTrue(lock["matching_equals_cycle234_530_sites"])
        self.assertEqual(
            lock["matching_equals_cycle234_530_sites"],
            STANDING_MATCHING_EQUALS_CYCLE234_530_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lock["matching_leftover_extra_remaining_after_700_local_4grams"],
            matching_leftover_extra_remaining_after_700_fwd530_local_4gram_rows(),
        )
        ...
        self.assertTrue(lock["i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530"])
        self.assertEqual(
            lock["i_leftover_extra_090_076_remaining_after_700_exactly_2_share_530"],
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_700_EXACTLY_2_SHARE_530,
        )
        ...
        self.assertTrue(lock["i_only_of_090_076_530_is_not_this_cycle"])
        self.assertTrue(lock["previous_4grams_of_090_076_700_are_not_this_cycle"])
        self.assertTrue(lock["unique_max_remaining_after_700_is_not_this_cycle"])
...
class TestMamariILeftoverExtra090076RemainingAfter700Fwd530ImageSnapshot(
    unittest.TestCase
):
    """Cycle 238 does not touch clustering. 83/62 / Hamming 6 stays."""
