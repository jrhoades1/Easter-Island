"""I's cycle-142 leftover overlap 3-gram forward 4-gram lock.

Cycle 143 text-search lock. Uses already-vendored A–V and the
...
For each leftover site, the forward 4-gram 076 010 079 X if
the line has a next stem after the 3-gram; end-of-line is
no-forward. Hypothesis: all three leftover sites share one
forward 4-gram (one stem X). Measured: N_leftover=3,
N_with_forward=3, N_distinct_forward_4grams=3
(076 010 079 090 at Ia5[139], 076 010 079 029 at Ia12[34],
076 010 079 053 at Ia14[83]). Claim that can lose:
i_leftover_076_010_079_share_one_forward_4gram. True only
if N_with_forward=3 and N_distinct_forward_4grams=1. The
...
STANDING_N3 = 3
STANDING_N4 = 4
STANDING_N_WITH_FORWARD = 3
STANDING_N_DISTINCT_FORWARD_4GRAMS = 3
STANDING_N_NO_FORWARD = 0
STANDING_FORWARD_4GRAM_090 = ("076", "010", "079", "090")
STANDING_FORWARD_4GRAM_029 = ("076", "010", "079", "029")
STANDING_FORWARD_4GRAM_053 = ("076", "010", "079", "053")
STANDING_DISTINCT_FORWARD_4GRAMS = (
    STANDING_FORWARD_4GRAM_090,
    STANDING_FORWARD_4GRAM_029,
    STANDING_FORWARD_4GRAM_053,
)
...
STANDING_SITES_PER_FORWARD_4GRAM = (
    (STANDING_FORWARD_4GRAM_090, ((SIDE_IA, "Ia5", 139),)),
    (STANDING_FORWARD_4GRAM_029, ((SIDE_IA, "Ia12", 34),)),
    (STANDING_FORWARD_4GRAM_053, ((SIDE_IA, "Ia14", 83),)),
)
STANDING_NO_FORWARD_SITES = ()
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_leftover_076_010_079_share_one_forward_4gram"
STANDING_I_LEFTOVER_076_010_079_SHARE_ONE_FORWARD_4GRAM = False
STANDING_RESULT = "i_leftover_076_010_079_forward_4gram"
...
def site_forward_4gram(
    ...
    """076 010 079 X if a next stem exists; None at end-of-line."""
    n3 = len(gram3)
...
def leftover_forward_4grams(
    i_sides: dict[str, list[list[str]]],
    sites: tuple[tuple[str, str, int], ...] = STANDING_LEFTOVER_SITES,
    gram3: tuple[str, ...] = GRAM3,
) -> tuple[tuple[str, ...] | None, ...]:
    """Per-site forward 4-gram or None for the locked leftover sites."""
    return tuple(
...
def sites_with_forward(
    ...
    """Leftover sites that have a next stem after the 3-gram."""
    return tuple(
        site
...
def group_sites_by_forward_4gram(
    sites: tuple[tuple[str, str, int], ...],
    forwards: tuple[tuple[str, ...] | None, ...],
) -> tuple[tuple[tuple[str, ...], tuple[tuple[str, str, int], ...]], ...]:
    """Distinct forward 4-grams in first-seen order, with their sites."""
...
def i_leftover_076_010_079_share_one_forward_4gram(
    n_with_forward: int,
    n_distinct_forward_4grams: int,
) -> bool:
    """True iff N_with_forward=3 and N_distinct_forward_4grams=1."""
    return n_with_forward == 3 and n_distinct_forward_4grams == 1


class TestILeftover076010079Forward4gramHelpers(unittest.TestCase):
    ...
    def test_forward_requires_next_stem_after_3gram(self):
        ...
        has_next = ["200", "076", "010", "079", "090", "076"]
        self.assertEqual(site_forward_4gram(has_next, 1), ("076", "010", "079", "090"))
        end_of_line = ["200", "076", "010", "079"]
        self.assertIsNone(site_forward_4gram(end_of_line, 1))
        self.assertIsNone(site_forward_4gram(["076", "010", "079"], 0))
        mismatch = ["076", "010", "006", "090"]
        self.assertIsNone(site_forward_4gram(mismatch, 0))
        gapped = ["076", "011", "079", "090"]
        self.assertIsNone(site_forward_4gram(gapped, 0))
        self.assertEqual(provider.get_call_history(), [])

    def test_share_one_requires_three_forwards_and_one_distinct(self):
        """Boolean is True only when N_with_forward=3 and N_distinct=1."""
        provider = MockProvider()
        self.assertTrue(i_leftover_076_010_079_share_one_forward_4gram(3, 1))
        self.assertFalse(i_leftover_076_010_079_share_one_forward_4gram(3, 3))
        self.assertFalse(i_leftover_076_010_079_share_one_forward_4gram(3, 2))
        self.assertFalse(i_leftover_076_010_079_share_one_forward_4gram(2, 1))
        self.assertFalse(i_leftover_076_010_079_share_one_forward_4gram(0, 0))
        self.assertFalse(i_leftover_076_010_079_share_one_forward_4gram(1, 1))
        self.assertEqual(STANDING_CLAIM, "i_leftover_076_010_079_share_one_forward_4gram")
        self.assertFalse(STANDING_I_LEFTOVER_076_010_079_SHARE_ONE_FORWARD_4GRAM)
        self.assertNotEqual(
            STANDING_I_LEFTOVER_076_010_079_SHARE_ONE_FORWARD_4GRAM,
    ...
    def test_grouping_keeps_first_seen_order_and_skips_none(self):
        ...
        self.assertEqual(
            group_sites_by_forward_4gram(sites, three_distinct),
            STANDING_SITES_PER_FORWARD_4GRAM,
        )
        shared = (
            STANDING_FORWARD_4GRAM_090,
        ...
        self.assertEqual(
            ...
            ((STANDING_FORWARD_4GRAM_090, sites),),
        )
        one_none = (STANDING_FORWARD_4GRAM_090, None, STANDING_FORWARD_4GRAM_090)
        self.assertEqual(
        ...
        self.assertEqual(sites_with_forward(sites, one_none), (sites[0], sites[2]))
        self.assertEqual(sites_without_forward(sites, one_none), (sites[1],))
        self.assertEqual(provider.get_call_history(), [])


class TestMamariILeftover076010079Forward4gramScoreboard(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.forwards = leftover_forward_4grams(self.i_sides, self.leftover_sites)
        ...
        self.claim_holds = i_leftover_076_010_079_share_one_forward_4gram(
            self.n_with_forward,
            self.n_distinct,
        )
    ...
    def test_three_leftovers_have_three_distinct_forwards_and_claim_loses(self):
        """N_leftover=3, N_with_forward=3, N_distinct=3. Claim loses."""
        ...
        self.assertEqual(STANDING_N_WITH_FORWARD, 3)
        ...
        self.assertEqual(STANDING_N_DISTINCT_FORWARD_4GRAMS, 3)
        ...
        expected = (
            (("076", "010", "079", "090"), (SIDE_IA, "Ia5", 139)),
            (("076", "010", "079", "029"), (SIDE_IA, "Ia12", 34)),
            (("076", "010", "079", "053"), (SIDE_IA, "Ia14", 83)),
        )
        for (fwd, site), (want_fwd, want_site) in zip(
            ...
            self.assertEqual(site_forward_4gram(stems, index), want_fwd)
            self.assertEqual(len(fwd), STANDING_N4)
            self.assertEqual(fwd[:STANDING_N3], GRAM3)
            self.assertNotEqual(fwd, MAXIMAL_N5_010[:STANDING_N4])
    ...
    def test_existing_142_141_140_103_and_w_scoreboards_still_compute(self):
        """Cycle 142 inside, 141 I-only, 140 overlap, 103 I-only, and W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-143 leftover forward-4 lock."""
        lock = self.survey["i_leftover_076_010_079_forward_4gram"]
        self.assertEqual(lock["cycle"], 143)
        ...
        self.assertEqual(lock["N_with_forward"], 3)
        ...
        self.assertEqual(lock["N_distinct_forward_4grams"], 3)
        self.assertEqual(
        ...
        self.assertFalse(lock["i_leftover_076_010_079_share_one_forward_4gram"])
        self.assertEqual(
            lock["i_leftover_076_010_079_share_one_forward_4gram"],
            STANDING_I_LEFTOVER_076_010_079_SHARE_ONE_FORWARD_4GRAM,
        )
...
class TestMamariILeftover076010079Forward4gramImageSnapshot(unittest.TestCase):
    """Cycle 143 does not touch clustering. 83/62 / Hamming 6 stays."""
