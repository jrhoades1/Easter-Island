"""Reframed closed-tradition hold-out of the locked leftover-6 next-34 grams.

Cycle 867 text-search lock. Uses already-vendored A–V and the
cycle-531 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 34-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 531 already locked the I-only claim
on these 34-grams as incomplete-set LOSE (N_with_next34=15,
N_no_next34=3, N_i_only=15, N_leak=0). Cycle 866 already locked
the reframed claim on the cycle-529 next-33 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–866.

Claim that can lose: each existing next-34 gram is an exact
...
Named claim: cycle531_next34_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next34 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next34=15,
N_no_next34=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-33 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700
076 076 053 177 700 076 057 741 430 extends to 604 076 071 600
999 050 076 000 002 999 076 092 535 999 208 076 532 244 999 090
076 057 600 700 076 076 053 177 700 076 057 741 430 076 at Ia9[9]
and hits I once. All fifteen next-33 grams extend; no new hole.
...
next 34-grams as a peel, next 35-grams as a peel, prev-184, or
next-185. The cycle-533 next-35 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next34_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE531_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT34_I_ONLY as CYCLE531_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT34_SITES as CYCLE531_SITES,
    STANDING_N as CYCLE531_N,
    STANDING_N_I_EACH as CYCLE531_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE531_N_I_ONLY,
    STANDING_N_LEAK as CYCLE531_N_LEAK,
    STANDING_N_NO_NEXT34 as CYCLE531_N_NO_NEXT34,
    STANDING_N_SEQUENCES as CYCLE531_N_SEQUENCES,
    STANDING_N_WITH_NEXT34 as CYCLE531_N_WITH_NEXT34,
    STANDING_NO_NEXT34_SITES as CYCLE531_NO_NEXT34_SITES,
    STANDING_RESULT as CYCLE531_RESULT,
    STANDING_SEQUENCES as CYCLE531_SEQUENCES,
)
...
STANDING_NO_NEXT34_SITES = CYCLE531_NO_NEXT34_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE531_SITES
STANDING_SEQUENCES = CYCLE531_SEQUENCES
...
STANDING_NEXT33_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT33_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next34_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next34_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 531
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 531
...
STANDING_DO_NOT_RELOCK_CYCLES_832_866 = True
...
class TestMamariILeftoverN6Next34ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 867 reframed closed-tradition lock on the cycle-531 next-34 population."""
    ...
    def test_population_is_the_locked_cycle_531_next34_grams(self):
        """Fifteen next-34 sequences and sites come from cycle 531. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE531_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE531_SITES)
        self.assertEqual(CYCLE531_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE531_N, 18)
        self.assertEqual(CYCLE531_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT34, CYCLE531_N_WITH_NEXT34)
        self.assertEqual(STANDING_N_NO_NEXT34, CYCLE531_N_NO_NEXT34)
        self.assertEqual(
            STANDING_NO_NEXT34_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT34_SITES, CYCLE866_NO_NEXT33_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE531_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE531_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE531_N_I_ONLY, 15)
        self.assertEqual(CYCLE531_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 531)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 34)
        prior = self.survey[CYCLE531_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_34grams_all_i_only"
        )
        self.assertEqual(CYCLE531_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next34_i_only"
        ))
        self.assertEqual(prior["cycle"], 531)
        ...
        self.assertFalse(prior[CYCLE531_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next34"], 3)
        self.assertEqual(prior["N_with_next34"], 15)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE866, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 531 I-only or the cycle 832–866 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 531)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_866)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE531_N_I_EACH)
        self.assertEqual(CYCLE531_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT33_ALL_EXTEND)
        for n33, n34, site33, site34 in zip(
            CYCLE866_SEQUENCES, STANDING_SEQUENCES, CYCLE866_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n34[:33], n33)
            self.assertEqual(site34, site33)
        still = self.rows[STANDING_NEXT33_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:33], CYCLE866_SEQUENCES[3])
        self.assertEqual(CYCLE866_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT34_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_533_next35_list(self):
        """Cycle 533 next-35 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE533_N, 18)
        self.assertEqual(CYCLE533_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE533_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 35 for tokens in CYCLE533_SEQUENCES))
        ...
        self.assertEqual(CYCLE533_N_WITH_NEXT35, 15)
        self.assertEqual(CYCLE533_N_NO_NEXT35, 3)
        self.assertEqual(
            CYCLE533_NO_NEXT35_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        ...
        self.assertNotIn("i_leftover_n6_next35_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle533_next35_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-867 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 867)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next34_sites"]),
            STANDING_NO_NEXT34_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_866"])
        ...
        self.assertEqual(lock["named_by_cycle865"], STANDING_NAMED_BY_CYCLE865)
        ...
        self.assertEqual(
            tuple(lock["next33_hapax_still_hapax"]),
            STANDING_NEXT33_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next33_all_extend"])
        self.assertEqual(lock["next33_all_extend"], STANDING_NEXT33_ALL_EXTEND)
