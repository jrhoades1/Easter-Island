"""Reframed closed-tradition hold-out of the locked leftover-6 next-32 grams.

Cycle 865 text-search lock. Uses already-vendored A–V and the
cycle-527 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 32-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 527 already locked the I-only claim
on these 32-grams as incomplete-set LOSE (N_with_next32=15,
N_no_next32=3, N_i_only=15, N_leak=0). Cycle 864 already locked
the reframed claim on the cycle-525 next-31 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–864.

Claim that can lose: each existing next-32 gram is an exact
...
Named claim: cycle527_next32_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next32 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next32=15,
N_no_next32=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-31 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700
076 076 053 177 700 076 057 extends to 604 076 071 600 999 050
076 000 002 999 076 092 535 999 208 076 532 244 999 090 076 057
600 700 076 076 053 177 700 076 057 741 at Ia9[9] and hits I
once. All fifteen next-31 grams extend; no new hole. Verdict:
incomplete-set LOSE.
...
next 32-grams as a peel, next 33-grams as a peel, prev-184, or
next-185. The cycle-529 next-33 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next32_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT32_SITES as CYCLE527_SITES,
    STANDING_N as CYCLE527_N,
    STANDING_N_I_EACH as CYCLE527_N_I_EACH,
...
STANDING_NO_NEXT32_SITES = CYCLE527_NO_NEXT32_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE527_SITES
STANDING_SEQUENCES = CYCLE527_SEQUENCES
...
STANDING_NEXT31_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT31_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next32_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next32_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 527
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 527
...
STANDING_DO_NOT_RELOCK_CYCLES_832_864 = True
...
class TestMamariILeftoverN6Next32ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 865 reframed closed-tradition lock on the cycle-527 next-32 population."""
    ...
    def test_population_is_the_locked_cycle_527_next32_grams(self):
        """Fifteen next-32 sequences and sites come from cycle 527. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE527_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE527_SITES)
        self.assertEqual(CYCLE527_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE527_N, 18)
        self.assertEqual(CYCLE527_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT32, CYCLE527_N_WITH_NEXT32)
        self.assertEqual(STANDING_N_NO_NEXT32, CYCLE527_N_NO_NEXT32)
        self.assertEqual(
            STANDING_NO_NEXT32_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT32_SITES, CYCLE864_NO_NEXT31_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 527)
        ...
        self.assertEqual(CYCLE527_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next32_i_only"
        ))
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 527 I-only or the cycle 832–864 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 527)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_864)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE527_N_I_EACH)
        self.assertEqual(CYCLE527_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT31_ALL_EXTEND)
        for n31, n32, site31, site32 in zip(
            CYCLE864_SEQUENCES, STANDING_SEQUENCES, CYCLE864_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n32[:31], n31)
            self.assertEqual(site32, site31)
        still = self.rows[STANDING_NEXT31_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:31], CYCLE864_SEQUENCES[3])
        self.assertEqual(CYCLE864_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT32_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_529_next33_list(self):
        """Cycle 529 next-33 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE529_N, 18)
        self.assertEqual(CYCLE529_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE529_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 33 for tokens in CYCLE529_SEQUENCES))
        ...
        self.assertEqual(CYCLE529_N_WITH_NEXT33, 15)
        self.assertEqual(CYCLE529_N_NO_NEXT33, 3)
        self.assertEqual(
            CYCLE529_NO_NEXT33_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        ...
        self.assertNotIn("i_leftover_n6_next33_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle529_next33_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-865 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 865)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next32_sites"]),
            STANDING_NO_NEXT32_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_864"])
        ...
        self.assertEqual(lock["named_by_cycle864"], STANDING_NAMED_BY_CYCLE864)
        ...
        self.assertEqual(
            tuple(lock["next31_hapax_still_hapax"]),
            STANDING_NEXT31_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next31_all_extend"])
        self.assertEqual(lock["next31_all_extend"], STANDING_NEXT31_ALL_EXTEND)
