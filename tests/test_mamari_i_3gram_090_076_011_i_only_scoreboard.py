"""I's cycle-247 leftover extra remaining-after-087 3-gram 090 076 011 off-I lock.

Cycle 248 text-search lock. Uses already-vendored A–V and the
cycle-247 leftover extra remaining-after-087 next stem G=011
(K=2 of N_remaining8=25). Does not retune that leftover extra
remaining-after-087 011 lock, leftover extra remaining-after-280
087, leftover extra remaining-after-530 280, leftover extra
remaining-after-700 530, leftover extra remaining-after-001
700, leftover extra remaining-after-001 unique-max (cycle 234
lost), leftover extra remaining-after-013 001, leftover extra
remaining-after-071 013, leftover extra remaining 071, leftover
...
Same claim-shape as cycle 245 (090 076 087 was I-only 5/0 extra
I=3), cycle 242 (090 076 280 was I-only 2/0), cycle 239
(090 076 530 was I-only 2/0), cycle 236 (090 076 700 was
I-only 2/0), cycle 232 (090 076 001 was I-only 3/0), cycle 229
(090 076 013 was I-only 5/0), cycle 195 (090 076 071 was
...
new leftover extra remaining-after-087 3-gram 090 076 011
only. 090 076 087, 090 076 280, 090 076 530, 090 076 700,
090 076 001, 090 076 013, 090 076 070, 090 076 071, and
090 076 without 011 do not count as this 3-gram. Leftover
extra 090 076 700 011 (Ia2[159]; cycle 237) is a different
4-gram: 011 is the 4th token, not contiguous 090 076 011.
Do not retune leftover n=4, 076-cells, or any detector. Do
not lock leftover extra remaining after 011 this cycle. Do
not lock the other two tied stems (005/000). Off-I T sites
of 090 076 are not this cycle except as off-I of 090 076 011
if they match. Do not assume the I-only result.

Locks exact consecutive hits of 090 076 011 on tablet I and
on every other vendored tablet A–H and J–V. Claim that can
lose: i_3gram_090_076_011_i_only (I hits ≥ 1 and off-I hits
== 0). True only if N_off_I == 0. Measured: Ia is exactly 4
at Ia2[37]/Ia2[107]/Ia12[47]/Ia14[54]; Ib unpublished 0;
every other vendored tablet is exact-0. Leftover extra
remaining-after-087 011 cluster is 2 of those 4 (Ia2[37]/
Ia12[47]; next 4-grams 090 076 011 678 / 130). Extra I sites
not in leftover extra remaining-after-087 = 2 at Ia2[107]/
Ia14[54] (inside leftover n=4 remaining 600 090 076 011;
leftover of leftover, not locked as remaining-after-011).
The claim is true. Not an n≥8 island. Not the cycle-103 I
5-gram. Nested leftover extra remaining-after-087 K=2 /
G=011 N_remaining8=25, cycle 246 5/0 hapax, cycle 245 5/0
extra I=3, cycle 244 K=2 / G=087, cycle 243 2/0 hapax, cycle
242 2/0, cycle 241 K=2 / G=280, cycle 240 2/0 hapax, cycle
239 2/0, cycle 238 K=2 / G=530, cycle 237 2/0 hapax, cycle
236 2/0, cycle 234 7-way tie at 2, cycle 233 3/0 hapax, cycle
232 3/0, cycle 223 69/3, cycle 195 6/0, and cycle 171 43/0
stay.
...
STANDING_IB_SITES = ()
STANDING_LEFTOVER_MATCHING_SITES = CYCLE247_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 2
STANDING_LEFTOVER_MATCHING_NEXT_4GRAMS = CYCLE247_MATCHING_NEXT_4GRAMS
...
STANDING_CLAIM = "i_3gram_090_076_011_i_only"
STANDING_I_3GRAM_090_076_011_I_ONLY = True
STANDING_RESULT = "i_3gram_090_076_011_i_only"
...
def i_3gram_090_076_011_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
def leftover_extra_remaining_after_087_011_subset(
    ...
    """True iff leftover extra remaining-after-087 011 sites ⊆ I 090 076 011."""
...
def extra_i_sites(
    ...
    """I 090 076 011 sites that are not leftover extra remaining-after-087 011."""
...
class TestI3gram090076011IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-247 leftover extra remaining-after-087 3-gram. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 3-gram counts; a gap is not a hit. 090 076 087 / 700 011 are not."""
        ...
        overlap = [["090", "076", "011", "090", "076", "011"]]
        ...
        self.assertEqual(ngram_hit_count([["090", "076", "087"]], GRAM3), 0)
        self.assertEqual(ngram_hit_count([["090", "076", "700", "011"]], GRAM3), 0)
        self.assertEqual(ngram_hit_count([["076", "011"]], GRAM3), 0)
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_090_076_011_i_only(1, 0))
        self.assertTrue(i_3gram_090_076_011_i_only(2, 0))
        self.assertTrue(i_3gram_090_076_011_i_only(4, 0))
        self.assertFalse(i_3gram_090_076_011_i_only(4, 1))
        self.assertFalse(i_3gram_090_076_011_i_only(1, 1))
        self.assertFalse(i_3gram_090_076_011_i_only(0, 0))
        self.assertFalse(i_3gram_090_076_011_i_only(0, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_090_076_011_i_only")
        self.assertTrue(STANDING_I_3GRAM_090_076_011_I_ONLY)
        self.assertTrue(HYPOTHESIS_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_090_076_011_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_leftover_matching_subset_and_extra_can_fail(self):
        """Leftover extra remaining-after-087 011 ⊆ I sites; extra is nonempty."""
        provider = MockProvider()
        self.assertTrue(
    ...
    def test_3gram_is_cycle247_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-247 leftover extra remaining-after-087 G, not priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "011"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], CYCLE247_G)
...
class TestMamariI3gram090076011IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover extra remaining-after-087 3-gram 090 076 011 off-I lock."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_090_076_011_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_247_leftover_not_retuned(self):
        """3-gram is the cycle-247 leftover extra remaining-after-087 G, not a new inventory."""
        self.assertEqual(GRAM3, ("090", "076", "011"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], "011")
        prior_247 = self.survey["i_leftover_extra_090_076_remaining_after_087_fwd011"]
        self.assertEqual(prior_247["cycle"], 247)
        self.assertEqual(tuple(prior_247["forward_3gram"]), GRAM3)
        ...
        measured_matching = [list(site) for site in CYCLE247_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_247["matching_leftover_extra_remaining_after_087_sites"]],
            measured_matching,
        )
        self.assertEqual(
            [list(gram) for gram in CYCLE247_MATCHING_NEXT_4GRAMS],
        ...
        self.assertEqual(STANDING_CYCLE247_N_REMAINING8, 25)
    ...
    def test_i_hits_are_four_on_ia_and_leftover_extra_011_is_subset(self):
        """3-gram is 4 on Ia; Ib 0. Leftover extra remaining-after-087 011 is 2 of those 4."""
        ...
        if self.leftover_matching != CYCLE247_MATCHING_SITES:
            self.fail("leftover extra remaining-after-087 011 set drifted")
        if not leftover_extra_remaining_after_087_011_subset(
            self.leftover_matching,
            self.i_sites,
        ):
            self.fail("leftover extra remaining-after-087 011 not subset of I 090 076 011")
        if self.extra != STANDING_EXTRA_I_SITES:
            self.fail("extra I 090 076 011 leftover-of-leftover sites drifted")
        ...
        self.assertEqual(STANDING_LEFTOVER_MATCHING_NEXT_4GRAMS, CYCLE247_MATCHING_NEXT_4GRAMS)
        self.assertEqual(
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_090_076_011_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_090_076_011_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_247_246_245_244_243_242_241_240_239_238_237_236_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 247 K=2/G=011 N_remaining8=25, 246 5/0 hapax, 245 5/0 extra I=3, 244 K=2/G=087, 243 2/0 hapax, 242 2/0, 241 K=2/G=280, 240 2/0 hapax, 239 2/0, 238 K=2/G=530, 237 2/0 hapax, 236 2/0, 234 7-way tie, 233 3/0, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-248 3-gram I-only lock."""
        lock = self.survey["i_3gram_090_076_011_i_only"]
        self.assertEqual(lock["cycle"], 248)
        ...
        self.assertEqual(
            lock["leftover_extra_remaining_after_087_011_count"],
            STANDING_LEFTOVER_MATCHING_COUNT,
        )
        self.assertEqual(lock["leftover_extra_remaining_after_087_011_count"], 2)
        self.assertTrue(lock["leftover_extra_remaining_after_087_011_subset_of_i_sites"])
        self.assertEqual(
        ...
        self.assertEqual(lock["nested_cycle247_N_remaining8"], STANDING_CYCLE247_N_REMAINING8)
        ...
        self.assertTrue(lock["i_3gram_090_076_011_i_only"])
        self.assertEqual(
            lock["i_3gram_090_076_011_i_only"],
            STANDING_I_3GRAM_090_076_011_I_ONLY,
        )
...
class TestMamariI3gram090076011IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 248 does not touch clustering. 83/62 / Hamming 6 stays."""
