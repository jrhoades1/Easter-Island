"""I's cycle-276 3-gram site previous 4-grams off-I hapax lock.

Cycle 277 text-search lock. Uses already-vendored A–V and the
cycle-276 I sites of 3-gram 071 090 076 (N_I=2, N_off_I=0,
extra I=0). Both I sites are the leftover extra remaining-
after-076 previous-071 pair (071-starts one token earlier;
090-starts Ia3[87]/Ia7[88]). Extra I of this 3-gram is empty
leftover-of-leftover; there is no extra I to include (unlike
cycle 246 / 263 / 269). Does not retune that 3-gram, leftover
extra remaining-after-076 previous-071 (cycle 275 K_071=2 /
N_remaining_after_090_and_076_and_071=31), leftover extra
...
(cycle 260 lost), leftover extra sites, leftover n=4 remaining
090 076 020 010, the leftover n=4 set, or the already-closed
leftover remaining family. Does not peel leftover extra
remaining-after-071 / remaining 045/009 this cycle. Does not
retune leftover n=4. Does not retune the forward peel of
leftover extra I 090 076 (cycles 225–259). Does not overwrite
cycle 167's 3-gram I-only 16/0 lock. Does not overwrite cycle
276's 3-gram I-only 2/0 lock. Does not retune cycle 272's
3-gram 090 090 076 I-only lose 3/1 on T extra I=1 or peel its
T / extra-I. Does not retune cycle 274's 3-gram 076 090 076
I-only lose 3/1 on T extra I=1 or peel its T / extra-I. Does
...
720 076 070 were all I-only hapax 1/0 x3) and cycle 269
(previous 4-grams of I-only 600 090 076 were all I-only
hapax 6/0). Token before G: each previous 4-gram is
contiguous W 071 090 076. Cycle 219 lost: 090 076 070 000
...
I-only but not all hapax (14/0, N_not_hapax=2). Cycle 275
listed previous 4-grams 076 071 090 076 / 092 071 090 076;
measure, do not assume. 090 076 without 071, 999 090 076,
600 090 076, 090 090 076, 076 090 076, 720 076 070,
090 076 070, 076 071 090, and leftover n=4 remaining
090 076 020 010 do not count as these 4-grams. Do not retune
leftover n=4, 076-cells, or any detector. Do not lock leftover
extra remaining-after-071. Do not assume hapax; count each
from fixtures.

Locks exact consecutive hits of each I 071 090 076 previous
...
previous-071 (extra I=0). Both have N_I=1 and N_off_I=0.
Claim that can lose:
i_071_090_076_previous_4grams_all_i_only_hapax. True only if
every I 071 090 076 site that has a previous token has
...
its previous 4-gram. Nested cycle 276 2/0 extra I=0, cycle
275 K_071=2 N_remaining_after_090_and_076_and_071=31
previous 4-grams 076 071 090 076 / 092 071 090 076, cycle
274 3/1 extra I=1, cycle 273 K_076=2 N=33, cycle 272 3/1
extra I=1, cycle 271 K_090=2 N=35, cycle 270 unique-max
false 5-way K=2 G=090, cycle 269 6/0 N_not_hapax=0, cycle
268 6/0 extra I=2, cycle 263 14/0 N_not_hapax=2, cycle 223
69/3, and cycle 207 8/1 on T stay.
...
from tests.test_mamari_i_3gram_071_090_076_i_only_scoreboard import (
    ...
    STANDING_I_PREVIOUS_4GRAMS as CYCLE276_PREVIOUS_4GRAMS,
    STANDING_I_SITE_ROWS as CYCLE276_SITE_ROWS,
    STANDING_I_SITES as CYCLE276_I_SITES,
    STANDING_LEFTOVER_3GRAM_SITES as CYCLE276_LEFTOVER_3GRAM_SITES,
    STANDING_LEFTOVER_MATCHING_SITES as CYCLE276_LEFTOVER_MATCHING_SITES,
...
STANDING_CYCLE276_SITES = CYCLE276_I_SITES
STANDING_PER_SITE_PREVIOUS_4GRAMS = CYCLE276_PREVIOUS_4GRAMS
...
STANDING_CYCLE276_SITES_BY_GRAM = (
    ((SIDE_IA, "Ia3", 86),),
    ((SIDE_IA, "Ia7", 87),),
)
STANDING_ROLES = (
...
STANDING_EXTRA_I_SITES = CYCLE276_EXTRA_I_SITES
STANDING_N_EXTRA = 0
STANDING_LEFTOVER_MATCHING_SITES = CYCLE276_LEFTOVER_MATCHING_SITES
STANDING_LEFTOVER_3GRAM_SITES = CYCLE276_LEFTOVER_3GRAM_SITES
STANDING_KNOWN_DISTINCT = True
STANDING_NOT_ASSUMED_HAPAX = True
STANDING_CLAIM = "i_071_090_076_previous_4grams_all_i_only_hapax"
STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY_HAPAX = True
STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY = True
STANDING_RESULT = "i_071_090_076_previous_4grams_i_only"
...
STANDING_090_076_WITHOUT_071_DOES_NOT_COUNT = True
...
def previous_4gram_start_site(
    cycle276_site: tuple[str, str, int],
) -> tuple[str, str, int] | None:
    """Previous 4-gram starts one token before 071 090 076; None at line start."""
    side, line, index = cycle276_site
    if index < 1:
        return None
...
def i_071_090_076_previous_4grams_all_i_only_hapax(
...
def i_sites_subset_of_previous_4gram(
    ...
    start = previous_4gram_start_site(cycle276_site)
    if start is None:
        return True
...
def site_row_as_survey(row: dict) -> dict:
    ...
    return {
        ...
        "cycle276_site": list(row["cycle276_site"]),
...
STANDING_SITE_ROWS = tuple(
    {
        ...
        "cycle276_site": (prior["side"], prior["line"], prior["index"]),
        "leftover_extra_090_076_site": prior["leftover_extra_090_076_site"],
        "previous_stem": prior["previous_4gram"][0],
        "role": (
            "extra_i"
            if not prior["in_cycle275_leftover_extra_2"]
            else "leftover_extra"
        ),
        "in_cycle275_leftover_extra_2": prior["in_cycle275_leftover_extra_2"],
    ...
    for prior in CYCLE276_SITE_ROWS
)


class TestI071090076Previous4gramsIOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-276 previous 4-grams. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 4-gram counts; a gap is not a hit. 999 / 090 076 / leftover n=4 are not."""
        provider = MockProvider()
        self.assertEqual(STANDING_SEQUENCES[0], ("076", "071", "090", "076"))
        self.assertEqual(STANDING_SEQUENCES[1], ("092", "071", "090", "076"))
        ...
        overlap = [["076", "071", "090", "076", "076", "071", "090", "076"]]
        ...
        self.assertEqual(ngram_hit_count([["071", "090", "076"]], STANDING_SEQUENCES[0]), 0)
        planted = ["076", "071", "090", "076"]
        self.assertEqual(site_previous_4gram_for_3gram(planted, 1, GRAM3), STANDING_SEQUENCES[0])
        self.assertIsNone(site_previous_4gram_for_3gram(["071", "090", "076"], 0, GRAM3))
        self.assertTrue(STANDING_090_076_WITHOUT_071_DOES_NOT_COUNT)
    ...
    def test_all_i_only_hapax_requires_n_i_1_and_zero_off_i_per_site(self):
        ...
        self.assertEqual(STANDING_CLAIM, "i_071_090_076_previous_4grams_all_i_only_hapax")
        self.assertTrue(STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY_HAPAX)
        self.assertTrue(STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY)
    ...
    def test_4grams_are_cycle_276_previous_not_retuned(self):
        """4-grams stay the cycle-276 I-site previous runs; extra I is empty."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("071", "090", "076"))
        self.assertEqual(STANDING_PER_SITE_PREVIOUS_4GRAMS, CYCLE276_PREVIOUS_4GRAMS)
        self.assertEqual(STANDING_CYCLE276_SITES, CYCLE276_I_SITES)
        self.assertEqual(STANDING_N_EXTRA, 0)
        self.assertEqual(STANDING_EXTRA_I_SITES, ())
        self.assertEqual(STANDING_SEQUENCES, CYCLE275_MATCHING_PREVIOUS_4GRAMS)
        leftover_set = set(CYCLE275_MATCHING_PREVIOUS_4GRAMS)
        self.assertTrue(leftover_set.issubset(set(STANDING_PER_SITE_PREVIOUS_4GRAMS)))
        self.assertEqual(len(CYCLE275_MATCHING_PREVIOUS_4GRAMS), 2)
...
class TestMamariI071090076Previous4gramsIOnlyScoreboard(unittest.TestCase):
    """Cited-fixture cycle-276 previous-4 I-only hapax lock. Mock only."""

    def setUp(self):
        self.provider = MockProvider()
        self.survey = load_corpus_survey()
        self.i_sides = load_i_sides()
        self.cycle276_sites = STANDING_CYCLE276_SITES
        ...
        self.per_site_previous = tuple(
            ...
            for site in self.cycle276_sites
        )
        self.line_initial = tuple(
            site
            for site, gram in zip(self.cycle276_sites, self.per_site_previous, strict=True)
            if gram is None
        )
        ...
        self.claim_holds = i_071_090_076_previous_4grams_all_i_only_hapax(
            self.per_site_previous,
            self.n_i_by_gram,
            self.n_off_i_by_gram,
        )

    def test_tokens_and_sites_are_cycle_276_lock_not_retuned(self):
        """4-grams and I sites stay the cycle-276 previous lock; cycle 167 stays 16/0."""
        self.assertEqual(GRAM3, ("071", "090", "076"))
        self.assertEqual(GRAM5, ("999", "071", "076", "010", "079"))
        self.assertEqual(self.cycle276_sites, STANDING_CYCLE276_SITES)
        self.assertEqual(len(self.cycle276_sites), 2)
        ...
        self.assertEqual(tuple(prior_276["tokens3"]), GRAM3)
        self.assertEqual(prior_276["N_I"], CYCLE276_N_I)
        self.assertEqual(prior_276["N_I"], 2)
        self.assertEqual(prior_276["N_off_I"], CYCLE276_N_OFF_I)
        self.assertEqual(prior_276["N_off_I"], 0)
        self.assertEqual(prior_276["N_extra"], CYCLE276_N_EXTRA)
        self.assertEqual(prior_276["N_extra"], 0)
        self.assertTrue(prior_276["i_3gram_071_090_076_i_only"])
        self.assertEqual(
            tuple(tuple(row) for row in prior_276["i_sites"]),
            STANDING_CYCLE276_SITES,
        )
        self.assertEqual(
            [list(gram) for gram in STANDING_PER_SITE_PREVIOUS_4GRAMS],
            prior_276["i_previous_4grams"],
        )
        self.assertEqual(
            tuple(tuple(row) for row in prior_276["extra_i_sites"]),
            STANDING_EXTRA_I_SITES,
        )
        self.assertTrue(leftover_extra_remaining_after_076_previous_071_subset())
        ...
        leftover_071_starts = tuple(
            (side, line, index - 1) for side, line, index in CYCLE275_MATCHING_SITES
        )
        self.assertTrue(set(leftover_071_starts).issubset(set(STANDING_CYCLE276_SITES)))
        ...
        self.assertNotEqual(STANDING_CLAIM, "i_3gram_071_090_076_i_only")
    ...
    def test_all_4grams_are_i_only_hapax_and_claim_holds(self):
        ...
        self.assertEqual(
            self.claim_holds,
            STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY_HAPAX,
        )
        self.assertTrue(STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY)
        self.assertTrue(HYPOTHESIS_ALL_I_ONLY_HAPAX)
        self.assertEqual(STANDING_CLAIM, "i_071_090_076_previous_4grams_all_i_only_hapax")
        ...
        self.assertEqual(leftover_rows[0]["cycle276_site"], (SIDE_IA, "Ia3", 86))
        self.assertEqual(leftover_rows[0]["tokens4"], ("076", "071", "090", "076"))
        self.assertEqual(leftover_rows[1]["cycle276_site"], (SIDE_IA, "Ia7", 87))
        self.assertEqual(leftover_rows[1]["tokens4"], ("092", "071", "090", "076"))
    ...
    def test_existing_276_275_274_273_272_271_270_269_268_263_223_207_167_still_compute(self):
        """Cycle 276 2/0 extra I=0, 275 K=2/31, 274 3/1, 273 K_076=2/33, 272 3/1, 271 K_090=2/35, 270 5-way lose, 269 6/0, 268 6/0 extra I=2, 263 14/0 N_not_hapax=2 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-277 previous-4 I-only hapax lock."""
        lock = self.survey["i_071_090_076_previous_4grams_i_only"]
        self.assertEqual(lock["cycle"], 277)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["leftover_extra_remaining_after_076_previous_071_sites"]),
            STANDING_LEFTOVER_MATCHING_SITES,
        )
        self.assertTrue(lock["leftover_extra_remaining_after_076_previous_071_subset_of_i_sites"])
        ...
        for row, gram, sites, cycle_sites, prev, role, n_on, n_off, hapax, off_sites in zip(
            ...
            self.assertEqual(tuple(tuple(site_row) for site_row in row["cycle276_sites"]), cycle_sites)
        ...
        self.assertTrue(lock["i_071_090_076_previous_4grams_all_i_only_hapax"])
        self.assertTrue(lock["i_071_090_076_previous_4grams_all_i_only"])
        self.assertEqual(
            lock["i_071_090_076_previous_4grams_all_i_only_hapax"],
            STANDING_I_071_090_076_PREVIOUS_4GRAMS_ALL_I_ONLY_HAPAX,
        )
        ...
        self.assertTrue(lock["090_076_without_071_does_not_count"])
...
class TestMamariI071090076Previous4gramsIOnlyImageSnapshot(unittest.TestCase):
    """Cycle 277 does not touch clustering. 83/62 / Hamming 6 stays."""
