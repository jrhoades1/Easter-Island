"""Reframed closed-tradition hold-out of the locked leftover-6 next-80 grams.

Cycle 913 text-search lock. Uses already-vendored A–V and the
cycle-623 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 80-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 623 already locked the
I-only claim on these 80-grams as incomplete-set LOSE
(N_with_next80=12, N_no_next80=6, N_i_only=12, N_leak=0). Cycle
912 already locked the reframed claim on the cycle-621 next-79
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–912.

Claim that can lose: each existing next-80 gram is an exact
...
next 80-grams as a peel, next 81-grams as a peel, prev-184, or
next-185. The cycle-625 next-81 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next80_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE623_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT80_I_ONLY as CYCLE623_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT80_SITES as CYCLE623_SITES,
    STANDING_N as CYCLE623_N,
    STANDING_N_I_EACH as CYCLE623_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE623_N_I_ONLY,
    STANDING_N_LEAK as CYCLE623_N_LEAK,
    STANDING_N_NO_NEXT80 as CYCLE623_N_NO_NEXT80,
    STANDING_N_SEQUENCES as CYCLE623_N_SEQUENCES,
    STANDING_N_WITH_NEXT80 as CYCLE623_N_WITH_NEXT80,
    STANDING_NO_NEXT80_SITES as CYCLE623_NO_NEXT80_SITES,
    STANDING_RESULT as CYCLE623_RESULT,
...
STANDING_NO_NEXT80_SITES = CYCLE623_NO_NEXT80_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE623_SITES
STANDING_SEQUENCES = CYCLE623_SEQUENCES
...
STANDING_NEXT79_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT79_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next80_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next80_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 623
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 623
...
class TestMamariILeftoverN6Next80ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 913 reframed closed-tradition lock on the cycle-623 next-80 population."""
    ...
    def test_population_is_the_locked_cycle_623_next80_grams(self):
        """Twelve next-80 sequences and sites come from cycle 623. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE623_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE623_SITES)
        self.assertEqual(CYCLE623_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE623_N, 18)
        self.assertEqual(CYCLE623_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT80, CYCLE623_N_WITH_NEXT80)
        self.assertEqual(STANDING_N_NO_NEXT80, CYCLE623_N_NO_NEXT80)
        self.assertEqual(
            STANDING_NO_NEXT80_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT80_SITES, CYCLE912_NO_NEXT79_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE623_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE623_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE623_N_I_ONLY, 12)
        self.assertEqual(CYCLE623_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 623)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 80)
        prior = self.survey[CYCLE623_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_80grams_all_i_only"
        )
        self.assertEqual(CYCLE623_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next80_i_only"
        ))
        self.assertEqual(prior["cycle"], 623)
        ...
        self.assertFalse(prior[CYCLE623_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next80"], 6)
        self.assertEqual(prior["N_with_next80"], 12)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE912, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 623 I-only or the cycle 832–912 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 623)
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE623_N_I_EACH)
        self.assertEqual(CYCLE623_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT79_ALL_EXTEND)
        for n79, n80, site79, site80 in zip(
            CYCLE912_SEQUENCES, STANDING_SEQUENCES, CYCLE912_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n80[:79], n79)
            self.assertEqual(site80, site79)
        still = self.rows[STANDING_NEXT79_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:79], CYCLE912_SEQUENCES[3])
        self.assertEqual(CYCLE912_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT80_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT80_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT80_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT80_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT80_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT80_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_625_next81_list(self):
        """Cycle 625 next-81 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE625_N, 18)
        self.assertEqual(CYCLE625_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE625_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 81 for tokens in CYCLE625_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next81_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle625_next81_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-913 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 913)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next80_sites"]),
            STANDING_NO_NEXT80_SITES,
        )
        ...
        self.assertEqual(
            tuple(lock["next79_hapax_still_hapax"]),
            STANDING_NEXT79_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next79_all_extend"])
        self.assertEqual(lock["next79_all_extend"], STANDING_NEXT79_ALL_EXTEND)
