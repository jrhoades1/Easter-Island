"""Reframed closed-tradition hold-out of the locked leftover-6 next-46 grams.

Cycle 879 text-search lock. Uses already-vendored A–V and the
cycle-555 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 46-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 555 already locked the
I-only claim on these 46-grams as incomplete-set LOSE
(N_with_next46=12, N_no_next46=6, N_i_only=12, N_leak=0). Cycle
878 already locked the reframed claim on the cycle-553 next-45
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–878.

Claim that can lose: each existing next-46 gram is an exact
...
Named claim: cycle555_next46_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 12 with-next46 grams: N=12, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=12, N_i_ge1=12, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=12. Parent N=18, N_with_next46=12,
N_no_next46=6 (Ia10[144], Ia10[143], Ia10[142], Ia8[167],
Ia8[166], Ia8[165]). Every scored gram is exact-0 on H/P/Q and
on A/B/C and an exact contiguous hit once on I. The next-45 hapax
...
074 379 002 076 244 280 001 at Ia9[9] extends
to 604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999
090 076 057 600 700 076 076 053 177 700 076 057 741 430 076 532 200 059 076
074 379 002 076 244 280 001 076 at Ia9[9]
and hits I once. All twelve next-45 grams extend; no new hole.
...
next 46-grams as a peel, next 47-grams as a peel, prev-184, or
next-185. The cycle-557 next-47 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next46_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT46_SITES as CYCLE555_SITES,
    STANDING_N as CYCLE555_N,
    STANDING_N_I_EACH as CYCLE555_N_I_EACH,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next47_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE557_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT47_I_ONLY as CYCLE557_ALL_I_ONLY_CLAIM,
    ...
    STANDING_N_SEQUENCES as CYCLE557_N_SEQUENCES,
    STANDING_N_WITH_NEXT47 as CYCLE557_N_WITH_NEXT47,
    STANDING_NO_NEXT47_SITES as CYCLE557_NO_NEXT47_SITES,
    STANDING_RESULT as CYCLE557_RESULT,
...
STANDING_NO_NEXT46_SITES = CYCLE555_NO_NEXT46_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE555_SITES
STANDING_SEQUENCES = CYCLE555_SEQUENCES
...
STANDING_NEXT45_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT45_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next46_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next46_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 555
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 555
...
STANDING_DO_NOT_RELOCK_CYCLES_832_878 = True
...
class TestMamariILeftoverN6Next46ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 879 reframed closed-tradition lock on the cycle-555 next-46 population."""
    ...
    def test_population_is_the_locked_cycle_555_next46_grams(self):
        """Twelve next-46 sequences and sites come from cycle 555. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE555_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE555_SITES)
        self.assertEqual(CYCLE555_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE555_N, 18)
        self.assertEqual(CYCLE555_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT46, CYCLE555_N_WITH_NEXT46)
        self.assertEqual(STANDING_N_NO_NEXT46, CYCLE555_N_NO_NEXT46)
        self.assertEqual(
            STANDING_NO_NEXT46_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT46_SITES, CYCLE878_NO_NEXT45_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 555)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 46)
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 555 I-only or the cycle 832–878 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 555)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_878)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE555_N_I_EACH)
        self.assertEqual(CYCLE555_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT45_ALL_EXTEND)
        for n45, n46, site45, site46 in zip(
            CYCLE878_SEQUENCES, STANDING_SEQUENCES, CYCLE878_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n46[:45], n45)
            self.assertEqual(site46, site45)
        still = self.rows[STANDING_NEXT45_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
                ...
                "002", "076", "244", "280", "001", "076",
            ),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:45], CYCLE878_SEQUENCES[3])
        self.assertEqual(CYCLE878_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT46_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT46_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT46_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT46_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT46_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT46_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_557_next47_list(self):
        """Cycle 557 next-47 I-only list stays locked. The reframed result key has landed; the claim id is not a survey key."""
        self.assertEqual(CYCLE557_N, 18)
        self.assertEqual(CYCLE557_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE557_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 47 for tokens in CYCLE557_SEQUENCES))
        self.assertTrue(CYCLE557_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE557_ALL_I_ONLY_CLAIM)
        ...
        prior = self.survey[CYCLE557_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_47grams_all_i_only"
        )
        self.assertEqual(CYCLE557_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next47_i_only"
        ))
        self.assertEqual(prior["cycle"], 557)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 12)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE557_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next47"], 6)
        self.assertEqual(prior["N_with_next47"], 12)
        self.assertNotIn(STANDING_NEXT_CHEAP_LOCK, self.survey)
        self.assertIn("i_leftover_n6_next47_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle557_next47_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-879 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 879)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next46_sites"]),
            STANDING_NO_NEXT46_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_878"])
        ...
        self.assertEqual(
            tuple(lock["next45_hapax_still_hapax"]),
            STANDING_NEXT45_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next45_all_extend"])
        self.assertEqual(lock["next45_all_extend"], STANDING_NEXT45_ALL_EXTEND)
