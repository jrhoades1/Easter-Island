"""Reframed closed-tradition hold-out of the locked leftover-6 next-33 grams.

Cycle 866 text-search lock. Uses already-vendored A–V and the
cycle-529 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 33-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 529 already locked the I-only claim
on these 33-grams as incomplete-set LOSE (N_with_next33=15,
N_no_next33=3, N_i_only=15, N_leak=0). Cycle 865 already locked
the reframed claim on the cycle-527 next-32 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–865.

Claim that can lose: each existing next-33 gram is an exact
...
Named claim: cycle529_next33_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next33 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next33=15,
N_no_next33=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-32 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700
076 076 053 177 700 076 057 741 extends to 604 076 071 600 999
050 076 000 002 999 076 092 535 999 208 076 532 244 999 090 076
057 600 700 076 076 053 177 700 076 057 741 430 at Ia9[9] and
hits I once. All fifteen next-32 grams extend; no new hole.
...
next 33-grams as a peel, next 34-grams as a peel, prev-184, or
next-185. The cycle-531 next-34 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next33_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE529_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT33_I_ONLY as CYCLE529_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT33_SITES as CYCLE529_SITES,
    STANDING_N as CYCLE529_N,
    STANDING_N_I_EACH as CYCLE529_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE529_N_I_ONLY,
    STANDING_N_LEAK as CYCLE529_N_LEAK,
    STANDING_N_NO_NEXT33 as CYCLE529_N_NO_NEXT33,
    STANDING_N_SEQUENCES as CYCLE529_N_SEQUENCES,
    STANDING_N_WITH_NEXT33 as CYCLE529_N_WITH_NEXT33,
    STANDING_NO_NEXT33_SITES as CYCLE529_NO_NEXT33_SITES,
    STANDING_RESULT as CYCLE529_RESULT,
...
STANDING_NO_NEXT33_SITES = CYCLE529_NO_NEXT33_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE529_SITES
STANDING_SEQUENCES = CYCLE529_SEQUENCES
...
STANDING_NEXT32_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT32_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next33_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next33_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 529
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 529
...
STANDING_DO_NOT_RELOCK_CYCLES_832_865 = True
...
class TestMamariILeftoverN6Next33ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 866 reframed closed-tradition lock on the cycle-529 next-33 population."""
    ...
    def test_population_is_the_locked_cycle_529_next33_grams(self):
        """Fifteen next-33 sequences and sites come from cycle 529. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE529_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE529_SITES)
        self.assertEqual(CYCLE529_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE529_N, 18)
        self.assertEqual(CYCLE529_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT33, CYCLE529_N_WITH_NEXT33)
        self.assertEqual(STANDING_N_NO_NEXT33, CYCLE529_N_NO_NEXT33)
        self.assertEqual(
            STANDING_NO_NEXT33_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT33_SITES, CYCLE865_NO_NEXT32_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE529_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE529_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE529_N_I_ONLY, 15)
        self.assertEqual(CYCLE529_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 529)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 33)
        prior = self.survey[CYCLE529_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_33grams_all_i_only"
        )
        self.assertEqual(CYCLE529_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next33_i_only"
        ))
        self.assertEqual(prior["cycle"], 529)
        ...
        self.assertFalse(prior[CYCLE529_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next33"], 3)
        self.assertEqual(prior["N_with_next33"], 15)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE865, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 529 I-only or the cycle 832–865 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 529)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_865)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE529_N_I_EACH)
        self.assertEqual(CYCLE529_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT32_ALL_EXTEND)
        for n32, n33, site32, site33 in zip(
            CYCLE865_SEQUENCES, STANDING_SEQUENCES, CYCLE865_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n33[:32], n32)
            self.assertEqual(site33, site32)
        still = self.rows[STANDING_NEXT32_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
                ...
                "076", "076", "053", "177", "700", "076", "057", "741", "430",
            ),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:32], CYCLE865_SEQUENCES[3])
        self.assertEqual(CYCLE865_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT33_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_531_next34_list(self):
        """Cycle 531 next-34 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE531_N, 18)
        self.assertEqual(CYCLE531_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE531_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 34 for tokens in CYCLE531_SEQUENCES))
        ...
        self.assertEqual(CYCLE531_N_WITH_NEXT34, 15)
        self.assertEqual(CYCLE531_N_NO_NEXT34, 3)
        self.assertEqual(
            CYCLE531_NO_NEXT34_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        ...
        self.assertNotIn("i_leftover_n6_next34_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle531_next34_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-866 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 866)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next33_sites"]),
            STANDING_NO_NEXT33_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_865"])
        ...
        self.assertEqual(
            tuple(lock["next32_hapax_still_hapax"]),
            STANDING_NEXT32_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next32_all_extend"])
        self.assertEqual(lock["next32_all_extend"], STANDING_NEXT32_ALL_EXTEND)
