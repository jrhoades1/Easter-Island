"""I's cycle-263 shared previous-090 cluster lock.

Cycle 264 text-search lock. Uses already-vendored A–V and the
...
Largest-id of those previous tokens is 090 (vs 000). This
cycle locks the previous-090 cluster as an exact-K claim.
Does not retune that 3-gram, those previous 4-grams, leftover
extra previous-999 (cycle 261 K_999=15 / N_remaining_after_999=41),
...
Same claim-shape as cycle 235 (exactly K share G after a
unique-max / hapax lose with a K=2 tie, largest-id first),
previous 090 of I 999 090 076 instead of forward 700 of
leftover extra remaining-after-001. Cycle 223 lost: 090 076
is not I-only (69/3 on T). Cycle 207 lost: 090 076 070 is
not I-only (8/1 on T). Cycle 220 5-gram 999 090 076 070 000
is a different n=5 (1/0). 090 076 without 999, 720 076 070,
and 999 090 076 071 do not count as this 3-gram. Do not lock
the tied previous-000 pair (000 999 090 076). Off-I T sites
are not this cycle. Do not assume K; count each from fixtures.
...
retune). Count I 999 090 076 sites whose previous token is
090. Lock those sites (3-gram start and 4-gram start). Lock
K_090, and that 090 is a max-K previous token (tied with 000
at K=2; largest-id G=090). Nested-check each previous-090 I
999 090 076 site ⊆ I 3-gram sites and has previous token 090.
Nested-check 4-gram 090 999 090 076 N_I==2 N_off_I==0 (do not
retune 263; still compute).

Hypothesis K=2: exactly 2 I 999 090 076 sites share previous
token 090 (4-gram 090 999 090 076). Measured: K_090=2 at
Ia12[46]/Ia14[139] (4-gram starts Ia12[45]/Ia14[138]). Claim
that can lose: i_999_090_076_exactly_2_share_previous_090.
True iff K_090==2 among I 999 090 076 sites. This can lose
if nested-check K differs from 2. The claim is true. Nested
cycle 263 14/0 N_not_hapax=2, cycle 262 16/0 extra I=1,
cycle 261 K_999=15 N_remaining=41, cycle 260 34 distinct
G=999 K=15, cycle 223 69/3, cycle 207 8/1 on T, and cycle
167 16/0 stay. Do not assume the result; measure. Do not
retune.
...
STANDING_MATCHING_EQUALS_CYCLE263_090_SITES = True
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_999_090_076_exactly_2_share_previous_090"
STANDING_I_999_090_076_EXACTLY_2_SHARE_PREVIOUS_090 = True
STANDING_RESULT = "i_999_090_076_previous_090"
...
STANDING_OTHER_TIED_STEM_000_NOT_LOCKED = True
STANDING_OFF_I_T_SITES_OF_090_076_ARE_NOT_THIS_CYCLE = True


def site_previous_stem_for_3gram(
    stems: list[str],
    index: int,
    gram3: tuple[str, ...] = GRAM3,
) -> str | None:
    """Token immediately before 999 090 076; None at start-of-line."""
    prev4 = site_previous_4gram_for_3gram(stems, index, gram3)
    if prev4 is None:
        return None
    return prev4[0]


def i_999_090_076_previous_stems(
    i_sides: dict[str, list[list[str]]],
    sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE262_SITES,
) -> tuple[str | None, ...]:
    """Previous token of each I 999 090 076 site."""
    return tuple(
        site_previous_stem_for_3gram(line_stems_for_site(i_sides, site), site[2], GRAM3)
        for site in sites
    )


def i_999_090_076_sites_with_previous_090(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """I 999 090 076 sites whose previous token is 090."""
    return tuple(
        site
...
def i_999_090_076_sites_without_previous_090(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """I 999 090 076 sites whose previous token is not 090, or none."""
    return tuple(
        site
        for site, prev in zip(sites, previous_stems, strict=True)
        if prev != stem
...
def matching_equals_cycle263_090_sites(
    matching_4gram_sites: tuple[tuple[str, str, int], ...],
    cycle263_090_sites: tuple[tuple[str, str, int], ...] = CYCLE263_NOT_HAPAX_I_SITES[1],
) -> bool:
    """True iff previous-090 4-gram starts equal cycle 263's 090 999 090 076 pair."""
    return matching_4gram_sites == cycle263_090_sites


def i_999_090_076_exactly_2_share_previous_090(
    k: int,
...
def matching_site_row_as_survey(
    ...
    previous_4gram: tuple[str, ...] = GRAM4_PREVIOUS_090,
) -> dict:
    """JSON-ready previous-090 I 999 090 076 site row."""
...
class TestI999090076Previous090Helpers(unittest.TestCase):
    """Helpers on I 999 090 076 previous 090. No CV, no LLM."""

    def test_previous_090_requires_stem_before_3gram(self):
        """Previous stem 090 is 090 999 090 076; line-initial is no-previous."""
        ...
        mismatch_071 = ["090", "999", "090", "071"]
        self.assertIsNone(site_previous_stem_for_3gram(mismatch_071, 1, GRAM3))
        self.assertEqual(ngram_hit_count([list(GRAM4_PREVIOUS_090)], GRAM4_PREVIOUS_090), 1)
        self.assertEqual(ngram_hit_count([list(GRAM4_PREVIOUS_000)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_720_076_070)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_090_076_070)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_090_076)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_999_090_076_071)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_5GRAM)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_LEFTOVER_N4_057)], GRAM4_PREVIOUS_090), 0)
        self.assertEqual(ngram_hit_count([["999", "090", "076"]], GRAM4_PREVIOUS_090), 0)
    ...
    def test_exactly_2_can_fail(self):
        """Boolean is True only when K_090=2."""
        provider = MockProvider()
        self.assertTrue(i_999_090_076_exactly_2_share_previous_090(2))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(0))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(1))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(3))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(14))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(16))
        planted = STANDING_MATCHING_SITES + ((SIDE_IA, "Ia1", 1),)
        planted_stems = ("090",) * 3
        self.assertEqual(
            i_999_090_076_sites_with_previous_090(planted, planted_stems),
            planted,
        )
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(len(planted)))
        self.assertEqual(STANDING_CLAIM, "i_999_090_076_exactly_2_share_previous_090")
        self.assertTrue(STANDING_I_999_090_076_EXACTLY_2_SHARE_PREVIOUS_090)
        self.assertEqual(
            STANDING_I_999_090_076_EXACTLY_2_SHARE_PREVIOUS_090,
            HYPOTHESIS_K == STANDING_K_090,
        )
        self.assertEqual(STANDING_K_090 + STANDING_N_WITHOUT_090, STANDING_N_I_3GRAM)
        self.assertEqual(2 + 14, 16)
        self.assertEqual(provider.get_call_history(), [])

    def test_cycle263_site_agreement_and_largest_id_tie_can_fail(self):
        """Matching 4-gram starts must equal cycle 263's 090 pair; 000 is not locked."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle263_090_sites(STANDING_MATCHING_4GRAM_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE263_090_SITES)
        self.assertEqual(STANDING_MATCHING_4GRAM_SITES, CYCLE263_NOT_HAPAX_I_SITES[1])
        ...
        self.assertFalse(matching_equals_cycle263_090_sites(planted))
        self.assertFalse(i_999_090_076_exactly_2_share_previous_090(len(planted)))
        swapped = STANDING_OTHER_TIED_4GRAM_SITES
        self.assertFalse(matching_equals_cycle263_090_sites(swapped))
        self.assertTrue(i_999_090_076_exactly_2_share_previous_090(len(swapped)))
        self.assertEqual(STANDING_OTHER_TIED_4GRAM_SITES, CYCLE263_NOT_HAPAX_I_SITES[0])
        self.assertTrue(STANDING_OTHER_TIED_STEM_000_NOT_LOCKED)
        self.assertEqual(STANDING_OTHER_TIED_STEM, "000")
...
class TestMamariI999090076Previous090Scoreboard(unittest.TestCase):
    """Cited-fixture I 999 090 076 previous-090 exact-K lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.matching = i_999_090_076_sites_with_previous_090(
            self.cycle262_sites,
            self.previous_stems,
        )
        self.without = i_999_090_076_sites_without_previous_090(
            self.cycle262_sites,
            self.previous_stems,
        )
        self.k_090 = len(self.matching)
        ...
        self.g, self.tiebreak_k, self.unique = select_previous_g(self.previous_stems)
        ...
        self.k_000 = sum(1 for stem in self.previous_stems if stem == "000")
        self.gram4_i_sites = nge4_sites(GRAM4_PREVIOUS_090, self.i_sides)
        self.n_i_4gram = ngram_hit_count(self.i_sides[SIDE_IA], GRAM4_PREVIOUS_090)
        ...
        self.equals_cycle263 = matching_equals_cycle263_090_sites(
            self.matching_4gram_sites
        )
        self.claim_holds = i_999_090_076_exactly_2_share_previous_090(self.k_090)

    def test_tokens_and_sites_are_cycle_262_263_locks_not_retuned(self):
        """3-gram sites stay cycle 262; shared 4-grams stay cycle 263. Cycle 167 stays 16/0."""
        self.assertEqual(GRAM3, ("999", "090", "076"))
    ...
    def test_counts_exactly_2_share_previous_090_and_hypothesis_holds(self):
        """K_090=2 at Ia12[46]/Ia14[139]. Largest-id G=090 tied with 000 at 2. Claim holds."""
        ...
        self.assertEqual(self.k_090, STANDING_K)
        self.assertEqual(len(self.without), STANDING_N_WITHOUT_090)
        self.assertEqual(STANDING_N_WITHOUT_090, 14)
        self.assertEqual(self.k_090 + len(self.without), STANDING_N_I_3GRAM)
        self.assertEqual(2 + 14, 16)
        ...
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE263_090_SITES)
        self.assertTrue(matching_equals_cycle263_090_sites(self.matching_4gram_sites))
        ...
        self.assertGreater(barthel_id(self.g), barthel_id(STANDING_OTHER_TIED_STEM))
        if self.k_090 != 2:
            self.fail("measured K_090 drifted from 2")
        if self.g != "090" or self.unique or self.tied != ("090", "000"):
            self.fail("largest-id max-K previous G=090 tied with 000 at K=2 drifted")
        self.assertTrue(i_999_090_076_exactly_2_share_previous_090(self.k_090))
        self.assertEqual(
            self.claim_holds,
            STANDING_I_999_090_076_EXACTLY_2_SHARE_PREVIOUS_090,
        )
        ...
        if self.n_i_4gram != 2 or self.n_off_i_4gram != 0:
            self.fail("4-gram 090 999 090 076 N_I/N_off_I drifted from 2/0")
    ...
    def test_previous_090_sites_subset_i_3gram_and_have_previous_090(self):
        """Each previous-090 site ⊆ I 3-gram sites and previous token is 090."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        expected = (
            ((SIDE_IA, "Ia12", 46), (SIDE_IA, "Ia12", 45), (SIDE_IA, "Ia12", 47)),
            ((SIDE_IA, "Ia14", 139), (SIDE_IA, "Ia14", 138), (SIDE_IA, "Ia14", 140)),
        )
        for site, start, leftover, (want_3, want_4, want_2) in zip(
            ...
            self.assertEqual(tuple(stems[start[2] : start[2] + STANDING_N4]), GRAM4_PREVIOUS_090)
            ...
            self.assertTrue(is_contiguous_substring(GRAM3, GRAM4_PREVIOUS_090))
        ...
        for tablet, count in zip(VENDORED_TABLETS, self.hits_by_tablet, strict=True):
            self.assertEqual(count, ngram_hit_count(self.by_tablet[tablet], GRAM4_PREVIOUS_090))
            self.assertEqual(count, 2 if tablet == "I" else 0)
        self.assertEqual(STANDING_IB_HITS, 0)
        self.assertEqual(STANDING_OFF_I_SITES, ())
        self.assertEqual(self.provider.get_call_history(), [])

    def test_existing_263_262_261_260_223_207_167_scoreboards_still_compute(self):
        """Cycle 263 14/0 N_not_hapax=2, 262 16/0 extra I=1, 261 K=15/41, 260 34/999/15, 223 69/3, 207 8/1, 167 16/0 stay."""
        ...
        self.assertEqual(CYCLE263_NOT_HAPAX_SEQUENCES[0], GRAM4_PREVIOUS_000)
        self.assertEqual(CYCLE263_NOT_HAPAX_SEQUENCES[1], GRAM4_PREVIOUS_090)
        self.assertEqual(CYCLE263_NOT_HAPAX_I_SITES[0], STANDING_OTHER_TIED_4GRAM_SITES)
        self.assertEqual(CYCLE263_NOT_HAPAX_I_SITES[1], STANDING_MATCHING_4GRAM_SITES)
        if (
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-264 previous-090 exact-K lock."""
        lock = self.survey["i_999_090_076_previous_090"]
        self.assertEqual(lock["cycle"], 264)
        ...
        self.assertEqual(tuple(lock["previous_4gram"]), GRAM4_PREVIOUS_090)
        ...
        self.assertEqual(lock["N_without_090"], STANDING_N_WITHOUT_090)
        self.assertEqual(lock["N_without_090"], 14)
        self.assertEqual(
        ...
        self.assertTrue(lock["matching_equals_cycle263_090_sites"])
        self.assertEqual(
        ...
        self.assertTrue(lock["i_999_090_076_exactly_2_share_previous_090"])
        self.assertEqual(
            lock["i_999_090_076_exactly_2_share_previous_090"],
            STANDING_I_999_090_076_EXACTLY_2_SHARE_PREVIOUS_090,
        )
        ...
        self.assertTrue(lock["other_tied_stem_000_not_locked"])
...
class TestMamariI999090076Previous090ImageSnapshot(unittest.TestCase):
    """Cycle 264 does not touch clustering. 83/62 / Hamming 6 stays."""
