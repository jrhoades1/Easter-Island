"""I's cycle-176 leftover 3-gram 076 071 600 off-I lock.

Cycle 177 text-search lock. Uses already-vendored A–V and the
cycle-176 leftover forward 3-gram 076 071 600 (the n=3 run
shared by leftover 076 071 sites Ia2[104] 076 071 600 090,
Ia2[169] 076 071 600 009, Ia9[10] 076 071 600 999, and
Ia14[106] 076 071 600 053). Does not retune that 3-gram,
...
Same claim-shape as cycle 174 (076 071 076 was I-only 6/0),
...
This cycle is the new leftover 3-gram 076 071 600 only.
071 999 (071 065 071 999) does not count. 076 076
(700 076 076 053) does not. Do not retune 076 071 076,
076 071, 999 090 076, 076 020 010, or 076 010 079. Cycle
176 leftover N=4 and leftover n=4 set stay. Do not assume
the I-only result. Include any inside-family 076 071 site
that also has this 3-gram, if one exists.

Locks exact consecutive hits of 076 071 600 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_076_071_600_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
4 at Ia2[104]/Ia2[169]/Ia9[10]/Ia14[106] (the four leftover
matching sites; no inside-family 076 071 site has this
3-gram); Ib unpublished 0; every other vendored tablet is
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE176_MATCHING_SITES
...
STANDING_CLAIM = "i_3gram_076_071_600_i_only"
STANDING_I_3GRAM_076_071_600_I_ONLY = True
STANDING_RESULT = "i_3gram_076_071_600_i_only"
...
def i_3gram_076_071_600_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
class TestI3gram076071600IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-176 leftover 3-gram tokens. No CV, no LLM."""
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_076_071_600_i_only(1, 0))
        self.assertTrue(i_3gram_076_071_600_i_only(4, 0))
        self.assertFalse(i_3gram_076_071_600_i_only(1, 1))
        self.assertFalse(i_3gram_076_071_600_i_only(0, 0))
        self.assertFalse(i_3gram_076_071_600_i_only(0, 4))
        self.assertEqual(STANDING_CLAIM, "i_3gram_076_071_600_i_only")
        self.assertTrue(STANDING_I_3GRAM_076_071_600_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_076_071_600_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle176_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-176 leftover, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_FORWARD)
...
class TestMamariI3gram076071600IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 076 071 600 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_076_071_600_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_176_leftover_not_retuned(self):
        """3-gram is the cycle-176 leftover lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3, ("076", "071", "600"))
        prior_176 = self.survey["i_leftover_076_071_forward_600"]
        self.assertEqual(prior_176["cycle"], 176)
        self.assertEqual(tuple(prior_176["forward_3gram"]), GRAM3)
        self.assertEqual(prior_176["N_with_forward_076_071_600"], 4)
        self.assertEqual(prior_176["N_without"], 30)
        self.assertEqual(prior_176["N_leftover"], 34)
        measured_matching = [list(site) for site in CYCLE176_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_176["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_176["i_leftover_076_071_exactly_4_forward_076_071_600"])
        self.assertTrue(prior_176["071_999_does_not_count"])
        self.assertTrue(prior_176["076_076_does_not_count"])
        self.assertNotEqual(GRAM3, GRAM5)
        self.assertNotEqual(GRAM3, CYCLE174_GRAM3)
        self.assertNotEqual(GRAM3, CYCLE171_GRAM2)
        self.assertNotEqual(GRAM3, CYCLE167_GRAM3)
    ...
    def test_i_hits_are_four_on_ia(self):
        """3-gram is 4 on Ia (4 leftover, 0 inside-family); Ib unpublished 0."""
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        """3-gram is 0 on A–H and J–V. Ia has exactly 4. W is not a tablet."""
        ...
        self.assertEqual(
            i_3gram_076_071_600_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_076_071_600_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_176_175_174_173_172_171_170_167_160_141_103_and_w_scoreboards_still_compute(self):
        """Cycle 176 leftover 4, 175/174 I-only, 173/172/171/170, 167/160/141, 103, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-177 3-gram I-only lock."""
        lock = self.survey["i_3gram_076_071_600_i_only"]
        self.assertEqual(lock["cycle"], 177)
        ...
        self.assertTrue(lock["i_3gram_076_071_600_i_only"])
        self.assertEqual(
            lock["i_3gram_076_071_600_i_only"],
            STANDING_I_3GRAM_076_071_600_I_ONLY,
        )
        ...
        self.assertEqual(self.survey["i_leftover_076_071_forward_600"]["N_without"], 30)
...
class TestMamariI3gram076071600IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 177 does not touch clustering. 83/62 / Hamming 6 stays."""
