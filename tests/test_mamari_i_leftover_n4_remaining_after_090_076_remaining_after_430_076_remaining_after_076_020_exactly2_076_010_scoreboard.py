"""I's leftover n=4 remaining remaining-after-090-076 remaining-after-430-076 remaining-after-076-020 exactly 2 share 076 010.

Cycle 336 text-search lock. Uses already-vendored A–V and the
...
remaining-after-076-020. Leftover n=4 remaining remaining-after-
090-076 = N=11 leftover 4-grams that do not contain 090 076.
Cycle 328 LOSE unique most frequent 2-gram (5-way tie at K=2,
G=430 076 largest-id). Cycle 329 HOLD: exactly 2 contain
contiguous 430 076. Cycle 330 LOSE: 2-gram 430 076 is not
I-only. Cycle 331 HOLD: leftover 4-grams that contain 430 076
are all I-only (extra I of those 4-grams=0). Cycle 332 LOSE
unique most frequent 2-gram among remaining-after-430-076
(N=9 leftover 4-grams that do not contain 090 076 and do not
contain contiguous 430 076). unique_max false, 4-way tie at
K=2: 076 020 / 076 010 / 020 010 / 010 079. G=076 020
largest-id labeling only. Matching leftovers of labeled G:
076 020 010 050, 053 076 020 010. N_remaining after peeling
G=7. Cycle 333 HOLD: exactly K=2 of those N=9 contain
contiguous 076 020, matching leftovers as labeled,
N_remaining=7. Nested cycle 332 unique_max false stays nested.
Cycle 334 HOLD: 2-gram 076 020 is I-only (N_I=12, N_off_I=0 /
N_leak=0, leftover matching sites 3 unique 2-gram sites
Ia2[120]/Ia12[1]/Ia14[110], extra I=9). Extra I ≠ 0 so
all-inside-family is false (skip analog of cycle 224). Leftover
4-grams that contain 076 020 are I-only by implication of the
2-gram I-only lock; do not spend a cycle re-locking that
tautology (unlike cycle 331, which was needed because cycle
330 2-gram leaked). Extra-I 4-grams of 076 020 I-only is the
same tautology; skip. Nested leftover n=4 remaining I 090 076
020 peels stay nested. Cycle 335 LOSE unique most frequent
2-gram among remaining-after-076-020 (N=7 leftover 4-grams
that do not contain 090 076, do not contain contiguous 430 076,
and do not contain contiguous 076 020). unique_max false,
2-way tie at K=2: 076 010 / 010 079. G=076 010 largest-id
labeling only. Matching leftovers of labeled G: 076 010 079 090,
072 076 010 079. N_remaining after peeling G=5. N_distinct=19.
This cycle's N=7 population is leftover n=4 remaining remaining-
after-090-076 remaining-after-430-076 remaining-after-076-020.
This cycle does not re-lock unique_max.

Already locked (do not re-lock): cycle 335, 334, 333, 332, 331,
330, 329, 328, 222, leftover n=4 remaining I 090 076 peels
(288–327), leftover extra 090 076 peels, cycles 220–221, or
cycle 223. Do not claim 2-gram 076 010 I-only or 4-gram I-only
this cycle.

This cycle is leftover-inventory, not I-only of G. HOLD iff
exactly K=2 of the N=7 remaining-after-076-020 leftover 4-grams
contain contiguous 2-gram 076 010, those two are 076 010 079 090
and 072 076 010 079, and N_remaining after peeling G=5. LOSE if
K≠2, the matching set differs, or N_remaining is not 5. Record
N=7, K, G=076 010, both matching 4-grams, N_remaining after
peeling G (=5). Nested cycle 335 unique_max false stays nested.

Analog: cycle 333 leftover n=4 remaining remaining-after-090-076
remaining-after-430-076 exactly 2 share 076 020 HOLD after cycle
332 unique-max LOSE. Also analog cycle 329 leftover n=4 remaining
remaining-after-090-076 exactly 2 share 430 076 HOLD after cycle
328 unique-max LOSE.

Measured: N=7, K=2, G=076 010, matching leftovers
076 010 079 090 and 072 076 010 079, N_remaining=5.
unique_max stays false (nested cycle 335). Claim that can
lose:
...
076 020, N=7 remaining-after-076-020, exactly K=2 contain
076 010, the matching set is those two 4-grams, and
N_remaining after peeling G=5. The claim is true. Do not
retune.
...
from tests.test_mamari_i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_next_2gram_scoreboard import (
    ...
    leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g,
    leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g,
)
...
STANDING_N_REMAINING_GREATGRANDPARENT = 16
STANDING_K_GREATGRANDPARENT = 5
STANDING_N_REMAINING_GRANDPARENT = 11
STANDING_K_GRANDPARENT = 2
STANDING_N_REMAINING_PARENT = 9
STANDING_K_PARENT = 2
STANDING_N = 7
GRAM2 = ("076", "010")
STANDING_G = GRAM2
...
STANDING_N_REMAINING = 5
STANDING_N_DISTINCT = 19
...
STANDING_TWOGRAM_I_ONLY_IS_NOT_THIS_CYCLE = True
STANDING_FOURGRAM_I_ONLY_IS_NOT_THIS_CYCLE = True
STANDING_CLAIM = (
    "i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010"
)
STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_EXACTLY_2_SHARE_076_010 = (
    True
)
STANDING_RESULT = (
...
STANDING_SAME_CLAIM_SHAPE_AS_CYCLE333 = True
STANDING_SAME_CLAIM_SHAPE_AS_CYCLE329 = True
...
def leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_matching_set(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[str, ...], ...]:
    """Tokens of remaining-after-076-020 leftovers that contain G."""
    return tuple(
        gram
        for gram, _n, _f, _s in leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g(
...
def i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010(
    ...
    expected_n_remaining: int = STANDING_N_REMAINING,
    ...
    """True iff exactly K=2 of N=7 remaining-after-076-020 contain G and N_remaining=5."""
    if not leftover_n4_family_counts_hold(leftovers):
        return False
    remaining = leftover_remaining_n4(leftovers)
    if len(remaining) != expected_remaining_greatgrandparent:
        return False
    if not i_leftover_n4_remaining_exactly_5_contain_090_076(leftovers):
        return False
    if len(leftover_remaining_with_g(remaining, peeled_090_076)) != expected_k_greatgrandparent:
        return False
    remaining_after_090 = leftover_remaining_without_g(remaining, peeled_090_076)
    if len(remaining_after_090) != expected_remaining_grandparent:
        return False
    if not i_leftover_n4_remaining_after_090_076_exactly_2_share_430_076(leftovers):
        return False
    if len(leftover_remaining_with_g(remaining_after_090, peeled_430_076)) != expected_k_grandparent:
        return False
    remaining_after_430 = leftover_remaining_without_g(
        remaining_after_090, peeled_430_076
    )
    if len(remaining_after_430) != expected_remaining_parent:
        return False
    if not i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020(
        leftovers
    ):
        return False
    if len(leftover_remaining_with_g(remaining_after_430, peeled_076_020)) != expected_k_parent:
        return False
    ...
    if len(matching) != expected_k:
        return False
    if not matching_set_is_labeled_g_pair(matching, expected_matching):
        return False
    without = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g(
        remaining_after, expected_g
    )
    return len(without) == expected_n_remaining


STANDING_REMAINING_GREATGRANDPARENT = leftover_remaining_n4(STANDING_LEFTOVER)
STANDING_REMAINING_GRANDPARENT = leftover_remaining_after_090_076(STANDING_LEFTOVER)
STANDING_REMAINING_PARENT = leftover_remaining_after_090_076_remaining_after_430_076(
    STANDING_LEFTOVER
)
STANDING_REMAINING = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
    STANDING_LEFTOVER
)
STANDING_WITH_ROWS = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g(
    STANDING_REMAINING
)
STANDING_WITHOUT_ROWS = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g(
    STANDING_REMAINING
)
STANDING_MATCHING_TOKENS = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_matching_set(
    STANDING_REMAINING
)
...
class TestILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Exactly2076010Helpers(
    unittest.TestCase
):
    """Helpers on remaining-after-076-020 exactly 2 share 076 010. No CV, no LLM."""

    def test_exactly_2_can_fail_on_k_or_matching_set_or_n_remaining(self):
        """N=7 holds at K=2 matching pair N_remaining=5; empty, K≠2, and swapped set lose."""
        ...
        self.assertEqual(len(remaining), STANDING_N_REMAINING_GREATGRANDPARENT)
        self.assertEqual(len(remaining), CYCLE222_N_REMAINING)
        remaining_grandparent = leftover_remaining_after_090_076(leftover)
        self.assertEqual(len(remaining_grandparent), STANDING_N_REMAINING_GRANDPARENT)
        self.assertEqual(len(remaining_grandparent), CYCLE329_N)
        remaining_parent = leftover_remaining_after_090_076_remaining_after_430_076(
            leftover
        )
        self.assertEqual(len(remaining_parent), STANDING_N_REMAINING_PARENT)
        self.assertEqual(len(remaining_parent), CYCLE333_N)
        ...
        self.assertEqual(len(remaining_after), 7)
        ...
        self.assertEqual(
            contiguous_2grams(("076", "010", "079", "090")),
            (("076", "010"), ("010", "079"), ("079", "090")),
        )
        self.assertEqual(
            contiguous_2grams(("072", "076", "010", "079")),
            (("072", "076"), ("076", "010"), ("010", "079")),
        )
        self.assertTrue(leftover_is_remaining(("076", "010", "079", "090")))
        self.assertTrue(leftover_is_remaining(("072", "076", "010", "079")))
        self.assertTrue(leftover_is_remaining(NEAR_MISS_028_076_011_076))
        self.assertTrue(leftover_is_remaining(NEAR_MISS_076_020_010_050))
        self.assertTrue(leftover_is_remaining(NEAR_MISS_430_076_001_076))
        ...
        self.assertFalse(
            is_contiguous_substring(LOCKED_G_090_076, ("076", "010", "079", "090"))
        )
        self.assertFalse(
            is_contiguous_substring(LOCKED_G_430_076, ("076", "010", "079", "090"))
        )
        self.assertFalse(
            is_contiguous_substring(LOCKED_G_076_020, ("076", "010", "079", "090"))
        )
        self.assertTrue(
        ...
        self.assertTrue(is_contiguous_substring(GRAM2, ("076", "010", "079", "090")))
        self.assertTrue(is_contiguous_substring(GRAM2, ("072", "076", "010", "079")))
        ...
        self.assertFalse(is_contiguous_substring(GRAM2, NEAR_MISS_430_076_001_076))
        self.assertFalse(
            i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010(
                ()
            )
        )
        with_gram = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g(
            remaining_after
        )
        without_gram = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g(
            remaining_after
        )
        matching = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_matching_set(
            remaining_after
        ...
        self.assertEqual(len(without_gram), STANDING_N_REMAINING)
        ...
        extra_after = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
            extra_g
        )
        self.assertEqual(len(extra_after), STANDING_N)
        extra_matching = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_matching_set(
            extra_after
        )
        ...
        dropped = tuple(
            row for row in leftover if row[0] != ("076", "010", "079", "090")
        )
        self.assertFalse(
            i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010(
                dropped
        ...
        swapped_after = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
            swapped
        )
        self.assertEqual(len(swapped_after), STANDING_N)
        swapped_matching = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_matching_set(
            swapped_after
        )
        ...
        self.assertEqual(STANDING_N_REMAINING, 5)
        self.assertTrue(STANDING_I_VS_OFF_I_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_TWOGRAM_I_ONLY_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_FOURGRAM_I_ONLY_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_SAME_CLAIM_SHAPE_AS_CYCLE333)
        self.assertTrue(STANDING_SAME_CLAIM_SHAPE_AS_CYCLE329)
    ...
    def test_nested_cycle335_unique_max_stays_false(self):
        """Cycle 335 unique-max LOSE stays nested; exactly-K is this claim."""
        provider = MockProvider()
        leftover = leftover_n4_rows()
        remaining_after = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
            leftover
        ...
        gram, count, unique = select_g(remaining_after)
        self.assertEqual(gram, CYCLE335_G)
        self.assertEqual(gram, STANDING_G)
        self.assertEqual(count, CYCLE335_K)
        self.assertEqual(count, STANDING_K)
        self.assertFalse(unique)
        ...
        self.assertEqual(CYCLE335_TIED, STANDING_TIED_2GRAMS)
        self.assertEqual(CYCLE335_MATCHING, STANDING_MATCHING_LEFTOVERS)
        self.assertEqual(CYCLE335_GRAM2, GRAM2)
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Exactly2076010Scoreboard(
    unittest.TestCase
):
    """Cited-fixture remaining-after-076-020 exactly 2 share 076 010. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining_greatgrandparent = leftover_remaining_n4(self.leftover)
        self.remaining_grandparent = leftover_remaining_after_090_076(self.leftover)
        self.remaining_parent = leftover_remaining_after_090_076_remaining_after_430_076(
            self.leftover
        )
        self.remaining = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
            self.leftover
        )
        self.with_gram = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g(
            self.remaining
        )
        self.without_gram = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g(
            self.remaining
        )
    ...
    def test_nested_leftover_n4_family_and_cycles_222_328_329_332_333_335(self):
        """Nested leftover n=4 27/7/4/1, 222 16/5/11, 328/329 11/2/9, 332/333 9/2/7, 335 7/2/5 stay."""
        ...
        self.assertEqual(len(self.remaining_greatgrandparent), 16)
        self.assertTrue(
        ...
        self.assertEqual(
            tuple(
                gram
                for gram, _n, _f, _s in leftover_remaining_with_g(
                    self.remaining_grandparent, LOCKED_G_430_076
                )
            ),
            CYCLE329_MATCHING,
        )
        self.assertEqual(len(self.remaining_grandparent), 11)
        self.assertTrue(
            i_leftover_n4_remaining_after_090_076_remaining_after_430_076_exactly_2_share_076_020(
                self.leftover
            )
        ...
        self.assertEqual(
            tuple(
                gram
                for gram, _n, _f, _s in leftover_remaining_with_g(
                    self.remaining_parent, LOCKED_G_076_020
                )
            ),
            CYCLE333_MATCHING,
        )
        self.assertEqual(len(self.remaining_parent), 9)
        ...
        self.assertEqual(STANDING_K_GRANDPARENT + STANDING_N_REMAINING_PARENT, 11)
        self.assertEqual(STANDING_K_GREATGRANDPARENT + STANDING_N_REMAINING_GRANDPARENT, 16)
        self.assertFalse(CYCLE335_UNIQUE)
        self.assertFalse(CYCLE335_UNIQUE_MAX_CLAIM)
        self.assertEqual(CYCLE335_G, STANDING_G)
    ...
    def test_remaining_after_7_exactly_2_share_076_010_holds(self):
        """7 remaining-after-076-020; exactly 2 share 076 010. Claim is true."""
        self.assertEqual(len(self.remaining), STANDING_N)
        self.assertEqual(len(self.remaining), 7)
        ...
        self.assertEqual(STANDING_N_DISTINCT, 19)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_per_remaining_after_only_the_expected_two_for_labeled_g(self):
        """Labeled G=076 010 matches two leftovers; peeled 090 076 / 430 076 / 076 020 do not count."""
        ...
        self.assertTrue(is_contiguous_substring(LOCKED_G_076_020, NEAR_MISS_090_076_020_010))
        self.assertTrue(STANDING_I_VS_OFF_I_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_TWOGRAM_I_ONLY_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_FOURGRAM_I_ONLY_IS_NOT_THIS_CYCLE)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_leftover_sites_on_i(self):
        """Remaining-after-076-020 leftover I sites; Ib unpublished."""
        self.assertEqual(self.remaining, STANDING_REMAINING)
    ...
    def test_existing_335_334_333_332_331_330_329_328_223_222_and_220_scoreboards_still_compute(self):
        """Cycle 335 unique-max lose, 334 12/0, 333 K=2, 332 unique-max lose stay."""
        leftover = leftover_n4_rows()
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-336 remaining-after-076-020 exactly-2 lock."""
        lock = self.survey[
            "i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly2_076_010"
        ]
        self.assertEqual(lock["cycle"], 336)
        ...
        self.assertEqual(lock["N_remaining_greatgrandparent"], 16)
        self.assertEqual(lock["K_greatgrandparent"], STANDING_K_GREATGRANDPARENT)
        self.assertEqual(lock["K_greatgrandparent"], 5)
        ...
        self.assertEqual(lock["N_remaining_grandparent"], 11)
        ...
        self.assertEqual(lock["N_remaining"], 5)
        self.assertEqual(lock["N_distinct"], STANDING_N_DISTINCT)
        self.assertEqual(lock["N_distinct"], 19)
        ...
        self.assertEqual(tuple(lock["near_miss_028_076_011_076"]), NEAR_MISS_028_076_011_076)
        ...
        self.assertTrue(lock["twogram_i_only_is_not_this_cycle"])
        self.assertTrue(lock["fourgram_i_only_is_not_this_cycle"])
        ...
        self.assertTrue(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010"]
        )
        self.assertEqual(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_exactly_2_share_076_010"],
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_EXACTLY_2_SHARE_076_010,
        ...
        self.assertTrue(lock["same_claim_shape_as_cycle333"])
        self.assertTrue(lock["same_claim_shape_as_cycle329"])
        ...
        self.assertEqual(lock["nested_cycle335_N"], 7)
        self.assertEqual(lock["nested_cycle335_N_remaining"], 5)
        ...
        self.assertEqual(CYCLE333_GRAM2, ("076", "020"))
        self.assertEqual(CYCLE329_GRAM2, ("430", "076"))
        self.assertEqual(CYCLE222_GRAM2, ("090", "076"))
        self.assertEqual(self.provider.get_call_history(), [])


class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Exactly2076010ImageSnapshot(
    unittest.TestCase
):
    """Cycle 336 does not touch clustering. 83/62 / Hamming 6 stays."""
