"""Reframed closed-tradition hold-out of the locked leftover-6 next-75 grams.

Cycle 908 text-search lock. Uses already-vendored A–V and the
cycle-613 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 75-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 613 already locked the
I-only claim on these 75-grams as incomplete-set LOSE
(N_with_next75=12, N_no_next75=6, N_i_only=12, N_leak=0). Cycle
907 already locked the reframed claim on the cycle-611 next-74
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–907.

Claim that can lose: each existing next-75 gram is an exact
...
Named claim: cycle613_next75_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 12 with-next75 grams: N=12, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=12, N_i_ge1=12, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=12. Parent N=18, N_with_next75=12,
N_no_next75=6 (Ia10[144], Ia10[143], Ia10[142], Ia8[167],
Ia8[166], Ia8[165]). Every scored gram is exact-0 on H/P/Q and
on A/B/C and an exact contiguous hit once on I. The next-74 hapax
...
420 519 999 006 076 073 430 076 006 006 040 999 002 076 240 382 999 at Ia9[9] extends
to 604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999
090 076 057 600 700 076 076 053 177 700 076 057 741 430 076 532 200 059 076
074 379 002 076 244 280 001 076 532 071 065 071 999 700 076 480 600 005 076
420 519 999 006 076 073 430 076 006 006 040 999 002 076 240 382 999 076 at Ia9[9]
and hits I once. All twelve next-74 grams extend; no new hole.
...
next 75-grams as a peel, next 76-grams as a peel, prev-184, or
next-185. The cycle-615 next-76 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next75_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE613_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT75_I_ONLY as CYCLE613_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT75_SITES as CYCLE613_SITES,
    STANDING_N as CYCLE613_N,
    STANDING_N_I_EACH as CYCLE613_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE613_N_I_ONLY,
    STANDING_N_LEAK as CYCLE613_N_LEAK,
    STANDING_N_NO_NEXT75 as CYCLE613_N_NO_NEXT75,
    STANDING_N_SEQUENCES as CYCLE613_N_SEQUENCES,
    STANDING_N_WITH_NEXT75 as CYCLE613_N_WITH_NEXT75,
    STANDING_NO_NEXT75_SITES as CYCLE613_NO_NEXT75_SITES,
    STANDING_RESULT as CYCLE613_RESULT,
...
STANDING_NO_NEXT75_SITES = CYCLE613_NO_NEXT75_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE613_SITES
STANDING_SEQUENCES = CYCLE613_SEQUENCES
...
STANDING_NEXT74_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT74_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next75_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next75_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 613
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 613
...
STANDING_DO_NOT_RELOCK_CYCLES_832_907 = True
...
class TestMamariILeftoverN6Next75ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 908 reframed closed-tradition lock on the cycle-613 next-75 population."""
    ...
    def test_population_is_the_locked_cycle_613_next75_grams(self):
        """Twelve next-75 sequences and sites come from cycle 613. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE613_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE613_SITES)
        self.assertEqual(CYCLE613_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE613_N, 18)
        self.assertEqual(CYCLE613_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT75, CYCLE613_N_WITH_NEXT75)
        self.assertEqual(STANDING_N_NO_NEXT75, CYCLE613_N_NO_NEXT75)
        self.assertEqual(
            STANDING_NO_NEXT75_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT75_SITES, CYCLE907_NO_NEXT74_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE613_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE613_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE613_N_I_ONLY, 12)
        self.assertEqual(CYCLE613_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 613)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 75)
        prior = self.survey[CYCLE613_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_75grams_all_i_only"
        )
        self.assertEqual(CYCLE613_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next75_i_only"
        ))
        self.assertEqual(prior["cycle"], 613)
        ...
        self.assertFalse(prior[CYCLE613_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next75"], 6)
        self.assertEqual(prior["N_with_next75"], 12)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE907, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 613 I-only or the cycle 832–907 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 613)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_907)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE613_N_I_EACH)
        self.assertEqual(CYCLE613_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT74_ALL_EXTEND)
        for n74, n75, site74, site75 in zip(
            CYCLE907_SEQUENCES, STANDING_SEQUENCES, CYCLE907_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n75[:74], n74)
            self.assertEqual(site75, site74)
        still = self.rows[STANDING_NEXT74_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:74], CYCLE907_SEQUENCES[3])
        self.assertEqual(CYCLE907_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT75_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT75_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT75_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT75_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT75_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT75_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_615_next76_list(self):
        """Cycle 615 next-76 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE615_N, 18)
        self.assertEqual(CYCLE615_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE615_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 76 for tokens in CYCLE615_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next76_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle615_next76_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-908 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 908)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next75_sites"]),
            STANDING_NO_NEXT75_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_907"])
        ...
        self.assertEqual(
            tuple(lock["next74_hapax_still_hapax"]),
            STANDING_NEXT74_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next74_all_extend"])
        self.assertEqual(lock["next74_all_extend"], STANDING_NEXT74_ALL_EXTEND)
