"""I's leftover n=4 remaining remaining-after-057 next-stem lock.

Cycle 296 text-search lock. Uses already-vendored A–V and the
...
after-087 unique next stem (cycle 294 lost), or leftover n=4
remaining remaining-after-087 exactly 2 share next 057
(cycle 295). Does not vendor a new tablet. Does not scrape X.
...
Leftover n=4 remaining remaining-after-057 next stems are
not yet locked as unique-max. Cycle 294 unique-max G/K is
inventory for the 057 peel, not remaining-after-057
unique-max. Cycle 294 frequency of remaining-after-087
minus 057 (011×2, hapax 607/021) is nested inventory, not
this claim; measure remaining-after-057 from the 4 sites.
Cycle 258 leftover extra 090 076 057 extra I=2 equals
leftover n=4 remaining remaining-after-087 057 sites; do
not retune 258/295. Cycle 248 leftover extra 090 076 011
I-only 4/0 extra I=2 may equal leftover n=4 remaining
remaining-after-057 011 sites; do not retune 247/248. Do
not peel a specific remaining-after-057 stem this cycle.
Do not lock I-only of remaining-after-057 3-grams this
...
Leftover n=4 remaining remaining-after-057 = leftover n=4
remaining I 090 076 sites whose next token is none of 020,
087, or 057. For each such site, take the next token if any
...
K_057==2, N_remaining_after_057==4 (do not retune
224/288/289/292/293/294/295). Nested-check cycle 258 extra
I equals leftover n=4 remaining remaining-after-087 057
sites (do not retune 258/295). Count next-stem frequencies
among remaining-after-057 sites that have a next token.
G = the next stem with the highest remaining-after-057
with-next count. If a tie, pick the larger Barthel id.
K = that count.

Claim that can lose:
i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem.
True iff remaining-after-057 leftover n=4 remaining I
090 076 has a unique most frequent next stem G with K ≥ 2
(no tie at max K). This can lose the same way cycle 234
lost (7-way tie at 2) and cycle 294 lost (2-way tie at
K=2), or hold the same way cycle 292 held (G=087 K=3).
Unique-max G/K is inventory for a later peel if the claim
holds or loses with K≥2. Measured: N_remaining_after_057=4,
N_with_next=4, N_no_next=0, N_distinct=3, unique-max G=011
K=2 at Ia2[107]/Ia14[54]. The claim is true. Nested cycle
295 K_057=2 N_remaining=4 extra I overlap, cycle 294
unique-max false 2-way tie G=057 K=2, cycle 293 K_087=3,
cycle 292 unique-max G=087 K=3, cycle 248 4/0 extra I=2 of
leftover extra 011, cycle 224 13/56, and cycle 223 69/3
...
LOCKED_FORWARD_STEMS = ("020", "087", "057")
STANDING_N2 = 2
STANDING_N3 = 3
STANDING_N4 = 4
...
STANDING_TIED_STEMS = ("011",)
STANDING_G_UNIQUELY_MOST_FREQUENT = True
STANDING_REMAINING_SITES = CYCLE295_REMAINING_AFTER_057_SITES
...
STANDING_CYCLE294_REMAINING_AFTER_057_INVENTORY = (
    ("011", 2),
    ("607", 1),
    ("021", 1),
)
...
STANDING_CLAIM = "i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem"
STANDING_I_LEFTOVER_N4_REMAINING_090_076_REMAINING_AFTER_057_UNIQUE_NEXT_STEM = True
STANDING_RESULT = "i_leftover_n4_remaining_090_076_remaining_after_057_next_stem"
...
STANDING_I_ONLY_OF_REMAINING_AFTER_057_3GRAM_IS_NOT_THIS_CYCLE = True
STANDING_DO_NOT_PEEL_A_SPECIFIC_REMAINING_STEM = True
STANDING_DO_NOT_PEEL_011 = True
STANDING_CYCLE294_FREQUENCY_MINUS_057_IS_NESTED_INVENTORY = True
STANDING_CYCLE258_EXTRA_I_EQUALS_REMAINING_AFTER_087_057 = True
...
STANDING_CYCLE167_268_295_NOT_OVERWRITTEN = True
STANDING_G_K_IS_INVENTORY_FOR_LATER_PEEL = True


def leftover_n4_remaining_remaining_after_057(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover n=4 remaining sites whose next token is none of 020, 087, 057."""
...
def leftover_n4_remaining_remaining_after_057_next_stems(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[str, ...]:
    """Next stems of remaining-after-057 sites that have a next token."""
    locked_set = set(locked)
    return tuple(
        nxt
...
def leftover_n4_remaining_remaining_after_057_with_next(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Remaining-after-057 leftover n=4 remaining sites that have a next token."""
    remaining = leftover_n4_remaining_remaining_after_057(sites, next_stems, locked)
    rem_next = tuple(
        nxt
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_n4_remaining_remaining_after_057_without_next(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Remaining-after-057 leftover n=4 remaining sites with no next token."""
    remaining = leftover_n4_remaining_remaining_after_057(sites, next_stems, locked)
    rem_next = tuple(
        nxt
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_n4_remaining_remaining_after_057_with_g(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover n=4 remaining remaining-after-057 sites whose next token is G."""
    remaining = set(leftover_n4_remaining_remaining_after_057(sites, next_stems, locked))
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_n4_remaining_remaining_after_057_without_g(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover n=4 remaining remaining-after-057 sites whose next token is not G."""
    locked_set = set(locked)
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def remaining_after_057_next_stem_counts(next_stems: tuple[str, ...]) -> Counter:
    """Counts of next stems among leftover n=4 remaining remaining-after-057 with-next."""
    return Counter(next_stems)


def rank_remaining_after_057_next_stems(
    counts: Counter,
) -> tuple[tuple[str, int], ...]:
    """Remaining-after-057 next stems by count, then larger Barthel id."""
    return tuple(
        sorted(
            counts.items(),
...
def select_remaining_after_057_g(
    remaining_stems: tuple[str, ...],
) -> tuple[str | None, int, bool]:
    """Return (G, K, uniquely_most_frequent). Empty remaining-after-057 has no G."""
    ranked = rank_remaining_after_057_next_stems(
        remaining_after_057_next_stem_counts(remaining_stems)
    )
    if not ranked:
        return (None, 0, False)
    gram, count = ranked[0]
...
def remaining_after_057_next_stem_frequency_table(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    ...
    """Remaining-after-057 next-stem frequency: highest count first, then larger id."""
    rem_sites = leftover_n4_remaining_remaining_after_057_with_next(
        sites, next_stems, locked
    )
    rem_stems = leftover_n4_remaining_remaining_after_057_next_stems(
        sites, next_stems, locked
    )
    locked_set = set(locked)
    rem_grams = tuple(
...
def remaining_after_057_next_stem_frequency_rows(
    table: tuple[
        tuple[str, int, tuple[tuple[str, str, int], ...], tuple[tuple[str, ...], ...]],
        ...,
    ] = STANDING_REMAINING_FREQUENCY,
) -> list[dict]:
    """Survey-shaped remaining-after-057 next-stem frequency table."""
    rows = []
        ...
        rows.append(
            {
                "next_stem": stem,
                "count": count,
                "leftover_n4_remaining_remaining_after_057_sites": [
...
def leftover_n4_remaining_remaining_after_057_nested_counts_hold(
    n_inside: int,
    k_020: int,
    ...
    expected_remaining: int = STANDING_N_REMAINING_AFTER_057,
) -> bool:
    ...
    return (
        ...
        and n_remaining == n_remaining_after_087 - k_057
    )


def i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem(
    inside_sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> bool:
    """True iff remaining-after-057 has a unique most frequent next stem with K ≥ 2."""
    remaining = leftover_n4_remaining_remaining_after_057(
        inside_sites,
        next_stems,
        locked,
    )
    remaining_stems = leftover_n4_remaining_remaining_after_057_next_stems(
        inside_sites,
        next_stems,
        locked,
    )
    if len(remaining) != STANDING_N_REMAINING_AFTER_057:
        return False
    if remaining != leftover_n4_remaining_remaining_after_087_without_forward_057(
        inside_sites,
        next_stems,
    ):
        return False
    gram, count, unique = select_remaining_after_057_g(remaining_stems)
    return bool(unique and gram is not None and count >= 2)


def matching_leftover_n4_remaining_remaining_after_057_local_4gram_rows(
    leftover_sites: tuple[tuple[str, str, int], ...] = STANDING_MATCHING_SITES,
    next_4grams: tuple[tuple[str, ...], ...] = STANDING_MATCHING_NEXT_4GRAMS,
) -> list[dict]:
    """Survey-shaped matching leftover n=4 remaining remaining-after-057 next-4-gram rows."""
    rows = []
...
def matching_equals_cycle248_extra_i(
    matching_sites: tuple[tuple[str, str, int], ...],
    extra_i: tuple[tuple[str, str, int], ...] = CYCLE248_EXTRA_I_SITES,
) -> bool:
    """True iff remaining-after-057 next-011 sites equal cycle-248 extra I of 011."""
    return matching_sites == extra_i


class TestILeftoverN4Remaining090076RemainingAfter057NextStemHelpers(unittest.TestCase):
    """Helpers on leftover n=4 remaining remaining-after-057 next stems. No CV, no LLM."""

    def test_remaining_after_057_requires_next_none_of_020_087_057(self):
        """Remaining-after-057 excludes next 020, 087, and 057; line-final is remaining, no-next."""
        ...
        planted_stems = ("011", "020", None, "607", "087", "057")
        rem = leftover_n4_remaining_remaining_after_057(planted_sites, planted_stems)
        ...
        self.assertTrue(STANDING_CYCLE294_FREQUENCY_MINUS_057_IS_NESTED_INVENTORY)
        self.assertEqual(provider.get_call_history(), [])

    def test_unique_max_can_fail(self):
        """Boolean is True only when remaining=4 and some G has unique K≥2."""
        provider = MockProvider()
        ...
        rem = leftover_n4_remaining_remaining_after_057(inside, stems)
        rem_stems = leftover_n4_remaining_remaining_after_057_next_stems(inside, stems)
        self.assertEqual(len(rem), STANDING_N_REMAINING_AFTER_057)
        ...
        g, k, unique = select_remaining_after_057_g(rem_stems)
        self.assertEqual(g, "011")
        ...
        # Remaining-after-057 already has 011×2, hapax 607/021. Promote 607
        # to 021 so unique-max false 2-way tie at K=2 (cycle-294 lose shape).
        promote = {(SIDE_IA, "Ia8", 106): "021"}
        for i, site in enumerate(inside):
            if site in promote:
                tie_stems[i] = promote[site]
        tied = tuple(tie_stems)
        tied_g, tied_k, tied_unique = select_remaining_after_057_g(
            leftover_n4_remaining_remaining_after_057_next_stems(inside, tied)
        )
        self.assertEqual(tied_g, "021")
        self.assertEqual(tied_k, 2)
        self.assertFalse(tied_unique)
        self.assertEqual(
            len(leftover_n4_remaining_remaining_after_057(inside, tied)),
            4,
        )
        self.assertFalse(
            i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem(
        ...
        hap_g, hap_k, hap_unique = select_remaining_after_057_g(
            leftover_n4_remaining_remaining_after_057_next_stems(inside, hapax)
        )
        self.assertEqual(hap_k, 1)
        self.assertFalse(hap_unique)
        self.assertEqual(
            len(leftover_n4_remaining_remaining_after_057(inside, hapax)),
            4,
        )
        self.assertFalse(
            i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem(
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_N4_REMAINING_090_076_REMAINING_AFTER_057_UNIQUE_NEXT_STEM
        )
        self.assertEqual(STANDING_K + STANDING_N_WITHOUT_G, STANDING_N_REMAINING_AFTER_057)
        ...
        self.assertTrue(STANDING_I_ONLY_OF_REMAINING_AFTER_057_3GRAM_IS_NOT_THIS_CYCLE)
        self.assertEqual(provider.get_call_history(), [])

    def test_tie_break_picks_larger_barthel_id(self):
        """Equal remaining-after-057 counts pick the larger Barthel id."""
        provider = MockProvider()
        counts = Counter({"011": 2, "607": 1, "021": 1})
        ranked = rank_remaining_after_057_next_stems(counts)
        self.assertEqual(ranked[0], ("011", 2))
        self.assertEqual(ranked[1], ("607", 1))
        self.assertEqual(ranked[2], ("021", 1))
        self.assertEqual(select_remaining_after_057_g(("011", "021", "011", "021"))[0], "021")
        self.assertFalse(select_remaining_after_057_g(("011", "021", "011", "021"))[2])
        self.assertEqual(select_remaining_after_057_g(("011", "011", "021"))[0], "011")
        self.assertTrue(select_remaining_after_057_g(("011", "011", "021"))[2])
        self.assertEqual(select_remaining_after_057_g(()), (None, 0, False))
...
class TestMamariILeftoverN4Remaining090076RemainingAfter057NextStemScoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover n=4 remaining remaining-after-057 next-stem lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.matching = leftover_n4_remaining_remaining_after_057_with_g(
            self.inside_sites,
            self.next_stems,
        )
        self.without = leftover_n4_remaining_remaining_after_057_without_g(
            self.inside_sites,
            self.next_stems,
        )
        self.frequency = remaining_after_057_next_stem_frequency_table(
            self.inside_sites,
            self.next_stems,
            self.forwards,
        )
        ...
        self.g, self.k, self.unique = select_remaining_after_057_g(self.remaining_stems)
    ...
    def test_tokens_and_nested_leftover_n4_remaining_13_4_9_3_6_2_4_not_retuned(self):
        ...
        unused_294_freq = CYCLE294_REMAINING_FREQUENCY
        self.assertEqual(unused_294_freq[0][0], "057")
        ...
        unused_295_match = CYCLE295_MATCHING_SITES
        self.assertEqual(unused_295_match, CYCLE294_MATCHING_SITES)
        self.assertTrue(CYCLE295_EQUALS_258)
        ...
        unused_259 = CYCLE259_EXTRA_I_BY_X["057"]
        self.assertEqual(unused_259, CYCLE258_EXTRA_I_057)
        ...
        self.assertTrue(STANDING_CYCLE167_268_295_NOT_OVERWRITTEN)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_counts_4_remaining_g_011_k_2_and_hypothesis_holds(self):
        """N_remaining=4, N_with_next=4, N_distinct=3, unique-max G=011 K=2. Claim holds."""
        ...
        self.assertTrue(STANDING_CYCLE258_EXTRA_I_EQUALS_REMAINING_AFTER_087_057)
        self.assertTrue(
            leftover_n4_remaining_remaining_after_020_nested_counts_hold(
                self.n_inside,
        ...
        self.assertEqual(
            self.n_remaining,
            self.n_inside - self.k_020 - self.k_087 - self.k_057,
        )
        self.assertEqual(13 - 4 - 3 - 2, 4)
        if self.n_remaining != 4:
            self.fail("measured N_remaining_after_057 drifted from 4")
        if self.n_remaining != self.n_remaining_after_087 - self.k_057:
            self.fail(
                "leftover n=4 remaining remaining-after-057 filter disagrees with nested 6−2"
            )
        ...
        for site in self.share_057:
            self.assertNotIn(site, self.remaining)
            self.assertIn(site, self.remaining_after_087)
        ...
        self.assertTrue(STANDING_G_UNIQUELY_MOST_FREQUENT)
        self.assertEqual(self.frequency[0][0], "011")
        ...
        self.assertEqual(2 + 2, 4)
        cycle294_remaining_after_057_inventory = tuple(
            (stem, count)
            for stem, count, _sites, _grams in CYCLE294_REMAINING_FREQUENCY
            if stem != "057"
        )
        self.assertEqual(
            cycle294_remaining_after_057_inventory,
            STANDING_CYCLE294_REMAINING_AFTER_057_INVENTORY,
        )
        self.assertEqual(
            set(cycle294_remaining_after_057_inventory),
    ...
    def test_matching_leftover_n4_remaining_remaining_after_057_sites_share_011(self):
        """Two leftover n=4 remaining remaining-after-057 sites are 090 076 011."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
    ...
    def test_existing_295_294_293_292_248_224_223_still_compute(self):
        """Cycle 295 K_057=2 N=4 extra I overlap, 294 unique-max false, 293 K_087=3, 292 G=087 K=3, 248 4/0 extra I=2, 224 13/56, 223 69/3 stay."""
        ...
        self.assertEqual(self.remaining_after_087, prior_293.without)
        ...
        self.assertEqual(self.matching, prior_248.extra)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-296 leftover n=4 remaining remaining-after-057 lock."""
        lock = self.survey["i_leftover_n4_remaining_090_076_remaining_after_057_next_stem"]
        self.assertEqual(lock["cycle"], 296)
        ...
        self.assertEqual(
            tuple(lock["remaining_after_057_next_stems"]),
            STANDING_REMAINING_NEXT_STEMS,
        )
        ...
        self.assertEqual(
            lock["remaining_after_057_next_stem_frequency"],
            remaining_after_057_next_stem_frequency_rows(),
        )
        self.assertEqual(
            [list(pair) for pair in STANDING_CYCLE294_REMAINING_AFTER_057_INVENTORY],
            lock["cycle294_remaining_after_057_inventory"],
        )
        ...
        self.assertTrue(lock["cycle258_extra_i_equals_remaining_after_087_057"])
        ...
        self.assertTrue(lock["i_leftover_n4_remaining_090_076_remaining_after_057_unique_next_stem"])
        ...
        self.assertTrue(lock["i_only_of_remaining_after_057_3gram_is_not_this_cycle"])
        self.assertTrue(lock["do_not_peel_a_specific_remaining_stem"])
        self.assertTrue(lock["do_not_peel_011"])
        self.assertTrue(lock["cycle294_frequency_minus_057_is_nested_inventory"])
        self.assertTrue(lock["cycle258_extra_i_equals_remaining_after_087_057"])
        self.assertTrue(lock["cycle248_extra_i_equals_remaining_after_057_011"])
        ...
        self.assertTrue(lock["cycle167_268_295_not_overwritten"])
        ...
        self.assertTrue(
            self.survey["i_leftover_n4_remaining_090_076_remaining_after_087_forward_057"][
...
class TestMamariILeftoverN4Remaining090076RemainingAfter057NextStemImageSnapshot(
    unittest.TestCase
):
    """Cycle 296 does not touch clustering. 83/62 / Hamming 6 stays."""
