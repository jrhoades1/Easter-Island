"""Reframed closed-tradition hold-out of the locked leftover-6 next-12 grams.

Cycle 845 text-search lock. Uses already-vendored A–V and the
cycle-487 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 12-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 487 already locked the I-only claim
on these 12-grams as incomplete-set LOSE (N_with_next12=15,
N_no_next12=3, N_i_only=15, N_leak=0). Cycle 844 already locked
the reframed claim on the cycle-485 next-11 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–844.

Claim that can lose: each existing next-12 gram is an exact
...
Measured on the 15 with-next12 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next12=15,
N_no_next12=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-11 hapax 604 076 071 600 999 050 076 000
002 999 076 extends to 604 076 071 600 999 050 076 000 002 999
076 092 at Ia9[9] and hits I once. All fifteen next-11 grams
extend; no new hole. Verdict: incomplete-set LOSE.
...
next 12-grams as a peel, next 13-grams as a peel, prev-184, or
next-185. The cycle-489 next-13 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_next11_closed_tradition_hpq_abc_scoreboard import (
    ABSENT_TABLETS,
    CYCLE485_NO_NEXT11_SITES,
    CYCLE487_ALL_I_ONLY_CLAIM,
    CYCLE487_HYPOTHESIS_ALL_I_ONLY,
    CYCLE487_N,
    CYCLE487_N_I_ONLY,
    CYCLE487_N_LEAK,
    CYCLE487_N_NO_NEXT12,
    CYCLE487_N_SEQUENCES,
    CYCLE487_N_WITH_NEXT12,
    ...
    MIN_HPQ_TABLETS,
    PROBE_TABLETS,
    STANDING_HITS_I as CYCLE844_HITS_I,
    STANDING_N_HOLD as CYCLE844_N_HOLD,
    STANDING_N_LEAK as CYCLE844_N_LEAK,
    STANDING_NEXT_CHEAP_LOCK as CYCLE844_NEXT_CHEAP_LOCK,
    STANDING_NO_NEXT11_SITES as CYCLE844_NO_NEXT11_SITES,
    STANDING_RESULT as CYCLE844_RESULT,
    STANDING_SEQUENCES as CYCLE844_SEQUENCES,
    STANDING_SITES as CYCLE844_SITES,
    STANDING_VERDICT as CYCLE844_VERDICT,
    absent_hit_sum,
    hpq_tablets_hit,
    reframed_verdict,
    row_holds,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next12_i_only_scoreboard import (
    STANDING_LEFTOVER_MATCHING_NEXT12_SITES as CYCLE487_SITES,
    STANDING_N_I_EACH as CYCLE487_N_I_EACH,
)
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next13_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE489_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT13_I_ONLY as CYCLE489_ALL_I_ONLY_CLAIM,
    ...
    STANDING_N_SEQUENCES as CYCLE489_N_SEQUENCES,
    STANDING_N_WITH_NEXT13 as CYCLE489_N_WITH_NEXT13,
    STANDING_NO_NEXT13_SITES as CYCLE489_NO_NEXT13_SITES,
    STANDING_RESULT as CYCLE489_RESULT,
...
STANDING_NO_NEXT12_SITES = CYCLE487_NO_NEXT12_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE487_SITES
STANDING_SEQUENCES = CYCLE487_SEQUENCES
...
STANDING_NEXT11_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT11_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next12_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next12_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 487
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 487
...
STANDING_DO_NOT_RELOCK_CYCLES_832_844 = True
...
STANDING_NAMED_BY_CYCLE844 = (
    "cycle487_next12_reframed_closed_tradition_hpq_ge1_exact0_abc"
)
STANDING_NEXT_CHEAP_LOCK = (
    "cycle489_next13_reframed_closed_tradition_hpq_ge1_exact0_abc"
)
...
class TestMamariILeftoverN6Next12ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 845 reframed closed-tradition lock on the cycle-487 next-12 population."""
    ...
    def test_population_is_the_locked_cycle_487_next12_grams(self):
        """Fifteen next-12 sequences and sites come from cycle 487. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE487_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE487_SITES)
        self.assertEqual(CYCLE487_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE487_N, 18)
        self.assertEqual(CYCLE487_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT12, CYCLE487_N_WITH_NEXT12)
        self.assertEqual(STANDING_N_NO_NEXT12, CYCLE487_N_NO_NEXT12)
        self.assertEqual(
            STANDING_NO_NEXT12_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT12_SITES, CYCLE844_NO_NEXT11_SITES)
        self.assertEqual(STANDING_NO_NEXT12_SITES, CYCLE485_NO_NEXT11_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 487)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 12)
        prior = self.survey[CYCLE487_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_12grams_all_i_only"
        )
        self.assertEqual(CYCLE487_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next12_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE844, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 487 I-only or the cycle 832–844 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 487)
        ...
        next11 = self.survey[CYCLE844_RESULT]
        self.assertEqual(next11["cycle"], STANDING_NEXT11_REFRAMED_CYCLE)
        self.assertEqual(next11["verdict"], CYCLE844_VERDICT)
        self.assertEqual(next11["verdict"], "incomplete-set LOSE")
        self.assertEqual(next11["N_hold"], CYCLE844_N_HOLD)
        self.assertEqual(next11["N_hold"], 0)
        self.assertEqual(next11["N_leak"], CYCLE844_N_LEAK)
        self.assertEqual(next11["N_leak"], 0)
        self.assertEqual(next11["hits_I"], CYCLE844_HITS_I)
        self.assertEqual(next11["hits_I"], 15)
        self.assertEqual(next11["N_hpq_ge1"], 0)
        self.assertEqual(next11["N_exact0_abc"], 15)
        self.assertEqual(next11["N_with_next11"], 15)
        self.assertEqual(next11["N_no_next11"], 3)
        self.assertEqual(next11["next_cheap_lock"], STANDING_NAMED_BY_CYCLE844)
        self.assertEqual(next11["next_cheap_lock"], CYCLE844_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE844, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_844)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE487_N_I_EACH)
        self.assertEqual(CYCLE487_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT11_ALL_EXTEND)
        for n11, n12, site11, site12 in zip(
            CYCLE844_SEQUENCES, STANDING_SEQUENCES, CYCLE844_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n12[:11], n11)
            self.assertEqual(site12, site11)
        still = self.rows[STANDING_NEXT11_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            ("604", "076", "071", "600", "999", "050", "076", "000", "002", "999", "076", "092"),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:11], CYCLE844_SEQUENCES[3])
        self.assertEqual(CYCLE844_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT12_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_489_next13_list(self):
        """Cycle 489 next-13 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE489_N, 18)
        self.assertEqual(CYCLE489_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE489_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 13 for tokens in CYCLE489_SEQUENCES))
        ...
        prior = self.survey[CYCLE489_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_13grams_all_i_only"
        )
        self.assertEqual(prior["cycle"], 489)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 15)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE489_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next13"], 3)
        self.assertEqual(prior["N_with_next13"], 15)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-845 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 845)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next12_sites"]),
            STANDING_NO_NEXT12_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_844"])
        ...
        self.assertEqual(lock["named_by_cycle844"], STANDING_NAMED_BY_CYCLE844)
        ...
        self.assertEqual(
            tuple(lock["next11_hapax_still_hapax"]),
            STANDING_NEXT11_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next11_all_extend"])
        self.assertEqual(lock["next11_all_extend"], STANDING_NEXT11_ALL_EXTEND)
