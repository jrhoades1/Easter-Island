"""I's cycle-173 leftover 3-gram 076 071 076 off-I lock.

Cycle 174 text-search lock. Uses already-vendored A–V and the
cycle-173 leftover forward 3-gram 076 071 076 (the n=3 run
shared by leftover 076 071 sites Ia2[43] 076 071 076 021,
Ia13[149] 076 071 076 090, Ia13[153] 076 071 076 430,
Ia14[81] 076 071 076 010, and Ia14[166] 076 071 076 011).
...
was I-only 8/0). This cycle is the new leftover 3-gram
076 071 076 only. 071 999 (071 065 071 999) does not
count. 076 076 (700 076 076 053) does not. Do not retune
076 071, 999 090 076, 076 020 010, or 076 010 079. Cycle
173 leftover N=5 and leftover n=4 set stay. Do not assume
the I-only result.

Locks exact consecutive hits of 076 071 076 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_076_071_076_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
6 at Ia2[43]/Ia3[81]/Ia13[149]/Ia13[153]/Ia14[81]/
Ia14[166] (five leftover matching sites plus inside-family
Ia3[81] 076 071 076 385); Ib unpublished 0; every other
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE173_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 5
STANDING_INSIDE_FAMILY_SITE = STANDING_INSIDE_FORWARD_076_SITE
STANDING_INSIDE_FAMILY_COUNT = 1
STANDING_INSIDE_FAMILY_NEXT_4GRAM = STANDING_INSIDE_FORWARD_076_NEXT_4GRAM
...
STANDING_CLAIM = "i_3gram_076_071_076_i_only"
STANDING_I_3GRAM_076_071_076_I_ONLY = True
STANDING_RESULT = "i_3gram_076_071_076_i_only"
...
def i_3gram_076_071_076_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
class TestI3gram076071076IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-173 leftover 3-gram tokens. No CV, no LLM."""
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_076_071_076_i_only(1, 0))
        self.assertTrue(i_3gram_076_071_076_i_only(6, 0))
        self.assertFalse(i_3gram_076_071_076_i_only(1, 1))
        self.assertFalse(i_3gram_076_071_076_i_only(0, 0))
        self.assertFalse(i_3gram_076_071_076_i_only(0, 6))
        self.assertEqual(STANDING_CLAIM, "i_3gram_076_071_076_i_only")
        self.assertTrue(STANDING_I_3GRAM_076_071_076_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_076_071_076_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle173_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-173 leftover, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_FORWARD)
...
class TestMamariI3gram076071076IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 076 071 076 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_076_071_076_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_173_leftover_not_retuned(self):
        """3-gram is the cycle-173 leftover lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3, ("076", "071", "076"))
        prior_173 = self.survey["i_leftover_076_071_forward_076"]
        self.assertEqual(prior_173["cycle"], 173)
        self.assertEqual(tuple(prior_173["forward_3gram"]), GRAM3)
        self.assertEqual(prior_173["N_with_forward_076_071_076"], 5)
        self.assertEqual(prior_173["N_without"], 29)
        self.assertEqual(prior_173["N_leftover"], 34)
        measured_matching = [list(site) for site in CYCLE173_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_173["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_173["i_leftover_076_071_exactly_5_forward_076_071_076"])
        self.assertTrue(prior_173["071_999_does_not_count"])
        self.assertTrue(prior_173["076_076_does_not_count"])
        self.assertNotEqual(GRAM3, GRAM5)
        self.assertNotEqual(GRAM3, CYCLE171_GRAM2)
        self.assertNotEqual(GRAM3, CYCLE167_GRAM3)
        self.assertNotEqual(GRAM3, CYCLE160_GRAM3)
    ...
    def test_i_hits_are_six_on_ia(self):
        """3-gram is 6 on Ia (5 leftover + Ia3[81]); Ib unpublished 0."""
        ...
        self.assertEqual(inside_in_i, (STANDING_INSIDE_FAMILY_SITE,))
        self.assertEqual(len(inside_in_i), STANDING_INSIDE_FAMILY_COUNT)
        self.assertEqual(STANDING_INSIDE_FAMILY_COUNT, 1)
        self.assertTrue(STANDING_INSIDE_FAMILY_SITE_INCLUDED)
        self.assertIn(STANDING_INSIDE_FAMILY_SITE, STANDING_I_SITES)
        self.assertNotIn(STANDING_INSIDE_FAMILY_SITE, STANDING_LEFTOVER_SITES)
        ...
        self.assertEqual(
            STANDING_I_SITES[:3],
            ((SIDE_IA, "Ia2", 43), (SIDE_IA, "Ia3", 81), (SIDE_IA, "Ia13", 149)),
        )
        self.assertEqual(
            STANDING_I_SITES[5],
            (SIDE_IA, "Ia14", 166),
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_076_071_076_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_076_071_076_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_173_172_171_170_167_160_141_103_and_w_scoreboards_still_compute(self):
        """Cycle 173 leftover 5, 172/171/170, 167/160/141 I-only, 103, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-174 3-gram I-only lock."""
        lock = self.survey["i_3gram_076_071_076_i_only"]
        self.assertEqual(lock["cycle"], 174)
        ...
        self.assertTrue(lock["i_3gram_076_071_076_i_only"])
        self.assertEqual(
            lock["i_3gram_076_071_076_i_only"],
            STANDING_I_3GRAM_076_071_076_I_ONLY,
        )
...
class TestMamariI3gram076071076IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 174 does not touch clustering. 83/62 / Hamming 6 stays."""
