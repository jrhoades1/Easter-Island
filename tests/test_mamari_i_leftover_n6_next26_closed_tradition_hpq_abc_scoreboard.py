"""Reframed closed-tradition hold-out of the locked leftover-6 next-26 grams.

Cycle 859 text-search lock. Uses already-vendored A–V and the
cycle-515 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 26-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 515 already locked the I-only claim
on these 26-grams as incomplete-set LOSE (N_with_next26=15,
N_no_next26=3, N_i_only=15, N_leak=0). Cycle 858 already locked
the reframed claim on the cycle-513 next-25 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–858.

Claim that can lose: each existing next-26 gram is an exact
...
Named claim: cycle515_next26_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next26 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next26=15,
N_no_next26=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-25 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700 076 extends to
604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076
532 244 999 090 076 057 600 700 076 076 at Ia9[9] and hits I once. All fifteen
next-25 grams extend; no new hole. Verdict: incomplete-set LOSE.
...
next 26-grams as a peel, next 27-grams as a peel, prev-184, or
next-185. The cycle-517 next-27 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next26_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE515_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT26_I_ONLY as CYCLE515_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT26_SITES as CYCLE515_SITES,
    STANDING_N as CYCLE515_N,
    STANDING_N_I_EACH as CYCLE515_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE515_N_I_ONLY,
    STANDING_N_LEAK as CYCLE515_N_LEAK,
    STANDING_N_NO_NEXT26 as CYCLE515_N_NO_NEXT26,
    STANDING_N_SEQUENCES as CYCLE515_N_SEQUENCES,
    STANDING_N_WITH_NEXT26 as CYCLE515_N_WITH_NEXT26,
    STANDING_NO_NEXT26_SITES as CYCLE515_NO_NEXT26_SITES,
    STANDING_RESULT as CYCLE515_RESULT,
    STANDING_SEQUENCES as CYCLE515_SEQUENCES,
)
...
STANDING_NO_NEXT26_SITES = CYCLE515_NO_NEXT26_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE515_SITES
STANDING_SEQUENCES = CYCLE515_SEQUENCES
...
STANDING_NEXT25_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT25_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next26_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next26_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 515
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 515
...
STANDING_DO_NOT_RELOCK_CYCLES_832_858 = True
...
class TestMamariILeftoverN6Next26ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 859 reframed closed-tradition lock on the cycle-515 next-26 population."""
    ...
    def test_population_is_the_locked_cycle_515_next26_grams(self):
        """Fifteen next-26 sequences and sites come from cycle 515. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE515_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE515_SITES)
        self.assertEqual(CYCLE515_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE515_N, 18)
        self.assertEqual(CYCLE515_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT26, CYCLE515_N_WITH_NEXT26)
        self.assertEqual(STANDING_N_NO_NEXT26, CYCLE515_N_NO_NEXT26)
        self.assertEqual(
            STANDING_NO_NEXT26_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT26_SITES, CYCLE858_NO_NEXT25_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE515_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE515_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE515_N_I_ONLY, 15)
        self.assertEqual(CYCLE515_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 515)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 26)
        prior = self.survey[CYCLE515_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_26grams_all_i_only"
        )
        self.assertEqual(CYCLE515_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next26_i_only"
        ))
        self.assertEqual(prior["cycle"], 515)
        ...
        self.assertFalse(prior[CYCLE515_RESULT])
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 515 I-only or the cycle 832–858 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 515)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_858)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE515_N_I_EACH)
        self.assertEqual(CYCLE515_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT25_ALL_EXTEND)
        for n25, n26, site25, site26 in zip(
            CYCLE858_SEQUENCES, STANDING_SEQUENCES, CYCLE858_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n26[:25], n25)
            self.assertEqual(site26, site25)
        still = self.rows[STANDING_NEXT25_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:25], CYCLE858_SEQUENCES[3])
        self.assertEqual(CYCLE858_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT26_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_517_next27_list(self):
        """Cycle 517 next-27 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE517_N, 18)
        self.assertEqual(CYCLE517_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE517_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 27 for tokens in CYCLE517_SEQUENCES))
        ...
        self.assertEqual(CYCLE517_N_WITH_NEXT27, 15)
        self.assertEqual(CYCLE517_N_NO_NEXT27, 3)
        self.assertEqual(
            CYCLE517_NO_NEXT27_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        ...
        self.assertNotIn("i_leftover_n6_next27_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle517_next27_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-859 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 859)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next26_sites"]),
            STANDING_NO_NEXT26_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_858"])
        ...
        self.assertEqual(lock["named_by_cycle858"], STANDING_NAMED_BY_CYCLE858)
        ...
        self.assertEqual(
            tuple(lock["next25_hapax_still_hapax"]),
            STANDING_NEXT25_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next25_all_extend"])
        self.assertEqual(lock["next25_all_extend"], STANDING_NEXT25_ALL_EXTEND)
