"""Reframed closed-tradition hold-out of the locked leftover-6 next-7 grams.

Cycle 840 text-search lock. Uses already-vendored A–V and the
cycle-477 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 7-grams (16 sequences; parent N=18; two
line-final holes at Ia8[167] and Ia8[166]). HEAD-check: that
gram list exists and is not yet locked under this reframed
rule. Cycle 477 already locked the I-only claim on these
7-grams as incomplete-set LOSE (N_with_next7=16, N_no_next7=2,
N_i_only=16, N_leak=0). Cycle 839 already locked the reframed
claim on the cycle-475 next-6 grams as incomplete-set LOSE.
This cycle does not re-lock either claim, and does not re-lock
Cycles 832–839.

Claim that can lose: each existing next-7 gram is an exact
...
Measured on the 16 with-next7 grams: N=16, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=16, N_i_ge1=16, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=16. Parent N=18, N_with_next7=16,
N_no_next7=2 (Ia8[167], Ia8[166]). Every scored gram is
exact-0 on H/P/Q and on A/B/C and an exact contiguous hit once
on I. The next-6 hapax grams stay hapax once extended: 604 076
071 600 999 050 076 at Ia9[9] hits I once; 720 076 070 701 214
076 298 at Ia8[171] hits I once. Verdict: incomplete-set LOSE.
...
Do not launch leftover-6 previous 7-grams, leftover 7-grams,
next 8-grams, prev-184, or next-185. The cycle-479 next-8
...
from tests.test_mamari_i_leftover_n6_next6_closed_tradition_hpq_abc_scoreboard import (
    ...
    STANDING_NEXT_CHEAP_LOCK as CYCLE839_NEXT_CHEAP_LOCK,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next7_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE477_HYPOTHESIS_ALL_I_ONLY,
    STANDING_CLAIM as CYCLE477_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT7_SITES as CYCLE477_SITES,
    STANDING_N as CYCLE477_N,
    STANDING_N_I_EACH as CYCLE477_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE477_N_I_ONLY,
    STANDING_N_LEAK as CYCLE477_N_LEAK,
    STANDING_N_NO_NEXT7 as CYCLE477_N_NO_NEXT7,
    STANDING_N_SEQUENCES as CYCLE477_N_SEQUENCES,
    STANDING_N_WITH_NEXT7 as CYCLE477_N_WITH_NEXT7,
    STANDING_NO_NEXT7_SITES as CYCLE477_NO_NEXT7_SITES,
    STANDING_RESULT as CYCLE477_RESULT,
...
STANDING_NEXT6_HAPAX_STILL_HAPAX = (3, 12)
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next7_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next7_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 477
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 477
...
class TestMamariILeftoverN6Next7ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 840 reframed closed-tradition lock on the cycle-477 next-7 population."""
    ...
    def test_population_is_the_locked_cycle_477_next7_grams(self):
        """Sixteen next-7 sequences and sites come from cycle 477. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE477_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE477_SITES)
        self.assertEqual(CYCLE477_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE477_N, 18)
        self.assertEqual(CYCLE477_N_SEQUENCES, 16)
        self.assertEqual(STANDING_N_WITH_NEXT7, CYCLE477_N_WITH_NEXT7)
        self.assertEqual(STANDING_N_NO_NEXT7, CYCLE477_N_NO_NEXT7)
        self.assertEqual(
            STANDING_NO_NEXT7_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166)),
        )
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE477_HYPOTHESIS_ALL_I_ONLY)
        self.assertEqual(CYCLE477_N_I_ONLY, 16)
        self.assertEqual(CYCLE477_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 477)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 7)
        prior = self.survey[CYCLE477_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_7grams_all_i_only"
        )
        self.assertEqual(CYCLE477_CLAIM, all_i_only_key)
        self.assertEqual(prior["cycle"], 477)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 16)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE477_RESULT])
        self.assertEqual(prior["N_i_only"], 16)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next7"], 2)
        self.assertEqual(prior["N_with_next7"], 16)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE839, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 477 I-only or the cycle 832–839 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 477)
        ...
        self.assertEqual(next6["N_exact0_abc"], 17)
        self.assertEqual(next6["N_with_next6"], 17)
        self.assertEqual(next6["N_no_next6"], 1)
        self.assertEqual(next6["next_cheap_lock"], STANDING_NAMED_BY_CYCLE839)
        self.assertEqual(next6["next_cheap_lock"], CYCLE839_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE839, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_839)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_measured_table_is_incomplete_set_lose(self):
        """H/P/Q and A/B/C are exact-0. hits_I is 16 and is not a leak."""
        ...
        self.assertEqual(self.summary["hits_I"], 16)
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE477_N_I_EACH)
        self.assertEqual(CYCLE477_N_I_EACH, (1,) * STANDING_N)
        ...
        once_a = self.rows[STANDING_NEXT6_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(once_a.tokens, ("604", "076", "071", "600", "999", "050", "076"))
        self.assertEqual(once_a.site, ("Ia", "Ia9", 9))
        self.assertEqual(once_a.as_map()["I"], 1)
        once_b = self.rows[STANDING_NEXT6_HAPAX_STILL_HAPAX[1]]
        self.assertEqual(once_b.tokens, ("720", "076", "070", "701", "214", "076", "298"))
    ...
    def test_window_peels_closed_and_prev1_list_is_absent(self):
        ...
        self.assertTrue(STANDING_DO_NOT_LAUNCH_PREV7)
        self.assertTrue(STANDING_DO_NOT_LAUNCH_LEFTOVER_7GRAMS)
        self.assertTrue(STANDING_DO_NOT_LAUNCH_NEXT8)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_479_next8_list(self):
        """Cycle 479 next-8 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE479_N, 18)
        self.assertEqual(CYCLE479_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE479_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 8 for tokens in CYCLE479_SEQUENCES))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-840 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 840)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next7_sites"]),
            STANDING_NO_NEXT7_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_839"])
        ...
        self.assertEqual(lock["named_by_cycle839"], STANDING_NAMED_BY_CYCLE839)
        ...
        self.assertEqual(
            tuple(lock["next6_hapax_still_hapax"]),
            STANDING_NEXT6_HAPAX_STILL_HAPAX,
        )
        ...
        self.assertEqual(len(lock["rows"]), 16)
