"""I's cycle-242 090 076 280 forward-4-grams off-I lock.

Cycle 243 text-search lock. Uses already-vendored A–V and the
cycle-242 I sites of 3-gram 090 076 280 (all 2 I sites; each
has a next stem). Does not retune those 4-grams, the 2 I
sites, leftover extra remaining-after-530 G=280 K=2
N_remaining6=29, leftover extra remaining-after-700 530,
leftover extra remaining-after-001 700, leftover extra
...
leftover extra sites, the leftover n=4 set, or the already-closed
...
1/0 x2), and cycle 240 (I 090 076 530 forward 4-grams all
I-only hapax 1/0 x2). Cycle 242 I 090 076 280 I-only holds
2/0, cycle 241 leftover extra remaining-after-530 K=2 / G=280
N_remaining6=29, cycle 240 2/0 hapax, cycle 239 2/0, cycle
238 K=2 / G=530, cycle 237 2/0 hapax, cycle 236 2/0, cycle
234 7-way tie at 2, cycle 233 3/0 hapax, cycle 232 3/0, cycle
223 69/3, cycle 195 6/0, and cycle 171 43/0 stay. Off-I T
sites of 090 076 are not this 3-gram and are not these
4-grams. Leftover extra remaining after 280 is not locked
this cycle. Previous 4-grams of the two I sites are not
locked this cycle. The other four tied stems
(087/011/005/000) are not locked. 090 076 530, 090 076 700,
090 076 001, 090 076 013, 090 076 070, 090 076 071, and
090 076 without 280 do not count. Do not retune leftover
n=4, 076-cells, or any detector.

Locks exact consecutive hits of each I 090 076 280 forward
4-gram on tablet I and on every other vendored tablet A–H
and J–V. The two 4-grams: 090 076 280 139, 090 076 280 067.
Do not assume hapax; count each from fixtures. Hypothesis:
all two are I-only. Measured: each N_I=1 at the cycle-242
...
from tests.test_mamari_i_3gram_090_076_280_i_only_scoreboard import (
    ...
    STANDING_I_NEXT_4GRAMS as CYCLE242_NEXT_4GRAMS,
    STANDING_I_PREVIOUS_4GRAMS as CYCLE242_PREVIOUS_4GRAMS,
    STANDING_I_SITES as CYCLE242_I_SITES,
    STANDING_N_I as CYCLE242_N_I,
    STANDING_N_OFF_I as CYCLE242_N_OFF_I,
    extra_i_sites,
    leftover_extra_remaining_after_530_280_subset,
    named_off_i_sites as cycle242_named_off_i_sites,
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_530_fwd280_scoreboard import (
    ...
    STANDING_OTHER_TIED_STEMS as CYCLE241_OTHER_TIED_STEMS,
...
STANDING_SEQUENCES = CYCLE242_NEXT_4GRAMS
STANDING_CYCLE242_SITES = CYCLE242_I_SITES
STANDING_NEXT_STEMS = (
    "139",
    "067",
)
STANDING_PREVIOUS_4GRAMS = CYCLE242_PREVIOUS_4GRAMS
...
STANDING_I_SITES = tuple((site,) for site in STANDING_CYCLE242_SITES)
STANDING_IB_HITS = 0
STANDING_IB_SITES = ()
...
STANDING_CLAIM = "i_090_076_280_forward_4grams_all_i_only"
STANDING_I_090_076_280_FORWARD_4GRAMS_ALL_I_ONLY = True
STANDING_I_090_076_280_FORWARD_4GRAMS_I_ONLY = True
STANDING_RESULT = "i_090_076_280_forward_4grams_i_only"
...
STANDING_090_076_WITHOUT_280_DOES_NOT_COUNT = True
STANDING_076_071_DOES_NOT_COUNT = True
STANDING_LEFTOVER_N4_SET_NOT_RETUNED = True
STANDING_LEFTOVER_EXTRA_REMAINING_AFTER_280_IS_NOT_THIS_CYCLE = True
...
STANDING_CYCLE241_K = 2
STANDING_CYCLE241_G = "280"
STANDING_CYCLE241_N_REMAINING6 = 29
STANDING_CYCLE242_N_I = 2
STANDING_CYCLE242_N_OFF_I = 0


def forward_4gram_start_site(
    cycle242_site: tuple[str, str, int],
) -> tuple[str, str, int]:
    """Forward 4-gram starts at the cycle-242 I 090 076 280 site."""
    return cycle242_site
...
def site_forward_4gram(
    ...
    """090 076 280 X if a next stem exists; None at end-of-line."""
    n3 = len(gram3)
...
def i_090_076_280_forward_4grams(
    i_sides: dict[str, list[list[str]]],
    sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE242_SITES,
    gram3: tuple[str, ...] = GRAM3,
...
def i_sites_without_forward(
    ...
    """I 090 076 280 sites that have no next stem after the 3-gram."""
    return tuple(
        site
...
def i_090_076_280_forward_4grams_all_i_only(
    n_i: tuple[int, ...],
    n_off_i: tuple[int, ...],
    expected_n: int = STANDING_N_SEQUENCES,
) -> bool:
    """True iff all 2 I 090 076 280 forward 4-grams are I-only.
...
class TestI090076280Forward4gramsIOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-242 I 090 076 280 forward 4-grams. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 4-gram counts; a gap is not a hit. 530 / 700 / 001 / 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(STANDING_SEQUENCES[0], ("090", "076", "280", "139"))
        self.assertEqual(STANDING_SEQUENCES[-1], ("090", "076", "280", "067"))
        ...
        overlap = [["090", "076", "280", "139", "090", "076", "280", "139"]]
        ...
        self.assertEqual(ngram_hit_count([["090", "076", "280"]], STANDING_SEQUENCES[0]), 0)
        self.assertEqual(ngram_hit_count([["090", "076"]], STANDING_SEQUENCES[0]), 0)
        self.assertEqual(ngram_hit_count([["090", "076", "530", "090"]], STANDING_SEQUENCES[0]), 0)
        self.assertEqual(ngram_hit_count([["090", "076", "700", "011"]], STANDING_SEQUENCES[0]), 0)
        planted = ["099", "090", "076", "280", "139"]
        ...
        self.assertTrue(STANDING_090_076_WITHOUT_280_DOES_NOT_COUNT)
    ...
    def test_all_i_only_requires_on_i_and_zero_off_i_for_each_4gram(self):
        ...
        self.assertTrue(i_090_076_280_forward_4grams_all_i_only(hold_ones, hold_zeros))
        self.assertTrue(
            i_090_076_280_forward_4grams_all_i_only((2,) + hold_ones[1:], hold_zeros)
        )
        lose_off = list(hold_zeros)
        lose_off[0] = 1
        self.assertFalse(
            i_090_076_280_forward_4grams_all_i_only(hold_ones, tuple(lose_off))
        )
        lose_missing_i = list(hold_ones)
        lose_missing_i[0] = 0
        self.assertFalse(
            i_090_076_280_forward_4grams_all_i_only(tuple(lose_missing_i), hold_zeros)
        )
        self.assertFalse(i_090_076_280_forward_4grams_all_i_only((), ()))
        self.assertFalse(
            i_090_076_280_forward_4grams_all_i_only(hold_ones[:-1], hold_zeros[:-1])
        )
        self.assertEqual(STANDING_CLAIM, "i_090_076_280_forward_4grams_all_i_only")
        self.assertTrue(STANDING_I_090_076_280_FORWARD_4GRAMS_ALL_I_ONLY)
        self.assertTrue(STANDING_I_090_076_280_FORWARD_4GRAMS_I_ONLY)
        self.assertEqual(
            STANDING_I_090_076_280_FORWARD_4GRAMS_ALL_I_ONLY,
    ...
    def test_4grams_are_cycle_242_forwards_not_retuned(self):
        """4-grams stay the cycle-242 I forwards; leftover / 530 / 700 / 001 / 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "280"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(STANDING_SEQUENCES, CYCLE242_NEXT_4GRAMS)
        self.assertEqual(STANDING_SEQUENCES, CYCLE241_MATCHING_NEXT_4GRAMS)
        self.assertEqual(STANDING_CYCLE242_SITES, CYCLE242_I_SITES)
        self.assertEqual(STANDING_CYCLE242_SITES, CYCLE241_MATCHING_SITES)
        self.assertEqual(len(STANDING_SEQUENCES), CYCLE242_N_I)
        ...
        self.assertTrue(STANDING_LEFTOVER_EXTRA_REMAINING_AFTER_280_IS_NOT_THIS_CYCLE)
...
class TestMamariI090076280Forward4gramsIOnlyScoreboard(unittest.TestCase):
    """Cited-fixture I 090 076 280 forward-4 off-I lock. Mock only."""

    def setUp(self):
        self.provider = MockProvider()
        self.survey = load_corpus_survey()
        self.i_sides = load_i_sides()
        self.cycle242_sites = STANDING_CYCLE242_SITES
        ...
        self.off_i_sites = tuple(
            cycle242_named_off_i_sites(gram) for gram in self.grams
        )
        self.measured_forwards = i_090_076_280_forward_4grams(
            self.i_sides,
            self.cycle242_sites,
            GRAM3,
        )
        ...
        self.leftover_matching = leftover_extra_remaining_after_530_with_280(
            STANDING_LEFTOVER_SITES,
            self.next_stems,
            CYCLE241_G,
        )
        ...
        self.extra = extra_i_sites(self.cycle242_sites, self.leftover_matching)
        ...
        self.claim_holds = i_090_076_280_forward_4grams_all_i_only(
            self.n_i,
            self.n_off_i,
        )
        self.n_i_only = sum(
    ...
    def test_tokens_and_sites_are_cycle_242_forwards_not_retuned(self):
        """4-grams and I sites stay the cycle-242 forward lock. Nested 2/0 must hold."""
        self.assertEqual(GRAM3, ("090", "076", "280"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM5, ("999", "071", "076", "010", "079"))
        self.assertEqual(self.cycle242_sites, STANDING_CYCLE242_SITES)
        self.assertEqual(self.cycle242_sites, CYCLE242_I_SITES)
        self.assertEqual(self.cycle242_sites, CYCLE241_MATCHING_SITES)
        self.assertEqual(self.measured_forwards, STANDING_SEQUENCES)
        self.assertEqual(self.measured_forwards, CYCLE242_NEXT_4GRAMS)
        self.assertEqual(self.measured_forwards, CYCLE241_MATCHING_NEXT_4GRAMS)
        self.assertEqual(self.leftover_matching_next_4grams, STANDING_SEQUENCES)
        self.assertEqual(self.no_forward, ())
        self.assertEqual(self.n3_i, STANDING_CYCLE242_N_I)
        self.assertEqual(self.n3_i, 2)
        self.assertEqual(self.n3_off_i, STANDING_CYCLE242_N_OFF_I)
        self.assertEqual(self.n3_off_i, 0)
        self.assertEqual(self.leftover_matching, CYCLE241_MATCHING_SITES)
        self.assertEqual(self.leftover_matching, CYCLE242_I_SITES)
        self.assertEqual(len(self.leftover_matching), STANDING_CYCLE241_K)
        self.assertEqual(STANDING_CYCLE241_K, 2)
        self.assertEqual(STANDING_CYCLE241_G, "280")
        ...
        if len(self.leftover_matching) != 2 or CYCLE241_G != "280":
            self.fail("nested cycle 241 leftover extra remaining-after-530 G=280 K=2 drifted")
        if self.extra:
            self.fail("extra I 090 076 280 sites appeared; leftover of leftover is not this cycle")
        ...
        self.assertEqual(
            ...
            prior_242["i_next_4grams"],
        )
        self.assertEqual(
            tuple(tuple(row) for row in prior_242["i_sites"]),
            STANDING_CYCLE242_SITES,
        )
        self.assertEqual(
            [list(gram) for gram in STANDING_PREVIOUS_4GRAMS],
            prior_242["i_previous_4grams"],
        )
        ...
        self.assertEqual(
            [list(gram) for gram in STANDING_SEQUENCES],
            prior_241["matching_next_4grams"],
        )
        ...
        self.assertEqual(CYCLE241_OTHER_TIED_STEMS, ("087", "011", "005", "000"))
        self.assertEqual(STANDING_OTHER_TIED_STEMS, ("087", "011", "005", "000"))
    ...
    def test_each_4gram_is_one_on_i_zero_off_i_and_claim_holds(self):
        ...
        for gram in STANDING_SEQUENCES:
            ...
            self.assertEqual(cycle242_named_off_i_sites(gram), ())
        ...
        self.assertEqual(
            i_090_076_280_forward_4grams_all_i_only(self.n_i, self.n_off_i),
            STANDING_I_090_076_280_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_242_241_240_239_238_237_236_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 242 2/0, 241 K=2/G=280 N_remaining6=29, 240 2/0 hapax, 239 2/0, 238 K=2/G=530, 237 2/0 hapax, 236 2/0, 234 7-way tie, 233 3/0 hapax, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
        ...
        self.assertEqual(prior_241.matching_next_4grams, STANDING_SEQUENCES)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-243 I-forward-4 I-only hold."""
        lock = self.survey["i_090_076_280_forward_4grams_i_only"]
        self.assertEqual(lock["cycle"], 243)
        ...
        self.assertEqual(lock["N_I"], CYCLE242_N_I)
        ...
        for row, gram, site, nxt, role, sites, n_on, n_off, off_sites, hits in zip(
            ...
            self.assertEqual(tuple(row["cycle242_site"]), site)
        ...
        self.assertTrue(lock["i_090_076_280_forward_4grams_all_i_only"])
        self.assertEqual(
            lock["i_090_076_280_forward_4grams_all_i_only"],
            STANDING_I_090_076_280_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertTrue(lock["i_090_076_280_forward_4grams_i_only"])
        ...
        self.assertEqual(lock["nested_cycle241_G"], STANDING_CYCLE241_G)
        self.assertEqual(lock["nested_cycle241_G"], "280")
        self.assertEqual(lock["nested_cycle241_K"], STANDING_CYCLE241_K)
        ...
        self.assertTrue(lock["090_076_without_280_does_not_count"])
        self.assertTrue(lock["076_071_does_not_count"])
        self.assertTrue(lock["leftover_n4_set_not_retuned"])
        self.assertTrue(lock["leftover_extra_remaining_after_280_is_not_this_cycle"])
        ...
        self.assertEqual(
            self.survey["i_leftover_extra_090_076_remaining_after_530_fwd280"]["N_remaining6"],
            29,
        )
...
class TestMamariI090076280Forward4gramsIOnlyImageSnapshot(unittest.TestCase):
    """Cycle 243 does not touch clustering. 83/62 / Hamming 6 stays."""
