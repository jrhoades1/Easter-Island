"""I's leftover n=4 remaining remaining-after-090-076 vs next 2-gram.

Cycle 328 text-search lock. Uses already-vendored A–V and the
...
contain 2-gram 090 076. The I-site peel of those five
(leftover n=4 remaining I 090 076, N_inside=13) is now
exhausted both directions through cycles 288–327. This
cycle's population is the N=11 leftover n=4 remaining
4-grams that do NOT contain 090 076 (16 − 5).

Already locked (do not re-lock): cycle 222 exactly-5-contain-
090 076 and the entire leftover n=4 remaining I 090 076 peel
(cycles 223–327). Leftover extra I 090 076 peels (cycles
225+). Closed leftover n=4 999 090 076 070 (cycles 220–221).

This cycle is leftover-inventory, not I-only of G. For each
remaining-after-090-076 leftover 4-gram, list its three
contiguous 2-grams. Count how many of the 11 contain each
2-gram (contain = the 2-gram is a substring of that leftover
4-gram). G = the 2-gram with the highest remaining-after-
090-076 leftover count. If a tie, pick the one whose first
...
222 leftover n=4 remaining next most frequent 2-gram). HOLD
iff unique_max is true (one 2-gram strictly outcounts every
other). LOSE on a tie for max frequency. Empty remainder
does not lose HOLD.

Measured: N=11, N_distinct=28, 5-way tie at K=2 (430 076 /
076 020 / 076 010 / 020 010 / 010 079), G=430 076 by
largest-id tie-break, unique-max false. Matching leftovers
of the labeled G: 430 076 001 076, 430 076 049 400.
N_remaining after peeling G = 9 (11−2). I vs off-I of G is
not this cycle. Nested cycle 222 leftover remaining K=5 /
G=090 076 / N_without=11, leftover n=4 remaining I 090 076
peel 288–327, leftover extra 090 076 peels, cycle 221
remaining 5-grams 4/4 I-only, cycle 220 leftover 000 5-gram
1/0, cycle 208 leftover 4-gram 5/0, and cycle 171 076 071
43/0 stay. Claim that can lose:
...
090 076, N=11 remaining-after-090-076, and G is uniquely
the most frequent under the tie-break. The claim is false.
Do not retune.
...
STANDING_CLAIM = "i_leftover_n4_remaining_after_090_076_unique_max_2gram"
STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_UNIQUE_MAX_2GRAM = False
STANDING_RESULT = "i_leftover_n4_remaining_after_090_076_next_2gram"
...
def leftover_remaining_after_090_076(
    ...
    """Leftover n=4 remaining 4-grams that do not contain 090 076."""
    return leftover_remaining_without_g(leftover_remaining_n4(leftovers), peeled)


def leftover_remaining_after_090_076_with_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-090-076 leftovers that contain the labeled G."""
    return leftover_remaining_with_g(remaining_after, needle)


def leftover_remaining_after_090_076_without_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-090-076 leftovers that do not contain the labeled G."""
    return leftover_remaining_without_g(remaining_after, needle)


def i_leftover_n4_remaining_after_090_076_unique_max_2gram(
    ...
    """True iff remaining-after-090-076 has a unique most frequent 2-gram."""
...
class TestILeftoverN4RemainingAfter090076Next2gramHelpers(unittest.TestCase):
    """Helpers on remaining-after-090-076 vs next 2-gram. No CV, no LLM."""

    def test_remaining_after_filter_and_unique_max_can_fail(self):
        """The 11 remaining-after-090-076 tie at 2; unique-max loses."""
        ...
        self.assertFalse(i_leftover_n4_remaining_after_090_076_unique_max_2gram(()))
        ...
        self.assertFalse(i_leftover_n4_remaining_after_090_076_unique_max_2gram(planted_family))
        replacements = {
            ("028", "076", "011", "076"): (
                ("430", "076", "011", "050"),
                4,
        ...
        self.assertEqual(len(leftover_remaining_n4(unique_leftover)), STANDING_N_REMAINING_PARENT)
        unique_after = leftover_remaining_after_090_076(unique_leftover)
        ...
        self.assertTrue(i_leftover_n4_remaining_after_090_076_unique_max_2gram(unique_leftover))
        self.assertEqual(
            tuple(gram for gram, _n, _f, _s in with_gram),
            STANDING_MATCHING_LEFTOVERS,
        )
        self.assertEqual(STANDING_CLAIM, "i_leftover_n4_remaining_after_090_076_unique_max_2gram")
        self.assertFalse(STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_UNIQUE_MAX_2GRAM)
    ...
    def test_tie_break_is_labeling_only_when_unique_max_false(self):
        ...
        self.assertEqual(ranked[0][0], ("430", "076"))
        self.assertEqual(ranked[0][1], 2)
        self.assertEqual(barthel_id("430"), 430)
        self.assertGreater(barthel_id("430"), barthel_id("076"))
        ...
        self.assertEqual(STANDING_RANKED_2GRAMS[STANDING_N_TIED_AT_K][1], STANDING_N_2GRAM_NEXT)
        gram, count, unique = select_g(remaining_after)
        self.assertEqual(gram, STANDING_G)
        self.assertEqual(count, STANDING_K)
        self.assertFalse(unique)
...
class TestMamariILeftoverN4RemainingAfter090076Next2gramScoreboard(unittest.TestCase):
    """Cited-fixture remaining-after-090-076 next 2-gram. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_leftover_n4_remaining_after_090_076_unique_max_2gram(
            self.leftover
        )
    ...
    def test_remaining_after_11_unique_max_loses(self):
        """11 remaining-after-090-076; unique-max is false. Claim is false."""
        self.assertEqual(len(self.remaining), STANDING_N)
        ...
        self.assertEqual(count, STANDING_K)
        self.assertFalse(unique)
        self.assertFalse(i_leftover_n4_remaining_after_090_076_unique_max_2gram(self.leftover))
        self.assertEqual(
            self.claim_holds,
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_UNIQUE_MAX_2GRAM,
        )
        self.assertFalse(STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_UNIQUE_MAX_2GRAM)
        self.assertEqual(STANDING_CLAIM, "i_leftover_n4_remaining_after_090_076_unique_max_2gram")
        self.assertEqual(
    ...
    def test_existing_222_221_220_208_171_and_w_scoreboards_still_compute(self):
        """Cycle 222 K=5, 221 4/4, 220 1/0, 208 5/0, 171 43/0, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-328 remaining-after-090-076 lock."""
        lock = self.survey["i_leftover_n4_remaining_after_090_076_next_2gram"]
        self.assertEqual(lock["cycle"], 328)
        ...
        self.assertEqual(lock["N_remaining"], STANDING_N_WITHOUT_G)
        ...
        self.assertFalse(lock["i_leftover_n4_remaining_after_090_076_unique_max_2gram"])
        self.assertEqual(
            lock["i_leftover_n4_remaining_after_090_076_unique_max_2gram"],
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_UNIQUE_MAX_2GRAM,
        )
...
class TestMamariILeftoverN4RemainingAfter090076Next2gramImageSnapshot(unittest.TestCase):
    """Cycle 328 does not touch clustering. 83/62 / Hamming 6 stays."""
