"""Reframed closed-tradition hold-out of the locked leftover-6 next-16 grams.

Cycle 849 text-search lock. Uses already-vendored A–V and the
cycle-495 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 16-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 495 already locked the I-only claim
on these 16-grams as incomplete-set LOSE (N_with_next16=15,
N_no_next16=3, N_i_only=15, N_leak=0). Cycle 848 already locked
the reframed claim on the cycle-493 next-15 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–848.

Claim that can lose: each existing next-16 gram is an exact
...
Named claim: cycle495_next16_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next16 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next16=15,
N_no_next16=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-15 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 extends to 604 076 071 600 999 050
076 000 002 999 076 092 535 999 208 076 at Ia9[9] and hits I
once. All fifteen next-15 grams extend; no new hole. Verdict:
incomplete-set LOSE.
...
next 16-grams as a peel, next 17-grams as a peel, prev-184, or
next-185. The cycle-497 next-17 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next16_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT16_SITES as CYCLE495_SITES,
    STANDING_N as CYCLE495_N,
    STANDING_N_I_EACH as CYCLE495_N_I_EACH,
...
STANDING_NO_NEXT16_SITES = CYCLE495_NO_NEXT16_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE495_SITES
STANDING_SEQUENCES = CYCLE495_SEQUENCES
...
STANDING_NEXT15_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT15_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next16_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next16_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 495
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 495
...
STANDING_DO_NOT_RELOCK_CYCLES_832_848 = True
...
class TestMamariILeftoverN6Next16ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 849 reframed closed-tradition lock on the cycle-495 next-16 population."""
    ...
    def test_population_is_the_locked_cycle_495_next16_grams(self):
        """Fifteen next-16 sequences and sites come from cycle 495. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE495_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE495_SITES)
        self.assertEqual(CYCLE495_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE495_N, 18)
        self.assertEqual(CYCLE495_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT16, CYCLE495_N_WITH_NEXT16)
        self.assertEqual(STANDING_N_NO_NEXT16, CYCLE495_N_NO_NEXT16)
        self.assertEqual(
            STANDING_NO_NEXT16_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT16_SITES, CYCLE848_NO_NEXT15_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 495)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 16)
        ...
        self.assertEqual(CYCLE495_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next16_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE848, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 495 I-only or the cycle 832–848 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 495)
        ...
        self.assertEqual(next15["next_cheap_lock"], STANDING_NAMED_BY_CYCLE848)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_848)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE495_N_I_EACH)
        self.assertEqual(CYCLE495_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT15_ALL_EXTEND)
        for n15, n16, site15, site16 in zip(
            CYCLE848_SEQUENCES, STANDING_SEQUENCES, CYCLE848_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n16[:15], n15)
            self.assertEqual(site16, site15)
        still = self.rows[STANDING_NEXT15_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:15], CYCLE848_SEQUENCES[3])
        self.assertEqual(CYCLE848_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT16_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_497_next17_list(self):
        """Cycle 497 next-17 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE497_N, 18)
        self.assertEqual(CYCLE497_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE497_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 17 for tokens in CYCLE497_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next17_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle497_next17_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-849 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 849)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next16_sites"]),
            STANDING_NO_NEXT16_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_848"])
        ...
        self.assertEqual(lock["named_by_cycle848"], STANDING_NAMED_BY_CYCLE848)
        ...
        self.assertEqual(
            tuple(lock["next15_hapax_still_hapax"]),
            STANDING_NEXT15_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next15_all_extend"])
        self.assertEqual(lock["next15_all_extend"], STANDING_NEXT15_ALL_EXTEND)
