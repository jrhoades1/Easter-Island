"""I's cycle-191 leftover 3-gram 700 076 071 off-I lock.

Cycle 192 text-search lock. Uses already-vendored A–V and the
cycle-191 leftover previous 3-gram 700 076 071 (the n=3 run
shared by leftover 076 071 sites Ia1[163] 090 700 076 071,
Ia2[51] 999 700 076 071, and Ia13[128] 040 700 076 071).
...
Same claim-shape as cycle 186 (076 071 061 was I-only 2/0),
cycle 183 (076 071 700 was I-only 3/0), cycle 180
(076 071 090 was I-only 5/0), cycle 177 (076 071 600 was
I-only 4/0), cycle 174 (076 071 076 was I-only 6/0), cycle
171 (076 071 was I-only 43/0), cycle 167 (999 090 076 was
I-only 16/0), cycle 160 (076 020 010 was I-only 12/0), and
cycle 141 (076 010 079 was I-only 8/0). This cycle is the
new leftover previous 3-gram 700 076 071 only. 071 999
(071 065 071 999) does not count. 076 076 (700 076 076 053)
does not. Near-miss Ia9[10] 700 604 076 071 does not (the
previous stem is 604). Do not retune 076 071 061, 076 071
700, 076 071 090, 076 071 600, 076 071 076, 076 071,
999 090 076, 076 020 010, or 076 010 079. Cycle 191 leftover
...
Locks exact consecutive hits of 700 076 071 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_700_076_071_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
3 at Ia1[162]/Ia2[50]/Ia13[127] (the three leftover-3 /
leftover-34 076 071 sites; no inside-family 076 071 site
has this 3-gram); Ib unpublished 0; every other vendored
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE191_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 3
STANDING_LEFTOVER_34_SITES = CYCLE191_MATCHING_SITES
...
STANDING_I_SITE_ROWS = (
    {
        ...
        "line": "Ia1",
        "index": 162,
        "previous_4gram": ("090", "700", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia2",
        "index": 50,
        "previous_4gram": ("999", "700", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia13",
        "index": 127,
        "previous_4gram": ("040", "700", "076", "071"),
...
STANDING_CLAIM = "i_3gram_700_076_071_i_only"
STANDING_I_3GRAM_700_076_071_I_ONLY = True
STANDING_RESULT = "i_3gram_700_076_071_i_only"
...
def leftover_076_071_site_for_3gram(
    ...
    """076 071 start one token after 700 076 071."""
...
def site_previous_4gram_for_3gram(
    ...
    """W 700 076 071 if a previous stem exists; None at start-of-line."""
...
def i_3gram_700_076_071_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
class TestI3gram700076071IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-191 leftover previous 3-gram tokens. No CV, no LLM."""
    ...
    def test_counts_require_consecutive_tokens(self):
        ...
        self.assertEqual(ngram_hit_count([list(STANDING_NEAR_MISS_LEFTOVER_700_604_PREVIOUS_4GRAM)], GRAM3), 0)
    ...
    def test_previous_4gram_requires_stem_before_3gram(self):
        """Previous 4-gram is W 700 076 071; start-of-line is none."""
        provider = MockProvider()
        has_prev = ["090", "700", "076", "071", "700", "076"]
        self.assertEqual(
            site_previous_4gram_for_3gram(has_prev, 1, GRAM3),
            ("090", "700", "076", "071"),
        )
        start_of_line = ["700", "076", "071", "010"]
        ...
        near_miss = list(STANDING_NEAR_MISS_LEFTOVER_700_604_PREVIOUS_4GRAM)
        self.assertIsNone(site_previous_4gram_for_3gram(near_miss, 1, GRAM3))
        self.assertEqual(
            leftover_076_071_site_for_3gram((SIDE_IA, "Ia1", 162)),
            (SIDE_IA, "Ia1", 163),
        )
        self.assertEqual(provider.get_call_history(), [])
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_700_076_071_i_only(1, 0))
        self.assertTrue(i_3gram_700_076_071_i_only(3, 0))
        self.assertFalse(i_3gram_700_076_071_i_only(1, 1))
        self.assertFalse(i_3gram_700_076_071_i_only(0, 0))
        self.assertFalse(i_3gram_700_076_071_i_only(0, 3))
        self.assertFalse(i_3gram_700_076_071_i_only(3, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_700_076_071_i_only")
        self.assertTrue(STANDING_I_3GRAM_700_076_071_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_700_076_071_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle191_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-191 leftover previous run, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        ...
        self.assertNotEqual(GRAM3, CYCLE186_GRAM3)
        ...
        for leftover in CYCLE191_MATCHING_PREVIOUS_4GRAMS:
...
class TestMamariI3gram700076071IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 700 076 071 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_700_076_071_i_only(
            self.i_hits,
            self.off_i_hits,
        )
    ...
    def test_tokens_are_cycle_191_leftover_not_retuned(self):
        """3-gram is the cycle-191 leftover previous lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        self.assertEqual(GRAM3, ("700", "076", "071"))
        ...
        self.assertEqual(prior_191["N_no_backward"], 0)
        measured_matching = [list(site) for site in CYCLE191_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_191["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_191["i_leftover_076_071_exactly_3_previous_700_076_071"])
        self.assertTrue(prior_191["071_999_does_not_count"])
        self.assertTrue(prior_191["076_076_does_not_count"])
        self.assertTrue(prior_191["near_miss_700_604_does_not_count"])
    ...
    def test_i_hits_are_three_on_ia(self):
        ...
        leftover3_in_i = tuple(
            ...
            if leftover_076_071_site_for_3gram(site) in CYCLE191_MATCHING_SITES
        )
        ...
        self.assertEqual(leftover3_in_i, CYCLE191_MATCHING_SITES)
        ...
        self.assertEqual(leftover34_in_i, CYCLE191_MATCHING_SITES)
        ...
        self.assertEqual(self.leftover_076_071_sites, CYCLE191_MATCHING_SITES)
        ...
        for site in STANDING_I_SITES:
            ...
            self.assertIn(gram2_site, CYCLE191_MATCHING_SITES)
        ...
        self.assertEqual(self.previous_4grams, CYCLE191_MATCHING_PREVIOUS_4GRAMS)
        self.assertEqual(
            STANDING_I_SITES,
            (
                (SIDE_IA, "Ia1", 162),
        ...
        self.assertNotIn(STANDING_NEAR_MISS_LEFTOVER_700_604_SITE, STANDING_I_SITES)
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_700_076_071_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_700_076_071_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-192 3-gram I-only lock."""
        lock = self.survey["i_3gram_700_076_071_i_only"]
        self.assertEqual(lock["cycle"], 192)
        ...
        self.assertTrue(lock["i_3gram_700_076_071_i_only"])
        self.assertEqual(
            lock["i_3gram_700_076_071_i_only"],
            STANDING_I_3GRAM_700_076_071_I_ONLY,
        )
        ...
        self.assertEqual(self.survey["i_leftover_076_071_previous_700"]["N_without"], 31)
...
class TestMamariI3gram700076071IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 192 does not touch clustering. 83/62 / Hamming 6 stays."""
