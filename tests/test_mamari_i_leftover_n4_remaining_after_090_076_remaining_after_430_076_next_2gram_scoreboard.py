"""I's leftover n=4 remaining remaining-after-090-076 remaining-after-430-076 vs next 2-gram.

Cycle 332 text-search lock. Uses already-vendored A–V and the
...
Histogram top: 430 076 / 076 020 / 076 010 / 020 010 / 010 079
all ×2. Matching leftovers of labeled G: 430 076 001 076 and
430 076 049 400. N_remaining after peeling G=9. Cycle 329 HOLD:
exactly 2 of the N=11 contain contiguous 430 076 (those two
leftovers). N_remaining=9. Cycle 330 LOSE: 2-gram 430 076 is
not I-only (N_I=30, N_off_I=16 including T at Ta4[19]/Ta8[2],
extra I=26, leftover matching sites 4). Extra I ≠ 0 is
recorded, not this cycle's claim. Do not re-lock 2-gram I-only
or all-I-inside-family (all-inside is false by extra I=26;
skip that analog of cycle 224). Cycle 331 HOLD: the two leftover
4-grams that contain 430 076 are all I-only (430 076 001 076
N_I=2 N_off_I=0 at Ia10[103]/Ia13[156]; 430 076 049 400 N_I=2
N_off_I=0 at Ia13[170]/Ia14[63]; extra I of the 4-grams=0, not
hapax). Nested cycle 330 2-gram leak stays nested (2-gram can
leak while 4-grams do not). Extra-I peel of those 4-grams is
closed (extra I=0). This cycle's N=9 population is leftover
n=4 remaining remaining-after-090-076 remaining-after-430-076:
the 9 leftover 4-grams that do not contain 090 076 and do not
contain contiguous 430 076.

Already locked (do not re-lock): cycle 331, 330, 329, 328, 222,
leftover n=4 remaining I 090 076 peels (288–327), leftover extra
090 076 peels, cycles 220–221, or cycle 223.

This cycle is leftover-inventory, not I-only of G. For each
remaining-after-430-076 leftover 4-gram, list its three
contiguous 2-grams. Count how many of the 9 contain each
2-gram (contain = the 2-gram is a substring of that leftover
4-gram). G = the 2-gram with the highest remaining-after-
430-076 leftover count. If a tie, pick the one whose first
...
328 leftover n=4 remaining remaining-after-090-076 unique most
frequent 2-gram, which LOSE 5-way tie). Also analog cycle 222
leftover n=4 remaining next most frequent 2-gram (HOLD G=090 076
K=5). HOLD iff unique_max is true (one 2-gram strictly
outcounts every other among those 9). LOSE on a tie
(unique_max false), including a hapax pile. G is largest-id
labeling only on a tie; do not treat labeled G as unique-max.
Empty remainder does not lose HOLD.

Measured: N=9, N_distinct=23, 4-way tie at K=2 (076 020 /
076 010 / 020 010 / 010 079), G=076 020 by largest-id
tie-break, unique-max false. Matching leftovers of the labeled
G: 076 020 010 050, 053 076 020 010. N_remaining after peeling
G = 7 (9−2). I vs off-I of G is not this cycle. Nested cycle
331 leftover 4-grams all I-only 2/0 extra I=0, cycle 330 2-gram
430 076 I-only 30/16 extra I=26, cycle 329 exactly-2-share
430 076, cycle 328 unique_max false, cycle 222 leftover
remaining K=5 / G=090 076 / N_without=11, leftover n=4 remaining
I 090 076 peel 288–327, leftover extra 090 076 peels, cycle 221
remaining 5-grams 4/4 I-only, cycle 220 leftover 000 5-gram
1/0, cycle 223 090 076 I-only 69/3, cycle 208 leftover 4-gram
5/0, and cycle 171 076 071 43/0 stay. Claim that can lose:
...
430 076, N=9 remaining-after-430-076, and G is uniquely the
...
LOCKED_G_090_076 = CYCLE222_GRAM2
LOCKED_G_430_076 = CYCLE329_GRAM2
...
STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_UNIQUE_MAX_2GRAM = (
    False
)
STANDING_RESULT = (
...
def leftover_remaining_after_090_076_remaining_after_430_076(
    ...
    """Leftover n=4 remaining 4-grams with neither 090 076 nor 430 076."""
    remaining_after_090 = leftover_remaining_without_g(
        leftover_remaining_n4(leftovers), peeled_090_076
    )
    return leftover_remaining_without_g(remaining_after_090, peeled_430_076)


def leftover_remaining_after_090_076_remaining_after_430_076_with_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-430-076 leftovers that contain the labeled G."""
    return leftover_remaining_with_g(remaining_after, needle)


def leftover_remaining_after_090_076_remaining_after_430_076_without_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-430-076 leftovers that do not contain the labeled G."""
    return leftover_remaining_without_g(remaining_after, needle)


def i_leftover_n4_remaining_after_090_076_remaining_after_430_076_unique_max_2gram(
    ...
    """True iff remaining-after-430-076 has a unique most frequent 2-gram."""
...
class TestILeftoverN4RemainingAfter090076RemainingAfter430076Next2gramHelpers(
    unittest.TestCase
):
    """Helpers on remaining-after-430-076 vs next 2-gram. No CV, no LLM."""

    def test_remaining_after_filter_and_unique_max_can_fail(self):
        """The 9 remaining-after-430-076 tie at 2; unique-max loses."""
        ...
        self.assertEqual(len(remaining_parent), CYCLE328_N)
        ...
        self.assertEqual(
            ...
            STANDING_N_REMAINING_GRANDPARENT,
        )
        unique_after = leftover_remaining_after_090_076_remaining_after_430_076(
            unique_leftover
        )
    ...
    def test_tie_break_is_labeling_only_when_unique_max_false(self):
        ...
        self.assertEqual(ranked[0][0], ("076", "020"))
        self.assertEqual(ranked[0][1], 2)
        self.assertEqual(barthel_id("076"), 76)
        self.assertGreater(barthel_id("076"), barthel_id("020"))
        self.assertGreater(barthel_id("020"), barthel_id("010"))
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076Next2gramScoreboard(
    unittest.TestCase
):
    """Cited-fixture remaining-after-430-076 next 2-gram. Mock only."""
    ...
    def test_nested_leftover_n4_family_and_cycles_222_328_329(self):
        """Nested leftover n=4 27/7/4/1, cycle 222 16/5/11, 328/329 11/2/9 stay."""
    ...
    def test_remaining_after_9_unique_max_loses(self):
        """9 remaining-after-430-076; unique-max is false. Claim is false."""
        self.assertEqual(len(self.remaining), STANDING_N)
    ...
    def test_existing_331_330_329_328_223_222_and_220_scoreboards_still_compute(self):
        """Cycle 331 2/0, 330 30/16, 329 K=2, 328 unique-max lose, 222 K=5 stay."""
        leftover = leftover_n4_rows()
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-332 remaining-after-430-076 lock."""
        lock = self.survey[
            "i_leftover_n4_remaining_after_090_076_remaining_after_430_076_next_2gram"
        ]
        self.assertEqual(lock["cycle"], 332)
        ...
        self.assertFalse(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_unique_max_2gram"]
        )
        self.assertEqual(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_unique_max_2gram"],
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_UNIQUE_MAX_2GRAM,
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076Next2gramImageSnapshot(
    unittest.TestCase
):
    """Cycle 332 does not touch clustering. 83/62 / Hamming 6 stays."""
