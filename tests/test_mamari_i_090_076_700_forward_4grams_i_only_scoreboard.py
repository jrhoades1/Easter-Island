"""I's cycle-236 090 076 700 forward-4-grams off-I lock.

Cycle 237 text-search lock. Uses already-vendored A–V and the
cycle-236 I sites of 3-gram 090 076 700 (all 2 I sites; each
has a next stem). Does not retune those 4-grams, the 2 I
sites, leftover extra remaining-after-001 G=700 K=2, leftover
extra remaining-after-001 unique-max (cycle 234 lost), leftover
extra remaining-after-013 001, leftover extra remaining-after-071
...
230 (I 090 076 013 forward 4-grams all I-only hapax 1/0 x5),
and cycle 233 (I 090 076 001 forward 4-grams all I-only hapax
1/0 x3). Cycle 236 I 090 076 700 I-only holds 2/0, cycle 235
leftover extra remaining-after-001 K=2 / G=700, cycle 234
7-way tie at 2, cycle 233 3/0 hapax, cycle 232 3/0, cycle
223 69/3, cycle 195 6/0, cycle 192 3/0 (700 076 071), cycle
183 3/0 (076 071 700), and cycle 171 43/0 stay. Off-I T sites
of 090 076 are not this 3-gram and are not these 4-grams.
Leftover extra remaining after 700 is not locked this cycle.
Previous 4-grams of the two I sites are not locked this cycle.
The other six tied stems (530/280/087/011/005/000) are not
locked. 076 after 700 is a different 4-gram from leftover
076 071 clusters and from 700 076 071. 090 076 001,
090 076 013, 090 076 070, 090 076 071, and 090 076 without
700 do not count. Do not retune leftover n=4, 076-cells, or
any detector.

Locks exact consecutive hits of each I 090 076 700 forward
4-gram on tablet I and on every other vendored tablet A–H
and J–V. The two 4-grams: 090 076 700 011, 090 076 700 076.
Do not assume hapax; count each from fixtures. Hypothesis:
all two are I-only. Measured: each N_I=1 at the cycle-236
...
from tests.test_mamari_i_3gram_090_076_700_i_only_scoreboard import (
    ...
    STANDING_I_NEXT_4GRAMS as CYCLE236_NEXT_4GRAMS,
    STANDING_I_PREVIOUS_4GRAMS as CYCLE236_PREVIOUS_4GRAMS,
    ...
    named_off_i_sites as cycle236_named_off_i_sites,
...
STANDING_SEQUENCES = CYCLE236_NEXT_4GRAMS
STANDING_CYCLE236_SITES = CYCLE236_I_SITES
STANDING_NEXT_STEMS = (
    "011",
    "076",
)
STANDING_PREVIOUS_4GRAMS = CYCLE236_PREVIOUS_4GRAMS
...
STANDING_I_SITES = tuple((site,) for site in STANDING_CYCLE236_SITES)
STANDING_IB_HITS = 0
STANDING_IB_SITES = ()
...
STANDING_CLAIM = "i_090_076_700_forward_4grams_all_i_only"
STANDING_I_090_076_700_FORWARD_4GRAMS_ALL_I_ONLY = True
STANDING_I_090_076_700_FORWARD_4GRAMS_I_ONLY = True
STANDING_RESULT = "i_090_076_700_forward_4grams_i_only"
STANDING_NEW_TABLET = False
STANDING_W_BARTHEL = False
STANDING_N_GE_8_ISLAND = False
STANDING_TABLET_ONLY = True
STANDING_SAME_AS_I_5GRAM = False
STANDING_SAME_AS_CYCLE183 = False
...
STANDING_076_AFTER_700_IS_NOT_LEFTOVER_076_071 = True
...
STANDING_CYCLE236_N_I = 2
STANDING_CYCLE236_N_OFF_I = 0


def forward_4gram_start_site(
    cycle236_site: tuple[str, str, int],
) -> tuple[str, str, int]:
    """Forward 4-gram starts at the cycle-236 I 090 076 700 site."""
    return cycle236_site
...
def site_forward_4gram(
    ...
    """090 076 700 X if a next stem exists; None at end-of-line."""
    n3 = len(gram3)
...
def i_090_076_700_forward_4grams(
    i_sides: dict[str, list[list[str]]],
    sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE236_SITES,
    gram3: tuple[str, ...] = GRAM3,
...
def i_sites_without_forward(
    ...
    """I 090 076 700 sites that have no next stem after the 3-gram."""
    return tuple(
        site
...
def i_090_076_700_forward_4grams_all_i_only(
    n_i: tuple[int, ...],
    n_off_i: tuple[int, ...],
    expected_n: int = STANDING_N_SEQUENCES,
) -> bool:
    """True iff all 2 I 090 076 700 forward 4-grams are I-only.
...
class TestI090076700Forward4gramsIOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-236 I 090 076 700 forward 4-grams. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 4-gram counts; a gap is not a hit. 001 / 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(STANDING_SEQUENCES[0], ("090", "076", "700", "011"))
        self.assertEqual(STANDING_SEQUENCES[-1], ("090", "076", "700", "076"))
        ...
        overlap = [["090", "076", "700", "011", "090", "076", "700", "011"]]
        ...
        self.assertEqual(ngram_hit_count([list(CYCLE183_GRAM3)], STANDING_SEQUENCES[0]), 0)
        self.assertEqual(ngram_hit_count([list(CYCLE192_GRAM3)], STANDING_SEQUENCES[0]), 0)
        ...
        self.assertEqual(ngram_hit_count([["090", "076", "700"]], STANDING_SEQUENCES[0]), 0)
        self.assertEqual(ngram_hit_count([["076", "071", "700", "076"]], STANDING_SEQUENCES[-1]), 0)
        self.assertEqual(ngram_hit_count([["700", "076", "071", "633"]], STANDING_SEQUENCES[-1]), 0)
        planted = ["497", "090", "076", "700", "011", "090"]
        ...
        self.assertTrue(STANDING_076_AFTER_700_IS_NOT_LEFTOVER_076_071)
    ...
    def test_all_i_only_requires_on_i_and_zero_off_i_for_each_4gram(self):
        ...
        self.assertTrue(i_090_076_700_forward_4grams_all_i_only(hold_ones, hold_zeros))
        self.assertTrue(
            i_090_076_700_forward_4grams_all_i_only((2,) + hold_ones[1:], hold_zeros)
        )
        lose_off = list(hold_zeros)
        lose_off[0] = 1
        self.assertFalse(
            i_090_076_700_forward_4grams_all_i_only(hold_ones, tuple(lose_off))
        )
        lose_missing_i = list(hold_ones)
        lose_missing_i[0] = 0
        self.assertFalse(
            i_090_076_700_forward_4grams_all_i_only(tuple(lose_missing_i), hold_zeros)
        )
        self.assertFalse(i_090_076_700_forward_4grams_all_i_only((), ()))
        self.assertFalse(
            i_090_076_700_forward_4grams_all_i_only(hold_ones[:-1], hold_zeros[:-1])
        )
        self.assertEqual(STANDING_CLAIM, "i_090_076_700_forward_4grams_all_i_only")
        self.assertTrue(STANDING_I_090_076_700_FORWARD_4GRAMS_ALL_I_ONLY)
        self.assertTrue(STANDING_I_090_076_700_FORWARD_4GRAMS_I_ONLY)
        self.assertEqual(
            STANDING_I_090_076_700_FORWARD_4GRAMS_ALL_I_ONLY,
    ...
    def test_4grams_are_cycle_236_forwards_not_retuned(self):
        """4-grams stay the cycle-236 I forwards; leftover / 001 / 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "700"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(STANDING_SEQUENCES, CYCLE236_NEXT_4GRAMS)
        self.assertEqual(STANDING_SEQUENCES, CYCLE235_MATCHING_NEXT_4GRAMS)
        self.assertEqual(STANDING_CYCLE236_SITES, CYCLE236_I_SITES)
        self.assertEqual(STANDING_CYCLE236_SITES, CYCLE235_MATCHING_SITES)
        self.assertEqual(len(STANDING_SEQUENCES), CYCLE236_N_I)
        ...
        self.assertEqual(CYCLE183_GRAM3, ("076", "071", "700"))
        self.assertEqual(CYCLE192_GRAM3, ("700", "076", "071"))
        for gram in STANDING_SEQUENCES:
            ...
            self.assertFalse(is_contiguous_substring(CYCLE183_GRAM3, gram))
            self.assertFalse(is_contiguous_substring(CYCLE192_GRAM3, gram))
            ...
            self.assertNotEqual(gram[:3], CYCLE183_GRAM3)
            self.assertNotEqual(gram[:3], CYCLE192_GRAM3)
            self.assertNotEqual(gram[:2], CYCLE171_GRAM2)
            self.assertNotEqual(gram, CYCLE219_LEAK_4GRAM)
        self.assertNotEqual(STANDING_SEQUENCES[-1], ("076", "071", "700", "076"))
        self.assertNotEqual(STANDING_SEQUENCES[-1][:3], CYCLE192_GRAM3)
        for site, start in zip(
            STANDING_CYCLE236_SITES,
        ...
        self.assertFalse(STANDING_SAME_AS_CYCLE183)
...
class TestMamariI090076700Forward4gramsIOnlyScoreboard(unittest.TestCase):
    """Cited-fixture I 090 076 700 forward-4 off-I lock. Mock only."""

    def setUp(self):
        self.provider = MockProvider()
        self.survey = load_corpus_survey()
        self.i_sides = load_i_sides()
        self.cycle236_sites = STANDING_CYCLE236_SITES
        ...
        self.off_i_sites = tuple(
            cycle236_named_off_i_sites(gram) for gram in self.grams
        )
        self.measured_forwards = i_090_076_700_forward_4grams(
            self.i_sides,
            self.cycle236_sites,
            GRAM3,
        )
        ...
        self.extra = extra_i_sites(self.cycle236_sites, self.leftover_matching)
        ...
        self.claim_holds = i_090_076_700_forward_4grams_all_i_only(
            self.n_i,
            self.n_off_i,
        )
        self.n_i_only = sum(
    ...
    def test_tokens_and_sites_are_cycle_236_forwards_not_retuned(self):
        """4-grams and I sites stay the cycle-236 forward lock. Nested 2/0 must hold."""
        self.assertEqual(GRAM3, ("090", "076", "700"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM5, ("999", "071", "076", "010", "079"))
        self.assertEqual(self.cycle236_sites, STANDING_CYCLE236_SITES)
        self.assertEqual(self.cycle236_sites, CYCLE236_I_SITES)
        self.assertEqual(self.cycle236_sites, CYCLE235_MATCHING_SITES)
        self.assertEqual(self.measured_forwards, STANDING_SEQUENCES)
        self.assertEqual(self.measured_forwards, CYCLE236_NEXT_4GRAMS)
        self.assertEqual(self.measured_forwards, CYCLE235_MATCHING_NEXT_4GRAMS)
        self.assertEqual(self.leftover_matching_next_4grams, STANDING_SEQUENCES)
        self.assertEqual(self.no_forward, ())
        self.assertEqual(self.n3_i, STANDING_CYCLE236_N_I)
        self.assertEqual(self.n3_i, 2)
        self.assertEqual(self.n3_off_i, STANDING_CYCLE236_N_OFF_I)
        self.assertEqual(self.n3_off_i, 0)
        self.assertEqual(self.leftover_matching, CYCLE235_MATCHING_SITES)
        self.assertEqual(self.leftover_matching, CYCLE236_I_SITES)
        ...
        self.assertEqual(len(self.remaining4), CYCLE235_N_REMAINING4)
        ...
        self.assertEqual(
            ...
            prior_236["i_next_4grams"],
        )
        self.assertEqual(
            tuple(tuple(row) for row in prior_236["i_sites"]),
            STANDING_CYCLE236_SITES,
        )
        self.assertEqual(
            [list(gram) for gram in STANDING_PREVIOUS_4GRAMS],
            prior_236["i_previous_4grams"],
        )
    ...
    def test_each_4gram_is_one_on_i_zero_off_i_and_claim_holds(self):
        ...
        for site, start, gram, nxt, prev4, role, sites, n_on, n_off, off_sites in zip(
            ...
            self.assertNotEqual(gram[:3], CYCLE192_GRAM3)
            self.assertNotEqual(gram, CYCLE219_LEAK_4GRAM)
            self.assertNotEqual(gram, ("076", "071", "700", "076"))
            ...
            self.assertNotIn(site, CYCLE183_I_SITES)
            self.assertNotIn(site, CYCLE192_I_SITES)
        ...
        for gram in STANDING_SEQUENCES:
            ...
            self.assertEqual(cycle236_named_off_i_sites(gram), ())
        ...
        self.assertEqual(
            i_090_076_700_forward_4grams_all_i_only(self.n_i, self.n_off_i),
            STANDING_I_090_076_700_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_236_235_234_233_232_223_195_192_183_and_171_still_compute(self):
        """Cycle 236 2/0, 235 K=2/G=700, 234 7-way tie, 233 3/0 hapax, 232 3/0, 223 69/3, 195 6/0, 192 3/0, 183 3/0, 171 43/0 stay."""
        ...
        self.assertEqual(prior_235.matching_next_4grams, STANDING_SEQUENCES)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-237 I-forward-4 I-only hold."""
        lock = self.survey["i_090_076_700_forward_4grams_i_only"]
        self.assertEqual(lock["cycle"], 237)
        ...
        self.assertEqual(lock["N_I"], CYCLE236_N_I)
        ...
        for row, gram, site, nxt, role, sites, n_on, n_off, off_sites, hits in zip(
            ...
            self.assertEqual(tuple(row["cycle236_site"]), site)
        ...
        self.assertTrue(lock["i_090_076_700_forward_4grams_all_i_only"])
        self.assertEqual(
            lock["i_090_076_700_forward_4grams_all_i_only"],
            STANDING_I_090_076_700_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertTrue(lock["i_090_076_700_forward_4grams_i_only"])
        ...
        self.assertFalse(lock["same_as_cycle183"])
        ...
        self.assertTrue(lock["076_after_700_is_not_leftover_076_071"])
...
class TestMamariI090076700Forward4gramsIOnlyImageSnapshot(unittest.TestCase):
    """Cycle 237 does not touch clustering. 83/62 / Hamming 6 stays."""
