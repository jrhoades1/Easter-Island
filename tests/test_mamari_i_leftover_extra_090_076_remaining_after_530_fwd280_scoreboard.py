"""I's cycle-240 leftover extra remaining-after-530 forward-280 cluster lock.

Cycle 241 text-search lock. Uses already-vendored A–V and the
...
(2/0 I-only; two forward 4-grams hapax 1/0). This cycle locks
the next tied stem as an exact-K claim on remaining-after-530.
Do not lock I-only of 090 076 280. Do not lock unique-max
remaining-after-530. Do not lock the other four tied stems
(087/011/005/000). Do not lock previous 4-grams of
090 076 530. Off-I T sites are not this cycle. I-only of
...
Leftover extra remaining-after-530 = leftover extra I 090 076
sites with a next token whose next token is none of 070, 071,
013, 001, 700, or 530. Nested-check leftover extra
...
remaining-after-700 N_remaining5==31 exactly 2 share 530 (do
not retune cycles 225–238). Nested-check cycle 239 090 076 530
I-only 2/0 and cycle 240 two forward 4-grams 1/0 each (do not
retune). Nested-check N_remaining6==29 (31−2). Ia2[174] has
next token 000 and is remaining-after-530; it is not no-next.

Hypothesis K=2: leftover extra remaining-after-530 includes
exactly 2 sites that share next stem 280 (forward 3-gram
090 076 280). Measured: K=2 at Ia7[2], Ia7[113]; next
4-grams 090 076 280 139 / 090 076 280 067. Cycle 234 listed
280×2 among the 7-way tie; re-measured, not assumed. Claim
that can lose:
...
238 (remaining-after-700 exactly 2 share 530). This can lose
if remaining-after-530 does not contain exactly 2 280
next-stem sites, or if N_remaining6 ≠ 29. Nested cycle 240
...
STANDING_OTHER_TIED_STEMS = ("087", "011", "005", "000")
...
STANDING_UNIQUE_MAX_REMAINING_AFTER_530_IS_NOT_THIS_CYCLE = True
...
STANDING_MATCHING_EQUALS_CYCLE234_280_SITES = True
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280"
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_530_EXACTLY_2_SHARE_280 = True
STANDING_RESULT = "i_leftover_extra_090_076_remaining_after_530_fwd280"
...
STANDING_I_ONLY_OF_090_076_280_IS_NOT_THIS_CYCLE = True
STANDING_PREVIOUS_4GRAMS_OF_090_076_530_ARE_NOT_THIS_CYCLE = True
...
def leftover_extra_remaining_after_530(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_530,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra with-next sites whose next token is none of 070/071/013/001/700/530."""
...
def leftover_extra_remaining_after_530_next_stems(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_530,
) -> tuple[str, ...]:
    """Next stems of leftover extra remaining-after-530 sites."""
    locked_set = set(locked)
    return tuple(
        nxt
...
def leftover_extra_remaining_after_530_with_280(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-530 sites whose next token is 280."""
    remaining6 = set(leftover_extra_remaining_after_530(sites, next_stems))
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
        if nxt == stem and site in remaining6
    )


def leftover_extra_remaining_after_530_without_280(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_530,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-530 sites whose next token is not 280."""
    locked_set = set(locked)
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_extra_remaining_after_530_nested_counts_hold(
    n_leftover: int,
    n_with_next: int,
    n_no_next: int,
    n_share_070: int,
    ...
    n_share_530: int,
    n_remaining6: int,
    expected_share_530: int = STANDING_N_SHARE_530,
    expected_remaining6: int = STANDING_N_REMAINING6,
) -> bool:
    """Nested leftover extra chain through remaining-after-530 N_remaining6==29."""
    return leftover_extra_remaining_after_700_nested_counts_hold(
        n_leftover,
        n_with_next,
        n_no_next,
        n_share_070,
        ...
        n_share_700,
        n_remaining5,
    ) and n_share_530 == expected_share_530 and n_remaining6 == expected_remaining6 and (
        n_remaining6 == n_remaining5 - n_share_530
    )


def matching_equals_cycle234_280_sites(
    matching_sites: tuple[tuple[str, str, int], ...],
    cycle234_sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE234_280_SITES,
) -> bool:
    """True iff remaining-after-530 090 076 280 sites equal cycle 234's 280 pair."""
    return matching_sites == cycle234_sites


def matching_leftover_extra_remaining_after_530_fwd280_local_4gram_rows(
    leftover_sites: tuple[tuple[str, str, int], ...] = STANDING_MATCHING_SITES,
    next_4grams: tuple[tuple[str, ...], ...] = STANDING_MATCHING_NEXT_4GRAMS,
) -> list[dict]:
    """Survey-shaped matching leftover extra remaining-after-530 280 next-4-gram rows."""
    rows = []
...
def i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(
    k: int,
...
class TestILeftoverExtra090076RemainingAfter530Fwd280Helpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-530 forward 280. No CV, no LLM."""

    def test_forward_280_requires_remaining_after_530_next_stem(self):
        """Next stem 280 is remaining-after-530; locked 070/071/013/001/700/530 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_FORWARD, ("090", "076", "280"))
        ...
        planted_stems = ("280", "530", "700", "070", "071", "013", "001", None, "087")
        rem6 = leftover_extra_remaining_after_530(planted_sites, planted_stems)
        self.assertEqual(rem6, (planted_sites[0], planted_sites[8]))
        self.assertEqual(
            leftover_extra_remaining_after_530_with_280(planted_sites, planted_stems),
            (planted_sites[0],),
        )
        self.assertEqual(
            leftover_extra_remaining_after_530_without_280(planted_sites, planted_stems),
            (planted_sites[8],),
        )
        self.assertNotIn(planted_sites[1], rem6)
        self.assertNotIn(planted_sites[2], rem6)
        self.assertNotIn(planted_sites[3], rem6)
        self.assertNotIn(planted_sites[4], rem6)
        self.assertNotIn(planted_sites[5], rem6)
        self.assertNotIn(planted_sites[6], rem6)
        self.assertNotIn(planted_sites[7], rem6)
    ...
    def test_exactly_2_can_fail(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(2))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(0))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(1))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(3))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(6))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(29))
        planted = STANDING_MATCHING_SITES + ((SIDE_IA, "Ia1", 2),)
        planted_stems = ("280",) * 3
        self.assertEqual(
            leftover_extra_remaining_after_530_with_280(planted, planted_stems),
            planted,
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(len(planted))
        )
        self.assertEqual(
            STANDING_CLAIM,
            "i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280",
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_530_EXACTLY_2_SHARE_280
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_530_EXACTLY_2_SHARE_280,
            HYPOTHESIS_K == STANDING_K,
        )
        self.assertEqual(STANDING_N_SHARE_530 + STANDING_N_REMAINING6, STANDING_N_REMAINING5)
        self.assertEqual(2 + 29, 31)
        self.assertEqual(STANDING_K + STANDING_N_WITHOUT, STANDING_N_REMAINING6)
        self.assertEqual(2 + 27, 29)
        self.assertEqual(provider.get_call_history(), [])

    def test_cycle234_280_site_agreement_and_tie_minus_700_530_can_fail(self):
        """Matching sites must equal cycle 234's Ia7[2]/Ia7[113]; unique-max stays unlocked."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle234_280_sites(STANDING_MATCHING_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_280_SITES)
        self.assertEqual(STANDING_MATCHING_SITES, STANDING_CYCLE234_280_SITES)
        planted = STANDING_MATCHING_SITES[:-1]
        self.assertFalse(matching_equals_cycle234_280_sites(planted))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(len(planted))
        )
        swapped = ((SIDE_IA, "Ia12", 42), (SIDE_IA, "Ia14", 9))
        self.assertFalse(matching_equals_cycle234_280_sites(swapped))
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(len(swapped))
        )
        self.assertEqual(swapped, CYCLE238_MATCHING_SITES)
        ...
        self.assertTrue(STANDING_UNIQUE_MAX_REMAINING_AFTER_530_IS_NOT_THIS_CYCLE)
        self.assertEqual(CYCLE234_N_TIED_AT_K, 7)
        self.assertEqual(CYCLE234_TIED_STEMS, STANDING_TIED_STEMS)
        self.assertEqual(
            tuple(stem for stem in CYCLE234_TIED_STEMS if stem not in ("700", "530")),
            STANDING_TIED_STEMS_MINUS_700_530,
        )
        self.assertEqual(len(STANDING_TIED_STEMS_MINUS_700_530), STANDING_N_TIED_MINUS_700_530)
        self.assertEqual(STANDING_N_TIED_MINUS_700_530, 5)
        self.assertTrue(STANDING_OTHER_TIED_STEMS_NOT_LOCKED)
        self.assertTrue(STANDING_I_ONLY_OF_090_076_280_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_PREVIOUS_4GRAMS_OF_090_076_530_ARE_NOT_THIS_CYCLE)
...
class TestMamariILeftoverExtra090076RemainingAfter530Fwd280Scoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-530 forward-280 lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.matching = leftover_extra_remaining_after_530_with_280(
            self.leftover_sites,
            self.next_stems,
        )
        self.without = leftover_extra_remaining_after_530_without_280(
            self.leftover_sites,
            self.next_stems,
        )
        ...
        self.equals_cycle234_280 = matching_equals_cycle234_280_sites(self.matching)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(
            self.k,
        )

    def test_tokens_and_nested_leftover_extra_through_530_not_retuned(self):
        """2-gram and leftover extra 56/55/1 through remaining-after-700 31/2/530 stay."""
        self.assertEqual(GRAM2, ("090", "076"))
    ...
    def test_counts_2_of_29_and_hypothesis_k_2_holds(self):
        """N_remaining6=29, 7-way-tie-minus-700-530 stays, K=2 share 280. Claim holds."""
        ...
        if (
            ...
            self.fail(
                ...
                "remaining-after-001 33/2/700 / remaining-after-700 31/2/530 drifted"
            )
        self.assertTrue(
        ...
        if self.n_remaining6 != 29:
            self.fail("measured N_remaining6 drifted from 29")
        if self.n_remaining6 != self.n_remaining5 - self.n_share_530:
            self.fail("leftover extra remaining-after-530 filter disagrees with nested 31−2")
        self.assertEqual(self.remaining6, STANDING_REMAINING6_SITES)
        self.assertEqual(len(self.remaining6), len(self.remaining6_stems))
        self.assertEqual(
            self.remaining6,
            leftover_extra_remaining_after_700_without_530(
                self.leftover_sites,
        ...
        self.assertNotIn(STANDING_NO_NEXT_SITES[0], self.remaining6)
        self.assertIn(STANDING_IA2_174, self.remaining6)
        self.assertEqual(STANDING_IA2_174_NEXT_STEM, "000")
        for site in self.share_070:
            self.assertNotIn(site, self.remaining6)
        ...
        self.assertEqual(STANDING_N_TIED_MINUS_700_530, 5)
        if self.tied_minus_700_530 != STANDING_TIED_STEMS_MINUS_700_530:
            self.fail("nested cycle 234 7-way-tie-minus-700-530 drifted")
        ...
        self.assertEqual(self.k + self.n_without, self.n_remaining6)
        self.assertEqual(2 + 27, 29)
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280(self.k)
        )
        self.assertEqual(
            self.claim_holds,
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_530_EXACTLY_2_SHARE_280,
        ...
        self.assertTrue(self.equals_cycle234_280)
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_280_SITES)
        self.assertTrue(matching_equals_cycle234_280_sites(self.matching))
        if len(self.matching) != 2 or not self.equals_cycle234_280:
            self.fail("leftover extra remaining-after-530 280 set drifted from cycle-234 pair")
    ...
    def test_matching_leftover_extra_remaining_after_530_sites_share_280(self):
        """Two leftover extra remaining-after-530 sites are 090 076 280."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_next_4grams, STANDING_MATCHING_NEXT_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia7", 2), ("090", "076", "280", "139")),
            ((SIDE_IA, "Ia7", 113), ("090", "076", "280", "067")),
        )
        for (site, nxt), (want_site, want_nxt) in zip(
            ...
            self.assertEqual(stems[index + STANDING_N2], "280")
            self.assertEqual(site_next_stem(stems, index, GRAM2), "280")
        ...
        self.assertEqual(self.matching, STANDING_CYCLE234_280_SITES)
        self.assertTrue(matching_equals_cycle234_280_sites(self.matching))
        ...
        other_tied = tuple(
            stem for stem in STANDING_TIED_STEMS_MINUS_700_530 if stem != "280"
        )
        self.assertEqual(other_tied, CYCLE235_OTHER_TIED_STEMS[2:])
        self.assertEqual(other_tied, STANDING_OTHER_TIED_STEMS)
        self.assertEqual(other_tied, ("087", "011", "005", "000"))
        for stem in other_tied:
            ...
            for site in other:
                self.assertNotIn(site, self.matching)
                self.assertIn(site, self.remaining6)
    ...
    def test_existing_240_239_238_237_236_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 240 2/0 hapax, 239 2/0, 238 K=2/G=530, 237 2/0 hapax, 236 2/0, 234 7-way tie, 233 3/0, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
        ...
        self.assertEqual(
            tuple(stem for stem in tied if stem not in ("700", "530")),
            STANDING_TIED_STEMS_MINUS_700_530,
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-241 leftover extra remaining-after-530 fwd280 lock."""
        lock = self.survey["i_leftover_extra_090_076_remaining_after_530_fwd280"]
        self.assertEqual(lock["cycle"], 241)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_530_sites"]),
            STANDING_MATCHING_SITES,
        )
        self.assertTrue(lock["matching_equals_cycle234_280_sites"])
        self.assertEqual(
            lock["matching_equals_cycle234_280_sites"],
            STANDING_MATCHING_EQUALS_CYCLE234_280_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lock["matching_leftover_extra_remaining_after_530_local_4grams"],
            matching_leftover_extra_remaining_after_530_fwd280_local_4gram_rows(),
        )
        ...
        self.assertTrue(lock["i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280"])
        self.assertEqual(
            lock["i_leftover_extra_090_076_remaining_after_530_exactly_2_share_280"],
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_530_EXACTLY_2_SHARE_280,
        )
        ...
        self.assertTrue(lock["i_only_of_090_076_280_is_not_this_cycle"])
        self.assertTrue(lock["previous_4grams_of_090_076_530_are_not_this_cycle"])
        self.assertTrue(lock["unique_max_remaining_after_530_is_not_this_cycle"])
...
class TestMamariILeftoverExtra090076RemainingAfter530Fwd280ImageSnapshot(
    unittest.TestCase
):
    """Cycle 241 does not touch clustering. 83/62 / Hamming 6 stays."""
