"""Reframed closed-tradition hold-out of the locked leftover-6 next-4 grams.

Cycle 837 text-search lock. Uses already-vendored A–V and the
cycle-470 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 4-grams (N=18). HEAD-check: that gram list
exists and is not yet locked under this reframed rule. Cycle
470 already locked the I-only claim on these 4-grams as HOLD
(N_i_only=18, N_leak=0, N_no_next4=0). Cycle 836 already
locked the reframed claim on the cycle-468 next-3 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–835.

Claim that can lose: each of the 18 locked next-4 grams is an
...
contiguous hit once on I. The next-3 multi-I grams are hapax
once extended: 604 076 071 600 at Ia9[9] hits I once; 720 076
070 701 at Ia8[171] hits I once. Verdict: incomplete-set LOSE.
...
from tests.test_mamari_i_leftover_n6_next3_closed_tradition_hpq_abc_scoreboard import (
    ...
    STANDING_NEXT_CHEAP_LOCK as CYCLE836_NEXT_CHEAP_LOCK,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next4_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT4_SITES as CYCLE470_SITES,
    STANDING_N as CYCLE470_N,
    STANDING_N_I_EACH as CYCLE470_N_I_EACH,
...
STANDING_SITES = CYCLE470_SITES
STANDING_SEQUENCES = CYCLE470_SEQUENCES
...
STANDING_NEXT3_MULTI_I_NOW_HAPAX = (3, 14)
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next4_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next4_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 470
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 470
...
STANDING_DO_NOT_RELOCK_CYCLES_832_836 = True
...
class TestMamariILeftoverN6Next4ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 837 reframed closed-tradition lock on the cycle-470 next-4 population."""
    ...
    def test_population_is_the_locked_cycle_470_next4_grams(self):
        """Eighteen next-4 sequences and sites come from cycle 470. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE470_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE470_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 470)
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 470 I-only or the cycle 832–836 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 470)
        ...
        self.assertEqual(next3["next_cheap_lock"], STANDING_NAMED_BY_CYCLE836)
        self.assertEqual(next3["next_cheap_lock"], CYCLE836_NEXT_CHEAP_LOCK)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_836)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE470_N_I_EACH)
        self.assertEqual(CYCLE470_N_I_EACH, (1,) * STANDING_N)
        ...
        once_a = self.rows[STANDING_NEXT3_MULTI_I_NOW_HAPAX[0]]
        self.assertEqual(once_a.tokens, ("604", "076", "071", "600"))
        self.assertEqual(once_a.site, ("Ia", "Ia9", 9))
        self.assertEqual(once_a.as_map()["I"], 1)
        once_b = self.rows[STANDING_NEXT3_MULTI_I_NOW_HAPAX[1]]
        self.assertEqual(once_b.tokens, ("720", "076", "070", "701"))
    ...
    def test_next_cheap_lock_is_the_locked_cycle_473_next5_list(self):
        """Cycle 473 next-5 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE473_N, 18)
        self.assertEqual(len(CYCLE473_SEQUENCES), 18)
        self.assertTrue(all(len(tokens) == 5 for tokens in CYCLE473_SEQUENCES))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-837 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 837)
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_836"])
        ...
        self.assertEqual(lock["named_by_cycle836"], STANDING_NAMED_BY_CYCLE836)
        ...
        self.assertEqual(tuple(lock["next3_multi_i_now_hapax"]), STANDING_NEXT3_MULTI_I_NOW_HAPAX)
