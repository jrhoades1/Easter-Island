"""I's cycle-241 leftover extra remaining-after-530 3-gram 090 076 280 off-I lock.

Cycle 242 text-search lock. Uses already-vendored A–V and the
cycle-241 leftover extra remaining-after-530 next stem G=280
(K=2 of N_remaining6=29). Does not retune that leftover extra
remaining-after-530 280 lock, leftover extra remaining-after-700
530, leftover extra remaining-after-001 700, leftover extra
remaining-after-001 unique-max (cycle 234 lost), leftover extra
remaining-after-013 001, leftover extra remaining-after-071 013,
...
Same claim-shape as cycle 239 (090 076 530 was I-only 2/0),
cycle 236 (090 076 700 was I-only 2/0), cycle 232 (090 076 001
was I-only 3/0), cycle 229 (090 076 013 was I-only 5/0), cycle
195 (090 076 071 was I-only 6/0), and cycle 171 (076 071 was
I-only 43/0). Cycle 207 lost: 090 076 070 is not I-only (8/1
on T). Cycle 223 lost: 090 076 is not I-only (69/3 on T). This
cycle is the new leftover extra remaining-after-530 3-gram
090 076 280 only. 090 076 530, 090 076 700, 090 076 001,
090 076 013, 090 076 070, 090 076 071, and 090 076 without
280 do not count as this 3-gram. Do not retune leftover n=4,
076-cells, or any detector. Do not lock leftover extra
remaining after 280 this cycle. Do not lock the other four
tied stems (087/011/005/000). Off-I T sites of 090 076 are
not this cycle except as off-I of 090 076 280 if they match.
Do not assume the I-only result.

Locks exact consecutive hits of 090 076 280 on tablet I and
on every other vendored tablet A–H and J–V. Claim that can
lose: i_3gram_090_076_280_i_only (I hits ≥ 1 and off-I hits
== 0). True only if N_off_I == 0. Measured: Ia is exactly 2
at Ia7[2]/Ia7[113]; Ib unpublished 0; every other vendored
tablet is exact-0. Those 2 equal the cycle-241 leftover extra
remaining-after-530 280 cluster (next 4-grams 090 076 280 139
/ 067). Extra I sites not in leftover extra remaining-after-530
= 0 (leftover of leftover would be a later cycle if any
appeared). The claim is true. Not an n≥8 island. Not the
cycle-103 I 5-gram. Nested leftover extra remaining-after-530
K=2 / G=280 N_remaining6=29, cycle 240 2/0 hapax, cycle 239
2/0, cycle 238 K=2 / G=530, cycle 237 2/0 hapax, cycle 236
2/0, cycle 234 7-way tie at 2, cycle 233 3/0 hapax, cycle 232
3/0, cycle 223 69/3, cycle 195 6/0, and cycle 171 43/0 stay.
...
STANDING_IB_SITES = ()
STANDING_LEFTOVER_MATCHING_SITES = CYCLE241_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 2
STANDING_LEFTOVER_MATCHING_NEXT_4GRAMS = CYCLE241_MATCHING_NEXT_4GRAMS
...
STANDING_CLAIM = "i_3gram_090_076_280_i_only"
STANDING_I_3GRAM_090_076_280_I_ONLY = True
STANDING_RESULT = "i_3gram_090_076_280_i_only"
...
def i_3gram_090_076_280_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
def leftover_extra_remaining_after_530_280_subset(
    ...
    """True iff leftover extra remaining-after-530 280 sites ⊆ I 090 076 280."""
...
def extra_i_sites(
    ...
    """I 090 076 280 sites that are not leftover extra remaining-after-530 280."""
...
class TestI3gram090076280IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-241 leftover extra remaining-after-530 3-gram. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 3-gram counts; a gap is not a hit. 090 076 530 / 700 / 001 are not."""
        ...
        overlap = [["090", "076", "280", "090", "076", "280"]]
        ...
        self.assertEqual(ngram_hit_count([["076", "280"]], GRAM3), 0)
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_090_076_280_i_only(1, 0))
        self.assertTrue(i_3gram_090_076_280_i_only(2, 0))
        self.assertFalse(i_3gram_090_076_280_i_only(2, 1))
        self.assertFalse(i_3gram_090_076_280_i_only(1, 1))
        self.assertFalse(i_3gram_090_076_280_i_only(0, 0))
        self.assertFalse(i_3gram_090_076_280_i_only(0, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_090_076_280_i_only")
        self.assertTrue(STANDING_I_3GRAM_090_076_280_I_ONLY)
        self.assertTrue(HYPOTHESIS_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_090_076_280_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_leftover_matching_subset_and_extra_can_fail(self):
        """Leftover extra remaining-after-530 280 ⊆ I sites; extra can be nonempty."""
        provider = MockProvider()
        self.assertTrue(
    ...
    def test_3gram_is_cycle241_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-241 leftover extra remaining-after-530 G, not priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "280"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], CYCLE241_G)
...
class TestMamariI3gram090076280IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover extra remaining-after-530 3-gram 090 076 280 off-I lock."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_090_076_280_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_241_leftover_not_retuned(self):
        """3-gram is the cycle-241 leftover extra remaining-after-530 G, not a new inventory."""
        self.assertEqual(GRAM3, ("090", "076", "280"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], "280")
        prior_241 = self.survey["i_leftover_extra_090_076_remaining_after_530_fwd280"]
        self.assertEqual(prior_241["cycle"], 241)
        self.assertEqual(tuple(prior_241["forward_3gram"]), GRAM3)
        ...
        measured_matching = [list(site) for site in CYCLE241_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_241["matching_leftover_extra_remaining_after_530_sites"]],
            measured_matching,
        )
        self.assertEqual(
            [list(gram) for gram in CYCLE241_MATCHING_NEXT_4GRAMS],
        ...
        self.assertEqual(STANDING_CYCLE241_N_REMAINING6, 29)
    ...
    def test_i_hits_are_two_on_ia_and_equal_leftover_extra_280(self):
        """3-gram is 2 on Ia; Ib 0. Those 2 equal leftover extra remaining-after-530 280."""
        ...
        if self.leftover_matching != self.i_sites:
            self.fail("leftover extra remaining-after-530 280 set drifted from I 090 076 280")
        if self.extra:
        ...
        for (side, line, index), prev4, nxt4 in zip(
            ...
            self.assertIn((side, line, index), CYCLE241_MATCHING_SITES)
            self.assertNotIn((side, line, index), CYCLE234_MATCHING_SITES)
            ...
            self.assertNotIn((side, line, index), CYCLE235_MATCHING_SITES)
            self.assertNotIn((side, line, index), CYCLE236_I_SITES)
            self.assertNotIn((side, line, index), CYCLE238_MATCHING_SITES)
            self.assertNotIn((side, line, index), CYCLE239_I_SITES)
        ...
        self.assertEqual(STANDING_I_NEXT_4GRAMS, CYCLE241_MATCHING_NEXT_4GRAMS)
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_090_076_280_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_090_076_280_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_241_240_239_238_237_236_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 241 K=2/G=280 N_remaining6=29, 240 2/0 hapax, 239 2/0, 238 K=2/G=530, 237 2/0 hapax, 236 2/0, 234 7-way tie, 233 3/0, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-242 3-gram I-only lock."""
        lock = self.survey["i_3gram_090_076_280_i_only"]
        self.assertEqual(lock["cycle"], 242)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["leftover_extra_remaining_after_530_280_sites"]),
            STANDING_LEFTOVER_MATCHING_SITES,
        )
        self.assertEqual(
            lock["leftover_extra_remaining_after_530_280_count"],
            STANDING_LEFTOVER_MATCHING_COUNT,
        )
        self.assertEqual(lock["leftover_extra_remaining_after_530_280_count"], 2)
        self.assertTrue(lock["leftover_extra_remaining_after_530_280_subset_of_i_sites"])
        ...
        self.assertEqual(lock["nested_cycle241_N_remaining6"], STANDING_CYCLE241_N_REMAINING6)
        ...
        self.assertTrue(lock["i_3gram_090_076_280_i_only"])
        self.assertEqual(
            lock["i_3gram_090_076_280_i_only"],
            STANDING_I_3GRAM_090_076_280_I_ONLY,
        )
...
class TestMamariI3gram090076280IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 242 does not touch clustering. 83/62 / Hamming 6 stays."""
