"""Reframed closed-tradition hold-out of the locked leftover-6 next-45 grams.

Cycle 878 text-search lock. Uses already-vendored A–V and the
cycle-553 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 45-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 553 already locked the
I-only claim on these 45-grams as incomplete-set LOSE
(N_with_next45=12, N_no_next45=6, N_i_only=12, N_leak=0). Cycle
877 already locked the reframed claim on the cycle-551 next-44
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–877.

Claim that can lose: each existing next-45 gram is an exact
...
Named claim: cycle553_next45_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 12 with-next45 grams: N=12, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=12, N_i_ge1=12, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=12. Parent N=18, N_with_next45=12,
N_no_next45=6 (Ia10[144], Ia10[143], Ia10[142], Ia8[167],
Ia8[166], Ia8[165]). Every scored gram is exact-0 on H/P/Q and
on A/B/C and an exact contiguous hit once on I. The next-44 hapax
604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076
532 244 999 090 076 057 600 700 076 076 053 177 700 076 057 741
430 076 532 200 059 076 074 379 002 076 244 280 at Ia9[9] extends
to 604 076 071 600 999 050 076 000 002 999 076 092 535 999 208
076 532 244 999 090 076 057 600 700 076 076 053 177 700 076 057
741 430 076 532 200 059 076 074 379 002 076 244 280 001 at Ia9[9]
and hits I once. The next-44 gram 048 700 999 073 064 076 075 720
999 059 076 400 280 999 379 076 200 059 999 280 076 391 002 999
200 076 445 499 049 076 509 009 600 999 430 076 168 076 059 076
532 204 048 076 at Ia10[148] does not extend (new hole at parent
Ia10[142]). Twelve of the thirteen next-44 grams extend. Verdict:
incomplete-set LOSE.
...
next 45-grams as a peel, next 46-grams as a peel, prev-184, or
next-185. The cycle-555 next-46 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next45_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT45_SITES as CYCLE553_SITES,
    STANDING_N as CYCLE553_N,
    STANDING_N_I_EACH as CYCLE553_N_I_EACH,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next46_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE555_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT46_I_ONLY as CYCLE555_ALL_I_ONLY_CLAIM,
    ...
    STANDING_N_SEQUENCES as CYCLE555_N_SEQUENCES,
    STANDING_N_WITH_NEXT46 as CYCLE555_N_WITH_NEXT46,
    STANDING_NO_NEXT46_SITES as CYCLE555_NO_NEXT46_SITES,
    STANDING_RESULT as CYCLE555_RESULT,
...
STANDING_NO_NEXT45_SITES = CYCLE553_NO_NEXT45_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE553_SITES
STANDING_SEQUENCES = CYCLE553_SEQUENCES
...
STANDING_NEXT44_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT44_DROPPED_INDEX = 9
STANDING_NEXT44_ALL_EXTEND = False
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next45_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next45_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 553
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 553
...
STANDING_DO_NOT_RELOCK_CYCLES_832_877 = True
...
class TestMamariILeftoverN6Next45ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 878 reframed closed-tradition lock on the cycle-553 next-45 population."""
    ...
    def test_population_is_the_locked_cycle_553_next45_grams(self):
        """Twelve next-45 sequences and sites come from cycle 553. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE553_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE553_SITES)
        self.assertEqual(CYCLE553_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE553_N, 18)
        self.assertEqual(CYCLE553_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT45, CYCLE553_N_WITH_NEXT45)
        self.assertEqual(STANDING_N_NO_NEXT45, CYCLE553_N_NO_NEXT45)
        self.assertEqual(
            STANDING_NO_NEXT45_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertNotEqual(STANDING_NO_NEXT45_SITES, CYCLE877_NO_NEXT44_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 553)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 45)
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 553 I-only or the cycle 832–877 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 553)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_877)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE553_N_I_EACH)
        self.assertEqual(CYCLE553_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertFalse(STANDING_NEXT44_ALL_EXTEND)
        kept_n44 = tuple(
            seq for index, seq in enumerate(CYCLE877_SEQUENCES)
            if index != STANDING_NEXT44_DROPPED_INDEX
        )
        kept_sites = tuple(
            site for index, site in enumerate(CYCLE877_SITES)
            if index != STANDING_NEXT44_DROPPED_INDEX
        )
        self.assertEqual(len(kept_n44), STANDING_N)
        for n44, n45, site44, site45 in zip(
            kept_n44, STANDING_SEQUENCES, kept_sites, STANDING_SITES, strict=True
        ):
            self.assertEqual(n45[:44], n44)
            self.assertEqual(site45, site44)
        still = self.rows[STANDING_NEXT44_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:44], CYCLE877_SEQUENCES[3])
        self.assertEqual(CYCLE877_SITES[3], ("Ia", "Ia9", 9))
        dropped_tokens = CYCLE877_SEQUENCES[STANDING_NEXT44_DROPPED_INDEX]
        dropped_site = CYCLE877_SITES[STANDING_NEXT44_DROPPED_INDEX]
        self.assertEqual(
            dropped_tokens,
            (
                "048", "700", "999", "073", "064", "076", "075", "720",
                "999", "059", "076", "400", "280", "999", "379", "076",
                "200", "059", "999", "280", "076", "391", "002", "999",
                "200", "076", "445", "499", "049", "076", "509", "009",
                "600", "999", "430", "076", "168", "076", "059", "076",
                "532", "204", "048", "076",
            ),
        )
        self.assertEqual(dropped_site, ("Ia", "Ia10", 148))
        self.assertFalse(any(tokens[:44] == dropped_tokens for tokens in STANDING_SEQUENCES))
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT45_SITES)
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT45_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT45_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT45_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_555_next46_list(self):
        """Cycle 555 next-46 I-only list stays locked. The reframed result key has landed; the claim id is not a survey key."""
        self.assertEqual(CYCLE555_N, 18)
        self.assertEqual(CYCLE555_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE555_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 46 for tokens in CYCLE555_SEQUENCES))
        self.assertTrue(CYCLE555_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE555_ALL_I_ONLY_CLAIM)
        ...
        prior = self.survey[CYCLE555_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_46grams_all_i_only"
        )
        self.assertEqual(CYCLE555_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next46_i_only"
        ))
        self.assertEqual(prior["cycle"], 555)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 12)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE555_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next46"], 6)
        self.assertEqual(prior["N_with_next46"], 12)
        self.assertNotIn(STANDING_NEXT_CHEAP_LOCK, self.survey)
        self.assertIn("i_leftover_n6_next46_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle555_next46_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-878 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 878)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next45_sites"]),
            STANDING_NO_NEXT45_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_877"])
        ...
        self.assertEqual(
            tuple(lock["next44_hapax_still_hapax"]),
            STANDING_NEXT44_HAPAX_STILL_HAPAX,
        )
        self.assertFalse(lock["next44_all_extend"])
        self.assertEqual(lock["next44_all_extend"], STANDING_NEXT44_ALL_EXTEND)
        self.assertEqual(lock["next44_dropped_index"], STANDING_NEXT44_DROPPED_INDEX)
