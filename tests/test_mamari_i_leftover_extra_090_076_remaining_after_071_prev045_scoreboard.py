"""I's cycle-275 leftover extra remaining-after-071 previous-045 lock.

Cycle 278 text-search lock. Uses already-vendored A–V and the
...
previous-4 I-only hapax 2/0 lock. Does not retune cycle 272's
3-gram 090 090 076 I-only lose 3/1 on T extra I=1. Does not
retune cycle 274's 3-gram 076 090 076 I-only lose 3/1 on T
extra I=1. Does not peel T / extra-I of 090 090 076 or of
076 090 076. Does not vendor a new tablet. Does not scrape X.
...
277. Next tied stem by the same largest-id order is 045. Do
not peel 009 this cycle. Do not lock 3-gram 045 090 076
I-only this cycle. Do not retune leftover n=4. Do not retune
the forward peel. Do not retune previous-999 / previous-600 /
previous-090 / previous-076 / previous-071 clusters. Off-I T
sites are not this cycle.

Hypothesis K=2: leftover extra remaining-after-600
remaining-after-090 remaining-after-076 remaining-after-071
I 090 076 sites include exactly 2 that share previous stem
045 (backward 3-gram 045 090 076). Nested-check leftover
extra remaining-after-076 exactly 2 share previous 071,
K_071==2, N_remaining_after_090_and_076_and_071==31,
leftover extra==56, N_I==69 (do not retune
223/224/270/271/272/273/274/275/276/277). On the remaining-
after-071 population (leftover extra I 090 076 whose previous
token is none of 600/090/076/071; N=31; also not 999 because
remaining-after-600 already peeled 999), count previous-token
frequencies. Lock K_045, the sites, and
N_remaining_after_090_and_076_and_071_and_045 = 31 − K_045.
Nested-check cycle 270's 5-way still includes 045 at K=2 on
remaining-after-600 (before subtracting 090/076/071), so after
peeling 090, 076, and 071 the remaining-after-071 K_045 is
expected 2 unless those two 045 sites were the 090/076/071
sites (they should not be: previous 045 vs previous
090/076/071). Measured: K_045=2 at Ia1[15], Ia2[37]; those
matching remaining-after-071 sites equal the cycle-270
remaining-after-600 previous-045 pair. Previous 4-grams
093 045 090 076 / 061 045 090 076.
N_remaining_after_090_and_076_and_071_and_045=29 (31−2).
Claim that can lose:
...
True only if K_045==2 among leftover extra remaining-after-071
I 090 076. The claim is true. This can lose if nested-check K
differs from 2. Nested cycle 277 prev4 hapax 2/0, cycle 276
2/0 extra I=0, cycle 275 K_071=2 N=31 previous 4-grams
076 071 090 076 / 092 071 090 076, cycle 274 3/1 extra I=1,
cycle 273 K_076=2 N=33, cycle 272 3/1 extra I=1, cycle 271
K_090=2 N=35, cycle 270 unique-max false 5-way K=2 G=090,
cycle 269 6/0 N_not_hapax=0, and cycle 268 6/0 extra I=2 stay.
...
STANDING_N_WITH_PREVIOUS = 31
STANDING_N_NO_PREVIOUS = 0
STANDING_NO_PREVIOUS_SITES = ()
STANDING_N_DISTINCT_REMAINING_AFTER_090_AND_076_AND_071 = 29
STANDING_K = 2
STANDING_K_045 = 2
STANDING_G = "045"
...
STANDING_OTHER_TIED_STEMS = ("009",)
STANDING_CYCLE270_K_045 = 2
STANDING_045_STILL_MAX_K_AFTER_600 = True
...
STANDING_REMAINING_AFTER_090_AND_076_AND_071_PREVIOUS_STEMS = (
    "045",
    "048",
    "380",
    "011",
...
STANDING_CYCLE270_045_SITES = (
    (SIDE_IA, "Ia1", 15),
    (SIDE_IA, "Ia2", 37),
)
STANDING_MATCHING_EQUALS_CYCLE270_045_SITES = True
...
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_REMAINING_AFTER_076_REMAINING_AFTER_071_EXACTLY_2_SHARE_PREVIOUS_045 = True
STANDING_RESULT = (
...
STANDING_I_ONLY_OF_045_090_076_IS_NOT_THIS_CYCLE = True
...
STANDING_PREVIOUS_999_600_090_076_071_NOT_RETUNED = True
...
def leftover_extra_remaining_after_090_and_076_and_071(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_071,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra sites whose previous token is not 999, 600, 090, 076, or 071."""
...
def leftover_extra_remaining_after_090_and_076_and_071_previous_stems(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_071,
) -> tuple[str, ...]:
    """Previous stems of remaining-after-071 sites that have a previous token."""
    locked_set = set(locked)
    return tuple(
        prev
...
def leftover_extra_remaining_after_090_and_076_and_071_with_previous_045(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_071,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-071 sites whose previous token is 045."""
    remaining = set(
        leftover_extra_remaining_after_090_and_076_and_071(sites, previous_stems, locked)
    )
    return tuple(
        site
        for site, prev in zip(sites, previous_stems, strict=True)
...
def leftover_extra_remaining_after_090_and_076_and_071_without_previous_045(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_071,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-071 sites whose previous token is not 045."""
    locked_set = set(locked)
    return tuple(
        site
        for site, prev in zip(sites, previous_stems, strict=True)
...
def leftover_extra_remaining_after_600_with_previous_045(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_600,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-600 sites whose previous token is 045."""
...
def matching_equals_cycle270_045_set(
    matching_sites: tuple[tuple[str, str, int], ...],
    cycle270_045_sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE270_045_SITES,
) -> bool:
    """True iff remaining-after-071 previous-045 sites equal the cycle-270 045 pair."""
    return matching_sites == cycle270_045_sites


def matching_leftover_extra_remaining_after_090_and_076_and_071_previous_045_local_4gram_rows(
    ...
    """Survey-shaped matching leftover extra remaining-after-071 previous-045 rows."""
    rows = []
...
def leftover_extra_remaining_after_090_and_076_and_071_nested_counts_hold(
    n_i: int,
    n_leftover: int,
    k_999: int,
    ...
    """Nested leftover extra 69/56, K_999=15, K_600=4, K_090=2, K_076=2, K_071=2, remaining-after-071=31."""
    return (
        n_i == expected_i
...
def i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(
    k: int,
    expected: int = HYPOTHESIS_K,
) -> bool:
    """True iff K_045 equals the hypothesized 2."""
    return k == expected


class TestILeftoverExtra090076RemainingAfter071Previous045Helpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-071 previous 045. No CV, no LLM."""

    def test_previous_045_requires_remaining_after_071_previous_stem(self):
        """Previous stem 045 is remaining-after-071; locked 999/600/090/076/071 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_BACKWARD, ("045", "090", "076"))
        ...
        self.assertEqual(LOCKED_PREVIOUS_STEMS_AFTER_071, ("999", "600", "090", "076", "071"))
        ...
        has_045 = ["093", "045", "090", "076"]
        self.assertEqual(site_previous_stem(has_045, 2, GRAM2), "045")
        self.assertEqual(site_backward_3gram(has_045, 2, GRAM2), GRAM3_BACKWARD)
        self.assertEqual(
            site_previous_4gram(has_045, 2, GRAM2),
            ("093", "045", "090", "076"),
        )
        has_061_045 = ["061", "045", "090", "076"]
        self.assertEqual(site_previous_stem(has_061_045, 2, GRAM2), "045")
        self.assertEqual(
            site_previous_4gram(has_061_045, 2, GRAM2),
            ("061", "045", "090", "076"),
        )
        has_071 = ["076", "071", "090", "076"]
        self.assertEqual(site_previous_stem(has_071, 2, GRAM2), "071")
        self.assertNotEqual(site_previous_stem(has_071, 2, GRAM2), "045")
        has_076 = ["071", "076", "090", "076"]
        self.assertEqual(site_previous_stem(has_076, 2, GRAM2), "076")
        self.assertNotEqual(site_previous_stem(has_076, 2, GRAM2), "045")
        has_090 = ["011", "090", "090", "076"]
        self.assertEqual(site_previous_stem(has_090, 2, GRAM2), "090")
        self.assertNotEqual(site_previous_stem(has_090, 2, GRAM2), "045")
        has_600 = ["076", "600", "090", "076"]
        self.assertEqual(site_previous_stem(has_600, 2, GRAM2), "600")
        self.assertNotEqual(site_previous_stem(has_600, 2, GRAM2), "045")
        has_999 = ["000", "999", "090", "076"]
        self.assertEqual(site_previous_stem(has_999, 2, GRAM2), "999")
        self.assertNotEqual(site_previous_stem(has_999, 2, GRAM2), "045")
        other_prev = ["093", "009", "090", "076"]
        self.assertEqual(site_previous_stem(other_prev, 2, GRAM2), "009")
        self.assertNotEqual(site_backward_3gram(other_prev, 2, GRAM2), GRAM3_BACKWARD)
        one_token_before = ["045", "090", "076"]
        self.assertEqual(site_previous_stem(one_token_before, 1, GRAM2), "045")
        ...
        planted_stems = ("045", "071", "076", "090", "999", "600", None, "009")
        rem = leftover_extra_remaining_after_090_and_076_and_071(planted_sites, planted_stems)
        self.assertEqual(rem, (planted_sites[0], planted_sites[6], planted_sites[7]))
        self.assertEqual(
    ...
    def test_exactly_2_can_fail(self):
        ...
        provider = MockProvider()
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(2)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(0)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(1)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(3)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(29)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(31)
        )
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_REMAINING_AFTER_076_REMAINING_AFTER_071_EXACTLY_2_SHARE_PREVIOUS_045
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_REMAINING_AFTER_076_REMAINING_AFTER_071_EXACTLY_2_SHARE_PREVIOUS_045,
            HYPOTHESIS_K == STANDING_K_045,
        )
        self.assertEqual(
    ...
    def test_cycle270_045_sites_survive_peeling_090_076_071_and_are_not_those_sites(self):
        """Cycle-270 previous-045 pair is disjoint from previous-090/076/071; peeling them keeps K_045=2."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle270_045_set(STANDING_MATCHING_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE270_045_SITES)
        self.assertEqual(STANDING_MATCHING_SITES, STANDING_CYCLE270_045_SITES)
        ...
        for site in STANDING_MATCHING_SITES:
            ...
            self.assertIn(site, CYCLE275_REMAINING_AFTER_090_AND_076_AND_071_SITES)
        for site in CYCLE275_MATCHING_SITES:
            self.assertNotIn(site, STANDING_MATCHING_SITES)
            self.assertNotIn(site, STANDING_REMAINING_AFTER_090_AND_076_AND_071_SITES)
        ...
        self.assertFalse(matching_equals_cycle270_045_set(planted))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(
                len(planted)
            )
        ...
        self.assertFalse(matching_equals_cycle270_045_set(swapped))
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(
                len(swapped)
            )
        ...
        self.assertEqual(STANDING_CYCLE270_K_045, 2)
        for stem, count, sites, _grams in CYCLE270_FREQUENCY:
            if stem == "045":
                self.assertEqual(count, 2)
                self.assertEqual(sites, STANDING_CYCLE270_045_SITES)
        self.assertTrue(STANDING_DO_NOT_PEEL_009)
        self.assertTrue(STANDING_I_ONLY_OF_045_090_076_IS_NOT_THIS_CYCLE)
...
class TestMamariILeftoverExtra090076RemainingAfter071Prev045Scoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-071 previous-045 lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining = leftover_extra_remaining_after_090_and_076_and_071(
            self.leftover_sites,
            self.previous_stems,
        )
        self.remaining_stems = leftover_extra_remaining_after_090_and_076_and_071_previous_stems(
            self.leftover_sites,
            self.previous_stems,
        )
        ...
        self.matching = leftover_extra_remaining_after_090_and_076_and_071_with_previous_045(
            self.leftover_sites,
            self.previous_stems,
        )
        self.without = leftover_extra_remaining_after_090_and_076_and_071_without_previous_045(
            self.leftover_sites,
            self.previous_stems,
        )
        self.share_045_after_600 = leftover_extra_remaining_after_600_with_previous_045(
            self.leftover_sites,
            self.previous_stems,
        )
        ...
        self.n_remaining_after_090_and_076_and_071 = len(self.remaining)
        self.n_with_previous = len(self.remaining_stems)
        self.n_no_previous = self.n_remaining_after_090_and_076_and_071 - self.n_with_previous
        ...
        self.k_045_after_600 = len(self.share_045_after_600)
        self.tied_after_600 = tuple(
            stem for stem, count, _sites, _grams in self.frequency_600 if count == 2
        )
        self.equals_cycle270_045 = matching_equals_cycle270_045_set(self.matching)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_exactly_2_share_previous_045(
            self.k_045,
        )
    ...
    def test_tokens_and_nested_leftover_extra_56_69_k071_2_n31_not_retuned(self):
        ...
        self.assertTrue(STANDING_PREVIOUS_999_600_090_076_071_NOT_RETUNED)
    ...
    def test_counts_2_of_31_and_hypothesis_k_2_holds(self):
        ...
        self.assertEqual(self.remaining, STANDING_REMAINING_AFTER_090_AND_076_AND_071_SITES)
        self.assertEqual(self.remaining, CYCLE275_REMAINING_AFTER_090_AND_076_AND_071_SITES)
        self.assertEqual(
        ...
        self.assertEqual(STANDING_N_WITH_PREVIOUS, 31)
        ...
        if "045" not in self.tied_after_600 or self.k_045_after_600 != 2:
            self.fail("nested cycle 270 5-way no longer includes 045 at K=2")
        self.assertEqual(self.k_045_after_600, STANDING_CYCLE270_K_045)
        self.assertEqual(STANDING_CYCLE270_K_045, 2)
        self.assertEqual(self.share_045_after_600, STANDING_CYCLE270_045_SITES)
        self.assertEqual(self.share_045_after_600, self.matching)
        self.assertTrue(STANDING_045_STILL_MAX_K_AFTER_600)
        self.assertEqual(self.k, STANDING_K)
        self.assertEqual(self.k_045, STANDING_K_045)
        self.assertEqual(STANDING_K_045, HYPOTHESIS_K)
        ...
        self.assertEqual(
            self.k_045 + self.n_remaining_after_090_and_076_and_071_and_045,
            self.n_remaining_after_090_and_076_and_071,
        )
        self.assertEqual(2 + 29, 31)
        if self.k_045 != 2:
        ...
        self.assertEqual(self.without, STANDING_REMAINING_AFTER_090_AND_076_AND_071_AND_045_SITES)
        self.assertTrue(self.equals_cycle270_045)
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE270_045_SITES)
        self.assertTrue(matching_equals_cycle270_045_set(self.matching))
        if len(self.matching) != 2 or not self.equals_cycle270_045:
            self.fail(
                "leftover extra remaining-after-071 previous-045 set drifted from cycle-270 045 pair"
            )
    ...
    def test_matching_remaining_after_071_sites_have_previous_045(self):
        """Two remaining-after-071 leftover extra sites are 045 090 076."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_previous_4grams, STANDING_MATCHING_PREVIOUS_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia1", 15), ("093", "045", "090", "076")),
            ((SIDE_IA, "Ia2", 37), ("061", "045", "090", "076")),
        )
        for (site, prev4), (want_site, want_prev) in zip(
            ...
            self.assertEqual(stems[index - 1], "045")
            self.assertEqual(site_previous_stem(stems, index, GRAM2), "045")
            ...
            self.assertIn(site, STANDING_CYCLE270_045_SITES)
        self.assertEqual(self.matching, STANDING_CYCLE270_045_SITES)
        self.assertTrue(matching_equals_cycle270_045_set(self.matching))
        for site in self.without:
            ...
            self.assertIn(site, self.remaining)
            self.assertIn(site, STANDING_LEFTOVER_SITES)
            self.assertNotIn(site, STANDING_CYCLE270_045_SITES)
        for site in self.share_999:
            self.assertNotIn(site, self.remaining)
            self.assertNotIn(site, self.matching)
        ...
        for stem in STANDING_OTHER_TIED_STEMS:
            other = leftover_extra_remaining_after_090_and_076_and_071_with_previous_045(
                self.leftover_sites,
                self.previous_stems,
                stem,
            )
            ...
            for site in other:
                self.assertNotIn(site, self.matching)
                self.assertIn(site, self.remaining)
        self.assertEqual(len(self.without), STANDING_N_REMAINING_AFTER_090_AND_076_AND_071_AND_045)
        ...
        self.assertEqual(
            matching_leftover_extra_remaining_after_090_and_076_and_071_previous_045_local_4gram_rows(),
            [
                {
                    "tablet": "I",
                    "side": SIDE_IA,
                    "line": "Ia1",
                    "index": 15,
                    "previous_4gram": ["093", "045", "090", "076"],
                    "backward_3gram": ["045", "090", "076"],
                },
                {
                    "tablet": "I",
                    "side": SIDE_IA,
                    "line": "Ia2",
                    "index": 37,
                    "previous_4gram": ["061", "045", "090", "076"],
                    "backward_3gram": ["045", "090", "076"],
                },
            ],
        )
    ...
    def test_existing_277_276_275_274_273_272_271_270_269_268_and_223_still_compute(self):
        """Cycle 277 2/0, 276 2/0 extra I=0, 275 K_071=2/31, 274 3/1, 273 K_076=2/33 stay."""
        ...
        if (
            ...
            self.fail(
                ...
                "5-way tie G=090 K=2 including 045 drifted"
            )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-278 leftover extra remaining-after-071 previous-045 lock."""
        lock = self.survey[
            "i_leftover_extra_090_076_remaining_after_600_remaining_after_090_remaining_after_076_remaining_after_071_previous_045"
        ]
        self.assertEqual(lock["cycle"], 278)
        ...
        self.assertEqual(tuple(lock["backward_3gram"]), ("045", "090", "076"))
        self.assertEqual(lock["n3"], STANDING_N3)
        self.assertEqual(lock["n4"], STANDING_N4)
        self.assertEqual(
            tuple(lock["locked_previous_stems_after_071"]),
            LOCKED_PREVIOUS_STEMS_AFTER_071,
        )
        self.assertEqual(
            tuple(lock["locked_previous_stems_after_071"]),
        ...
        self.assertEqual(
            tuple(lock["remaining_after_090_and_076_and_071_previous_stems"]),
            STANDING_REMAINING_AFTER_090_AND_076_AND_071_PREVIOUS_STEMS,
        )
        self.assertEqual(lock["N_with_previous"], STANDING_N_WITH_PREVIOUS)
        self.assertEqual(lock["N_with_previous"], 31)
        ...
        self.assertEqual(
            lock["N_distinct_remaining_after_090_and_076_and_071"],
            STANDING_N_DISTINCT_REMAINING_AFTER_090_AND_076_AND_071,
        )
        self.assertEqual(lock["N_distinct_remaining_after_090_and_076_and_071"], 29)
        self.assertEqual(lock["G"], STANDING_G)
        self.assertEqual(lock["G"], "045")
        ...
        self.assertEqual(lock["cycle270_K_045"], STANDING_CYCLE270_K_045)
        self.assertEqual(lock["cycle270_K_045"], 2)
        self.assertTrue(lock["045_still_max_K_after_600"])
        ...
        self.assertTrue(lock["matching_equals_cycle270_045_sites"])
        self.assertEqual(
            lock["matching_equals_cycle270_045_sites"],
            STANDING_MATCHING_EQUALS_CYCLE270_045_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["cycle270_045_sites"]),
            STANDING_CYCLE270_045_SITES,
        )
        ...
        self.assertTrue(lock["i_only_of_045_090_076_is_not_this_cycle"])
        ...
        self.assertTrue(lock["previous_999_600_090_076_071_not_retuned"])
...
class TestMamariILeftoverExtra090076RemainingAfter071Prev045ImageSnapshot(
    unittest.TestCase
):
    """Cycle 278 does not touch clustering. 83/62 / Hamming 6 stays."""
