"""Reframed closed-tradition hold-out of the locked leftover-6 next-9 grams.

Cycle 842 text-search lock. Uses already-vendored A–V and the
cycle-481 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 9-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 481 already locked the I-only claim
on these 9-grams as incomplete-set LOSE (N_with_next9=15,
N_no_next9=3, N_i_only=15, N_leak=0). Cycle 841 already locked
the reframed claim on the cycle-479 next-8 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–841.

Claim that can lose: each existing next-9 gram is an exact
...
Measured on the 15 with-next9 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next9=15,
N_no_next9=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-8 hapax 604 076 071 600 999 050 076 000
extends to 604 076 071 600 999 050 076 000 002 at Ia9[9] and
hits I once. All fifteen next-8 grams extend; no new hole.
...
Do not launch leftover-6 previous 9-grams, leftover 9-grams,
next 10-grams, prev-184, or next-185. The cycle-483 next-10
...
from tests.test_mamari_i_leftover_n6_next8_closed_tradition_hpq_abc_scoreboard import (
    CYCLE840_SEQUENCES,
    CYCLE840_SITES,
    STANDING_HITS_I as CYCLE841_HITS_I,
    STANDING_N_HOLD as CYCLE841_N_HOLD,
    STANDING_N_LEAK as CYCLE841_N_LEAK,
    STANDING_NEXT7_HAPAX_DROPPED_INDEX as CYCLE841_NEXT7_DROPPED_INDEX,
    STANDING_NEXT_CHEAP_LOCK as CYCLE841_NEXT_CHEAP_LOCK,
    STANDING_NO_NEXT8_SITES as CYCLE841_NO_NEXT8_SITES,
    STANDING_RESULT as CYCLE841_RESULT,
    STANDING_SEQUENCES as CYCLE841_SEQUENCES,
    STANDING_SITES as CYCLE841_SITES,
    STANDING_VERDICT as CYCLE841_VERDICT,
)
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next9_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT9_SITES as CYCLE481_SITES,
    STANDING_N as CYCLE481_N,
    STANDING_N_I_EACH as CYCLE481_N_I_EACH,
...
STANDING_NO_NEXT9_SITES = CYCLE481_NO_NEXT9_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE481_SITES
STANDING_SEQUENCES = CYCLE481_SEQUENCES
...
STANDING_NEXT8_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT8_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next9_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next9_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 481
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 481
...
class TestMamariILeftoverN6Next9ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 842 reframed closed-tradition lock on the cycle-481 next-9 population."""
    ...
    def test_population_is_the_locked_cycle_481_next9_grams(self):
        """Fifteen next-9 sequences and sites come from cycle 481. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE481_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE481_SITES)
        self.assertEqual(CYCLE481_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE481_N, 18)
        self.assertEqual(CYCLE481_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT9, CYCLE481_N_WITH_NEXT9)
        self.assertEqual(STANDING_N_NO_NEXT9, CYCLE481_N_NO_NEXT9)
        self.assertEqual(
            STANDING_NO_NEXT9_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT9_SITES, CYCLE841_NO_NEXT8_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 481)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 9)
        prior = self.survey[CYCLE481_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_9grams_all_i_only"
        )
        self.assertEqual(CYCLE481_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next9_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE841, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 481 I-only or the cycle 832–841 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 481)
        ...
        next8 = self.survey[CYCLE841_RESULT]
        self.assertEqual(next8["cycle"], STANDING_NEXT8_REFRAMED_CYCLE)
        self.assertEqual(next8["verdict"], CYCLE841_VERDICT)
        self.assertEqual(next8["verdict"], "incomplete-set LOSE")
        self.assertEqual(next8["N_hold"], CYCLE841_N_HOLD)
        self.assertEqual(next8["N_hold"], 0)
        self.assertEqual(next8["N_leak"], CYCLE841_N_LEAK)
        self.assertEqual(next8["N_leak"], 0)
        self.assertEqual(next8["hits_I"], CYCLE841_HITS_I)
        self.assertEqual(next8["hits_I"], 15)
        self.assertEqual(next8["N_hpq_ge1"], 0)
        self.assertEqual(next8["N_exact0_abc"], 15)
        self.assertEqual(next8["N_with_next8"], 15)
        self.assertEqual(next8["N_no_next8"], 3)
        self.assertEqual(next8["next_cheap_lock"], STANDING_NAMED_BY_CYCLE841)
        self.assertEqual(next8["next_cheap_lock"], CYCLE841_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE841, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_841)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE481_N_I_EACH)
        self.assertEqual(CYCLE481_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT8_ALL_EXTEND)
        for n8, n9, site8, site9 in zip(
            CYCLE841_SEQUENCES, STANDING_SEQUENCES, CYCLE841_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n9[:8], n8)
            self.assertEqual(site9, site8)
        still = self.rows[STANDING_NEXT8_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            ("604", "076", "071", "600", "999", "050", "076", "000", "002"),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:8], CYCLE841_SEQUENCES[3])
        self.assertEqual(CYCLE841_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT9_SITES)
    ...
    def test_window_peels_closed_and_prev1_list_is_absent(self):
        ...
        self.assertTrue(STANDING_DO_NOT_LAUNCH_PREV9)
        self.assertTrue(STANDING_DO_NOT_LAUNCH_LEFTOVER_9GRAMS)
        self.assertTrue(STANDING_DO_NOT_LAUNCH_NEXT10)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_483_next10_list(self):
        """Cycle 483 next-10 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE483_N, 18)
        self.assertEqual(CYCLE483_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE483_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 10 for tokens in CYCLE483_SEQUENCES))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-842 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 842)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next9_sites"]),
            STANDING_NO_NEXT9_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_841"])
        ...
        self.assertTrue(lock["do_not_launch_leftover_9grams"])
        ...
        self.assertEqual(lock["named_by_cycle841"], STANDING_NAMED_BY_CYCLE841)
        ...
        self.assertEqual(
            tuple(lock["next8_hapax_still_hapax"]),
            STANDING_NEXT8_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next8_all_extend"])
        self.assertEqual(lock["next8_all_extend"], STANDING_NEXT8_ALL_EXTEND)
