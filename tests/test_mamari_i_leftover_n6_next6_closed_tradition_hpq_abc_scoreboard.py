"""Reframed closed-tradition hold-out of the locked leftover-6 next-6 grams.

Cycle 839 text-search lock. Uses already-vendored A–V and the
cycle-475 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 6-grams (17 sequences; parent N=18; one
line-final hole at Ia8[167]). HEAD-check: that gram list exists
and is not yet locked under this reframed rule. Cycle 475
already locked the I-only claim on these 6-grams as
incomplete-set LOSE (N_with_next6=17, N_no_next6=1, N_i_only=17,
N_leak=0). Cycle 838 already locked the reframed claim on the
cycle-473 next-5 grams as incomplete-set LOSE. This cycle does
not re-lock either claim, and does not re-lock Cycles 832–838.

Claim that can lose: each existing next-6 gram is an exact
...
Measured on the 17 with-next6 grams: N=17, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=17, N_i_ge1=17, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=17. Parent N=18, N_with_next6=17,
N_no_next6=1 (Ia8[167]). Every scored gram is exact-0 on H/P/Q
and on A/B/C and an exact contiguous hit once on I. The next-5
hapax grams stay hapax once extended: 604 076 071 600 999 050
at Ia9[9] hits I once; 720 076 070 701 214 076 at Ia8[171]
hits I once. Verdict: incomplete-set LOSE.
...
from tests.test_mamari_i_leftover_n6_next5_closed_tradition_hpq_abc_scoreboard import (
    ...
    STANDING_NEXT_CHEAP_LOCK as CYCLE838_NEXT_CHEAP_LOCK,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next6_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT6_SITES as CYCLE475_SITES,
    STANDING_N as CYCLE475_N,
    STANDING_N_I_EACH as CYCLE475_N_I_EACH,
...
STANDING_NEXT5_HAPAX_STILL_HAPAX = (3, 13)
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next6_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next6_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 475
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 475
...
STANDING_DO_NOT_RELOCK_CYCLES_832_838 = True
...
class TestMamariILeftoverN6Next6ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 839 reframed closed-tradition lock on the cycle-475 next-6 population."""
    ...
    def test_population_is_the_locked_cycle_475_next6_grams(self):
        """Seventeen next-6 sequences and sites come from cycle 475. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE475_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE475_SITES)
        self.assertEqual(CYCLE475_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE475_N, 18)
        self.assertEqual(CYCLE475_N_SEQUENCES, 17)
        self.assertEqual(STANDING_N_WITH_NEXT6, CYCLE475_N_WITH_NEXT6)
        self.assertEqual(STANDING_N_NO_NEXT6, CYCLE475_N_NO_NEXT6)
        self.assertEqual(STANDING_NO_NEXT6_SITES, (("Ia", "Ia8", 167),))
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 475)
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 475 I-only or the cycle 832–838 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 475)
        ...
        self.assertEqual(next5["next_cheap_lock"], STANDING_NAMED_BY_CYCLE838)
        self.assertEqual(next5["next_cheap_lock"], CYCLE838_NEXT_CHEAP_LOCK)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_838)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_measured_table_is_incomplete_set_lose(self):
        """H/P/Q and A/B/C are exact-0. hits_I is 17 and is not a leak."""
        ...
        self.assertEqual(self.summary["hits_I"], 17)
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE475_N_I_EACH)
        self.assertEqual(CYCLE475_N_I_EACH, (1,) * STANDING_N)
        ...
        once_a = self.rows[STANDING_NEXT5_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(once_a.tokens, ("604", "076", "071", "600", "999", "050"))
        self.assertEqual(once_a.site, ("Ia", "Ia9", 9))
        self.assertEqual(once_a.as_map()["I"], 1)
        once_b = self.rows[STANDING_NEXT5_HAPAX_STILL_HAPAX[1]]
        self.assertEqual(once_b.tokens, ("720", "076", "070", "701", "214", "076"))
    ...
    def test_next_cheap_lock_is_the_locked_cycle_477_next7_list(self):
        """Cycle 477 next-7 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE477_N, 18)
        self.assertEqual(CYCLE477_N_SEQUENCES, 16)
        self.assertEqual(len(CYCLE477_SEQUENCES), 16)
        self.assertTrue(all(len(tokens) == 7 for tokens in CYCLE477_SEQUENCES))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-839 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 839)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next6_sites"]),
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_838"])
        ...
        self.assertEqual(lock["named_by_cycle838"], STANDING_NAMED_BY_CYCLE838)
        ...
        self.assertEqual(
            tuple(lock["next5_hapax_still_hapax"]),
            STANDING_NEXT5_HAPAX_STILL_HAPAX,
        )
        self.assertNotIn("leak_index", lock)
        self.assertFalse(lock["new_tablet"])
        self.assertEqual(lock["from_cycle"], STANDING_FROM_CYCLE)
        self.assertEqual(lock["image_track"], "parked")
        self.assertEqual(len(lock["rows"]), 17)
