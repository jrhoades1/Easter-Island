"""Reframed closed-tradition hold-out of the locked leftover-6 next-62 grams.

Cycle 895 text-search lock. Uses already-vendored A–V and the
cycle-587 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 62-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 587 already locked the
I-only claim on these 62-grams as incomplete-set LOSE
(N_with_next62=12, N_no_next62=6, N_i_only=12, N_leak=0). Cycle
894 already locked the reframed claim on the cycle-585 next-61
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–894.

Claim that can lose: each existing next-62 gram is an exact
...
Named claim: cycle587_next62_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 12 with-next62 grams: N=12, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=12, N_i_ge1=12, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=12. Parent N=18, N_with_next62=12,
N_no_next62=6 (Ia10[144], Ia10[143], Ia10[142], Ia8[167],
Ia8[166], Ia8[165]). Every scored gram is exact-0 on H/P/Q and
on A/B/C and an exact contiguous hit once on I. The next-61 hapax
...
074 379 002 076 244 280 001 076 532 071 065 071 999 700 076 480 600 005 076 420 519 999 006 at Ia9[9] extends
to 604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999
090 076 057 600 700 076 076 053 177 700 076 057 741 430 076 532 200 059 076
074 379 002 076 244 280 001 076 532 071 065 071 999 700 076 480 600 005 076 420 519 999 006 076 at Ia9[9]
and hits I once. All twelve next-61 grams extend; no new hole.
...
next 62-grams as a peel, next 63-grams as a peel, prev-184, or
next-185. The cycle-589 next-63 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next62_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE587_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT62_I_ONLY as CYCLE587_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT62_SITES as CYCLE587_SITES,
    STANDING_N as CYCLE587_N,
    STANDING_N_I_EACH as CYCLE587_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE587_N_I_ONLY,
    STANDING_N_LEAK as CYCLE587_N_LEAK,
    STANDING_N_NO_NEXT62 as CYCLE587_N_NO_NEXT62,
    STANDING_N_SEQUENCES as CYCLE587_N_SEQUENCES,
    STANDING_N_WITH_NEXT62 as CYCLE587_N_WITH_NEXT62,
    STANDING_NO_NEXT62_SITES as CYCLE587_NO_NEXT62_SITES,
    STANDING_RESULT as CYCLE587_RESULT,
...
STANDING_NO_NEXT62_SITES = CYCLE587_NO_NEXT62_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE587_SITES
STANDING_SEQUENCES = CYCLE587_SEQUENCES
...
STANDING_NEXT61_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT61_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next62_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next62_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 587
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 587
...
STANDING_DO_NOT_RELOCK_CYCLES_832_894 = True
...
class TestMamariILeftoverN6Next62ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 895 reframed closed-tradition lock on the cycle-587 next-62 population."""
    ...
    def test_population_is_the_locked_cycle_587_next62_grams(self):
        """Twelve next-62 sequences and sites come from cycle 587. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE587_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE587_SITES)
        self.assertEqual(CYCLE587_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE587_N, 18)
        self.assertEqual(CYCLE587_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT62, CYCLE587_N_WITH_NEXT62)
        self.assertEqual(STANDING_N_NO_NEXT62, CYCLE587_N_NO_NEXT62)
        self.assertEqual(
            STANDING_NO_NEXT62_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT62_SITES, CYCLE894_NO_NEXT61_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE587_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE587_ALL_I_ONLY_CLAIM)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 62)
        prior = self.survey[CYCLE587_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_62grams_all_i_only"
        )
        self.assertEqual(CYCLE587_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next62_i_only"
        ))
        self.assertEqual(prior["cycle"], 587)
        ...
        self.assertFalse(prior[CYCLE587_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next62"], 6)
        self.assertEqual(prior["N_with_next62"], 12)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE894, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 587 I-only or the cycle 832–894 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 587)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_894)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE587_N_I_EACH)
        self.assertEqual(CYCLE587_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT61_ALL_EXTEND)
        for n61, n62, site61, site62 in zip(
            CYCLE894_SEQUENCES, STANDING_SEQUENCES, CYCLE894_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n62[:61], n61)
            self.assertEqual(site62, site61)
        still = self.rows[STANDING_NEXT61_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:61], CYCLE894_SEQUENCES[3])
        self.assertEqual(CYCLE894_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT62_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT62_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT62_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT62_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT62_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT62_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_589_next63_list(self):
        """Cycle 589 next-63 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE589_N, 18)
        self.assertEqual(CYCLE589_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE589_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 63 for tokens in CYCLE589_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next63_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle589_next63_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-895 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 895)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next62_sites"]),
            STANDING_NO_NEXT62_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_894"])
        ...
        self.assertEqual(
            tuple(lock["next61_hapax_still_hapax"]),
            STANDING_NEXT61_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next61_all_extend"])
        self.assertEqual(lock["next61_all_extend"], STANDING_NEXT61_ALL_EXTEND)
