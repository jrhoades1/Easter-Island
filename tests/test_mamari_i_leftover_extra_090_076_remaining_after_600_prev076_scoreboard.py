"""I's cycle-271 leftover extra remaining-after-090 previous-076 lock.

Cycle 273 text-search lock. Uses already-vendored A–V and the
...
previous-090 lock. Does not retune cycle 272's 3-gram
090 090 076 I-only lose 3/1 on T extra I=1. Does not peel T /
extra-I of 090 090 076. Does not vendor a new tablet. Does
...
its T / extra-I this cycle. Next tied stem by the same
largest-id order is 076. Do not peel 071/045/009 this cycle.
Do not lock 3-gram 076 090 076 I-only this cycle. Do not
retune leftover n=4. Do not retune the forward peel. Do not
retune previous-999 / previous-600 / previous-090 clusters.
Off-I T sites are not this cycle.

Hypothesis K=2: leftover extra remaining-after-600
remaining-after-090 I 090 076 sites include exactly 2 that
share previous stem 076 (backward 3-gram 076 090 076).
Nested-check leftover extra remaining-after-600 exactly 2
share previous 090, K_090==2, N_remaining_after_090==35,
leftover extra==56, N_I==69 (do not retune 223/224/270/271/272).
On the remaining-after-090 population (leftover extra I
090 076 whose previous token is neither 600 nor 090; N=35;
also not 999 because remaining-after-600 already peeled 999),
count previous-token frequencies. Lock K_076, the sites, and
N_remaining_after_090_and_076 = 35 − K_076. Nested-check
cycle 270's 5-way still includes 076 at K=2 on remaining-
after-600 (before subtracting 090), so after peeling 090 the
remaining-after-090 K_076 is expected 2 unless those two 076
sites were the 090 sites (they should not be: previous 076
vs previous 090). Measured: K_076=2 at Ia2[165], Ia13[152];
those matching remaining-after-090 sites equal the cycle-270
remaining-after-600 previous-076 pair. N_remaining_after_090_and_076=33
(35−2). Claim that can lose:
i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076.
True only if K_076==2 among leftover extra remaining-after-090
I 090 076. The claim is true. This can lose if nested-check K
differs from 2. Nested cycle 272 3/1 on T extra I=1, cycle 271
K_090=2 N_remaining_after_090=35 both previous 4-grams
011 090 090 076, cycle 270 unique-max false 5-way K=2 G=090,
and cycle 268 6/0 extra I=2 stay. Do not assume the result;
...
STANDING_N_WITH_PREVIOUS = 35
STANDING_N_NO_PREVIOUS = 0
STANDING_NO_PREVIOUS_SITES = ()
STANDING_N_DISTINCT_REMAINING_AFTER_090 = 31
STANDING_K = 2
STANDING_K_076 = 2
STANDING_G = "076"
STANDING_N_WITHOUT = 33
...
STANDING_OTHER_TIED_STEMS = ("071", "045", "009")
STANDING_CYCLE270_K_076 = 2
STANDING_076_STILL_MAX_K_AFTER_600 = True
STANDING_REMAINING_AFTER_090_SITES = (
    (SIDE_IA, "Ia1", 15),
    (SIDE_IA, "Ia1", 27),
    (SIDE_IA, "Ia1", 59),
    (SIDE_IA, "Ia1", 96),
...
STANDING_REMAINING_AFTER_090_PREVIOUS_STEMS = (
    "045",
    "048",
    "380",
    "011",
...
STANDING_CYCLE270_076_SITES = (
    (SIDE_IA, "Ia2", 165),
    (SIDE_IA, "Ia13", 152),
)
STANDING_MATCHING_EQUALS_CYCLE270_076_SITES = True
...
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_EXACTLY_2_SHARE_PREVIOUS_076 = True
STANDING_RESULT = (
...
STANDING_I_ONLY_OF_076_090_076_IS_NOT_THIS_CYCLE = True
...
STANDING_PREVIOUS_999_600_090_NOT_RETUNED = True
...
def leftover_extra_remaining_after_090(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_090,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra sites whose previous token is not 999, 600, or 090."""
...
def leftover_extra_remaining_after_090_previous_stems(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_090,
) -> tuple[str, ...]:
    """Previous stems of remaining-after-090 sites that have a previous token."""
    locked_set = set(locked)
    return tuple(
        prev
...
def leftover_extra_remaining_after_090_with_previous_076(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_090,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-090 sites whose previous token is 076."""
    remaining = set(leftover_extra_remaining_after_090(sites, previous_stems, locked))
    return tuple(
        site
        for site, prev in zip(sites, previous_stems, strict=True)
...
def leftover_extra_remaining_after_090_without_previous_076(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_090,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-090 sites whose previous token is not 076."""
    locked_set = set(locked)
    return tuple(
        site
        for site, prev in zip(sites, previous_stems, strict=True)
...
def leftover_extra_remaining_after_600_with_previous_076(
    sites: tuple[tuple[str, str, int], ...],
    previous_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_PREVIOUS_STEMS_AFTER_600,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-600 sites whose previous token is 076."""
...
def matching_equals_cycle270_076_set(
    matching_sites: tuple[tuple[str, str, int], ...],
    cycle270_076_sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE270_076_SITES,
) -> bool:
    """True iff remaining-after-090 previous-076 sites equal the cycle-270 076 pair."""
    return matching_sites == cycle270_076_sites


def matching_leftover_extra_remaining_after_090_previous_076_local_4gram_rows(
    ...
    """Survey-shaped matching leftover extra remaining-after-090 previous-076 rows."""
    rows = []
...
def leftover_extra_remaining_after_090_nested_counts_hold(
    n_i: int,
    n_leftover: int,
    k_999: int,
    ...
    """Nested leftover extra 69/56, K_999=15, K_600=4, K_090=2, remaining-after-090=35."""
    return (
        n_i == expected_i
...
def i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(
    k: int,
    expected: int = HYPOTHESIS_K,
) -> bool:
    """True iff K_076 equals the hypothesized 2."""
    return k == expected


class TestILeftoverExtra090076RemainingAfter090Previous076Helpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-090 previous 076. No CV, no LLM."""

    def test_previous_076_requires_remaining_after_090_previous_stem(self):
        """Previous stem 076 is remaining-after-090; locked 999/600/090 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_BACKWARD, ("076", "090", "076"))
        ...
        has_076 = ["076", "076", "090", "076"]
        self.assertEqual(site_previous_stem(has_076, 2, GRAM2), "076")
        self.assertEqual(site_backward_3gram(has_076, 2, GRAM2), GRAM3_BACKWARD)
        self.assertEqual(
            site_previous_4gram(has_076, 2, GRAM2),
            ("076", "076", "090", "076"),
        )
        has_071_076 = ["071", "076", "090", "076"]
        self.assertEqual(site_previous_stem(has_071_076, 2, GRAM2), "076")
        self.assertEqual(
            site_previous_4gram(has_071_076, 2, GRAM2),
            ("071", "076", "090", "076"),
        )
        has_090 = ["011", "090", "090", "076"]
        self.assertEqual(site_previous_stem(has_090, 2, GRAM2), "090")
        self.assertNotEqual(site_previous_stem(has_090, 2, GRAM2), "076")
        has_600 = ["076", "600", "090", "076"]
        self.assertEqual(site_previous_stem(has_600, 2, GRAM2), "600")
        self.assertNotEqual(site_previous_stem(has_600, 2, GRAM2), "076")
        has_999 = ["000", "999", "090", "076"]
        self.assertEqual(site_previous_stem(has_999, 2, GRAM2), "999")
        self.assertNotEqual(site_previous_stem(has_999, 2, GRAM2), "076")
        ...
        one_token_before = ["076", "090", "076"]
        self.assertEqual(site_previous_stem(one_token_before, 1, GRAM2), "076")
        ...
        planted_stems = ("076", "090", "999", "600", None, "045")
        rem = leftover_extra_remaining_after_090(planted_sites, planted_stems)
        self.assertEqual(rem, (planted_sites[0], planted_sites[4], planted_sites[5]))
        self.assertEqual(
    ...
    def test_exactly_2_can_fail(self):
        ...
        provider = MockProvider()
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(2)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(0)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(1)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(3)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(33)
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(35)
        )
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_EXACTLY_2_SHARE_PREVIOUS_076
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_600_REMAINING_AFTER_090_EXACTLY_2_SHARE_PREVIOUS_076,
            HYPOTHESIS_K == STANDING_K_076,
        )
        self.assertEqual(
            STANDING_K_076 + STANDING_N_REMAINING_AFTER_090_AND_076,
            STANDING_N_REMAINING_AFTER_090,
        )
    ...
    def test_cycle270_076_sites_survive_peeling_090_and_are_not_090_sites(self):
        """Cycle-270 previous-076 pair is disjoint from previous-090; peeling 090 keeps K_076=2."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle270_076_set(STANDING_MATCHING_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE270_076_SITES)
        self.assertEqual(STANDING_MATCHING_SITES, STANDING_CYCLE270_076_SITES)
        ...
        for site in CYCLE271_MATCHING_SITES:
            ...
            self.assertNotIn(site, STANDING_REMAINING_AFTER_090_SITES)
        planted = STANDING_MATCHING_SITES[:-1]
        self.assertFalse(matching_equals_cycle270_076_set(planted))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(
                len(planted)
            )
        )
        swapped = CYCLE271_MATCHING_SITES
        self.assertFalse(matching_equals_cycle270_076_set(swapped))
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(
                len(swapped)
            )
        ...
        self.assertIn("076", CYCLE270_TIED_STEMS)
        self.assertEqual(CYCLE270_N_TIED_AT_K, 5)
        self.assertEqual(STANDING_CYCLE270_K_076, 2)
        for stem, count, sites, _grams in CYCLE270_FREQUENCY:
            if stem == "076":
                self.assertEqual(count, 2)
                self.assertEqual(sites, STANDING_CYCLE270_076_SITES)
        self.assertTrue(STANDING_DO_NOT_PEEL_071_045_009)
        self.assertTrue(STANDING_I_ONLY_OF_076_090_076_IS_NOT_THIS_CYCLE)
...
class TestMamariILeftoverExtra090076RemainingAfter600Prev076Scoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-090 previous-076 lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining = leftover_extra_remaining_after_090(
            self.leftover_sites,
            self.previous_stems,
        )
        self.remaining_stems = leftover_extra_remaining_after_090_previous_stems(
            self.leftover_sites,
            self.previous_stems,
        )
        ...
        self.matching = leftover_extra_remaining_after_090_with_previous_076(
            self.leftover_sites,
            self.previous_stems,
        )
        self.without = leftover_extra_remaining_after_090_without_previous_076(
            self.leftover_sites,
            self.previous_stems,
        )
        self.share_076_after_600 = leftover_extra_remaining_after_600_with_previous_076(
            self.leftover_sites,
            self.previous_stems,
        )
        ...
        self.n_remaining_after_090 = len(self.remaining)
        self.n_with_previous = len(self.remaining_stems)
        self.n_no_previous = self.n_remaining_after_090 - self.n_with_previous
        self.k = len(self.matching)
        self.k_076 = self.k
        self.n_without = len(self.without)
        self.n_remaining_after_090_and_076 = self.n_remaining_after_090 - self.k_076
        self.k_076_after_600 = len(self.share_076_after_600)
        self.tied_after_600 = tuple(
            stem for stem, count, _sites, _grams in self.frequency_600 if count == 2
        )
        self.equals_cycle270_076 = matching_equals_cycle270_076_set(self.matching)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_600_remaining_after_090_exactly_2_share_previous_076(
            self.k_076,
        )

    def test_tokens_and_nested_leftover_extra_56_69_k090_2_n35_not_retuned(self):
        """2-gram and leftover extra 56 / N_I=69 / K_090=2 / N_remaining_after_090=35 stay."""
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_BACKWARD, ("076", "090", "076"))
        self.assertEqual(GRAM3_NESTED_090, ("090", "090", "076"))
        self.assertEqual(GRAM3_NESTED_600, ("600", "090", "076"))
        ...
        self.assertTrue(
            prior_271["i_leftover_extra_090_076_remaining_after_600_exactly_2_share_previous_090"]
        )
        self.assertEqual(
        ...
        if (
            ...
            self.fail(
                "nested cycle 271 K_090=2 N_remaining_after_090=35 leftover extra=56 N_I=69 drifted"
            )
        ...
        self.assertIn("076", prior_270["tied_stems_at_K"])
        ...
        self.assertTrue(STANDING_PREVIOUS_999_600_090_NOT_RETUNED)
    ...
    def test_counts_2_of_35_and_hypothesis_k_2_holds(self):
        """N_remaining_after_090=35, cycle-270 076 still K=2, K_076=2. Claim holds."""
        ...
        self.assertEqual(self.remaining, STANDING_REMAINING_AFTER_090_SITES)
        self.assertEqual(self.remaining_stems, STANDING_REMAINING_AFTER_090_PREVIOUS_STEMS)
        self.assertEqual(
            self.remaining,
            leftover_extra_remaining_after_600_without_previous_090(
                self.leftover_sites,
        ...
        self.assertEqual(STANDING_N_WITH_PREVIOUS, 35)
        self.assertEqual(self.n_no_previous, STANDING_N_NO_PREVIOUS)
        self.assertEqual(STANDING_N_NO_PREVIOUS, 0)
        self.assertEqual(self.n_with_previous + self.n_no_previous, self.n_remaining_after_090)
        self.assertEqual(35 + 0, 35)
        ...
        self.assertIn("076", self.tied_after_600)
        self.assertEqual(len(self.tied_after_600), STANDING_N_TIED_AT_K_AFTER_600)
        self.assertEqual(STANDING_N_TIED_AT_K_AFTER_600, 5)
        if "076" not in self.tied_after_600 or self.k_076_after_600 != 2:
            self.fail("nested cycle 270 5-way no longer includes 076 at K=2")
        self.assertEqual(self.k_076_after_600, STANDING_CYCLE270_K_076)
        self.assertEqual(STANDING_CYCLE270_K_076, 2)
        self.assertEqual(self.share_076_after_600, STANDING_CYCLE270_076_SITES)
        self.assertEqual(self.share_076_after_600, self.matching)
        self.assertTrue(STANDING_076_STILL_MAX_K_AFTER_600)
        self.assertEqual(self.k, STANDING_K)
        self.assertEqual(self.k_076, STANDING_K_076)
        self.assertEqual(STANDING_K_076, HYPOTHESIS_K)
        self.assertEqual(STANDING_K_076, 2)
        self.assertEqual(self.n_without, STANDING_N_WITHOUT)
        self.assertEqual(STANDING_N_WITHOUT, 33)
        ...
        self.assertEqual(self.k_076 + self.n_remaining_after_090_and_076, self.n_remaining_after_090)
        self.assertEqual(2 + 33, 35)
        if self.k_076 != 2:
        ...
        self.assertEqual(self.without, STANDING_REMAINING_AFTER_090_AND_076_SITES)
        self.assertTrue(self.equals_cycle270_076)
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE270_076_SITES)
        self.assertTrue(matching_equals_cycle270_076_set(self.matching))
        if len(self.matching) != 2 or not self.equals_cycle270_076:
            self.fail(
                "leftover extra remaining-after-090 previous-076 set drifted from cycle-270 076 pair"
            )
    ...
    def test_matching_remaining_after_090_sites_have_previous_076(self):
        """Two remaining-after-090 leftover extra sites are 076 090 076."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_previous_4grams, STANDING_MATCHING_PREVIOUS_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia2", 165), ("076", "076", "090", "076")),
            ((SIDE_IA, "Ia13", 152), ("071", "076", "090", "076")),
        )
        for (site, prev4), (want_site, want_prev) in zip(
            ...
            self.assertEqual(stems[index - 1], "076")
            self.assertEqual(site_previous_stem(stems, index, GRAM2), "076")
            ...
            self.assertIn(site, STANDING_CYCLE270_076_SITES)
        self.assertEqual(self.matching, STANDING_CYCLE270_076_SITES)
        self.assertTrue(matching_equals_cycle270_076_set(self.matching))
        for site in self.without:
            ...
            self.assertIn(site, self.remaining)
            self.assertIn(site, STANDING_LEFTOVER_SITES)
            self.assertNotIn(site, STANDING_CYCLE270_076_SITES)
        for site in self.share_999:
            self.assertNotIn(site, self.remaining)
            self.assertNotIn(site, self.matching)
        ...
        for stem in STANDING_OTHER_TIED_STEMS:
            other = leftover_extra_remaining_after_090_with_previous_076(
                self.leftover_sites,
                self.previous_stems,
                stem,
            )
            ...
            for site in other:
                self.assertNotIn(site, self.matching)
                self.assertIn(site, self.remaining)
        self.assertEqual(len(self.without), STANDING_N_REMAINING_AFTER_090_AND_076)
        ...
        self.assertEqual(
            matching_leftover_extra_remaining_after_090_previous_076_local_4gram_rows(),
            [
                {
                    "tablet": "I",
                    "side": SIDE_IA,
    ...
    def test_existing_272_271_270_268_and_223_still_compute(self):
        """Cycle 272 3/1 extra I=1, 271 K_090=2/35, 270 5-way G=090, 268 6/0 extra I=2 stay."""
        ...
        if (
            ...
            self.fail(
                ...
                "5-way tie G=090 K=2 including 076 drifted"
            )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-273 leftover extra remaining-after-090 previous-076 lock."""
        lock = self.survey[
            "i_leftover_extra_090_076_remaining_after_600_remaining_after_090_previous_076"
        ]
        self.assertEqual(lock["cycle"], 273)
        ...
        self.assertEqual(tuple(lock["backward_3gram"]), ("076", "090", "076"))
        self.assertEqual(lock["n3"], STANDING_N3)
        self.assertEqual(lock["n4"], STANDING_N4)
        self.assertEqual(
            tuple(lock["locked_previous_stems_after_090"]),
            LOCKED_PREVIOUS_STEMS_AFTER_090,
        )
        self.assertEqual(tuple(lock["locked_previous_stems_after_090"]), ("999", "600", "090"))
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["remaining_after_090_sites"]),
            STANDING_REMAINING_AFTER_090_SITES,
        )
        self.assertEqual(
            tuple(lock["remaining_after_090_previous_stems"]),
            STANDING_REMAINING_AFTER_090_PREVIOUS_STEMS,
        )
        self.assertEqual(lock["N_with_previous"], STANDING_N_WITH_PREVIOUS)
        self.assertEqual(lock["N_with_previous"], 35)
        ...
        self.assertEqual(lock["N_distinct_remaining_after_090"], STANDING_N_DISTINCT_REMAINING_AFTER_090)
        self.assertEqual(lock["N_distinct_remaining_after_090"], 31)
        self.assertEqual(lock["G"], STANDING_G)
        self.assertEqual(lock["G"], "076")
        ...
        self.assertEqual(lock["cycle270_K_076"], STANDING_CYCLE270_K_076)
        self.assertEqual(lock["cycle270_K_076"], 2)
        self.assertTrue(lock["076_still_max_K_after_600"])
        ...
        self.assertEqual(lock["N_without"], 33)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_090_sites"]),
            STANDING_MATCHING_SITES,
        )
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_090_sites"]),
        ...
        self.assertTrue(lock["matching_equals_cycle270_076_sites"])
        self.assertEqual(
            lock["matching_equals_cycle270_076_sites"],
            STANDING_MATCHING_EQUALS_CYCLE270_076_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lock["matching_leftover_extra_remaining_after_090_local_4grams"],
            matching_leftover_extra_remaining_after_090_previous_076_local_4gram_rows(),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["cycle270_076_sites"]),
            STANDING_CYCLE270_076_SITES,
        )
        ...
        self.assertIn("076", lock["cycle270_tied_stems_at_K"])
        ...
        self.assertIn("076", lock["nested_cycle270_tied_stems_at_K"])
        ...
        self.assertTrue(lock["i_only_of_076_090_076_is_not_this_cycle"])
        ...
        self.assertTrue(lock["previous_999_600_090_not_retuned"])
...
class TestMamariILeftoverExtra090076RemainingAfter600Prev076ImageSnapshot(
    unittest.TestCase
):
    """Cycle 273 does not touch clustering. 83/62 / Hamming 6 stays."""
