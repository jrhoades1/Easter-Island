"""Reframed closed-tradition hold-out of the locked leftover-6 next-81 grams.

Cycle 914 text-search lock. Uses already-vendored A–V and the
cycle-625 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 81-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 625 already locked the
I-only claim on these 81-grams as incomplete-set LOSE
(N_with_next81=12, N_no_next81=6, N_i_only=12, N_leak=0). Cycle
913 already locked the reframed claim on the cycle-623 next-80
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–913.

Claim that can lose: each existing next-81 gram is an exact
...
next 81-grams as a peel, next 82-grams as a peel, prev-184, or
next-185. The cycle-627 next-82 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next81_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE625_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT81_I_ONLY as CYCLE625_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT81_SITES as CYCLE625_SITES,
    STANDING_N as CYCLE625_N,
    STANDING_N_I_EACH as CYCLE625_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE625_N_I_ONLY,
    STANDING_N_LEAK as CYCLE625_N_LEAK,
    STANDING_N_NO_NEXT81 as CYCLE625_N_NO_NEXT81,
    STANDING_N_SEQUENCES as CYCLE625_N_SEQUENCES,
    STANDING_N_WITH_NEXT81 as CYCLE625_N_WITH_NEXT81,
    STANDING_NO_NEXT81_SITES as CYCLE625_NO_NEXT81_SITES,
    STANDING_RESULT as CYCLE625_RESULT,
...
STANDING_NO_NEXT81_SITES = CYCLE625_NO_NEXT81_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE625_SITES
STANDING_SEQUENCES = CYCLE625_SEQUENCES
...
STANDING_NEXT80_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT80_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next81_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next81_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 625
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 625
...
class TestMamariILeftoverN6Next81ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 914 reframed closed-tradition lock on the cycle-625 next-81 population."""
    ...
    def test_population_is_the_locked_cycle_625_next81_grams(self):
        """Twelve next-81 sequences and sites come from cycle 625. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE625_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE625_SITES)
        self.assertEqual(CYCLE625_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE625_N, 18)
        self.assertEqual(CYCLE625_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT81, CYCLE625_N_WITH_NEXT81)
        self.assertEqual(STANDING_N_NO_NEXT81, CYCLE625_N_NO_NEXT81)
        self.assertEqual(
            STANDING_NO_NEXT81_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT81_SITES, CYCLE913_NO_NEXT80_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE625_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE625_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE625_N_I_ONLY, 12)
        self.assertEqual(CYCLE625_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 625)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 81)
        prior = self.survey[CYCLE625_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_81grams_all_i_only"
        )
        self.assertEqual(CYCLE625_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next81_i_only"
        ))
        self.assertEqual(prior["cycle"], 625)
        ...
        self.assertFalse(prior[CYCLE625_RESULT])
        self.assertEqual(prior["N_i_only"], 12)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next81"], 6)
        self.assertEqual(prior["N_with_next81"], 12)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE913, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 625 I-only or the cycle 832–913 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 625)
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE625_N_I_EACH)
        self.assertEqual(CYCLE625_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT80_ALL_EXTEND)
        for n80, n81, site80, site81 in zip(
            CYCLE913_SEQUENCES, STANDING_SEQUENCES, CYCLE913_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n81[:80], n80)
            self.assertEqual(site81, site80)
        still = self.rows[STANDING_NEXT80_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:80], CYCLE913_SEQUENCES[3])
        self.assertEqual(CYCLE913_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT81_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT81_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT81_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT81_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT81_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT81_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_627_next82_list(self):
        """Cycle 627 next-82 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE627_N, 18)
        self.assertEqual(CYCLE627_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE627_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 82 for tokens in CYCLE627_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next82_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle627_next82_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-914 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 914)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next81_sites"]),
            STANDING_NO_NEXT81_SITES,
        )
        ...
        self.assertEqual(
            tuple(lock["next80_hapax_still_hapax"]),
            STANDING_NEXT80_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next80_all_extend"])
        self.assertEqual(lock["next80_all_extend"], STANDING_NEXT80_ALL_EXTEND)
