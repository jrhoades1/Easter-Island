"""I's cycle-232 090 076 001 forward-4-grams off-I lock.

Cycle 233 text-search lock. Uses already-vendored A–V and the
cycle-232 I sites of 3-gram 090 076 001 (all 3 I sites; each
has a next stem). Does not retune those 4-grams, the 3 I
sites, leftover extra remaining-after-013 G=001 K=3, leftover
extra remaining-after-071 013, leftover extra remaining 071,
...
all I-only lost 7/8; 090 076 070 000 leaks 1/1 on T) and
cycle 230 (I 090 076 013 forward 4-grams all I-only hapax
1/0 x5). Cycle 232 I 090 076 001 I-only holds 3/0, cycle 231
leftover extra remaining-after-013 K=3 / G=001, cycle 230
5/0 hapax, cycle 229 5/0, cycle 223 69/3, cycle 195 6/0, and
cycle 171 43/0 stay. Off-I T sites of 090 076 are not this
3-gram and are not these 4-grams. Leftover extra remaining
after 001 is not locked this cycle. Previous 4-grams of the
three I sites are not locked this cycle. 090 076 013,
090 076 070, 090 076 071, and 090 076 without 001 do not
count. Do not retune leftover n=4, 076-cells, or any
detector.

Locks exact consecutive hits of each I 090 076 001 forward
4-gram on tablet I and on every other vendored tablet A–H
and J–V. The three 4-grams: 090 076 001 048, 090 076 001 224,
090 076 001 071.
Do not assume hapax; count each from fixtures. Hypothesis:
all three are I-only. Measured: each N_I=1 at the cycle-232
I site; all N_off_I=0. Claim that can lose:
i_090_076_001_forward_4grams_all_i_only.
True only if ALL three have N_off_I=0 and N_I>=1. The claim
...
from tests.test_mamari_i_3gram_090_076_001_i_only_scoreboard import (
    GRAM3,
    STANDING_I_3GRAM_090_076_001_I_ONLY as CYCLE232_I_ONLY,
    STANDING_I_NEXT_4GRAMS as CYCLE232_NEXT_4GRAMS,
    STANDING_I_PREVIOUS_4GRAMS as CYCLE232_PREVIOUS_4GRAMS,
    STANDING_I_SITES as CYCLE232_I_SITES,
    STANDING_N_I as CYCLE232_N_I,
    STANDING_N_OFF_I as CYCLE232_N_OFF_I,
    extra_i_sites,
    leftover_extra_remaining_after_013_001_subset,
    named_off_i_sites as cycle232_named_off_i_sites,
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_013_next_stem_scoreboard import (
    ...
    STANDING_MATCHING_NEXT_4GRAMS as CYCLE231_MATCHING_NEXT_4GRAMS,
...
STANDING_SEQUENCES = CYCLE232_NEXT_4GRAMS
STANDING_CYCLE232_SITES = CYCLE232_I_SITES
STANDING_NEXT_STEMS = (
    "048",
    "224",
    "071",
)
STANDING_PREVIOUS_4GRAMS = CYCLE232_PREVIOUS_4GRAMS
...
STANDING_I_SITES = tuple((site,) for site in STANDING_CYCLE232_SITES)
STANDING_IB_HITS = 0
STANDING_IB_SITES = ()
...
STANDING_CLAIM = "i_090_076_001_forward_4grams_all_i_only"
STANDING_I_090_076_001_FORWARD_4GRAMS_ALL_I_ONLY = True
STANDING_I_090_076_001_FORWARD_4GRAMS_I_ONLY = True
STANDING_RESULT = "i_090_076_001_forward_4grams_i_only"
...
STANDING_090_076_WITHOUT_001_DOES_NOT_COUNT = True
STANDING_076_071_DOES_NOT_COUNT = True
STANDING_LEFTOVER_N4_SET_NOT_RETUNED = True
STANDING_LEFTOVER_EXTRA_REMAINING_AFTER_001_IS_NOT_THIS_CYCLE = True
STANDING_PREVIOUS_4GRAMS_ARE_NOT_THIS_CYCLE = True
STANDING_OFF_I_T_SITES_OF_090_076_ARE_NOT_THIS_CYCLE = True
STANDING_CYCLE231_K = 3
STANDING_CYCLE231_G = "001"
STANDING_CYCLE232_N_I = 3
STANDING_CYCLE232_N_OFF_I = 0


def forward_4gram_start_site(
    cycle232_site: tuple[str, str, int],
) -> tuple[str, str, int]:
    """Forward 4-gram starts at the cycle-232 I 090 076 001 site."""
    return cycle232_site
...
def site_forward_4gram(
    ...
    """090 076 001 X if a next stem exists; None at end-of-line."""
    n3 = len(gram3)
...
def i_090_076_001_forward_4grams(
    i_sides: dict[str, list[list[str]]],
    sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE232_SITES,
    gram3: tuple[str, ...] = GRAM3,
...
def i_sites_without_forward(
    ...
    """I 090 076 001 sites that have no next stem after the 3-gram."""
    return tuple(
        site
...
def i_090_076_001_forward_4grams_all_i_only(
    n_i: tuple[int, ...],
    n_off_i: tuple[int, ...],
    expected_n: int = STANDING_N_SEQUENCES,
) -> bool:
    """True iff all 3 I 090 076 001 forward 4-grams are I-only.

    Claim holds only if every gram has N_off_I=0. N_I>=1 is
    required so a missing I hit is not I-only. Hapax is not
    assumed; N_I may be greater than 1. Length must stay 3.
    """
    return (
        len(n_i) == expected_n
...
class TestI090076001Forward4gramsIOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-232 I 090 076 001 forward 4-grams. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 4-gram counts; a gap is not a hit. 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(STANDING_SEQUENCES[0], ("090", "076", "001", "048"))
        self.assertEqual(STANDING_SEQUENCES[-1], ("090", "076", "001", "071"))
        ...
        overlap = [["090", "076", "001", "048", "090", "076", "001", "048"]]
        ...
        self.assertEqual(ngram_hit_count([["090", "076", "001"]], STANDING_SEQUENCES[0]), 0)
        planted = ["999", "090", "076", "001", "048", "090"]
        ...
        self.assertTrue(STANDING_090_076_WITHOUT_001_DOES_NOT_COUNT)
    ...
    def test_all_i_only_requires_on_i_and_zero_off_i_for_each_4gram(self):
        """Boolean is True only when all 3 forward 4-grams are I-only."""
        provider = MockProvider()
        hold_ones = STANDING_N_I_EACH
        hold_zeros = STANDING_N_OFF_I_EACH
        self.assertTrue(i_090_076_001_forward_4grams_all_i_only(hold_ones, hold_zeros))
        self.assertTrue(
            i_090_076_001_forward_4grams_all_i_only((2,) + hold_ones[1:], hold_zeros)
        )
        lose_off = list(hold_zeros)
        lose_off[0] = 1
        self.assertFalse(
            i_090_076_001_forward_4grams_all_i_only(hold_ones, tuple(lose_off))
        )
        lose_missing_i = list(hold_ones)
        lose_missing_i[0] = 0
        self.assertFalse(
            i_090_076_001_forward_4grams_all_i_only(tuple(lose_missing_i), hold_zeros)
        )
        self.assertFalse(i_090_076_001_forward_4grams_all_i_only((), ()))
        self.assertFalse(
            i_090_076_001_forward_4grams_all_i_only(hold_ones[:-1], hold_zeros[:-1])
        )
        self.assertEqual(STANDING_CLAIM, "i_090_076_001_forward_4grams_all_i_only")
        self.assertTrue(STANDING_I_090_076_001_FORWARD_4GRAMS_ALL_I_ONLY)
        self.assertTrue(STANDING_I_090_076_001_FORWARD_4GRAMS_I_ONLY)
        self.assertEqual(
            STANDING_I_090_076_001_FORWARD_4GRAMS_ALL_I_ONLY,
    ...
    def test_4grams_are_cycle_232_forwards_not_retuned(self):
        """4-grams stay the cycle-232 I forwards; leftover / 013 / 070 / 071 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "001"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(STANDING_SEQUENCES, CYCLE232_NEXT_4GRAMS)
        self.assertEqual(STANDING_SEQUENCES, CYCLE231_MATCHING_NEXT_4GRAMS)
        self.assertEqual(STANDING_CYCLE232_SITES, CYCLE232_I_SITES)
        self.assertEqual(STANDING_CYCLE232_SITES, CYCLE231_MATCHING_SITES)
        self.assertEqual(len(STANDING_SEQUENCES), CYCLE232_N_I)
        self.assertEqual(CYCLE232_N_I, 3)
        self.assertEqual(CYCLE232_N_OFF_I, 0)
        self.assertEqual(len(set(STANDING_NEXT_STEMS)), 3)
        ...
        self.assertTrue(STANDING_LEFTOVER_EXTRA_REMAINING_AFTER_001_IS_NOT_THIS_CYCLE)
...
class TestMamariI090076001Forward4gramsIOnlyScoreboard(unittest.TestCase):
    """Cited-fixture I 090 076 001 forward-4 off-I lock. Mock only."""

    def setUp(self):
        self.provider = MockProvider()
        self.survey = load_corpus_survey()
        self.i_sides = load_i_sides()
        self.cycle232_sites = STANDING_CYCLE232_SITES
        ...
        self.off_i_sites = tuple(
            cycle232_named_off_i_sites(gram) for gram in self.grams
        )
        self.measured_forwards = i_090_076_001_forward_4grams(
            self.i_sides,
            self.cycle232_sites,
            GRAM3,
        )
        ...
        self.leftover_matching = leftover_extra_remaining_after_013_with_g(
            STANDING_LEFTOVER_SITES,
            self.next_stems,
            CYCLE231_G,
        )
        ...
        self.extra = extra_i_sites(self.cycle232_sites, self.leftover_matching)
        ...
        self.claim_holds = i_090_076_001_forward_4grams_all_i_only(
            self.n_i,
            self.n_off_i,
        )
        self.n_i_only = sum(
    ...
    def test_tokens_and_sites_are_cycle_232_forwards_not_retuned(self):
        """4-grams and I sites stay the cycle-232 forward lock. Nested 3/0 must hold."""
        self.assertEqual(GRAM3, ("090", "076", "001"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM5, ("999", "071", "076", "010", "079"))
        self.assertEqual(self.cycle232_sites, STANDING_CYCLE232_SITES)
        self.assertEqual(self.cycle232_sites, CYCLE232_I_SITES)
        self.assertEqual(self.cycle232_sites, CYCLE231_MATCHING_SITES)
        self.assertEqual(self.measured_forwards, STANDING_SEQUENCES)
        self.assertEqual(self.measured_forwards, CYCLE232_NEXT_4GRAMS)
        self.assertEqual(self.measured_forwards, CYCLE231_MATCHING_NEXT_4GRAMS)
        self.assertEqual(self.leftover_matching_next_4grams, STANDING_SEQUENCES)
        self.assertEqual(self.no_forward, ())
        self.assertEqual(self.n3_i, STANDING_CYCLE232_N_I)
        self.assertEqual(self.n3_i, 3)
        self.assertEqual(self.n3_off_i, STANDING_CYCLE232_N_OFF_I)
        self.assertEqual(self.n3_off_i, 0)
        self.assertEqual(self.leftover_matching, CYCLE231_MATCHING_SITES)
        self.assertEqual(self.leftover_matching, CYCLE232_I_SITES)
        self.assertEqual(len(self.leftover_matching), STANDING_CYCLE231_K)
        self.assertEqual(STANDING_CYCLE231_K, 3)
        self.assertEqual(STANDING_CYCLE231_G, "001")
        self.assertEqual(CYCLE231_G, "001")
        self.assertEqual(CYCLE231_K, 3)
        self.assertEqual(len(self.remaining3), CYCLE231_N_REMAINING3)
        self.assertEqual(len(self.remaining3), 36)
        self.assertTrue(
            leftover_extra_remaining_after_013_001_subset(
                self.leftover_matching,
                self.cycle232_sites,
        ...
        if self.n3_i != 3 or self.n3_off_i != 0:
            self.fail("nested cycle 232 090 076 001 I-only 3/0 drifted")
        if len(self.leftover_matching) != 3 or CYCLE231_G != "001":
            self.fail("nested cycle 231 leftover extra remaining-after-013 G=001 K=3 drifted")
        if self.extra:
            self.fail("extra I 090 076 001 sites appeared; leftover of leftover is not this cycle")
        ...
        self.assertTrue(CYCLE232_I_ONLY)
        self.assertEqual(
            [list(gram) for gram in STANDING_SEQUENCES],
            prior_232["i_next_4grams"],
        )
        self.assertEqual(
            tuple(tuple(row) for row in prior_232["i_sites"]),
            STANDING_CYCLE232_SITES,
        )
        self.assertEqual(
            [list(gram) for gram in STANDING_PREVIOUS_4GRAMS],
            prior_232["i_previous_4grams"],
        )
        ...
        self.assertEqual(
            [list(gram) for gram in STANDING_SEQUENCES],
            prior_231["matching_next_4grams"],
        )
    ...
    def test_each_4gram_is_one_on_i_zero_off_i_and_claim_holds(self):
        """Each forward 4-gram is 1/0. All three I-only. Claim holds."""
        ...
        if self.n_off_i != STANDING_N_OFF_I_EACH:
            self.fail("measured N_off_I drifted from the locked 3/0")
        for site, start, gram, nxt, prev4, role, sites, n_on, n_off, off_sites in zip(
            ...
            self.assertIn(site, CYCLE231_MATCHING_SITES)
        ...
        if self.n_i_only != STANDING_N_I_ONLY:
            self.fail("measured N_i_only drifted from 3")
        ...
        for gram in STANDING_SEQUENCES:
            ...
            self.assertEqual(cycle232_named_off_i_sites(gram), ())
        ...
        self.assertEqual(
            i_090_076_001_forward_4grams_all_i_only(self.n_i, self.n_off_i),
            STANDING_I_090_076_001_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_232_231_230_229_223_195_and_171_still_compute(self):
        """Cycle 232 3/0, 231 K=3/G=001, 230 5/0 hapax, 229 5/0, 223 69/3, 195 6/0, 171 43/0 stay."""
        ...
        self.assertEqual(prior_231.matching_next_4grams, STANDING_SEQUENCES)
        self.assertTrue(prior_231.claim_holds)
        self.assertTrue(CYCLE231_CLAIM)
        if prior_231.k != 3 or prior_231.g != "001" or prior_231.n_remaining3 != 36:
            self.fail("nested cycle 231 leftover extra remaining-after-013 G=001 K=3 drifted")
        ...
        self.assertEqual(prior_230.n_i_only, 5)
        self.assertEqual(prior_230.n_not_i_only, 0)
        self.assertEqual(prior_230.n_off_i, (0, 0, 0, 0, 0))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-233 I-forward-4 I-only hold."""
        lock = self.survey["i_090_076_001_forward_4grams_i_only"]
        self.assertEqual(lock["cycle"], 233)
        ...
        self.assertEqual(lock["N_I"], CYCLE232_N_I)
        ...
        for row, gram, site, nxt, role, sites, n_on, n_off, off_sites, hits in zip(
            ...
            self.assertEqual(tuple(row["cycle232_site"]), site)
        ...
        self.assertTrue(lock["i_090_076_001_forward_4grams_all_i_only"])
        self.assertEqual(
            lock["i_090_076_001_forward_4grams_all_i_only"],
            STANDING_I_090_076_001_FORWARD_4GRAMS_ALL_I_ONLY,
        )
        self.assertTrue(lock["i_090_076_001_forward_4grams_i_only"])
        ...
        self.assertEqual(lock["nested_cycle231_G"], STANDING_CYCLE231_G)
        self.assertEqual(lock["nested_cycle231_G"], "001")
        self.assertEqual(lock["nested_cycle231_K"], STANDING_CYCLE231_K)
        self.assertEqual(lock["nested_cycle231_K"], 3)
        self.assertEqual(lock["nested_cycle231_N_remaining3"], 36)
        self.assertEqual(lock["nested_cycle230_N_i_only"], 5)
        self.assertEqual(lock["nested_cycle230_N_not_i_only"], 0)
        ...
        self.assertTrue(lock["090_076_without_001_does_not_count"])
        self.assertTrue(lock["076_071_does_not_count"])
        self.assertTrue(lock["leftover_n4_set_not_retuned"])
        self.assertTrue(lock["leftover_extra_remaining_after_001_is_not_this_cycle"])
...
class TestMamariI090076001Forward4gramsIOnlyImageSnapshot(unittest.TestCase):
    """Cycle 233 does not touch clustering. 83/62 / Hamming 6 stays."""
