"""Reframed closed-tradition hold-out of the locked leftover-6 next-37 grams.

Cycle 870 text-search lock. Uses already-vendored A–V and the
cycle-537 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 37-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 537 already locked the I-only claim
on these 37-grams as incomplete-set LOSE (N_with_next37=15,
N_no_next37=3, N_i_only=15, N_leak=0). Cycle 869 already locked
the reframed claim on the cycle-535 next-36 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–869.

Claim that can lose: each existing next-37 gram is an exact
...
Named claim: cycle537_next37_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next37 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next37=15,
N_no_next37=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-36 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700
076 076 053 177 700 076 057 741 430 076 532 200 extends to 604
076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532
244 999 090 076 057 600 700 076 076 053 177 700 076 057 741 430
076 532 200 059 at Ia9[9] and hits I once. All fifteen next-36
...
next 37-grams as a peel, next 38-grams as a peel, prev-184, or
next-185. The cycle-539 next-38 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next37_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE537_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT37_I_ONLY as CYCLE537_ALL_I_ONLY_CLAIM,
    STANDING_LEFTOVER_MATCHING_NEXT37_SITES as CYCLE537_SITES,
    STANDING_N as CYCLE537_N,
    STANDING_N_I_EACH as CYCLE537_N_I_EACH,
    STANDING_N_I_ONLY as CYCLE537_N_I_ONLY,
    STANDING_N_LEAK as CYCLE537_N_LEAK,
    STANDING_N_NO_NEXT37 as CYCLE537_N_NO_NEXT37,
    STANDING_N_SEQUENCES as CYCLE537_N_SEQUENCES,
    STANDING_N_WITH_NEXT37 as CYCLE537_N_WITH_NEXT37,
    STANDING_NO_NEXT37_SITES as CYCLE537_NO_NEXT37_SITES,
    STANDING_RESULT as CYCLE537_RESULT,
...
STANDING_NO_NEXT37_SITES = CYCLE537_NO_NEXT37_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE537_SITES
STANDING_SEQUENCES = CYCLE537_SEQUENCES
...
STANDING_NEXT36_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT36_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next37_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next37_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 537
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 537
...
STANDING_DO_NOT_RELOCK_CYCLES_832_869 = True
...
class TestMamariILeftoverN6Next37ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 870 reframed closed-tradition lock on the cycle-537 next-37 population."""
    ...
    def test_population_is_the_locked_cycle_537_next37_grams(self):
        """Fifteen next-37 sequences and sites come from cycle 537. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE537_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE537_SITES)
        self.assertEqual(CYCLE537_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE537_N, 18)
        self.assertEqual(CYCLE537_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT37, CYCLE537_N_WITH_NEXT37)
        self.assertEqual(STANDING_N_NO_NEXT37, CYCLE537_N_NO_NEXT37)
        self.assertEqual(
            STANDING_NO_NEXT37_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT37_SITES, CYCLE869_NO_NEXT36_SITES)
        self.assertFalse(STANDING_PARENT_COMPLETE)
        self.assertTrue(CYCLE537_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE537_ALL_I_ONLY_CLAIM)
        self.assertEqual(CYCLE537_N_I_ONLY, 15)
        self.assertEqual(CYCLE537_N_LEAK, 0)
        self.assertEqual(STANDING_FROM_CYCLE, 537)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 37)
        prior = self.survey[CYCLE537_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_37grams_all_i_only"
        )
        self.assertEqual(CYCLE537_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next37_i_only"
        ))
        self.assertEqual(prior["cycle"], 537)
        ...
        self.assertFalse(prior[CYCLE537_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next37"], 3)
        self.assertEqual(prior["N_with_next37"], 15)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE869, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 537 I-only or the cycle 832–869 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 537)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_869)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE537_N_I_EACH)
        self.assertEqual(CYCLE537_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT36_ALL_EXTEND)
        for n36, n37, site36, site37 in zip(
            CYCLE869_SEQUENCES, STANDING_SEQUENCES, CYCLE869_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n37[:36], n36)
            self.assertEqual(site37, site36)
        still = self.rows[STANDING_NEXT36_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:36], CYCLE869_SEQUENCES[3])
        self.assertEqual(CYCLE869_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT37_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_539_next38_list(self):
        """Cycle 539 next-38 I-only list stays locked. The reframed result key has landed; the claim id is not a survey key."""
        self.assertEqual(CYCLE539_N, 18)
        self.assertEqual(CYCLE539_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE539_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 38 for tokens in CYCLE539_SEQUENCES))
        ...
        self.assertIn("i_leftover_n6_next38_closed_tradition_hpq_abc", self.survey)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-870 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 870)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next37_sites"]),
            STANDING_NO_NEXT37_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_869"])
        ...
        self.assertEqual(
            tuple(lock["next36_hapax_still_hapax"]),
            STANDING_NEXT36_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next36_all_extend"])
        self.assertEqual(lock["next36_all_extend"], STANDING_NEXT36_ALL_EXTEND)
