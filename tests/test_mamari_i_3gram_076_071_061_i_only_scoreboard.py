"""I's cycle-185 leftover 3-gram 076 071 061 off-I lock.

Cycle 186 text-search lock. Uses already-vendored A–V and the
cycle-185 leftover forward 3-gram 076 071 061 (the n=3 run
shared by leftover 076 071 sites Ia3[147] 076 071 061 011
and Ia12[59] 076 071 061 290). Does not retune that 3-gram,
...
Same claim-shape as cycle 183 (076 071 700 was I-only 3/0),
cycle 180 (076 071 090 was I-only 5/0), cycle 177
(076 071 600 was I-only 4/0), cycle 174 (076 071 076 was
I-only 6/0), cycle 171 (076 071 was I-only 43/0), cycle 167
(999 090 076 was I-only 16/0), cycle 160 (076 020 010 was
I-only 12/0), and cycle 141 (076 010 079 was I-only 8/0).
This cycle is the new leftover 3-gram 076 071 061 only.
071 999 (071 065 071 999) does not count. 076 076
(700 076 076 053) does not. Do not retune 076 071 700,
076 071 090, 076 071 600, 076 071 076, 076 071,
999 090 076, 076 020 010, or 076 010 079. Cycle 185
leftover N=2 and leftover n=4 set stay. Do not assume the
I-only result. Include any inside-family 076 071 site that
also has this 3-gram, if one exists.

Locks exact consecutive hits of 076 071 061 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_076_071_061_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
2 at Ia3[147]/Ia12[59] (the two leftover matching sites;
no inside-family 076 071 site has this 3-gram); Ib
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE185_MATCHING_SITES
...
STANDING_I_SITE_ROWS = (
    {
        ...
        "line": "Ia3",
        "index": 147,
        "next_4gram": ("076", "071", "061", "011"),
        "in_cycle185_leftover_2": True,
...
STANDING_CLAIM = "i_3gram_076_071_061_i_only"
STANDING_I_3GRAM_076_071_061_I_ONLY = True
STANDING_RESULT = "i_3gram_076_071_061_i_only"
...
def i_3gram_076_071_061_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
def i_site_row_as_survey(row: dict) -> dict:
    ...
    return {
        ...
        "in_cycle185_leftover_2": row["in_cycle185_leftover_2"],
        "in_cycle172_leftover_34": row["in_cycle172_leftover_34"],
        "inside_family": row["inside_family"],
    }


class TestI3gram076071061IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-185 leftover 3-gram tokens. No CV, no LLM."""
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_076_071_061_i_only(1, 0))
        self.assertTrue(i_3gram_076_071_061_i_only(2, 0))
        self.assertFalse(i_3gram_076_071_061_i_only(1, 1))
        self.assertFalse(i_3gram_076_071_061_i_only(0, 0))
        self.assertFalse(i_3gram_076_071_061_i_only(0, 2))
        self.assertEqual(STANDING_CLAIM, "i_3gram_076_071_061_i_only")
        self.assertTrue(STANDING_I_3GRAM_076_071_061_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_076_071_061_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle185_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-185 leftover, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_FORWARD)
...
class TestMamariI3gram076071061IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 076 071 061 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_076_071_061_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_185_leftover_not_retuned(self):
        """3-gram is the cycle-185 leftover lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3, ("076", "071", "061"))
        prior_185 = self.survey["i_leftover_076_071_forward_061"]
        self.assertEqual(prior_185["cycle"], 185)
        self.assertEqual(tuple(prior_185["forward_3gram"]), GRAM3)
        self.assertEqual(prior_185["N_with_forward_076_071_061"], 2)
        self.assertEqual(prior_185["N_without"], 32)
        self.assertEqual(prior_185["N_leftover"], 34)
        measured_matching = [list(site) for site in CYCLE185_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_185["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_185["i_leftover_076_071_exactly_2_forward_076_071_061"])
        self.assertTrue(prior_185["071_999_does_not_count"])
        self.assertTrue(prior_185["076_076_does_not_count"])
        self.assertNotEqual(GRAM3, GRAM5)
        self.assertNotEqual(GRAM3, CYCLE183_GRAM3)
        self.assertNotEqual(GRAM3, CYCLE180_GRAM3)
        self.assertNotEqual(GRAM3, CYCLE177_GRAM3)
    ...
    def test_i_hits_are_two_on_ia(self):
        ...
        leftover2_in_i = tuple(
            site for site in STANDING_I_SITES if site in CYCLE185_MATCHING_SITES
        )
        inside_in_i = tuple(
        ...
        self.assertEqual(leftover2_in_i, CYCLE185_MATCHING_SITES)
        ...
        for site in STANDING_I_SITES:
            ...
            self.assertIn(site, CYCLE185_MATCHING_SITES)
        ...
        for row, site, nxt in zip(
            ...
            self.assertTrue(row["in_cycle185_leftover_2"])
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_076_071_061_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_076_071_061_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_185_184_183_182_181_180_179_178_177_176_175_174_173_172_171_170_167_160_141_103_and_w_scoreboards_still_compute(self):
        """Cycle 185 leftover 2, 184/183 I-only, 182 leftover 3, 181/180 I-only, 179/178/177/176/175/174/173/172/171/170, 167/160/141, 103, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-186 3-gram I-only lock."""
        lock = self.survey["i_3gram_076_071_061_i_only"]
        self.assertEqual(lock["cycle"], 186)
        ...
        self.assertTrue(lock["i_3gram_076_071_061_i_only"])
        self.assertEqual(
            lock["i_3gram_076_071_061_i_only"],
            STANDING_I_3GRAM_076_071_061_I_ONLY,
        )
        ...
        self.assertEqual(self.survey["i_leftover_076_071_forward_061"]["N_without"], 32)
        self.assertEqual(self.survey["i_leftover_076_071_forward_061"]["N_leftover"], 34)
...
class TestMamariI3gram076071061IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 186 does not touch clustering. 83/62 / Hamming 6 stays."""
