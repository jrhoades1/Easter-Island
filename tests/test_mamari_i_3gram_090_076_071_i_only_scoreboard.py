"""I's cycle-194 leftover 3-gram 090 076 071 off-I lock.

Cycle 195 text-search lock. Uses already-vendored A–V and the
cycle-194 leftover previous 3-gram 090 076 071 (the n=3 run
shared by leftover 076 071 sites Ia5[67] 000 090 076 071,
Ia13[153] 076 090 076 071, and Ia14[106] 090 090 076 071).
...
cycle 186 (076 071 061 was I-only 2/0), cycle 183
(076 071 700 was I-only 3/0), cycle 180 (076 071 090 was
I-only 5/0), cycle 177 (076 071 600 was I-only 4/0), cycle
174 (076 071 076 was I-only 6/0), cycle 171 (076 071 was
I-only 43/0), cycle 167 (999 090 076 was I-only 16/0), cycle
160 (076 020 010 was I-only 12/0), and cycle 141
(076 010 079 was I-only 8/0). This cycle is the new leftover
previous 3-gram 090 076 071 only. 071 999 (071 065 071 999)
does not count. 076 076 (700 076 076 053) does not.
Near-miss Ia1[163] 090 700 076 071 does not (the previous
stem is 700). Near-miss Ia12[71] 090 305 076 071 does not
(the previous stem is 305). Do not retune 700 076 071,
076 071 090, 076 071, 999 090 076, 076 020 010, or
076 010 079. Cycle 194 leftover N=3, cycle 191 leftover
N=3, cycle 190 N_distinct=28, cycle 172 leftover N=34, and
leftover n=4 set stay. Do not assume the I-only result.
Include any inside-family 076 071 site that also has this
3-gram: leftover n=4 maximal 999 090 076 071 at Ia4[155],
Ia5[3], and Ia5[24] (cycle 172 inside-family) count as real
I hits because they are consecutive 090 076 071.

Locks exact consecutive hits of 090 076 071 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_090_076_071_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
6 at Ia4[154]/Ia5[2]/Ia5[23]/Ia5[66]/Ia13[152]/Ia14[105]
(the three leftover-3 / leftover-34 076 071 sites plus
three inside-family leftover n=4 maximals; no extra I
hits); Ib unpublished 0; every other vendored tablet is
exact-0. Not an n≥8 island. Not the cycle-103 I 5-gram.
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE194_MATCHING_SITES
...
STANDING_LEFTOVER_34_SITES = CYCLE194_MATCHING_SITES
STANDING_LEFTOVER_34_COUNT = 3
STANDING_INSIDE_FAMILY_SITES = STANDING_INSIDE_PREVIOUS_090_SITES
...
STANDING_INSIDE_FAMILY_PREVIOUS_4GRAM = STANDING_INSIDE_PREVIOUS_090_PREVIOUS_4GRAM
...
STANDING_I_SITE_ROWS = (
    {
        ...
        "line": "Ia4",
        "index": 154,
        "previous_4gram": ("999", "090", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia5",
        "index": 66,
        "previous_4gram": ("000", "090", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia13",
        "index": 152,
        "previous_4gram": ("076", "090", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia14",
        "index": 105,
        "previous_4gram": ("090", "090", "076", "071"),
...
STANDING_CLAIM = "i_3gram_090_076_071_i_only"
STANDING_I_3GRAM_090_076_071_I_ONLY = True
STANDING_RESULT = "i_3gram_090_076_071_i_only"
...
def leftover_076_071_site_for_3gram(
    ...
    """076 071 start one token after 090 076 071."""
...
def site_previous_4gram_for_3gram(
    ...
    """W 090 076 071 if a previous stem exists; None at start-of-line."""
...
def i_3gram_090_076_071_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
class TestI3gram090076071IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-194 leftover previous 3-gram tokens. No CV, no LLM."""
    ...
    def test_counts_require_consecutive_tokens(self):
        ...
        self.assertEqual(ngram_hit_count([["305", "076", "071"]], GRAM3), 0)
        self.assertEqual(
            ngram_hit_count(
    ...
    def test_previous_4gram_requires_stem_before_3gram(self):
        """Previous 4-gram is W 090 076 071; start-of-line is none."""
        provider = MockProvider()
        has_prev = ["000", "090", "076", "071", "004"]
        self.assertEqual(
            site_previous_4gram_for_3gram(has_prev, 1, GRAM3),
            ("000", "090", "076", "071"),
        )
        start_of_line = ["090", "076", "071", "004"]
        ...
        self.assertEqual(
            leftover_076_071_site_for_3gram((SIDE_IA, "Ia5", 66)),
            (SIDE_IA, "Ia5", 67),
        )
        self.assertEqual(provider.get_call_history(), [])
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_090_076_071_i_only(1, 0))
        self.assertTrue(i_3gram_090_076_071_i_only(6, 0))
        self.assertFalse(i_3gram_090_076_071_i_only(1, 1))
        self.assertFalse(i_3gram_090_076_071_i_only(0, 0))
        self.assertFalse(i_3gram_090_076_071_i_only(0, 6))
        self.assertFalse(i_3gram_090_076_071_i_only(6, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_090_076_071_i_only")
        self.assertTrue(STANDING_I_3GRAM_090_076_071_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_090_076_071_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle194_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-194 leftover previous run, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        ...
        for leftover in CYCLE194_MATCHING_PREVIOUS_4GRAMS:
            self.assertTrue(is_contiguous_substring(GRAM3, leftover))
        self.assertTrue(
            is_contiguous_substring(GRAM3, STANDING_INSIDE_FAMILY_PREVIOUS_4GRAM)
        )
        self.assertEqual(STANDING_INSIDE_FAMILY_PREVIOUS_4GRAM, LEFTOVER_N4_090071)
        self.assertEqual(
            STANDING_INSIDE_FAMILY_PREVIOUS_4GRAM,
            ("999", "090", "076", "071"),
        )
...
class TestMamariI3gram090076071IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 090 076 071 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_090_076_071_i_only(
            self.i_hits,
            self.off_i_hits,
        )
    ...
    def test_tokens_are_cycle_194_leftover_not_retuned(self):
        """3-gram is the cycle-194 leftover previous lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        self.assertEqual(GRAM3, ("090", "076", "071"))
        ...
        self.assertEqual(prior_194["N_no_backward"], 0)
        measured_matching = [list(site) for site in CYCLE194_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_194["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_194["i_leftover_076_071_exactly_3_previous_090_076_071"])
        self.assertTrue(prior_194["071_999_does_not_count"])
        self.assertTrue(prior_194["076_076_does_not_count"])
        self.assertTrue(prior_194["inside_family_does_not_count"])
        self.assertTrue(prior_194["near_miss_090_700_does_not_count"])
        self.assertTrue(prior_194["disjoint_from_cycle191_leftover_3"])
    ...
    def test_i_hits_are_six_on_ia(self):
        """3-gram is 6 on Ia (3 leftover-3/leftover-34, 3 inside-family); Ib 0."""
        ...
        leftover3_in_i = tuple(
            leftover_076_071_site_for_3gram(site)
            for site in STANDING_I_SITES
            if leftover_076_071_site_for_3gram(site) in CYCLE194_MATCHING_SITES
        )
        ...
        extra_in_i = tuple(
            leftover_076_071_site_for_3gram(site)
            for site in STANDING_I_SITES
            if leftover_076_071_site_for_3gram(site) not in CYCLE194_MATCHING_SITES
            and leftover_076_071_site_for_3gram(site) not in STANDING_INSIDE_SITES
        )
        self.assertEqual(leftover3_in_i, STANDING_LEFTOVER_MATCHING_SITES)
        self.assertEqual(leftover3_in_i, CYCLE194_MATCHING_SITES)
        self.assertEqual(len(leftover3_in_i), STANDING_LEFTOVER_MATCHING_COUNT)
        self.assertEqual(STANDING_LEFTOVER_MATCHING_COUNT, 3)
        self.assertEqual(leftover34_in_i, STANDING_LEFTOVER_34_SITES)
        self.assertEqual(leftover34_in_i, CYCLE194_MATCHING_SITES)
        self.assertEqual(len(leftover34_in_i), STANDING_LEFTOVER_34_COUNT)
        self.assertEqual(STANDING_LEFTOVER_34_COUNT, 3)
        self.assertEqual(inside_in_i, STANDING_INSIDE_FAMILY_SITES)
        self.assertEqual(inside_in_i, STANDING_LEFTOVER_090071_COVERED)
        ...
        self.assertEqual(
            tuple(
                leftover_076_071_site_for_3gram(site)
                for site in STANDING_INSIDE_FAMILY_3GRAM_SITES
            ),
            STANDING_LEFTOVER_090071_COVERED,
        )
        for site in STANDING_INSIDE_SITES:
            if site not in STANDING_LEFTOVER_090071_COVERED:
        ...
        for site in STANDING_I_SITES:
            ...
            if gram2_site in CYCLE194_MATCHING_SITES:
                ...
                self.assertIn(gram2_site, STANDING_INSIDE_SITES)
                self.assertIn(gram2_site, STANDING_LEFTOVER_090071_COVERED)
                self.assertNotIn(gram2_site, STANDING_LEFTOVER_SITES)
        self.assertTrue(STANDING_DISJOINT_FROM_CYCLE191_LEFTOVER_3)
        self.assertEqual(
            set(CYCLE194_MATCHING_SITES) & set(CYCLE191_MATCHING_SITES),
            set(),
        )
        ...
        for row, site, prev4 in zip(
            ...
            if site in STANDING_LEFTOVER_MATCHING_3GRAM_SITES:
        ...
        self.assertNotEqual(
            tuple(inside_near_stems[inside_near_index - 1 : inside_near_index + 2]),
            GRAM3,
        )
        self.assertNotIn(STANDING_NEAR_MISS_INSIDE_090_305_SITE, STANDING_I_SITES)
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        """3-gram is 0 on A–H and J–V. Ia has exactly 6. W is not a tablet."""
        ...
        self.assertEqual(
            i_3gram_090_076_071_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_090_076_071_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_194_191_172_171_scoreboards_still_compute(self):
        """Cycle 194 leftover-3, 191 leftover-3, 172 leftover-34, and 171 I-only 43/0 stay."""
        ...
        self.assertEqual(
            set(prior_194.with_sites) & set(prior_191.with_sites),
            set(),
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-195 3-gram I-only lock."""
        lock = self.survey["i_3gram_090_076_071_i_only"]
        self.assertEqual(lock["cycle"], 195)
        ...
        self.assertEqual(lock["leftover_34_count"], 3)
        self.assertEqual(
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["inside_family_3gram_sites"]),
            STANDING_INSIDE_FAMILY_3GRAM_SITES,
        )
        self.assertEqual(
            [list(gram) for gram in STANDING_INSIDE_FAMILY_PREVIOUS_4GRAMS],
            lock["inside_family_previous_4grams"],
        )
        ...
        self.assertTrue(lock["i_3gram_090_076_071_i_only"])
        self.assertEqual(
            lock["i_3gram_090_076_071_i_only"],
            STANDING_I_3GRAM_090_076_071_I_ONLY,
        )
...
class TestMamariI3gram090076071IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 195 does not touch clustering. 83/62 / Hamming 6 stays."""
