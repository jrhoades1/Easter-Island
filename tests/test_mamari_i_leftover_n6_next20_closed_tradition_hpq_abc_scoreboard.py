"""Reframed closed-tradition hold-out of the locked leftover-6 next-20 grams.

Cycle 853 text-search lock. Uses already-vendored A–V and the
cycle-503 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 20-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 503 already locked the I-only claim
on these 20-grams as incomplete-set LOSE (N_with_next20=15,
N_no_next20=3, N_i_only=15, N_leak=0). Cycle 852 already locked
the reframed claim on the cycle-501 next-19 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–852.

Claim that can lose: each existing next-20 gram is an exact
...
Named claim: cycle503_next20_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next20 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next20=15,
N_no_next20=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-19 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 extends to 604 076
071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244
999 090 at Ia9[9] and hits I once. All fifteen next-19 grams
...
next 20-grams as a peel, next 21-grams as a peel, prev-184, or
next-185. The cycle-505 next-21 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_next19_closed_tradition_hpq_abc_scoreboard import (
    ABSENT_TABLETS,
    CYCLE457_RESULT,
    ...
    PREV1_N6_SCOREBOARD,
    PROBE_TABLETS,
    STANDING_HITS_I as CYCLE852_HITS_I,
    STANDING_N_HOLD as CYCLE852_N_HOLD,
    STANDING_N_LEAK as CYCLE852_N_LEAK,
    STANDING_NEXT_CHEAP_LOCK as STANDING_NAMED_BY_CYCLE852,
    STANDING_NO_NEXT19_SITES as CYCLE852_NO_NEXT19_SITES,
    STANDING_RESULT as CYCLE852_RESULT,
    STANDING_SEQUENCES as CYCLE852_SEQUENCES,
    STANDING_SITES as CYCLE852_SITES,
    STANDING_VERDICT as CYCLE852_VERDICT,
    absent_hit_sum,
    hpq_tablets_hit,
    reframed_verdict,
    row_holds,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next20_i_only_scoreboard import (
    STANDING_LEFTOVER_MATCHING_NEXT20_SITES as CYCLE503_SITES,
    STANDING_N_I_EACH as CYCLE503_N_I_EACH,
)
...
STANDING_NO_NEXT20_SITES = CYCLE503_NO_NEXT20_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE503_SITES
STANDING_SEQUENCES = CYCLE503_SEQUENCES
...
STANDING_NEXT19_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT19_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next20_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next20_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 503
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 503
...
STANDING_DO_NOT_RELOCK_CYCLES_832_852 = True
...
class TestMamariILeftoverN6Next20ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 853 reframed closed-tradition lock on the cycle-503 next-20 population."""
    ...
    def test_population_is_the_locked_cycle_503_next20_grams(self):
        """Fifteen next-20 sequences and sites come from cycle 503. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE503_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE503_SITES)
        self.assertEqual(CYCLE503_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE503_N, 18)
        self.assertEqual(CYCLE503_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT20, CYCLE503_N_WITH_NEXT20)
        self.assertEqual(STANDING_N_NO_NEXT20, CYCLE503_N_NO_NEXT20)
        self.assertEqual(
            STANDING_NO_NEXT20_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT20_SITES, CYCLE852_NO_NEXT19_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 503)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 20)
        ...
        self.assertEqual(CYCLE503_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next20_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE852, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 503 I-only or the cycle 832–852 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 503)
        ...
        self.assertEqual(next19["next_cheap_lock"], STANDING_NAMED_BY_CYCLE852)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE852, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_852)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE503_N_I_EACH)
        self.assertEqual(CYCLE503_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT19_ALL_EXTEND)
        for n19, n20, site19, site20 in zip(
            CYCLE852_SEQUENCES, STANDING_SEQUENCES, CYCLE852_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n20[:19], n19)
            self.assertEqual(site20, site19)
        still = self.rows[STANDING_NEXT19_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:19], CYCLE852_SEQUENCES[3])
        self.assertEqual(CYCLE852_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT20_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_505_next21_list(self):
        """Cycle 505 next-21 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE505_N, 18)
        self.assertEqual(CYCLE505_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE505_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 21 for tokens in CYCLE505_SEQUENCES))
        ...
        self.assertEqual(CYCLE505_N_WITH_NEXT21, 15)
        self.assertEqual(CYCLE505_N_NO_NEXT21, 3)
        self.assertEqual(
            CYCLE505_NO_NEXT21_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        ...
        self.assertNotIn("i_leftover_n6_next21_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle505_next21_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-853 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 853)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next20_sites"]),
            STANDING_NO_NEXT20_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_852"])
        ...
        self.assertEqual(lock["named_by_cycle852"], STANDING_NAMED_BY_CYCLE852)
        ...
        self.assertEqual(
            tuple(lock["next19_hapax_still_hapax"]),
            STANDING_NEXT19_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next19_all_extend"])
        self.assertEqual(lock["next19_all_extend"], STANDING_NEXT19_ALL_EXTEND)
