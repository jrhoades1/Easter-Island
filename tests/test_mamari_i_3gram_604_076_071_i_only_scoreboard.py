"""I's cycle-197 leftover 3-gram 604 076 071 off-I lock.

Cycle 198 text-search lock. Uses already-vendored A–V and the
cycle-197 leftover previous 3-gram 604 076 071 (the n=3 run
shared by leftover 076 071 sites Ia3[147] 600 604 076 071
and Ia9[10] 700 604 076 071). Does not retune that 3-gram,
...
cycle 192 (700 076 071 was I-only 3/0), cycle 186
(076 071 061 was I-only 2/0), cycle 183 (076 071 700 was
I-only 3/0), cycle 180 (076 071 090 was I-only 5/0), cycle
177 (076 071 600 was I-only 4/0), cycle 174 (076 071 076
was I-only 6/0), cycle 171 (076 071 was I-only 43/0), cycle
167 (999 090 076 was I-only 16/0), cycle 160 (076 020 010
was I-only 12/0), and cycle 141 (076 010 079 was I-only
8/0). This cycle is the new leftover previous 3-gram
604 076 071 only. 071 999 (071 065 071 999) does not count.
076 076 (700 076 076 053) does not. Cycle 191 leftover
700 076 071 does not. Cycle 194 leftover 090 076 071 does
not. Near-miss Ia1[163] 090 700 076 071 does not (the
previous stem is 700). Forward 604 after leftover 076 071
at Ia3[13] 076 071 274 604 and Ia3[133] 076 071 202 604
does not. Cycle 191's near-miss Ia9[10] 700 604 076 071
does count here (the previous stem is 604). Do not retune
090 076 071, 700 076 071, 076 071, 999 090 076, 076 020
010, or 076 010 079. Cycle 197 leftover N=2, cycle 194
leftover N=3, cycle 191 leftover N=3, cycle 190
N_distinct=28, cycle 172 leftover N=34, and leftover n=4
set stay. Do not assume the I-only result. Include any
inside-family 076 071 site that also has this 3-gram, if
one exists (none does; cycle 197 already measured that).

Locks exact consecutive hits of 604 076 071 on tablet I
and on every other vendored tablet A–H and J–V. Claim that
can lose: i_3gram_604_076_071_i_only (I hits ≥ 1 and
off-I hits == 0). True only if N_off_I == 0. Ia is exactly
2 at Ia3[146]/Ia9[9] (the two leftover-2 / leftover-34
...
STANDING_LEFTOVER_MATCHING_SITES = CYCLE197_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 2
STANDING_LEFTOVER_MATCHING_3GRAM_SITES = (
    (SIDE_IA, "Ia3", 146),
    (SIDE_IA, "Ia9", 9),
)
STANDING_LEFTOVER_34_SITES = CYCLE197_MATCHING_SITES
...
STANDING_I_SITE_ROWS = (
    {
        ...
        "line": "Ia3",
        "index": 146,
        "previous_4gram": ("600", "604", "076", "071"),
    ...
    },
    {
        ...
        "line": "Ia9",
        "index": 9,
        "previous_4gram": ("700", "604", "076", "071"),
...
STANDING_CLAIM = "i_3gram_604_076_071_i_only"
STANDING_I_3GRAM_604_076_071_I_ONLY = True
STANDING_RESULT = "i_3gram_604_076_071_i_only"
...
def leftover_076_071_site_for_3gram(
    ...
    """076 071 start one token after 604 076 071."""
...
def site_previous_4gram_for_3gram(
    ...
    """W 604 076 071 if a previous stem exists; None at start-of-line."""
...
def i_3gram_604_076_071_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
class TestI3gram604076071IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-197 leftover previous 3-gram tokens. No CV, no LLM."""
    ...
    def test_counts_require_consecutive_tokens(self):
        ...
        self.assertEqual(ngram_hit_count([["076", "071", "274", "604"]], GRAM3), 0)
        self.assertEqual(ngram_hit_count([["076", "071", "202", "604"]], GRAM3), 0)
        self.assertEqual(
            ngram_hit_count(
    ...
    def test_previous_4gram_requires_stem_before_3gram(self):
        """Previous 4-gram is W 604 076 071; start-of-line is none."""
        provider = MockProvider()
        has_prev = ["600", "604", "076", "071", "061"]
        self.assertEqual(
            site_previous_4gram_for_3gram(has_prev, 1, GRAM3),
            ("600", "604", "076", "071"),
        )
        start_of_line = ["604", "076", "071", "061"]
        ...
        self.assertEqual(
            site_previous_4gram_for_3gram(cycle191_near, 1, GRAM3),
            ("700", "604", "076", "071"),
        )
        self.assertEqual(
            leftover_076_071_site_for_3gram((SIDE_IA, "Ia3", 146)),
            (SIDE_IA, "Ia3", 147),
        )
        self.assertEqual(
            leftover_076_071_site_for_3gram((SIDE_IA, "Ia9", 9)),
            (SIDE_IA, "Ia9", 10),
        )
        self.assertEqual(provider.get_call_history(), [])
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_604_076_071_i_only(1, 0))
        self.assertTrue(i_3gram_604_076_071_i_only(2, 0))
        self.assertFalse(i_3gram_604_076_071_i_only(1, 1))
        self.assertFalse(i_3gram_604_076_071_i_only(0, 0))
        self.assertFalse(i_3gram_604_076_071_i_only(0, 2))
        self.assertFalse(i_3gram_604_076_071_i_only(2, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_604_076_071_i_only")
        self.assertTrue(STANDING_I_3GRAM_604_076_071_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_604_076_071_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle197_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-197 leftover previous run, not 071 999, 076 076, or the priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        ...
        for leftover in CYCLE197_MATCHING_PREVIOUS_4GRAMS:
...
class TestMamariI3gram604076071IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 604 076 071 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_604_076_071_i_only(
            self.i_hits,
            self.off_i_hits,
        )
    ...
    def test_tokens_are_cycle_197_leftover_not_retuned(self):
        """3-gram is the cycle-197 leftover previous lock, not a new inventory."""
        self.assertEqual(GRAM3, GRAM3_BACKWARD)
        self.assertEqual(GRAM3, ("604", "076", "071"))
        ...
        self.assertEqual(prior_197["N_no_backward"], 0)
        measured_matching = [list(site) for site in CYCLE197_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_197["matching_leftover_sites"]],
            measured_matching,
        )
        self.assertTrue(prior_197["i_leftover_076_071_exactly_2_previous_604_076_071"])
        self.assertTrue(prior_197["071_999_does_not_count"])
        self.assertTrue(prior_197["076_076_does_not_count"])
        self.assertTrue(prior_197["inside_family_does_not_count"])
        self.assertTrue(prior_197["near_miss_forward_604_does_not_count"])
        self.assertTrue(prior_197["disjoint_from_cycle191_leftover_3"])
        self.assertTrue(prior_197["disjoint_from_cycle194_leftover_3"])
    ...
    def test_i_hits_are_two_on_ia(self):
        ...
        leftover2_in_i = tuple(
            ...
            if leftover_076_071_site_for_3gram(site) in CYCLE197_MATCHING_SITES
        )
        ...
        extra_in_i = tuple(
            leftover_076_071_site_for_3gram(site)
            for site in STANDING_I_SITES
            if leftover_076_071_site_for_3gram(site) not in CYCLE197_MATCHING_SITES
        ...
        self.assertEqual(leftover2_in_i, CYCLE197_MATCHING_SITES)
        ...
        self.assertEqual(leftover34_in_i, CYCLE197_MATCHING_SITES)
        ...
        self.assertEqual(self.leftover_076_071_sites, CYCLE197_MATCHING_SITES)
        ...
        for site in STANDING_I_SITES:
            ...
            self.assertIn(gram2_site, CYCLE197_MATCHING_SITES)
        ...
        self.assertEqual(self.previous_4grams, CYCLE197_MATCHING_PREVIOUS_4GRAMS)
        ...
        self.assertIn(STANDING_NEAR_MISS_LEFTOVER_700_604_SITE, self.leftover_076_071_sites)
        self.assertEqual(
            leftover_076_071_site_for_3gram((SIDE_IA, "Ia9", 9)),
            STANDING_NEAR_MISS_LEFTOVER_700_604_SITE,
        )
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        ...
        self.assertEqual(
            i_3gram_604_076_071_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_604_076_071_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_197_194_191_172_171_scoreboards_still_compute(self):
        """Cycle 197 leftover-2, 194 leftover-3, 191 leftover-3, 172 leftover-34, and 171 I-only 43/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-198 3-gram I-only lock."""
        lock = self.survey["i_3gram_604_076_071_i_only"]
        self.assertEqual(lock["cycle"], 198)
        ...
        self.assertTrue(lock["i_3gram_604_076_071_i_only"])
        self.assertEqual(
            lock["i_3gram_604_076_071_i_only"],
            STANDING_I_3GRAM_604_076_071_I_ONLY,
        )
...
class TestMamariI3gram604076071IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 198 does not touch clustering. 83/62 / Hamming 6 stays."""
