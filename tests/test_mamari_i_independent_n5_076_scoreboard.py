"""I's cycle-136 independent n=5 maximals vs stem 076.

Cycle 139 text-search lock. Uses already-vendored A–V and the
...
For each independent n=5 maximal, whether the gram contains
token 076 (Santiago staff formulaic glyph). Hypothesis: all 4
contain at least one 076. Measured: 4 of 4 do. No exceptions.
Claim that can lose:
i_independent_n5_maximals_all_contain_076. The claim is true.
Do not retune.
...
STANDING_EXCEPTIONS = ()
STANDING_CONTAINS_076 = (True, True, True, True)
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_independent_n5_maximals_all_contain_076"
STANDING_I_INDEPENDENT_N5_MAXIMALS_ALL_CONTAIN_076 = True
STANDING_RESULT = "i_independent_n5_maximals_076"
...
def independent_n5_rows(
    maximals: tuple[tuple[tuple[str, ...], int, int, tuple], ...] = STANDING_MAXIMALS,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Cycle-136 independent n=5 maximals. Does not retune the set."""
    return tuple(row for row in maximals if row[1] == 5)
...
def n5_with_076(
    ...
    """Independent n=5 maximals that contain stem 076."""
...
def n5_without_076(
    ...
    """Independent n=5 maximals that do not contain stem 076."""
    return tuple(row for row in rows if stem not in row[0])


def i_independent_n5_maximals_all_contain_076(
    rows: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    stem: str = STEM_076,
) -> bool:
    """True iff every independent n=5 maximal contains stem 076.
...
class TestIIndependentN5Maximals076Helpers(unittest.TestCase):
    """Helpers on cycle-136 n=5 tokens. No CV, no LLM."""

    def test_contains_076_and_all_contain_can_fail(self):
        """The four n=5 maximals hold; empty and a 076-less gram lose."""
        ...
        self.assertTrue(gram_contains_076(MAXIMAL_N5_430))
        self.assertTrue(gram_contains_076(MAXIMAL_N5_011))
        self.assertTrue(gram_contains_076(MAXIMAL_N5_400))
        self.assertTrue(gram_contains_076(MAXIMAL_N5_010))
        self.assertFalse(gram_contains_076(("400", "070", "020", "010", "071")))
        self.assertFalse(i_independent_n5_maximals_all_contain_076(()))
        with_076 = n5_with_076(n5)
        without_076 = n5_without_076(n5)
        self.assertEqual(len(with_076), STANDING_WITH_076)
        self.assertEqual(len(without_076), STANDING_WITHOUT_076)
        self.assertEqual(with_076, STANDING_N5)
        self.assertEqual(without_076, STANDING_EXCEPTIONS)
        self.assertTrue(i_independent_n5_maximals_all_contain_076(with_076))
        self.assertFalse(i_independent_n5_maximals_all_contain_076(without_076))
        self.assertTrue(i_independent_n5_maximals_all_contain_076(n5))
        dropped = (
            (MAXIMAL_N5_430, 5, 2, STANDING_N5[0][3]),
            (("400", "070", "020", "010", "071"), 5, 2, STANDING_N5[2][3]),
        )
        self.assertFalse(i_independent_n5_maximals_all_contain_076(dropped))
        self.assertEqual(
            tuple(STEM_076 in gram for gram, _n, _f, _s in n5),
            STANDING_CONTAINS_076,
        )
        self.assertEqual(STANDING_CLAIM, "i_independent_n5_maximals_all_contain_076")
        self.assertTrue(STANDING_I_INDEPENDENT_N5_MAXIMALS_ALL_CONTAIN_076)
        self.assertEqual(
            STANDING_I_INDEPENDENT_N5_MAXIMALS_ALL_CONTAIN_076,
            HYPOTHESIS_ALL_CONTAIN_076,
        )
        self.assertEqual(STANDING_WITH_076 + STANDING_WITHOUT_076, STANDING_INDEPENDENT_N5)
...
class TestMamariIIndependentN5076Scoreboard(unittest.TestCase):
    """Cited-fixture independent n=5 maximals vs 076. Mock only."""
    ...
    def setUp(self):
        ...
        self.with_076 = n5_with_076(self.n5)
        self.without_076 = n5_without_076(self.n5)
        self.claim_holds = i_independent_n5_maximals_all_contain_076(self.n5)
    ...
    def test_counts_4_of_4_and_hypothesis_all_contain_holds(self):
        """4 independent 5-grams contain 076; 0 do not. Claim is true."""
        ...
        self.assertEqual(STANDING_WITH_076, 4)
        self.assertEqual(STANDING_WITHOUT_076, 0)
        self.assertEqual(
            STANDING_INDEPENDENT_N5,
            STANDING_WITH_076 + STANDING_WITHOUT_076,
        )
        self.assertTrue(HYPOTHESIS_ALL_CONTAIN_076)
        self.assertTrue(i_independent_n5_maximals_all_contain_076(self.n5))
        self.assertEqual(
            self.claim_holds,
            STANDING_I_INDEPENDENT_N5_MAXIMALS_ALL_CONTAIN_076,
        )
        ...
        self.assertEqual(STANDING_CLAIM, "i_independent_n5_maximals_all_contain_076")
        self.assertEqual(
            tuple(STEM_076 in gram for gram, _n, _f, _s in self.n5),
            STANDING_CONTAINS_076,
        )
        self.assertEqual(
            sum(1 for flag in STANDING_CONTAINS_076 if flag),
            STANDING_WITH_076,
        )
        self.assertEqual(
    ...
    def test_no_exceptions(self):
        """Every independent n=5 maximal contains 076; exception list is empty."""
        self.assertEqual(self.without_076, STANDING_EXCEPTIONS)
        self.assertEqual(len(STANDING_EXCEPTIONS), 0)
        self.assertEqual(STANDING_EXCEPTIONS, ())
        for gram, n, freq, _sites in self.n5:
            self.assertEqual(n, 5)
            self.assertEqual(freq, 2)
            self.assertTrue(gram_contains_076(gram))
            self.assertIn(STEM_076, gram)
            self.assertNotEqual(gram, GRAM5)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_existing_138_137_136_103_and_w_scoreboards_still_compute(self):
        """Cycle 138 I-only, 137 leftover 076, 136 maximals, 103 I-only, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-139 independent n=5 076 lock."""
        lock = self.survey["i_independent_n5_maximals_076"]
        self.assertEqual(lock["cycle"], 139)
        ...
        self.assertEqual(lock["exception_tokens"], [])
        self.assertEqual(lock["exception_sites"], [])
        self.assertTrue(lock["known_distinct"])
        self.assertEqual(lock["known_distinct"], STANDING_KNOWN_DISTINCT)
        self.assertEqual(lock["claim"], STANDING_CLAIM)
        self.assertTrue(lock["i_independent_n5_maximals_all_contain_076"])
        self.assertEqual(
            lock["i_independent_n5_maximals_all_contain_076"],
            STANDING_I_INDEPENDENT_N5_MAXIMALS_ALL_CONTAIN_076,
        )
...
class TestMamariIIndependentN5076ImageSnapshot(unittest.TestCase):
    """Cycle 139 does not touch clustering. 83/62 / Hamming 6 stays."""
