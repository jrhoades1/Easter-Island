"""I's cycle-228 leftover extra remaining-after-013 next-stem lock.

Cycle 231 text-search lock. Uses already-vendored A–V and the
...
Leftover extra remaining-after-013 = leftover extra I 090 076
sites with a next token whose next token is none of 070, 071,
or 013. Nested-check leftover extra N_leftover==56,
...
G=013 K=5 (do not retune cycles 225–228). Nested-check cycle
229 090 076 013 I-only 5/0 and cycle 230 five forward 4-grams
1/0 each (do not retune). Nested-check N_remaining3==36
(41−5). Ia2[174] has next token 000 and is remaining-after-013;
it is not no-next. Off-I T sites are not this cycle. I-only of
090 076 G is leftover-of-leftover for a later cycle if K≥2.
Previous 4-grams of 090 076 013 are not this cycle. 076 071
and 076 070 do not count as this 2-gram. Inside-family sites
do not count as leftover extra.

Among remaining-after-013, G = the next stem with the highest
remaining-after-013 count. If a tie, pick the larger Barthel
id. K = that count. Measured: N_remaining3=36,
N_distinct_remaining3=27, G=001 uniquely most frequent (next
is 700×2), K=3 at Ia4[134], Ia6[134], Ia7[88]. Cycle 228
reported next-after-G as 001×3; measure it, do not assume it.
Claim that can lose:
...
stay, N_remaining3==36, G is uniquely most frequent under the
tie-break, and exactly K remaining-after-013 sites share G.
The claim is true. Same claim-shape as cycle 228 (leftover
extra remaining-after-071 exactly 5 share 013), cycle 227
(leftover extra remaining exactly 6 share 071), and cycle 226
(leftover extra exactly 8 share forward 070). This can lose
if N_remaining3 ≠ 36 or the leftover extra remaining-after-013
filter disagrees with nested counts. Nested cycle 230 5/0
hapax, cycle 229 5/0, cycle 226 K=8 / G=070, cycle 223 69/3,
cycle 195 6/0, and cycle 171 43/0 stay. Do not assume the
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_071_next_stem_scoreboard import (
    ...
    leftover_extra_remaining_after_071_next_stems,
...
STANDING_N_DISTINCT_REMAINING3 = 27
STANDING_N_HAPAX_REMAINING3 = 19
STANDING_G = "001"
STANDING_K = 3
STANDING_N_WITHOUT_G = 33
STANDING_N_NEXT_AFTER_G = 2
STANDING_N_NEXT_AFTER_G_STEM = "700"
STANDING_G_UNIQUELY_MOST_FREQUENT = True
STANDING_REMAINING3_SITES = (
    (SIDE_IA, "Ia1", 2),
    (SIDE_IA, "Ia1", 15),
    (SIDE_IA, "Ia1", 27),
    (SIDE_IA, "Ia1", 59),
...
STANDING_REMAINING3_NEXT_STEMS = (
    "012",
    "175",
    "755",
    "470",
...
STANDING_CLAIM = "i_leftover_extra_090_076_remaining_after_013_exactly_K_share_G"
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_013_EXACTLY_K_SHARE_G = True
STANDING_RESULT = "i_leftover_extra_090_076_remaining_after_013_next_stem"
...
STANDING_PREVIOUS_4GRAMS_OF_090_076_013_ARE_NOT_THIS_CYCLE = True
...
def leftover_extra_remaining_after_013(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra with-next sites whose next token is not 070, 071, or 013."""
...
def leftover_extra_remaining_after_013_next_stems(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[str, ...]:
    """Next stems of leftover extra remaining-after-013 sites."""
    locked_set = set(locked)
    return tuple(
        nxt
...
def leftover_extra_remaining_after_013_with_g(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-013 sites whose next token is G."""
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
        if nxt == stem
...
def leftover_extra_remaining_after_013_without_g(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-013 sites whose next token is not G."""
    locked_set = set(locked)
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def remaining_after_013_next_stem_counts(next_stems: tuple[str, ...]) -> Counter:
    """Counts of next stems among leftover extra remaining-after-013."""
    return Counter(next_stems)


def rank_remaining_after_013_next_stems(
    counts: Counter,
) -> tuple[tuple[str, int], ...]:
    """Remaining-after-013 next stems by count, then larger Barthel id."""
    return tuple(
        sorted(
            counts.items(),
...
def select_remaining_after_013_g(
    remaining_stems: tuple[str, ...],
) -> tuple[str | None, int, bool]:
    """Return (G, K, uniquely_most_frequent). Empty remaining-after-013 has no G."""
    ranked = rank_remaining_after_013_next_stems(
        remaining_after_013_next_stem_counts(remaining_stems)
    )
    if not ranked:
        return (None, 0, False)
    gram, count = ranked[0]
...
def remaining_after_013_next_stem_frequency_table(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    ...
    """Remaining-after-013 next-stem frequency: highest count first, then larger id."""
    rem_sites = leftover_extra_remaining_after_013(sites, next_stems, locked)
    rem_stems = leftover_extra_remaining_after_013_next_stems(sites, next_stems, locked)
    locked_set = set(locked)
    rem_grams = tuple(
        gram
...
def remaining_after_013_next_stem_frequency_rows(
    table: tuple[
        tuple[str, int, tuple[tuple[str, str, int], ...], tuple[tuple[str, ...], ...]],
        ...,
    ] = STANDING_REMAINING3_FREQUENCY,
) -> list[dict]:
    """Survey-shaped remaining-after-013 next-stem frequency table."""
    rows = []
        ...
        rows.append(
            {
                "next_stem": stem,
                "count": count,
                "leftover_extra_remaining_after_013_sites": [
...
def matching_leftover_extra_remaining_after_013_local_4gram_rows(
    leftover_sites: tuple[tuple[str, str, int], ...] = STANDING_MATCHING_SITES,
    next_4grams: tuple[tuple[str, ...], ...] = STANDING_MATCHING_NEXT_4GRAMS,
) -> list[dict]:
    """Survey-shaped matching leftover extra remaining-after-013 next-4-gram rows."""
    rows = []
...
def leftover_extra_remaining_after_013_nested_counts_hold(
    n_leftover: int,
    n_with_next: int,
    n_no_next: int,
    n_share_070: int,
    ...
    """Nested leftover extra 56/55/1, 8/070, 47/6/071, 41/5/013, remaining3=36."""
    return (
        leftover_extra_remaining_after_071_nested_counts_hold(
            n_leftover,
            n_with_next,
        ...
        and n_share_013 == expected_share_013
        and n_remaining3 == expected_remaining3
...
def i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
    ...
    """True iff remaining3=36, G is unique max, and exactly K share G."""
    ...
    remaining3 = leftover_extra_remaining_after_013(leftover_sites, next_stems, locked)
    rem3_stems = leftover_extra_remaining_after_013_next_stems(
        leftover_sites,
        next_stems,
        locked,
    )
    if not leftover_extra_remaining_after_013_nested_counts_hold(
        len(leftover_sites),
        len(with_next),
        len(no_next),
        len(share_070),
        ...
        return False
    if len(remaining3) != expected_remaining3:
        return False
    if remaining3 != leftover_extra_remaining_after_071_without_g(
        leftover_sites,
        next_stems,
        LOCKED_FORWARD_STEM_013,
    ):
        return False
    gram, count, unique = select_remaining_after_013_g(rem3_stems)
...
class TestILeftoverExtra090076RemainingAfter013NextStemHelpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-013 next stems. No CV, no LLM."""

    def test_remaining_after_013_requires_with_next_not_070_not_071_not_013(self):
        """Remaining-after-013 excludes no-next and the locked 070/071/013 clusters."""
        provider = MockProvider()
        self.assertEqual(GRAM2, ("090", "076"))
        self.assertEqual(GRAM3_FORWARD, ("090", "076", "001"))
        self.assertEqual(GRAM3_NESTED_013, ("090", "076", "013"))
        self.assertEqual(GRAM3_NESTED_071, ("090", "076", "071"))
        self.assertEqual(LOCKED_FORWARD_STEMS, ("070", "071", "013"))
        ...
        self.assertEqual(site_forward_3gram(has_001, 1, GRAM2), GRAM3_FORWARD)
        self.assertEqual(
            site_next_4gram(has_001, 1, GRAM2),
            ("090", "076", "001", "048"),
        )
        has_013 = ["999", "090", "076", "013", "073"]
        self.assertEqual(site_next_stem(has_013, 1, GRAM2), "013")
        self.assertNotEqual(site_next_stem(has_013, 1, GRAM2), "001")
        has_071 = ["999", "090", "076", "071", "633"]
        self.assertEqual(site_next_stem(has_071, 1, GRAM2), "071")
        self.assertNotEqual(site_next_stem(has_071, 1, GRAM2), "001")
        has_070 = ["999", "090", "076", "070", "499"]
        self.assertEqual(site_next_stem(has_070, 1, GRAM2), "070")
        self.assertNotEqual(site_next_stem(has_070, 1, GRAM2), "001")
        ...
        planted_stems = ("001", "070", "071", "013", None, "700")
        self.assertEqual(
            leftover_extra_remaining_after_013(planted_sites, planted_stems),
            (planted_sites[0], planted_sites[5]),
        )
        self.assertEqual(
            leftover_extra_remaining_after_013_next_stems(planted_sites, planted_stems),
            ("001", "700"),
        )
        self.assertNotIn(
            planted_sites[1],
    ...
    def test_exactly_k_can_fail(self):
        """Boolean is True only when remaining3=36, unique G=001, K=3."""
        provider = MockProvider()
        ...
        rem3 = leftover_extra_remaining_after_013(leftover, stems)
        rem3_stems = leftover_extra_remaining_after_013_next_stems(leftover, stems)
        self.assertEqual(len(rem3), STANDING_N_REMAINING3)
        self.assertEqual(len(rem3), 36)
        self.assertEqual(rem3, STANDING_REMAINING3_SITES)
        self.assertEqual(rem3_stems, STANDING_REMAINING3_NEXT_STEMS)
        g, k, unique = select_remaining_after_013_g(rem3_stems)
        self.assertEqual(g, "001")
        ...
        dropped = []
        for nxt in stems:
            if not changed and nxt == "001":
                dropped.append("700")
                changed = True
            else:
                dropped.append(nxt)
        dropped_stems = tuple(dropped)
        self.assertEqual(len(leftover_extra_remaining_after_013(leftover, dropped_stems)), 36)
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
                leftover,
                dropped_stems,
        ...
        one_more_700 = tuple(
            "700"
            if nxt == "755" and leftover[i] == (SIDE_IA, "Ia1", 27)
            else nxt
            for i, nxt in enumerate(stems)
        )
        tie_g, tie_k, tie_unique = select_remaining_after_013_g(
            leftover_extra_remaining_after_013_next_stems(leftover, one_more_700)
        )
        self.assertEqual(tie_k, 3)
        self.assertFalse(tie_unique)
        self.assertIn(tie_g, ("001", "700"))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
                leftover,
                one_more_700,
        ...
        make_013 = tuple("013" if nxt == "001" else nxt for nxt in stems)
        self.assertEqual(len(leftover_extra_remaining_after_013(leftover, make_013)), 33)
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
                leftover,
                make_013,
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_013_EXACTLY_K_SHARE_G
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_013_EXACTLY_K_SHARE_G,
            HYPOTHESIS_K == STANDING_K,
        )
        self.assertEqual(STANDING_K + STANDING_N_WITHOUT_G, STANDING_N_REMAINING3)
        self.assertEqual(3 + 33, 36)
        self.assertEqual(
            STANDING_N_REMAINING2 - STANDING_N_SHARE_013,
            STANDING_N_REMAINING3,
        )
        self.assertEqual(41 - 5, 36)
        self.assertEqual(CYCLE228_N_WITHOUT_G, STANDING_N_REMAINING3)
        self.assertTrue(STANDING_I_ONLY_OF_090_076_G_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_PREVIOUS_4GRAMS_OF_090_076_013_ARE_NOT_THIS_CYCLE)
        self.assertEqual(provider.get_call_history(), [])

    def test_tie_break_picks_larger_barthel_id(self):
        """Equal remaining-after-013 counts pick the larger Barthel id."""
        provider = MockProvider()
        counts = Counter({"001": 3, "700": 3, "530": 2})
        ranked = rank_remaining_after_013_next_stems(counts)
        self.assertEqual(ranked[0], ("700", 3))
        self.assertEqual(ranked[1], ("001", 3))
        self.assertEqual(
            select_remaining_after_013_g(
                ("001", "700", "001", "700", "001", "700")
            )[0],
            "700",
        )
        self.assertFalse(
        ...
        self.assertEqual(select_remaining_after_013_g(("005", "700", "005", "700"))[0], "700")
        self.assertFalse(select_remaining_after_013_g(("005", "700", "005", "700"))[2])
        self.assertEqual(select_remaining_after_013_g(()), (None, 0, False))
...
class TestMamariILeftoverExtra090076RemainingAfter013NextStemScoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-013 next-stem lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.remaining3_stems = leftover_extra_remaining_after_013_next_stems(
            self.leftover_sites,
            self.next_stems,
        )
        self.matching = leftover_extra_remaining_after_013_with_g(
            self.leftover_sites,
            self.next_stems,
        )
        self.without = leftover_extra_remaining_after_013_without_g(
            self.leftover_sites,
            self.next_stems,
        )
        self.frequency = remaining_after_013_next_stem_frequency_table(
            self.leftover_sites,
            self.next_stems,
            self.forwards,
        )
        ...
        self.n_distinct_remaining3 = len(self.frequency)
        self.g, self.k, self.unique = select_remaining_after_013_g(self.remaining3_stems)
        self.n_without = len(self.without)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
            self.leftover_sites,
            self.next_stems,
        )

    def test_tokens_and_nested_leftover_extra_56_55_8_and_47_6_071_and_41_5_013_not_retuned(self):
        """2-gram and leftover extra 56/55/1 / 8 share 070 / 47/6/071 / 41/5/013 stay."""
        self.assertEqual(GRAM2, ("090", "076"))
        ...
        self.assertEqual(prior_228["N_with_next"], 55)
        self.assertEqual(prior_228["N_no_next"], 1)
        self.assertEqual(prior_228["N_share_070"], 8)
        self.assertEqual(prior_228["N_remaining"], 47)
        self.assertEqual(prior_228["N_share_071"], 6)
        self.assertEqual(prior_228["N_remaining2"], 41)
        self.assertEqual(prior_228["G"], "013")
        self.assertEqual(prior_228["K"], 5)
        self.assertEqual(prior_228["N_without_G"], 36)
        self.assertEqual(prior_228["next_after_G"], "001")
        self.assertEqual(prior_228["N_next_after_G"], 3)
        ...
        self.assertEqual(CYCLE228_N_WITHOUT_G, 36)
    ...
    def test_counts_36_remaining3_g_001_k_3_and_hypothesis_holds(self):
        """N_remaining3=36, N_distinct=27, G=001 K=3 unique. Claim holds."""
        ...
        self.assertEqual(STANDING_N_REMAINING2, CYCLE228_N_REMAINING2)
        ...
        self.assertEqual(STANDING_N_SHARE_013, CYCLE228_K)
        ...
        if (
            ...
            self.fail(
                ...
                "remaining-after-071 41/5/013 drifted"
            )
        self.assertTrue(
        ...
        self.assertEqual(41 - 5, 36)
        self.assertEqual(self.n_remaining3, CYCLE228_N_WITHOUT_G)
        if self.n_remaining3 != 36:
            self.fail("measured N_remaining3 drifted from 36")
        if self.n_remaining3 != self.n_remaining2 - self.n_share_013:
            self.fail("leftover extra remaining-after-013 filter disagrees with nested 41−5")
        self.assertEqual(self.remaining3, STANDING_REMAINING3_SITES)
        self.assertEqual(self.remaining3_stems, STANDING_REMAINING3_NEXT_STEMS)
        self.assertEqual(len(self.remaining3), len(self.remaining3_stems))
        self.assertEqual(
            self.remaining3,
            leftover_extra_remaining_after_071_without_g(
                self.leftover_sites,
        ...
        self.assertNotIn(STANDING_NO_NEXT_SITES[0], self.remaining3)
        self.assertIn(STANDING_IA2_174, self.remaining3)
        self.assertEqual(STANDING_IA2_174_NEXT_STEM, "000")
        for site in self.share_070:
            self.assertNotIn(site, self.remaining3)
        for site in self.share_071:
            self.assertNotIn(site, self.remaining3)
        for site in self.share_013:
            self.assertNotIn(site, self.remaining3)
        self.assertEqual(self.n_distinct_remaining3, STANDING_N_DISTINCT_REMAINING3)
        self.assertEqual(STANDING_N_DISTINCT_REMAINING3, 27)
        self.assertEqual(self.n_distinct_remaining3, CYCLE228_N_DISTINCT - 1)
        self.assertEqual(self.frequency, STANDING_REMAINING3_FREQUENCY)
        self.assertEqual(self.g, STANDING_G)
        self.assertEqual(STANDING_G, "001")
        ...
        self.assertTrue(STANDING_G_UNIQUELY_MOST_FREQUENT)
        self.assertEqual(self.frequency[0][0], "001")
        ...
        self.assertEqual(STANDING_N_WITHOUT_G, 33)
        self.assertEqual(self.k + self.n_without, self.n_remaining3)
        self.assertEqual(3 + 33, 36)
        hapax = sum(1 for _stem, count, _sites, _grams in self.frequency if count == 1)
        self.assertEqual(hapax, STANDING_N_HAPAX_REMAINING3)
        self.assertEqual(STANDING_N_HAPAX_REMAINING3, 19)
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_013_exactly_k_share_g(
                self.leftover_sites,
                self.next_stems,
    ...
    def test_matching_leftover_extra_remaining_after_013_sites_share_001(self):
        """Three leftover extra remaining-after-013 sites are 090 076 001."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_next_4grams, STANDING_MATCHING_NEXT_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia4", 134), ("090", "076", "001", "048")),
            ((SIDE_IA, "Ia6", 134), ("090", "076", "001", "224")),
            ((SIDE_IA, "Ia7", 88), ("090", "076", "001", "071")),
        )
        for (site, nxt), (want_site, want_nxt) in zip(
            ...
            self.assertEqual(stems[index + STANDING_N2], "001")
            self.assertEqual(site_next_stem(stems, index, GRAM2), "001")
            ...
            self.assertIn(site, STANDING_REMAINING3_SITES)
    ...
    def test_existing_230_229_228_227_226_223_195_and_171_still_compute(self):
        """Cycle 230 5/0 hapax, 229 5/0, 228 41/5/013, 227 6/071, 226 8/070, 223 69/3, 195 6/0, 171 43/0 stay."""
        ...
        self.assertTrue(CYCLE228_CLAIM)
        if (
            prior_228.n_leftover != 56
            or prior_228.n_with_next != 55
            or prior_228.n_no_next != 1
            or prior_228.n_remaining2 != 41
            or prior_228.k != 5
        ):
            self.fail("nested cycle 228 leftover extra remaining-after-071 41 G=013 K=5 drifted")
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-231 leftover extra remaining-after-013 lock."""
        lock = self.survey["i_leftover_extra_090_076_remaining_after_013_next_stem"]
        self.assertEqual(lock["cycle"], 231)
        ...
        self.assertEqual(tuple(lock["locked_forward_stems"]), ("070", "071", "013"))
        ...
        self.assertEqual(
            tuple(lock["remaining_after_013_next_stems"]),
            STANDING_REMAINING3_NEXT_STEMS,
        )
        self.assertEqual(lock["N_distinct_remaining3"], STANDING_N_DISTINCT_REMAINING3)
        self.assertEqual(lock["N_distinct_remaining3"], 27)
        self.assertEqual(lock["N_hapax_remaining3"], STANDING_N_HAPAX_REMAINING3)
        self.assertEqual(lock["N_hapax_remaining3"], 19)
        self.assertEqual(lock["G"], STANDING_G)
        self.assertEqual(lock["G"], "001")
        ...
        self.assertEqual(lock["N_without_G"], 33)
        ...
        self.assertEqual(lock["next_after_G"], "700")
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_013_sites"]),
            STANDING_MATCHING_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lock["matching_leftover_extra_remaining_after_013_local_4grams"],
            matching_leftover_extra_remaining_after_013_local_4gram_rows(),
        )
        self.assertEqual(
            lock["remaining_after_013_next_stem_frequency"],
            remaining_after_013_next_stem_frequency_rows(),
        )
        self.assertTrue(lock["known_distinct"])
        self.assertEqual(lock["claim"], STANDING_CLAIM)
        self.assertTrue(lock["i_leftover_extra_090_076_remaining_after_013_exactly_K_share_G"])
        self.assertEqual(
            lock["i_leftover_extra_090_076_remaining_after_013_exactly_K_share_G"],
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_013_EXACTLY_K_SHARE_G,
        )
        ...
        self.assertTrue(lock["previous_4grams_of_090_076_013_are_not_this_cycle"])
        ...
        self.assertEqual(
            self.survey["i_leftover_extra_090_076_remaining_after_071_next_stem"]["N_without_G"],
            36,
        )
...
class TestMamariILeftoverExtra090076RemainingAfter013NextStemImageSnapshot(
    unittest.TestCase
):
    """Cycle 231 does not touch clustering. 83/62 / Hamming 6 stays."""
