"""I's cycle-205 leftover 3-gram 090 076 070 off-I lock.

Cycle 207 text-search lock. Uses already-vendored A–V and the
cycle-205 leftover 3-gram 090 076 070 (the n=3 run inside leftover
n=4 maximal 999 090 076 070). Does not retune that 3-gram or
...
and cycle 171 (076 071 was I-only 43/0). Cycle 206 lost:
2-gram 076 070 is not I-only (19/5). Cycle 205 holds: exactly
1 leftover n=4 maximal contains consecutive 076 070
(999 090 076 070, freq 5). This cycle is the new leftover
3-gram 090 076 070 only. 076 070 without 090 does not count
as this 3-gram. 090 076 071 is a different 3-gram (already
I-only 6/0). Do not retune 076 070, 999 090 076, 076 071, or
090 076 071. Do not assume the I-only result.

Locks exact consecutive hits of 090 076 070 on tablet I and
on every other vendored tablet A–H and J–V. Claim that can lose:
i_3gram_090_076_070_i_only (I hits ≥ 1 and off-I hits == 0).
True only if N_off_I == 0. Measured: Ia is exactly 8; Ib
unpublished 0; off-I is 1 on T (Ta9[2]). The claim is false.
Not an n≥8 island. Not the cycle-103 I 5-gram. Five of the 8
I sites sit inside leftover n=4 999 090 076 070; the other 3
are extra.
...
STANDING_EXTRA_PREVIOUS_4GRAMS = (
    ("036", "090", "076", "070"),
    ("161", "090", "076", "070"),
    ("400", "090", "076", "070"),
)
...
STANDING_OFF_I_PREVIOUS_4GRAM = ("059", "090", "076", "070")
...
STANDING_CLAIM = "i_3gram_090_076_070_i_only"
STANDING_I_3GRAM_090_076_070_I_ONLY = False
STANDING_RESULT = "i_3gram_090_076_070_i_only"
...
def leftover_contained_3gram_sites(
    leftover_n4_sites: tuple[tuple[str, str, int], ...] = STANDING_LEFTOVER_N4_SITES,
    offset: int = 1,
) -> tuple[tuple[str, str, int], ...]:
    """090 076 070 starts one token after leftover n=4 999 090 076 070."""
...
def extra_i_sites(
    ...
    leftover_3gram: tuple[tuple[str, str, int], ...] = STANDING_LEFTOVER_3GRAM_SITES,
) -> tuple[tuple[str, str, int], ...]:
    """I 090 076 070 sites that do not sit inside leftover n=4 999 090 076 070."""
    leftover_set = set(leftover_3gram)
...
class TestI3gram090076070IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-205 leftover 3-gram tokens. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 3-gram counts; a gap is not a hit. 090 076 071 / 076 070 are not."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "070"))
        adjacent = [list(GRAM3), list(GRAM3)]
        self.assertEqual(ngram_hit_count(adjacent, GRAM3), 2)
        overlap = [["090", "076", "070", "090", "076", "070"]]
        ...
        self.assertEqual(ngram_hit_count([list(NEAR_MISS_076_070)], GRAM3), 0)
        ...
        self.assertTrue(STANDING_076_070_WITHOUT_090_DOES_NOT_COUNT)
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        """Boolean is True only when I ≥ 1 and off-I is 0. Measured 8/1 loses."""
        provider = MockProvider()
        self.assertTrue(i_3gram_090_076_070_i_only(1, 0))
        self.assertTrue(i_3gram_090_076_070_i_only(8, 0))
        self.assertFalse(i_3gram_090_076_070_i_only(8, 1))
        self.assertFalse(i_3gram_090_076_070_i_only(1, 1))
        self.assertFalse(i_3gram_090_076_070_i_only(0, 0))
        self.assertFalse(i_3gram_090_076_070_i_only(0, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_090_076_070_i_only")
        self.assertFalse(STANDING_I_3GRAM_090_076_070_I_ONLY)
        self.assertTrue(HYPOTHESIS_I_ONLY)
        self.assertNotEqual(
            STANDING_I_3GRAM_090_076_070_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_3gram_is_cycle205_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-205 leftover run, not 076 070, 090 076 071, or 999 090 076."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "070"))
        self.assertEqual(GRAM3, STANDING_LEFTOVER_N4[1:])
        self.assertNotEqual(GRAM3, CYCLE206_GRAM2)
        self.assertNotEqual(GRAM3, CYCLE205_GRAM2)
        ...
        self.assertTrue(is_contiguous_substring(CYCLE206_GRAM2, STANDING_LEFTOVER_N4))
        self.assertTrue(is_contiguous_substring(GRAM3, STANDING_LEFTOVER_N4))
        self.assertEqual(provider.get_call_history(), [])

    def test_leftover_vs_extra_split_can_fail(self):
        """Five I sites sit inside leftover n=4; the other 3 are extra."""
        provider = MockProvider()
        leftover_3gram = leftover_contained_3gram_sites()
        extra = extra_i_sites()
        self.assertEqual(leftover_3gram, STANDING_LEFTOVER_3GRAM_SITES)
        self.assertEqual(extra, STANDING_EXTRA_I_SITES)
        self.assertEqual(len(leftover_3gram), STANDING_N_INSIDE_LEFTOVER)
        ...
        self.assertEqual(
            tuple(sorted(leftover_3gram + extra)),
            tuple(sorted(STANDING_I_SITES)),
        )
        planted = leftover_3gram + (STANDING_EXTRA_I_SITES[0],)
...
class TestMamariI3gram090076070IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover 3-gram 090 076 070 off-I lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.leftover_3gram = leftover_contained_3gram_sites()
        self.extra = extra_i_sites(self.i_sites, self.leftover_3gram)
        self.claim_holds = i_3gram_090_076_070_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_205_leftover_not_retuned(self):
        """3-gram is the cycle-205 leftover lock, not a new inventory."""
        self.assertEqual(GRAM3, ("090", "076", "070"))
        self.assertEqual(GRAM3, STANDING_LEFTOVER_N4[1:])
    ...
    def test_i_hits_are_eight_on_ia(self):
        """3-gram is 8 on Ia; Ib unpublished 0. N_I must not drift."""
        self.assertEqual(self.i_sites, STANDING_I_SITES)
        self.assertEqual(len(STANDING_I_SITES), 8)
        ...
        self.assertEqual(
            STANDING_I_SITES[:3],
            ((SIDE_IA, "Ia2", 10), (SIDE_IA, "Ia3", 4), (SIDE_IA, "Ia4", 112)),
        )
        self.assertEqual(
            STANDING_I_SITES[7],
            (SIDE_IA, "Ia14", 140),
    ...
    def test_leftover_vs_extra_on_i(self):
        """Five I sites sit inside leftover 999 090 076 070; 3 are extra."""
        self.assertEqual(self.leftover_3gram, STANDING_LEFTOVER_3GRAM_SITES)
        self.assertEqual(self.extra, STANDING_EXTRA_I_SITES)
        self.assertEqual(len(self.leftover_3gram), STANDING_N_INSIDE_LEFTOVER)
        ...
        self.assertEqual(
            leftover_contained_3gram_sites(STANDING_LEFTOVER_N4_SITES),
            STANDING_LEFTOVER_3GRAM_SITES,
        )
        ...
        for side, line, index in STANDING_LEFTOVER_N4_SITES:
            ...
            self.assertIn((side, line, index + 1), leftover_set)
    ...
    def test_3gram_is_one_off_i_and_not_i_only(self):
        """3-gram is 1 off-I (T Ta9[2]). Ia has exactly 8. Claim loses."""
    ...
    def test_existing_206_205_171_and_167_scoreboards_still_compute(self):
        """Cycle 206 19/5 loss, 205 leftover-1, 171 I-only 43/0, 167 I-only 16/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-207 3-gram I-only loss."""
        lock = self.survey["i_3gram_090_076_070_i_only"]
        self.assertEqual(lock["cycle"], 207)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["extra_previous_4grams"]),
            STANDING_EXTRA_PREVIOUS_4GRAMS,
        )
        ...
        self.assertFalse(lock["i_3gram_090_076_070_i_only"])
        self.assertEqual(
            lock["i_3gram_090_076_070_i_only"],
            STANDING_I_3GRAM_090_076_070_I_ONLY,
        )
        ...
        self.assertTrue(lock["076_070_without_090_does_not_count"])
...
class TestMamariI3gram090076070IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 207 does not touch clustering. 83/62 / Hamming 6 stays."""
