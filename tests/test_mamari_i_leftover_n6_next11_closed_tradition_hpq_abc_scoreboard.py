"""Reframed closed-tradition hold-out of the locked leftover-6 next-11 grams.

Cycle 844 text-search lock. Uses already-vendored A–V and the
cycle-485 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 11-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 485 already locked the I-only claim
on these 11-grams as incomplete-set LOSE (N_with_next11=15,
N_no_next11=3, N_i_only=15, N_leak=0). Cycle 843 already locked
the reframed claim on the cycle-483 next-10 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–843.

Claim that can lose: each existing next-11 gram is an exact
...
Measured on the 15 with-next11 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next11=15,
N_no_next11=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-10 hapax 604 076 071 600 999 050 076 000
002 999 extends to 604 076 071 600 999 050 076 000 002 999 076
at Ia9[9] and hits I once. All fifteen next-10 grams extend;
no new hole. Verdict: incomplete-set LOSE.
...
next 12-grams as a peel, prev-184, or next-185. The cycle-487
next-12 I-only list stays the named next lock and is not re-scored.
...
from tests.test_mamari_i_leftover_n6_next10_closed_tradition_hpq_abc_scoreboard import (
    STANDING_HITS_I as CYCLE843_HITS_I,
    STANDING_N_HOLD as CYCLE843_N_HOLD,
    STANDING_N_LEAK as CYCLE843_N_LEAK,
    STANDING_NEXT_CHEAP_LOCK as CYCLE843_NEXT_CHEAP_LOCK,
    STANDING_NO_NEXT10_SITES as CYCLE843_NO_NEXT10_SITES,
    STANDING_RESULT as CYCLE843_RESULT,
    STANDING_SEQUENCES as CYCLE843_SEQUENCES,
    STANDING_SITES as CYCLE843_SITES,
    STANDING_VERDICT as CYCLE843_VERDICT,
)
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next11_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT11_SITES as CYCLE485_SITES,
    STANDING_N as CYCLE485_N,
    STANDING_N_I_EACH as CYCLE485_N_I_EACH,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next12_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE487_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT12_I_ONLY as CYCLE487_ALL_I_ONLY_CLAIM,
    ...
    STANDING_N_SEQUENCES as CYCLE487_N_SEQUENCES,
    STANDING_N_WITH_NEXT12 as CYCLE487_N_WITH_NEXT12,
    STANDING_NO_NEXT12_SITES as CYCLE487_NO_NEXT12_SITES,
    STANDING_RESULT as CYCLE487_RESULT,
...
STANDING_NO_NEXT11_SITES = CYCLE485_NO_NEXT11_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE485_SITES
STANDING_SEQUENCES = CYCLE485_SEQUENCES
...
STANDING_NEXT10_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT10_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next11_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next11_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 485
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 485
...
STANDING_DO_NOT_RELOCK_CYCLES_832_843 = True
...
class TestMamariILeftoverN6Next11ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 844 reframed closed-tradition lock on the cycle-485 next-11 population."""
    ...
    def test_population_is_the_locked_cycle_485_next11_grams(self):
        """Fifteen next-11 sequences and sites come from cycle 485. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE485_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE485_SITES)
        self.assertEqual(CYCLE485_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE485_N, 18)
        self.assertEqual(CYCLE485_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT11, CYCLE485_N_WITH_NEXT11)
        self.assertEqual(STANDING_N_NO_NEXT11, CYCLE485_N_NO_NEXT11)
        self.assertEqual(
            STANDING_NO_NEXT11_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT11_SITES, CYCLE843_NO_NEXT10_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 485)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 11)
        prior = self.survey[CYCLE485_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_11grams_all_i_only"
        )
        self.assertEqual(CYCLE485_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next11_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE843, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 485 I-only or the cycle 832–843 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 485)
        ...
        self.assertEqual(next10["N_exact0_abc"], 15)
        self.assertEqual(next10["N_with_next10"], 15)
        self.assertEqual(next10["N_no_next10"], 3)
        self.assertEqual(next10["next_cheap_lock"], STANDING_NAMED_BY_CYCLE843)
        self.assertEqual(next10["next_cheap_lock"], CYCLE843_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE843, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_843)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE485_N_I_EACH)
        self.assertEqual(CYCLE485_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT10_ALL_EXTEND)
        for n10, n11, site10, site11 in zip(
            CYCLE843_SEQUENCES, STANDING_SEQUENCES, CYCLE843_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n11[:10], n10)
            self.assertEqual(site11, site10)
        still = self.rows[STANDING_NEXT10_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            ("604", "076", "071", "600", "999", "050", "076", "000", "002", "999", "076"),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:10], CYCLE843_SEQUENCES[3])
        self.assertEqual(CYCLE843_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT11_SITES)
    ...
    def test_window_peels_closed_and_prev1_list_is_absent(self):
        ...
        self.assertTrue(STANDING_DO_NOT_LAUNCH_NEXT12)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_487_next12_list(self):
        """Cycle 487 next-12 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE487_N, 18)
        self.assertEqual(CYCLE487_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE487_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 12 for tokens in CYCLE487_SEQUENCES))
        self.assertTrue(CYCLE487_HYPOTHESIS_ALL_I_ONLY)
        self.assertFalse(CYCLE487_ALL_I_ONLY_CLAIM)
        ...
        self.assertEqual(
            CYCLE487_NO_NEXT12_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        prior = self.survey[CYCLE487_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_12grams_all_i_only"
        )
        self.assertEqual(prior["cycle"], 487)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 15)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE487_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
        self.assertEqual(prior["N_no_next12"], 3)
        self.assertEqual(prior["N_with_next12"], 15)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-844 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 844)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next11_sites"]),
            STANDING_NO_NEXT11_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_843"])
        ...
        self.assertEqual(lock["named_by_cycle843"], STANDING_NAMED_BY_CYCLE843)
        ...
        self.assertEqual(
            tuple(lock["next10_hapax_still_hapax"]),
            STANDING_NEXT10_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next10_all_extend"])
        self.assertEqual(lock["next10_all_extend"], STANDING_NEXT10_ALL_EXTEND)
