"""I's cycle-172 leftover 2-gram forward 700 cluster lock.

Cycle 182 text-search lock. Uses already-vendored A–V and the
...
is 700 (forward 3-gram 076 071 700; the local next 4-gram
starts with 076 071 700). End-of-line is no-forward. The 9
inside-family sites do not count as leftover (none of them
is 076 071 700). Cycle 173's five leftover 076 071 076
sites, cycle 176's four leftover 076 071 600 sites, and
cycle 179's three leftover 076 071 090 sites are different
clusters and do not count toward this 700 cluster. 071 999
and 076 076 do not count as this 2-gram. Same inventory
move as cycles 173, 176, and 179: lock the next-largest
leftover continuation cluster (after cycle 173 N=5 forward
076, cycle 176 N=4 forward 600, and cycle 179 N=3 forward
090, this is N=3 forward 700). Cycle 181's four 076 071 090
forward 4-grams stay I-only (1/0, 1/0, 1/0, 2/0).

Hypothesis N=3: exactly 3 of the 34 leftover sites share
forward 3-gram 076 071 700. Measured: N_leftover=34,
N_with_forward_076_071_700=3 at Ia1[163] 076 071 700 076,
Ia13[54] 076 071 700 430, Ia13[92] 076 071 700 128;
N_without=31; N_no_forward=0. Claim that can lose:
i_leftover_076_071_exactly_3_forward_076_071_700. True only
if N_with_forward_076_071_700=3. The claim is true. Do not
retune.
...
STANDING_CLAIM = "i_leftover_076_071_exactly_3_forward_076_071_700"
STANDING_I_LEFTOVER_076_071_EXACTLY_3_FORWARD_076_071_700 = True
STANDING_RESULT = "i_leftover_076_071_forward_700"
...
def leftover_without_forward_076_071_700(
    sites: tuple[tuple[str, str, int], ...],
    forwards: tuple[tuple[str, ...] | None, ...],
    needle: tuple[str, ...] = GRAM3_FORWARD,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover sites that have a next stem other than 700, or none."""
    return tuple(
        site
        for site, fwd in zip(sites, forwards, strict=True)
        if fwd != needle
...
def i_leftover_076_071_exactly_3_forward_076_071_700(
    n_with_forward_076_071_700: int,
    expected: int = HYPOTHESIS_N,
) -> bool:
    """True iff N_with_forward_076_071_700 equals the hypothesized 3."""
    return n_with_forward_076_071_700 == expected


class TestILeftover076071Forward700Helpers(unittest.TestCase):
    ...
    def test_forward_requires_next_stem_after_2gram(self):
        ...
        self.assertEqual(GRAM3_FORWARD, ("076", "071", "700"))
    ...
    def test_exactly_3_can_fail(self):
        """Boolean is True only when N_with_forward_076_071_700=3."""
        provider = MockProvider()
        self.assertTrue(i_leftover_076_071_exactly_3_forward_076_071_700(3))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(0))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(2))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(4))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(5))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(34))
        self.assertFalse(i_leftover_076_071_exactly_3_forward_076_071_700(1))
        planted = STANDING_MATCHING_SITES + (CYCLE179_MATCHING_SITES[0],)
        planted_forwards = (GRAM3_FORWARD,) * 4
        self.assertEqual(
            leftover_with_forward_076_071_700(planted, planted_forwards),
            planted,
        )
        self.assertFalse(
            i_leftover_076_071_exactly_3_forward_076_071_700(len(planted))
        )
        self.assertEqual(STANDING_CLAIM, "i_leftover_076_071_exactly_3_forward_076_071_700")
        self.assertTrue(STANDING_I_LEFTOVER_076_071_EXACTLY_3_FORWARD_076_071_700)
        self.assertEqual(
            STANDING_I_LEFTOVER_076_071_EXACTLY_3_FORWARD_076_071_700,
            HYPOTHESIS_N == STANDING_N_WITH_FORWARD_076_071_700,
        )
        self.assertEqual(STANDING_N_WITH_FORWARD_076_071_700 + STANDING_N_WITHOUT, 34)
        self.assertEqual(provider.get_call_history(), [])

    def test_inside_family_prior_clusters_and_near_misses_do_not_count(self):
        """Inside family, cycle-173 076, cycle-176 600, cycle-179 090, 071 999, and 076 076 are not this 700 cluster."""
        provider = MockProvider()
        self.assertEqual(STANDING_INSIDE_FORWARD_700_SITES, ())
...
class TestMamariILeftover076071Forward700Scoreboard(unittest.TestCase):
    """Cited-fixture leftover 076 071 forward-700 cluster. Mock only."""
    ...
    def setUp(self):
        ...
        self.with_sites = leftover_with_forward_076_071_700(
            self.leftover_sites,
            self.forwards,
        )
        self.without_sites = leftover_without_forward_076_071_700(
            self.leftover_sites,
            self.forwards,
        )
        ...
        self.claim_holds = i_leftover_076_071_exactly_3_forward_076_071_700(
            self.n_with,
        )
    ...
    def test_counts_3_of_34_and_hypothesis_n_3_holds(self):
        ...
        self.assertEqual(self.n_with, STANDING_N_WITH_FORWARD_076_071_700)
        self.assertEqual(STANDING_N_WITH_FORWARD_076_071_700, 3)
        ...
        self.assertTrue(i_leftover_076_071_exactly_3_forward_076_071_700(self.n_with))
        self.assertEqual(
            self.claim_holds,
            STANDING_I_LEFTOVER_076_071_EXACTLY_3_FORWARD_076_071_700,
        )
    ...
    def test_matching_leftover_sites_and_next_4grams(self):
        """Three leftover sites share 076 071 700; next 4-grams stay distinct."""
        ...
        expected = (
            ((SIDE_IA, "Ia1", 163), ("076", "071", "700", "076")),
            ((SIDE_IA, "Ia13", 54), ("076", "071", "700", "430")),
            ((SIDE_IA, "Ia13", 92), ("076", "071", "700", "128")),
        )
        ...
        self.assertEqual(len(set(STANDING_MATCHING_NEXT_4GRAMS)), 3)
        ...
        for site, nxt in zip(CYCLE176_MATCHING_SITES, CYCLE176_MATCHING_NEXT_4GRAMS):
            self.assertIn(site, self.without_sites)
            self.assertNotIn(site, self.with_sites)
            stems = line_stems_for_site(self.i_sides, site)
            index = site[2]
            self.assertEqual(site_forward_3gram(stems, index, GRAM2), CYCLE176_GRAM3_FORWARD)
            self.assertEqual(site_next_4gram(stems, index, GRAM2), nxt)
        for site, nxt in zip(CYCLE179_MATCHING_SITES, CYCLE179_MATCHING_NEXT_4GRAMS):
            ...
            stems = line_stems_for_site(self.i_sides, site)
            index = site[2]
            self.assertEqual(site_forward_3gram(stems, index, GRAM2), CYCLE179_GRAM3_FORWARD)
            self.assertEqual(site_next_4gram(stems, index, GRAM2), nxt)
        for site in STANDING_INSIDE_SITES:
            self.assertNotIn(site, self.with_sites)
    ...
    def test_existing_181_179_176_173_172_171_170_103_and_w_scoreboards_still_compute(self):
        """Cycle 181 I-only 4-grams, 179 N=3, 176 N=4, 173 N=5, 172 leftover 34, 171, 170, 103, W stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-182 leftover forward-700 lock."""
        lock = self.survey["i_leftover_076_071_forward_700"]
        self.assertEqual(lock["cycle"], 182)
        ...
        self.assertEqual(lock["N_with_forward_076_071_700"], STANDING_N_WITH_FORWARD_076_071_700)
        self.assertEqual(lock["N_with_forward_076_071_700"], 3)
        ...
        self.assertEqual(lock["inside_forward_700_sites"], [])
        self.assertEqual(
        ...
        self.assertTrue(lock["i_leftover_076_071_exactly_3_forward_076_071_700"])
        self.assertEqual(
            lock["i_leftover_076_071_exactly_3_forward_076_071_700"],
            STANDING_I_LEFTOVER_076_071_EXACTLY_3_FORWARD_076_071_700,
        )
...
class TestMamariILeftover076071Forward700ImageSnapshot(unittest.TestCase):
    """Cycle 182 does not touch clustering. 83/62 / Hamming 6 stays."""
