"""Reframed closed-tradition hold-out of the locked leftover-6 next-14 grams.

Cycle 847 text-search lock. Uses already-vendored A–V and the
cycle-491 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 14-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 491 already locked the I-only claim
on these 14-grams as incomplete-set LOSE (N_with_next14=15,
N_no_next14=3, N_i_only=15, N_leak=0). Cycle 846 already locked
the reframed claim on the cycle-489 next-13 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–846.

Claim that can lose: each existing next-14 gram is an exact
...
Named claim: cycle491_next14_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next14 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next14=15,
N_no_next14=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-13 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 extends to 604 076 071 600 999 050 076 000
002 999 076 092 535 999 at Ia9[9] and hits I once. All fifteen
next-13 grams extend; no new hole. Verdict: incomplete-set LOSE.
...
next 14-grams as a peel, next 15-grams as a peel, prev-184, or
next-185. The cycle-493 next-15 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_next13_closed_tradition_hpq_abc_scoreboard import (
    ABSENT_TABLETS,
    CYCLE457_RESULT,
    ...
    PREV1_N6_SCOREBOARD,
    PROBE_TABLETS,
    STANDING_HITS_I as CYCLE846_HITS_I,
    STANDING_N_HOLD as CYCLE846_N_HOLD,
    STANDING_N_LEAK as CYCLE846_N_LEAK,
    STANDING_NEXT_CHEAP_LOCK as CYCLE846_NEXT_CHEAP_LOCK,
    STANDING_NO_NEXT13_SITES as CYCLE846_NO_NEXT13_SITES,
    STANDING_RESULT as CYCLE846_RESULT,
    STANDING_SEQUENCES as CYCLE846_SEQUENCES,
    STANDING_SITES as CYCLE846_SITES,
    STANDING_VERDICT as CYCLE846_VERDICT,
    absent_hit_sum,
    hpq_tablets_hit,
    reframed_verdict,
    row_holds,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next14_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT14_SITES as CYCLE491_SITES,
    STANDING_N as CYCLE491_N,
    STANDING_N_I_EACH as CYCLE491_N_I_EACH,
...
STANDING_NO_NEXT14_SITES = CYCLE491_NO_NEXT14_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE491_SITES
STANDING_SEQUENCES = CYCLE491_SEQUENCES
...
STANDING_NEXT13_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT13_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next14_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next14_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 491
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 491
...
STANDING_DO_NOT_RELOCK_CYCLES_832_846 = True
...
class TestMamariILeftoverN6Next14ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 847 reframed closed-tradition lock on the cycle-491 next-14 population."""
    ...
    def test_population_is_the_locked_cycle_491_next14_grams(self):
        """Fifteen next-14 sequences and sites come from cycle 491. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE491_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE491_SITES)
        self.assertEqual(CYCLE491_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE491_N, 18)
        self.assertEqual(CYCLE491_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT14, CYCLE491_N_WITH_NEXT14)
        self.assertEqual(STANDING_N_NO_NEXT14, CYCLE491_N_NO_NEXT14)
        self.assertEqual(
            STANDING_NO_NEXT14_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT14_SITES, CYCLE846_NO_NEXT13_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 491)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 14)
        ...
        self.assertEqual(CYCLE491_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next14_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE846, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 491 I-only or the cycle 832–846 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 491)
        ...
        next13 = self.survey[CYCLE846_RESULT]
        self.assertEqual(next13["cycle"], STANDING_NEXT13_REFRAMED_CYCLE)
        self.assertEqual(next13["verdict"], CYCLE846_VERDICT)
        self.assertEqual(next13["verdict"], "incomplete-set LOSE")
        self.assertEqual(next13["N_hold"], CYCLE846_N_HOLD)
        self.assertEqual(next13["N_hold"], 0)
        self.assertEqual(next13["N_leak"], CYCLE846_N_LEAK)
        self.assertEqual(next13["N_leak"], 0)
        self.assertEqual(next13["hits_I"], CYCLE846_HITS_I)
        self.assertEqual(next13["hits_I"], 15)
        self.assertEqual(next13["N_hpq_ge1"], 0)
        self.assertEqual(next13["N_exact0_abc"], 15)
        self.assertEqual(next13["N_with_next13"], 15)
        self.assertEqual(next13["N_no_next13"], 3)
        self.assertEqual(next13["next_cheap_lock"], STANDING_NAMED_BY_CYCLE846)
        self.assertEqual(next13["next_cheap_lock"], CYCLE846_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE846, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_846)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE491_N_I_EACH)
        self.assertEqual(CYCLE491_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT13_ALL_EXTEND)
        for n13, n14, site13, site14 in zip(
            CYCLE846_SEQUENCES, STANDING_SEQUENCES, CYCLE846_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n14[:13], n13)
            self.assertEqual(site14, site13)
        still = self.rows[STANDING_NEXT13_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
                "604", "076", "071", "600", "999", "050", "076", "000",
                "002", "999", "076", "092", "535", "999",
            ),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:13], CYCLE846_SEQUENCES[3])
        self.assertEqual(CYCLE846_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT14_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_493_next15_list(self):
        """Cycle 493 next-15 grams are already locked and not yet reframed."""
        self.assertEqual(CYCLE493_N, 18)
        self.assertEqual(CYCLE493_N_SEQUENCES, 15)
        self.assertEqual(len(CYCLE493_SEQUENCES), 15)
        self.assertTrue(all(len(tokens) == 15 for tokens in CYCLE493_SEQUENCES))
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-847 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 847)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next14_sites"]),
            STANDING_NO_NEXT14_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_846"])
        ...
        self.assertEqual(lock["named_by_cycle846"], STANDING_NAMED_BY_CYCLE846)
        ...
        self.assertEqual(
            tuple(lock["next13_hapax_still_hapax"]),
            STANDING_NEXT13_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next13_all_extend"])
        self.assertEqual(lock["next13_all_extend"], STANDING_NEXT13_ALL_EXTEND)
