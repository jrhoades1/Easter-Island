"""Reframed closed-tradition hold-out of the locked leftover-6 next-25 grams.

Cycle 858 text-search lock. Uses already-vendored A–V and the
cycle-513 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 25-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 513 already locked the I-only claim
on these 25-grams as incomplete-set LOSE (N_with_next25=15,
N_no_next25=3, N_i_only=15, N_leak=0). Cycle 857 already locked
the reframed claim on the cycle-511 next-24 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–857.

Claim that can lose: each existing next-25 gram is an exact
...
Named claim: cycle513_next25_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next25 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next25=15,
N_no_next25=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-24 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700 extends to
604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076
532 244 999 090 076 057 600 700 076 at Ia9[9] and hits I once. All fifteen
next-24 grams extend; no new hole. Verdict: incomplete-set LOSE.
...
next 25-grams as a peel, next 26-grams as a peel, prev-184, or
next-185. The cycle-515 next-26 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next25_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT25_SITES as CYCLE513_SITES,
    STANDING_N as CYCLE513_N,
    STANDING_N_I_EACH as CYCLE513_N_I_EACH,
...
STANDING_NO_NEXT25_SITES = CYCLE513_NO_NEXT25_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE513_SITES
STANDING_SEQUENCES = CYCLE513_SEQUENCES
...
STANDING_NEXT24_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT24_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next25_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next25_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 513
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 513
...
STANDING_DO_NOT_RELOCK_CYCLES_832_857 = True
...
class TestMamariILeftoverN6Next25ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 858 reframed closed-tradition lock on the cycle-513 next-25 population."""
    ...
    def test_population_is_the_locked_cycle_513_next25_grams(self):
        """Fifteen next-25 sequences and sites come from cycle 513. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE513_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE513_SITES)
        self.assertEqual(CYCLE513_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE513_N, 18)
        self.assertEqual(CYCLE513_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT25, CYCLE513_N_WITH_NEXT25)
        self.assertEqual(STANDING_N_NO_NEXT25, CYCLE513_N_NO_NEXT25)
        self.assertEqual(
            STANDING_NO_NEXT25_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT25_SITES, CYCLE857_NO_NEXT24_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 513)
        ...
        self.assertEqual(CYCLE513_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next25_i_only"
        ))
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 513 I-only or the cycle 832–857 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 513)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_857)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE513_N_I_EACH)
        self.assertEqual(CYCLE513_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT24_ALL_EXTEND)
        for n24, n25, site24, site25 in zip(
            CYCLE857_SEQUENCES, STANDING_SEQUENCES, CYCLE857_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n25[:24], n24)
            self.assertEqual(site25, site24)
        still = self.rows[STANDING_NEXT24_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:24], CYCLE857_SEQUENCES[3])
        self.assertEqual(CYCLE857_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT25_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_515_next26_list(self):
        ...
        self.assertTrue(all(len(tokens) == 26 for tokens in CYCLE515_SEQUENCES))
        ...
        self.assertEqual(CYCLE515_N_WITH_NEXT26, 15)
        self.assertEqual(CYCLE515_N_NO_NEXT26, 3)
        self.assertEqual(
            CYCLE515_NO_NEXT26_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-858 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 858)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next25_sites"]),
            STANDING_NO_NEXT25_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_857"])
        ...
        self.assertEqual(lock["named_by_cycle857"], STANDING_NAMED_BY_CYCLE857)
        ...
        self.assertEqual(
            tuple(lock["next24_hapax_still_hapax"]),
            STANDING_NEXT24_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next24_all_extend"])
        self.assertEqual(lock["next24_all_extend"], STANDING_NEXT24_ALL_EXTEND)
