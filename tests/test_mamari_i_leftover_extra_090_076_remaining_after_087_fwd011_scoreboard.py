"""I's cycle-247 leftover extra remaining-after-087 forward-011 cluster lock.

Cycle 247 text-search lock. Uses already-vendored A–V and the
...
forward 4-grams hapax 1/0). This cycle locks the next tied
stem as an exact-K claim on remaining-after-087.
Do not lock I-only of 090 076 011. Do not lock unique-max
remaining-after-087. Do not lock the other two tied stems
(005/000). Do not lock leftover n=4 021 090 076 087.
Previous 4-grams of 090 076 087 are not this cycle. Off-I T
sites are not this cycle. I-only of leftover extra 4-grams is
leftover-of-leftover for a later cycle. 076 071 and 076 070
do not count as this 2-gram. Inside-family sites do not count
as leftover extra. Do not count leftover extra 090 076 700 011
as remaining-after-087 090 076 011.

Leftover extra remaining-after-087 = leftover extra I 090 076
sites with a next token whose next token is none of 070, 071,
013, 001, 700, 530, 280, or 087. Nested-check leftover extra
...
share 280, leftover extra remaining-after-280 N_remaining7==27
exactly 2 share 087 (do not retune cycles 225–244). Nested-check
cycle 245 090 076 087 I-only 5/0 extra I=3 and cycle 246 five
forward 4-grams 1/0 each (do not retune). Nested-check
N_remaining8==25 (27−2). Ia2[174] has next token 000 and is
remaining-after-087; it is not no-next. Ia2[159] 090 076 700 011
is remaining-after-001 700, not remaining-after-087 011.

Hypothesis K=2: leftover extra remaining-after-087 includes
exactly 2 sites that share next stem 011 (forward 3-gram
090 076 011). Measured: K=2 at Ia2[37], Ia12[47]; next
4-grams 090 076 011 678 / 090 076 011 130. Cycle 234 listed
011×2 among the 7-way tie; re-measured, not assumed. Claim
that can lose:
...
244 (remaining-after-280 exactly 2 share 087). This can lose
if remaining-after-087 does not contain exactly 2 011
next-stem sites, or if N_remaining8 ≠ 25. Nested cycle 246
5/0 hapax, cycle 245 5/0 extra I=3, cycle 244 K=2 / G=087,
cycle 243 2/0 hapax, cycle 242 2/0, cycle 241 K=2 / G=280,
cycle 240 2/0 hapax, cycle 239 2/0, cycle 238 K=2 / G=530,
cycle 237 2/0 hapax, cycle 236 2/0, cycle 234 7-way tie at 2,
cycle 233 3/0 hapax, cycle 232 3/0, cycle 223 69/3, cycle 195
6/0, and cycle 171 43/0 stay. Do not assume the result; measure.
Do not retune.
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_280_fwd087_scoreboard import (
    ...
    leftover_extra_remaining_after_280_next_stems,
...
STANDING_OTHER_TIED_STEMS = ("005", "000")
...
STANDING_G = "011"
STANDING_K = 2
STANDING_N_WITHOUT = 23
STANDING_G_UNIQUELY_MOST_FREQUENT = False
STANDING_UNIQUE_MAX_REMAINING_AFTER_087_IS_NOT_THIS_CYCLE = True
STANDING_REMAINING7_SITES = CYCLE244_REMAINING7_SITES
...
STANDING_CYCLE234_011_SITES = CYCLE234_FREQUENCY[4][2]
STANDING_CYCLE234_087_SITES = CYCLE234_FREQUENCY[3][2]
STANDING_CYCLE234_280_SITES = CYCLE234_FREQUENCY[2][2]
STANDING_MATCHING_EQUALS_CYCLE234_011_SITES = True
STANDING_KNOWN_DISTINCT = True
STANDING_CLAIM = "i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011"
STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_087_EXACTLY_2_SHARE_011 = True
STANDING_RESULT = "i_leftover_extra_090_076_remaining_after_087_fwd011"
...
STANDING_PREVIOUS_4GRAMS_OF_090_076_087_ARE_NOT_THIS_CYCLE = True
...
STANDING_LEFTOVER_N4_021_090_076_087_NOT_RETUNED = True
STANDING_090_076_700_011_IS_NOT_REMAINING_AFTER_087_011 = True
STANDING_CYCLE224_NO_NEXT_4GRAM_IS_NOT_NO_NEXT_TOKEN = True


def leftover_extra_remaining_after_087(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_087,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra with-next sites whose next token is none of 070/071/013/001/700/530/280/087."""
...
def leftover_extra_remaining_after_087_next_stems(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_087,
) -> tuple[str, ...]:
    """Next stems of leftover extra remaining-after-087 sites."""
    locked_set = set(locked)
    return tuple(
        nxt
...
def leftover_extra_remaining_after_087_with_011(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-087 sites whose next token is 011."""
    remaining8 = set(leftover_extra_remaining_after_087(sites, next_stems))
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
        if nxt == stem and site in remaining8
    )


def leftover_extra_remaining_after_087_without_011(
    sites: tuple[tuple[str, str, int], ...],
    next_stems: tuple[str | None, ...],
    stem: str = STANDING_G,
    locked: tuple[str, ...] = LOCKED_FORWARD_STEMS_AFTER_087,
) -> tuple[tuple[str, str, int], ...]:
    """Leftover extra remaining-after-087 sites whose next token is not 011."""
    locked_set = set(locked)
    return tuple(
        site
        for site, nxt in zip(sites, next_stems, strict=True)
...
def leftover_extra_remaining_after_087_nested_counts_hold(
    n_leftover: int,
    n_with_next: int,
    n_no_next: int,
    n_share_070: int,
    ...
    n_share_087: int,
    n_remaining8: int,
    expected_share_087: int = STANDING_N_SHARE_087,
    expected_remaining8: int = STANDING_N_REMAINING8,
) -> bool:
    """Nested leftover extra chain through remaining-after-087 N_remaining8==25."""
    return leftover_extra_remaining_after_280_nested_counts_hold(
        n_leftover,
        n_with_next,
        n_no_next,
        n_share_070,
        ...
        n_share_280,
        n_remaining7,
    ) and n_share_087 == expected_share_087 and n_remaining8 == expected_remaining8 and (
        n_remaining8 == n_remaining7 - n_share_087
    )


def matching_equals_cycle234_011_sites(
    matching_sites: tuple[tuple[str, str, int], ...],
    cycle234_sites: tuple[tuple[str, str, int], ...] = STANDING_CYCLE234_011_SITES,
) -> bool:
    """True iff remaining-after-087 090 076 011 sites equal cycle 234's 011 pair."""
    return matching_sites == cycle234_sites


def matching_leftover_extra_remaining_after_087_fwd011_local_4gram_rows(
    leftover_sites: tuple[tuple[str, str, int], ...] = STANDING_MATCHING_SITES,
    next_4grams: tuple[tuple[str, ...], ...] = STANDING_MATCHING_NEXT_4GRAMS,
) -> list[dict]:
    """Survey-shaped matching leftover extra remaining-after-087 011 next-4-gram rows."""
    rows = []
...
def i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(
    k: int,
...
class TestILeftoverExtra090076RemainingAfter087Fwd011Helpers(unittest.TestCase):
    """Helpers on leftover extra remaining-after-087 forward 011. No CV, no LLM."""

    def test_forward_011_requires_remaining_after_087_next_stem(self):
        """Next stem 011 is remaining-after-087; locked 070/071/013/001/700/530/280/087 are not."""
        ...
        self.assertEqual(site_forward_3gram(has_011, 1, GRAM2), GRAM3_FORWARD)
        self.assertEqual(
            site_next_4gram(has_011, 1, GRAM2),
            ("090", "076", "011", "678"),
        )
        has_087 = ["999", "090", "076", "087", "499"]
        self.assertEqual(site_next_stem(has_087, 1, GRAM2), "087")
        self.assertNotEqual(site_next_stem(has_087, 1, GRAM2), "011")
        self.assertEqual(site_forward_3gram(has_087, 1, GRAM2), GRAM3_NESTED_087)
        has_280 = ["999", "090", "076", "280", "139"]
        self.assertEqual(site_next_stem(has_280, 1, GRAM2), "280")
        self.assertNotEqual(site_next_stem(has_280, 1, GRAM2), "011")
        has_530 = ["999", "090", "076", "530", "090"]
        self.assertEqual(site_next_stem(has_530, 1, GRAM2), "530")
        self.assertNotEqual(site_next_stem(has_530, 1, GRAM2), "011")
        has_700 = ["999", "090", "076", "700", "011"]
        self.assertEqual(site_next_stem(has_700, 1, GRAM2), "700")
        self.assertNotEqual(site_next_stem(has_700, 1, GRAM2), "011")
        ...
        self.assertNotEqual(site_next_stem(has_001, 1, GRAM2), "011")
        has_013 = ["999", "090", "076", "013", "073"]
        self.assertEqual(site_next_stem(has_013, 1, GRAM2), "013")
        self.assertNotEqual(site_next_stem(has_013, 1, GRAM2), "011")
        has_071 = ["999", "090", "076", "071", "633"]
        self.assertEqual(site_next_stem(has_071, 1, GRAM2), "071")
        self.assertNotEqual(site_next_stem(has_071, 1, GRAM2), "011")
        has_070 = ["999", "090", "076", "070", "499"]
        self.assertEqual(site_next_stem(has_070, 1, GRAM2), "070")
        self.assertNotEqual(site_next_stem(has_070, 1, GRAM2), "011")
        end_of_line = ["011", "678", "090", "076"]
        ...
        planted_stems = (
            "011", "087", "280", "530", "700", "070", "071", "013", "001", None, "005",
        )
        rem8 = leftover_extra_remaining_after_087(planted_sites, planted_stems)
        self.assertEqual(rem8, (planted_sites[0], planted_sites[10]))
        self.assertEqual(
            leftover_extra_remaining_after_087_with_011(planted_sites, planted_stems),
            (planted_sites[0],),
        )
        self.assertEqual(
            leftover_extra_remaining_after_087_without_011(planted_sites, planted_stems),
            (planted_sites[10],),
        )
        rem7 = leftover_extra_remaining_after_280(planted_sites, planted_stems)
        self.assertEqual(rem7, (planted_sites[0], planted_sites[1], planted_sites[10]))
        self.assertEqual(
            leftover_extra_remaining_after_280_with_087(planted_sites, planted_stems),
            (planted_sites[1],),
        )
        self.assertNotIn(planted_sites[1], rem8)
        self.assertNotIn(planted_sites[2], rem8)
        self.assertNotIn(planted_sites[3], rem8)
        self.assertNotIn(planted_sites[4], rem8)
        self.assertNotIn(planted_sites[5], rem8)
        self.assertNotIn(planted_sites[6], rem8)
        self.assertNotIn(planted_sites[7], rem8)
        self.assertNotIn(planted_sites[8], rem8)
        ...
        self.assertTrue(STANDING_090_076_700_011_IS_NOT_REMAINING_AFTER_087_011)
        self.assertEqual(provider.get_call_history(), [])

    def test_exactly_2_can_fail(self):
        """Boolean is True only when K=2."""
        provider = MockProvider()
        self.assertTrue(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(2))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(0))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(1))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(3))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(6))
        self.assertFalse(i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(29))
        planted = STANDING_MATCHING_SITES + ((SIDE_IA, "Ia1", 2),)
        planted_stems = ("011",) * 3
        self.assertEqual(
            leftover_extra_remaining_after_087_with_011(planted, planted_stems),
            planted,
        )
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(len(planted))
        )
        self.assertEqual(
            STANDING_CLAIM,
            "i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011",
        ...
        self.assertTrue(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_087_EXACTLY_2_SHARE_011
        )
        self.assertEqual(
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_087_EXACTLY_2_SHARE_011,
            HYPOTHESIS_K == STANDING_K,
        )
        self.assertEqual(STANDING_N_SHARE_087 + STANDING_N_REMAINING8, STANDING_N_REMAINING7)
        self.assertEqual(2 + 25, 27)
        self.assertEqual(STANDING_K + STANDING_N_WITHOUT, STANDING_N_REMAINING8)
        self.assertEqual(2 + 23, 25)
        self.assertEqual(provider.get_call_history(), [])

    def test_cycle234_011_site_agreement_and_tie_minus_700_530_280_087_can_fail(self):
        """Matching sites must equal cycle 234's Ia2[37]/Ia12[47]; unique-max stays unlocked."""
        provider = MockProvider()
        self.assertTrue(matching_equals_cycle234_011_sites(STANDING_MATCHING_SITES))
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_011_SITES)
        self.assertEqual(STANDING_MATCHING_SITES, STANDING_CYCLE234_011_SITES)
        planted = STANDING_MATCHING_SITES[:-1]
        self.assertFalse(matching_equals_cycle234_011_sites(planted))
        self.assertFalse(
            i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(len(planted))
        )
        swapped = ((SIDE_IA, "Ia3", 87), (SIDE_IA, "Ia4", 162))
        self.assertFalse(matching_equals_cycle234_011_sites(swapped))
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(len(swapped))
        )
        self.assertEqual(swapped, CYCLE244_MATCHING_SITES)
        ...
        self.assertTrue(STANDING_UNIQUE_MAX_REMAINING_AFTER_087_IS_NOT_THIS_CYCLE)
        ...
        self.assertEqual(
            tuple(
                ...
                if stem not in ("700", "530", "280", "087")
            ),
            STANDING_TIED_STEMS_MINUS_700_530_280_087,
        )
        self.assertEqual(
            len(STANDING_TIED_STEMS_MINUS_700_530_280_087),
            STANDING_N_TIED_MINUS_700_530_280_087,
        )
        self.assertEqual(STANDING_N_TIED_MINUS_700_530_280_087, 3)
        self.assertTrue(STANDING_OTHER_TIED_STEMS_NOT_LOCKED)
        self.assertTrue(STANDING_I_ONLY_OF_090_076_011_IS_NOT_THIS_CYCLE)
        self.assertTrue(STANDING_PREVIOUS_4GRAMS_OF_090_076_087_ARE_NOT_THIS_CYCLE)
        ...
        self.assertTrue(STANDING_SAME_CLAIM_SHAPE_AS_CYCLE244)
...
class TestMamariILeftoverExtra090076RemainingAfter087Fwd011Scoreboard(
    unittest.TestCase
):
    """Cited-fixture leftover extra remaining-after-087 forward-011 lock. Mock only."""
    ...
    def setUp(self):
        ...
        self.matching = leftover_extra_remaining_after_087_with_011(
            self.leftover_sites,
            self.next_stems,
        )
        self.without = leftover_extra_remaining_after_087_without_011(
            self.leftover_sites,
            self.next_stems,
        )
        ...
        self.tied_minus_700_530_280_087 = tuple(
            stem for stem in self.tied if stem not in ("700", "530", "280", "087")
        )
        self.equals_cycle234_011 = matching_equals_cycle234_011_sites(self.matching)
        self.claim_holds = i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(
            self.k,
        )
    ...
    def test_tokens_and_nested_leftover_extra_through_530_not_retuned(self):
        ...
        self.assertEqual(prior_244["N_without"], 25)
        ...
        self.assertEqual(CYCLE244_N_REMAINING7, 27)
        self.assertEqual(CYCLE244_N_WITHOUT, 25)
    ...
    def test_counts_2_of_29_and_hypothesis_k_2_holds(self):
        ...
        self.assertEqual(self.remaining7, CYCLE244_REMAINING7_SITES)
        ...
        self.assertEqual(STANDING_N_SHARE_087, 2)
        self.assertEqual(self.share_087, CYCLE244_MATCHING_SITES)
        self.assertEqual(self.share_087, CYCLE245_I_SITES[:1] + CYCLE245_I_SITES[2:3])
        if self.share_087 != CYCLE244_MATCHING_SITES:
            self.fail("leftover extra remaining-after-280 2 share 087 drifted from cycle-244 pair")
        self.assertEqual(self.n_remaining8, STANDING_N_REMAINING8)
        self.assertEqual(STANDING_N_REMAINING8, 25)
        self.assertEqual(self.n_remaining8, CYCLE244_N_WITHOUT)
        self.assertEqual(self.n_remaining8, self.n_remaining7 - self.n_share_087)
        self.assertEqual(27 - 2, 25)
        if self.n_remaining8 != 25:
            self.fail("measured N_remaining8 drifted from 25")
        if self.n_remaining8 != self.n_remaining7 - self.n_share_087:
            self.fail("leftover extra remaining-after-087 filter disagrees with nested 27−2")
        self.assertEqual(self.remaining8, STANDING_REMAINING8_SITES)
        self.assertEqual(len(self.remaining8), len(self.remaining8_stems))
        self.assertEqual(
            self.remaining8,
            leftover_extra_remaining_after_280_without_087(
                self.leftover_sites,
        ...
        self.assertNotIn(STANDING_NO_NEXT_SITES[0], self.remaining8)
        self.assertIn(STANDING_IA2_174, self.remaining8)
        self.assertEqual(STANDING_IA2_174_NEXT_STEM, "000")
        self.assertNotIn(STANDING_IA2_159, self.remaining8)
        self.assertEqual(STANDING_IA2_159_NEXT_STEM, "700")
        self.assertTrue(STANDING_090_076_700_011_IS_NOT_REMAINING_AFTER_087_011)
        for site in self.share_070:
            self.assertNotIn(site, self.remaining8)
        ...
        self.assertEqual(
            self.tied_minus_700_530_280_087,
            STANDING_TIED_STEMS_MINUS_700_530_280_087,
        )
        self.assertEqual(
            len(self.tied_minus_700_530_280_087),
            STANDING_N_TIED_MINUS_700_530_280_087,
        )
        self.assertEqual(STANDING_N_TIED_MINUS_700_530_280_087, 3)
        if self.tied_minus_700_530_280_087 != STANDING_TIED_STEMS_MINUS_700_530_280_087:
            self.fail("nested cycle 234 7-way-tie-minus-700-530-280-087 drifted")
        ...
        self.assertEqual(STANDING_N_WITHOUT, 23)
        self.assertEqual(self.k + self.n_without, self.n_remaining8)
        self.assertEqual(2 + 23, 25)
        self.assertTrue(
            i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011(self.k)
        )
        self.assertEqual(
            self.claim_holds,
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_087_EXACTLY_2_SHARE_011,
        ...
        self.assertTrue(self.equals_cycle234_011)
        self.assertTrue(STANDING_MATCHING_EQUALS_CYCLE234_011_SITES)
        self.assertTrue(matching_equals_cycle234_011_sites(self.matching))
        if len(self.matching) != 2 or not self.equals_cycle234_011:
            self.fail("leftover extra remaining-after-087 011 set drifted from cycle-234 pair")
    ...
    def test_matching_leftover_extra_remaining_after_087_sites_share_011(self):
        """Two leftover extra remaining-after-087 sites are 090 076 011."""
        self.assertEqual(self.matching, STANDING_MATCHING_SITES)
        self.assertEqual(self.matching_next_4grams, STANDING_MATCHING_NEXT_4GRAMS)
        expected = (
            ((SIDE_IA, "Ia2", 37), ("090", "076", "011", "678")),
            ((SIDE_IA, "Ia12", 47), ("090", "076", "011", "130")),
        )
        ...
        self.assertEqual(self.matching, STANDING_CYCLE234_011_SITES)
        self.assertTrue(matching_equals_cycle234_011_sites(self.matching))
        ...
        other_tied = tuple(
            stem
            for stem in STANDING_TIED_STEMS_MINUS_700_530_280_087
            if stem != "011"
        )
        self.assertEqual(other_tied, STANDING_OTHER_TIED_STEMS)
        self.assertEqual(other_tied, ("005", "000"))
        for stem in other_tied:
            ...
            for site in other:
                self.assertNotIn(site, self.matching)
                self.assertIn(site, self.remaining8)
    ...
    def test_existing_246_245_244_243_242_241_240_239_238_237_236_234_233_232_223_195_and_171_still_compute(self):
        """Cycle 246 5/0 hapax, 245 5/0 extra I=3, 244 K=2/G=087, 243 2/0 hapax, 242 2/0, 241 K=2/G=280, 240 2/0 hapax, 239 2/0, 238 K=2/G=530, 237 2/0 hapax, 236 2/0, 234 7-way tie, 233 3/0, 232 3/0, 223 69/3, 195 6/0, 171 43/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-247 leftover extra remaining-after-087 fwd011 lock."""
        lock = self.survey["i_leftover_extra_090_076_remaining_after_087_fwd011"]
        self.assertEqual(lock["cycle"], 247)
        ...
        self.assertEqual(lock["N_share_087"], STANDING_N_SHARE_087)
        self.assertEqual(lock["N_share_087"], 2)
        self.assertEqual(
            tuple(tuple(row) for row in lock["share_087_sites"]),
            CYCLE244_MATCHING_SITES,
        )
        self.assertEqual(lock["N_remaining8"], STANDING_N_REMAINING8)
        ...
        self.assertEqual(
            tuple(lock["tied_stems_minus_700_530_280_087"]),
            STANDING_TIED_STEMS_MINUS_700_530_280_087,
        )
        self.assertEqual(
            lock["N_tied_minus_700_530_280_087"],
            STANDING_N_TIED_MINUS_700_530_280_087,
        )
        self.assertEqual(lock["N_tied_minus_700_530_280_087"], 3)
        self.assertEqual(lock["N_without"], STANDING_N_WITHOUT)
        self.assertEqual(lock["N_without"], 23)
        self.assertEqual(
            tuple(tuple(row) for row in lock["matching_leftover_extra_remaining_after_087_sites"]),
            STANDING_MATCHING_SITES,
        )
        self.assertTrue(lock["matching_equals_cycle234_011_sites"])
        self.assertEqual(
            lock["matching_equals_cycle234_011_sites"],
            STANDING_MATCHING_EQUALS_CYCLE234_011_SITES,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lock["matching_leftover_extra_remaining_after_087_local_4grams"],
            matching_leftover_extra_remaining_after_087_fwd011_local_4gram_rows(),
        )
        self.assertEqual(lock["cycle246_N_i_only"], CYCLE246_N_I_ONLY)
        self.assertEqual(lock["cycle246_N_i_only"], 5)
        self.assertEqual(lock["cycle246_N_not_i_only"], CYCLE246_N_NOT_I_ONLY)
        self.assertEqual(lock["cycle246_N_not_i_only"], 0)
        self.assertEqual(lock["cycle245_N_I"], CYCLE245_N_I)
        self.assertEqual(lock["cycle245_N_I"], 5)
        self.assertEqual(lock["cycle245_N_off_I"], CYCLE245_N_OFF_I)
        self.assertEqual(lock["cycle245_N_off_I"], 0)
        self.assertEqual(lock["cycle245_N_extra"], CYCLE245_N_EXTRA)
        self.assertEqual(lock["cycle245_N_extra"], 3)
        self.assertEqual(lock["cycle244_G"], CYCLE244_G)
        self.assertEqual(lock["cycle244_G"], "087")
        self.assertEqual(lock["cycle244_K"], CYCLE244_K)
        self.assertEqual(lock["cycle244_K"], 2)
        self.assertEqual(lock["cycle244_N_remaining7"], CYCLE244_N_REMAINING7)
        ...
        self.assertTrue(lock["i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011"])
        self.assertEqual(
            lock["i_leftover_extra_090_076_remaining_after_087_exactly_2_share_011"],
            STANDING_I_LEFTOVER_EXTRA_090_076_REMAINING_AFTER_087_EXACTLY_2_SHARE_011,
        )
        ...
        self.assertTrue(lock["previous_4grams_of_090_076_087_are_not_this_cycle"])
        self.assertTrue(lock["unique_max_remaining_after_087_is_not_this_cycle"])
        ...
        self.assertTrue(lock["leftover_n4_021_090_076_087_not_retuned"])
        self.assertTrue(lock["090_076_700_011_is_not_remaining_after_087_011"])
        ...
        self.assertEqual(
            self.survey["i_leftover_extra_090_076_remaining_after_280_fwd087"]["G"],
            "087",
        )
        self.assertEqual(
            self.survey["i_leftover_extra_090_076_remaining_after_280_fwd087"]["K"],
            2,
        )
...
class TestMamariILeftoverExtra090076RemainingAfter087Fwd011ImageSnapshot(
    unittest.TestCase
):
    """Cycle 247 does not touch clustering. 83/62 / Hamming 6 stays."""
