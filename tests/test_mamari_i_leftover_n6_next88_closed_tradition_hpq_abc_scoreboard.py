"""Reframed closed-tradition hold-out of the locked leftover-6 next-88 grams.

Cycle 921 text-search lock. Uses already-vendored A–V and the
cycle-639 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 88-grams (12 sequences; parent N=18; six
holes at Ia10[144], Ia10[143], Ia10[142], Ia8[167], Ia8[166],
and Ia8[165]). HEAD-check: that gram list exists and is not yet
locked under this reframed rule. Cycle 639 already locked the
I-only claim on these 88-grams as incomplete-set LOSE
(N_with_next88=12, N_no_next88=6, N_i_only=12, N_leak=0). Cycle
920 already locked the reframed claim on the cycle-637 next-87
grams as incomplete-set LOSE. This cycle does not re-lock either
claim, and does not re-lock Cycles 832–920.

Claim that can lose: each existing next-88 gram is an exact
...
Named claim: cycle639_next88_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 12 with-next88 grams: N=12, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=12, N_i_ge1=12, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=12. Parent N=18, N_with_next88=12,
N_no_next88=6 (Ia10[144], Ia10[143], Ia10[142], Ia8[167],
Ia8[166], Ia8[165]). Every scored gram is exact-0 on H/P/Q and
on A/B/C and an exact contiguous hit once on I. The next-87 hapax
604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700 076 076 053 177 700 076 057 741 430 076 532 200 059 076 074 379 002 076 244 280 001 076 532 071 065 071 999 700 076 480 600 005 076 420 519 999 006 076 073 430 076 006 006 040 999 002 076 240 382 999 076 053 472 021 386 076 001 037 150 076 001 526 383 at Ia9[9] extends
to 604 076 071 600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999 090 076 057 600 700 076 076 053 177 700 076 057 741 430 076 532 200 059 076 074 379 002 076 244 280 001 076 532 071 065 071 999 700 076 480 600 005 076 420 519 999 006 076 073 430 076 006 006 040 999 002 076 240 382 999 076 053 472 021 386 076 001 037 150 076 001 526 383 076 at Ia9[9]
and hits I once. All twelve next-87 grams extend; no new hole.
...
next 88-grams as a peel, next 89-grams as a peel, prev-184, or
next-185. The cycle-641 next-89 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_next87_closed_tradition_hpq_abc_scoreboard import (
    ...
    STANDING_NO_NEXT87_SITES as CYCLE920_NO_NEXT87_SITES,
    STANDING_RESULT as CYCLE920_RESULT,
    STANDING_SEQUENCES as CYCLE920_SEQUENCES,
    STANDING_SITES as CYCLE920_SITES,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next88_i_only_scoreboard import (
    ...
    STANDING_LEFTOVER_MATCHING_NEXT88_SITES as CYCLE639_SITES,
    STANDING_N as CYCLE639_N,
    STANDING_N_I_EACH as CYCLE639_N_I_EACH,
...
STANDING_NO_NEXT88_SITES = CYCLE639_NO_NEXT88_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE639_SITES
STANDING_SEQUENCES = CYCLE639_SEQUENCES
...
STANDING_NEXT87_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT87_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next88_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next88_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 639
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 639
...
class TestMamariILeftoverN6Next88ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 921 reframed closed-tradition lock on the cycle-639 next-88 population."""
    ...
    def test_population_is_the_locked_cycle_639_next88_grams(self):
        """Twelve next-88 sequences and sites come from cycle 639. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE639_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE639_SITES)
        self.assertEqual(CYCLE639_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE639_N, 18)
        self.assertEqual(CYCLE639_N_SEQUENCES, 12)
        self.assertEqual(STANDING_N_WITH_NEXT88, CYCLE639_N_WITH_NEXT88)
        self.assertEqual(STANDING_N_NO_NEXT88, CYCLE639_N_NO_NEXT88)
        self.assertEqual(
            STANDING_NO_NEXT88_SITES,
            (
                ("Ia", "Ia10", 144),
                ("Ia", "Ia10", 143),
                ("Ia", "Ia10", 142),
        ...
        self.assertEqual(STANDING_NO_NEXT88_SITES, CYCLE920_NO_NEXT87_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 639)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 88)
    ...
    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 639 I-only or the cycle 832–920 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 639)
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE639_N_I_EACH)
        self.assertEqual(CYCLE639_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT87_ALL_EXTEND)
        for n87, n88, site87, site88 in zip(
            CYCLE920_SEQUENCES, STANDING_SEQUENCES, CYCLE920_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n88[:87], n87)
            self.assertEqual(site88, site87)
        still = self.rows[STANDING_NEXT87_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
        ...
        self.assertEqual(still.tokens[:87], CYCLE920_SEQUENCES[3])
        self.assertEqual(CYCLE920_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia10", 144), STANDING_NO_NEXT88_SITES)
        self.assertIn(("Ia", "Ia10", 143), STANDING_NO_NEXT88_SITES)
        self.assertIn(("Ia", "Ia10", 142), STANDING_NO_NEXT88_SITES)
        self.assertIn(("Ia", "Ia8", 167), STANDING_NO_NEXT88_SITES)
        self.assertIn(("Ia", "Ia8", 166), STANDING_NO_NEXT88_SITES)
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT88_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_641_next89_list(self):
        """Cycle 641 next-89 I-only list stays locked. The claim name is not a survey key."""
        self.assertEqual(CYCLE641_N, 18)
        self.assertEqual(CYCLE641_N_SEQUENCES, 12)
        self.assertEqual(len(CYCLE641_SEQUENCES), 12)
        self.assertTrue(all(len(tokens) == 89 for tokens in CYCLE641_SEQUENCES))
        ...
        self.assertNotIn("i_leftover_n6_next89_closed_tradition_hpq_abc", self.survey)
        self.assertEqual(
            STANDING_NEXT_CHEAP_LOCK,
            "cycle641_next89_reframed_closed_tradition_hpq_ge1_exact0_abc",
        )
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-921 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 921)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next88_sites"]),
            STANDING_NO_NEXT88_SITES,
        )
        ...
        self.assertEqual(
            tuple(lock["next87_hapax_still_hapax"]),
            STANDING_NEXT87_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next87_all_extend"])
        self.assertEqual(lock["next87_all_extend"], STANDING_NEXT87_ALL_EXTEND)
