"""I's leftover n=4 remaining remaining-after-090-076 remaining-after-430-076 remaining-after-076-020 vs next 2-gram.

Cycle 335 text-search lock. Uses already-vendored A–V and the
...
020 peels stay nested. This cycle's N=7 population is leftover
n=4 remaining remaining-after-090-076 remaining-after-430-076
remaining-after-076-020: the 7 leftover 4-grams that do not
contain 090 076, do not contain contiguous 430 076, and do not
contain contiguous 076 020.

Already locked (do not re-lock): cycle 334, 333, 332, 331,
330, 329, 328, 222, leftover n=4 remaining I 090 076 peels
(288–327), leftover extra 090 076 peels, cycles 220–221, or
cycle 223. Do not claim leftover 4-gram I-only or extra-I
4-gram I-only of 076 020 this cycle.

This cycle is leftover-inventory, not I-only of G. For each
remaining-after-076-020 leftover 4-gram, list its three
contiguous 2-grams. Count how many of the 7 contain each
2-gram (contain = the 2-gram is a substring of that leftover
4-gram). G = the 2-gram with the highest remaining-after-
076-020 leftover count. If a tie, pick the one whose first
...
332 leftover n=4 remaining remaining-after-090-076 remaining-
after-430-076 unique most frequent 2-gram, which LOSE 4-way
tie). Also analog cycle 328 leftover n=4 remaining remaining-
after-090-076 unique most frequent 2-gram (LOSE 5-way tie).
HOLD iff unique_max is true (one 2-gram strictly outcounts
every other among those 7). LOSE on a tie (unique_max false),
including a hapax pile. G is largest-id labeling only on a
tie; do not treat labeled G as unique-max. Empty remainder
does not lose HOLD.

Measured: N=7, N_distinct=19, 2-way tie at K=2 (076 010 /
010 079), G=076 010 by largest-id tie-break, unique-max false.
Matching leftovers of the labeled G: 076 010 079 090,
072 076 010 079. N_remaining after peeling G = 5 (7−2). I vs
off-I of G is not this cycle. Nested cycle 334 2-gram 076 020
I-only 12/0 extra I=9, cycle 333 exactly-2-share 076 020,
cycle 332 unique_max false, cycle 331 leftover 4-grams all
I-only 2/0 extra I=0, cycle 330 2-gram 430 076 I-only 30/16
extra I=26, cycle 329 exactly-2-share 430 076, cycle 328
unique_max false, cycle 222 leftover remaining K=5 / G=090 076
/ N_without=11, leftover n=4 remaining I 090 076 peel 288–327,
leftover extra 090 076 peels, cycle 221 remaining 5-grams 4/4
I-only, cycle 220 leftover 000 5-gram 1/0, cycle 223 090 076
I-only 69/3, cycle 208 leftover 4-gram 5/0, and cycle 171
076 071 43/0 stay. Claim that can lose:
...
076 020, N=7 remaining-after-076-020, and G is uniquely the
...
LOCKED_G_076_020 = CYCLE333_GRAM2
...
STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_UNIQUE_MAX_2GRAM = (
    False
)
STANDING_RESULT = (
...
def leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
    ...
    """Leftover n=4 remaining 4-grams with neither 090 076 nor 430 076 nor 076 020."""
    remaining_after_430 = leftover_remaining_after_090_076_remaining_after_430_076(
        leftovers, peeled_090_076, peeled_430_076
    )
    return leftover_remaining_without_g(remaining_after_430, peeled_076_020)


def leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_with_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-076-020 leftovers that contain the labeled G."""
    return leftover_remaining_with_g(remaining_after, needle)


def leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_without_g(
    remaining_after: tuple[tuple[tuple[str, ...], int, int, tuple], ...],
    needle: tuple[str, ...] = STANDING_G,
) -> tuple[tuple[tuple[str, ...], int, int, tuple], ...]:
    """Remaining-after-076-020 leftovers that do not contain the labeled G."""
    return leftover_remaining_without_g(remaining_after, needle)


def i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_unique_max_2gram(
    ...
    """True iff remaining-after-076-020 has a unique most frequent 2-gram."""
...
class TestILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Next2gramHelpers(
    unittest.TestCase
):
    """Helpers on remaining-after-076-020 vs next 2-gram. No CV, no LLM."""

    def test_remaining_after_filter_and_unique_max_can_fail(self):
        """The 7 remaining-after-076-020 tie at 2; unique-max loses."""
        ...
        self.assertEqual(len(remaining_grandparent), CYCLE328_N)
        ...
        self.assertEqual(len(remaining_parent), CYCLE332_N)
        ...
        self.assertEqual(
            ...
            STANDING_N_REMAINING_GREATGRANDPARENT,
        )
        unique_after = leftover_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020(
            unique_leftover
        )
    ...
    def test_tie_break_is_labeling_only_when_unique_max_false(self):
        ...
        self.assertEqual(ranked[0][0], ("076", "010"))
        self.assertEqual(ranked[0][1], 2)
        self.assertEqual(barthel_id("076"), 76)
        self.assertGreater(barthel_id("076"), barthel_id("010"))
        for pair in STANDING_TIED_2GRAMS:
            self.assertEqual(counts[pair], STANDING_K)
        self.assertEqual(len(counts), STANDING_N_DISTINCT)
        self.assertEqual(
            STANDING_RANKED_2GRAMS[STANDING_N_TIED_AT_K][1], STANDING_N_2GRAM_NEXT
        )
        gram, count, unique = select_g(remaining_after)
        self.assertEqual(gram, STANDING_G)
        self.assertEqual(count, STANDING_K)
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Next2gramScoreboard(
    unittest.TestCase
):
    """Cited-fixture remaining-after-076-020 next 2-gram. Mock only."""
    ...
    def test_nested_leftover_n4_family_and_cycles_222_328_329_332_333(self):
        """Nested leftover n=4 27/7/4/1, 222 16/5/11, 328/329 11/2/9, 332/333 9/2/7 stay."""
    ...
    def test_remaining_after_7_unique_max_loses(self):
        """7 remaining-after-076-020; unique-max is false. Claim is false."""
        self.assertEqual(len(self.remaining), STANDING_N)
    ...
    def test_existing_334_333_332_331_330_329_328_223_222_and_220_scoreboards_still_compute(self):
        """Cycle 334 12/0, 333 K=2, 332 unique-max lose, 331 2/0, 330 30/16 stay."""
        leftover = leftover_n4_rows()
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-335 remaining-after-076-020 lock."""
        lock = self.survey[
            "i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_next_2gram"
        ]
        self.assertEqual(lock["cycle"], 335)
        ...
        self.assertFalse(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_unique_max_2gram"]
        )
        self.assertEqual(
            lock["i_leftover_n4_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_unique_max_2gram"],
            STANDING_I_LEFTOVER_N4_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_UNIQUE_MAX_2GRAM,
...
class TestMamariILeftoverN4RemainingAfter090076RemainingAfter430076RemainingAfter076020Next2gramImageSnapshot(
    unittest.TestCase
):
    """Cycle 335 does not touch clustering. 83/62 / Hamming 6 stays."""
