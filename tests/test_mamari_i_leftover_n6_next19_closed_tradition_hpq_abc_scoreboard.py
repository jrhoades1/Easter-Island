"""Reframed closed-tradition hold-out of the locked leftover-6 next-19 grams.

Cycle 852 text-search lock. Uses already-vendored A–V and the
cycle-501 leftover n=6 remaining remaining-after-090-076
remaining-after-430-076 remaining-after-076-020 remaining-
after-076-010 next 19-grams (15 sequences; parent N=18; three
line-final holes at Ia8[167], Ia8[166], and Ia8[165]).
HEAD-check: that gram list exists and is not yet locked under
this reframed rule. Cycle 501 already locked the I-only claim
on these 19-grams as incomplete-set LOSE (N_with_next19=15,
N_no_next19=3, N_i_only=15, N_leak=0). Cycle 851 already locked
the reframed claim on the cycle-499 next-18 grams as
incomplete-set LOSE. This cycle does not re-lock either claim,
and does not re-lock Cycles 832–851.

Claim that can lose: each existing next-19 gram is an exact
...
Named claim: cycle501_next19_reframed_closed_tradition_hpq_ge1_exact0_abc.

Measured on the 15 with-next19 grams: N=15, N_hpq_ge1=0,
N_hpq_ge2=0, N_exact0_abc=15, N_i_ge1=15, N_exact0_i=0,
N_leak=0, N_hold=0, hits_I=15. Parent N=18, N_with_next19=15,
N_no_next19=3 (Ia8[167], Ia8[166], Ia8[165]). Every scored gram
is exact-0 on H/P/Q and on A/B/C and an exact contiguous hit
once on I. The next-18 hapax 604 076 071 600 999 050 076 000
002 999 076 092 535 999 208 076 532 244 extends to 604 076 071
600 999 050 076 000 002 999 076 092 535 999 208 076 532 244 999
at Ia9[9] and hits I once. All fifteen next-18 grams extend; no
...
next 19-grams as a peel, next 20-grams as a peel, prev-184, or
next-185. The cycle-503 next-20 I-only list stays the named
...
from tests.test_mamari_i_leftover_n6_next18_closed_tradition_hpq_abc_scoreboard import (
    ABSENT_TABLETS,
    CYCLE457_RESULT,
    ...
    PREV1_N6_SCOREBOARD,
    PROBE_TABLETS,
    STANDING_HITS_I as CYCLE851_HITS_I,
    STANDING_N_HOLD as CYCLE851_N_HOLD,
    STANDING_N_LEAK as CYCLE851_N_LEAK,
    STANDING_NAMED_BY_CYCLE850,
    STANDING_NEXT_CHEAP_LOCK as CYCLE851_NEXT_CHEAP_LOCK,
    STANDING_NO_NEXT18_SITES as CYCLE851_NO_NEXT18_SITES,
    STANDING_RESULT as CYCLE851_RESULT,
    STANDING_SEQUENCES as CYCLE851_SEQUENCES,
    STANDING_SITES as CYCLE851_SITES,
    STANDING_VERDICT as CYCLE851_VERDICT,
    absent_hit_sum,
    hpq_tablets_hit,
    reframed_verdict,
    row_holds,
...
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next19_i_only_scoreboard import (
    STANDING_LEFTOVER_MATCHING_NEXT19_SITES as CYCLE501_SITES,
    STANDING_N_I_EACH as CYCLE501_N_I_EACH,
)
from tests.test_mamari_i_leftover_n6_remaining_after_090_076_remaining_after_430_076_remaining_after_076_020_remaining_after_076_010_next20_i_only_scoreboard import (
    HYPOTHESIS_ALL_I_ONLY as CYCLE503_HYPOTHESIS_ALL_I_ONLY,
    STANDING_I_LEFTOVER_N6_REMAINING_AFTER_090_076_REMAINING_AFTER_430_076_REMAINING_AFTER_076_020_REMAINING_AFTER_076_010_NEXT20_I_ONLY as CYCLE503_ALL_I_ONLY_CLAIM,
    ...
    STANDING_N_SEQUENCES as CYCLE503_N_SEQUENCES,
    STANDING_N_WITH_NEXT20 as CYCLE503_N_WITH_NEXT20,
    STANDING_NO_NEXT20_SITES as CYCLE503_NO_NEXT20_SITES,
    STANDING_RESULT as CYCLE503_RESULT,
...
STANDING_NO_NEXT19_SITES = CYCLE501_NO_NEXT19_SITES
STANDING_PARENT_COMPLETE = False
STANDING_HITS_EACH = (0, 0, 0, 0, 0, 0, 1)
STANDING_HITS = (STANDING_HITS_EACH,) * STANDING_N
STANDING_SITES = CYCLE501_SITES
STANDING_SEQUENCES = CYCLE501_SEQUENCES
...
STANDING_NEXT18_HAPAX_STILL_HAPAX = (3,)
STANDING_NEXT18_ALL_EXTEND = True
STANDING_HITS_H = 0
STANDING_HITS_P = 0
STANDING_HITS_Q = 0
STANDING_HITS_A = 0
...
STANDING_CLAIM = "leftover6_next19_reframed_closed_tradition_hpq_ge1_and_exact0_abc"
STANDING_RESULT = "i_leftover_n6_next19_closed_tradition_hpq_abc"
STANDING_AXIS = "closed-tradition-reframed"
STANDING_FROM_CYCLE = 501
STANDING_I_ONLY_ALREADY_LOCKED = True
STANDING_I_ONLY_CYCLE = 501
...
STANDING_DO_NOT_RELOCK_CYCLES_832_851 = True
...
class TestMamariILeftoverN6Next19ClosedTraditionHpqAbcScoreboard(unittest.TestCase):
    """Cycle 852 reframed closed-tradition lock on the cycle-501 next-19 population."""
    ...
    def test_population_is_the_locked_cycle_501_next19_grams(self):
        """Fifteen next-19 sequences and sites come from cycle 501. None invented."""
        self.assertEqual(len(STANDING_SEQUENCES), STANDING_N)
        self.assertEqual(len(STANDING_SITES), STANDING_N)
        self.assertEqual(STANDING_SEQUENCES, CYCLE501_SEQUENCES)
        self.assertEqual(STANDING_SITES, CYCLE501_SITES)
        self.assertEqual(CYCLE501_N, STANDING_N_PARENT)
        self.assertEqual(CYCLE501_N, 18)
        self.assertEqual(CYCLE501_N_SEQUENCES, 15)
        self.assertEqual(STANDING_N_WITH_NEXT19, CYCLE501_N_WITH_NEXT19)
        self.assertEqual(STANDING_N_NO_NEXT19, CYCLE501_N_NO_NEXT19)
        self.assertEqual(
            STANDING_NO_NEXT19_SITES,
            (("Ia", "Ia8", 167), ("Ia", "Ia8", 166), ("Ia", "Ia8", 165)),
        )
        self.assertEqual(STANDING_NO_NEXT19_SITES, CYCLE851_NO_NEXT18_SITES)
        ...
        self.assertEqual(STANDING_FROM_CYCLE, 501)
        ...
        for row, tokens, site in zip(self.rows, STANDING_SEQUENCES, STANDING_SITES, strict=True):
            ...
            self.assertEqual(len(row.tokens), 19)
        ...
        self.assertEqual(CYCLE501_RESULT, (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next19_i_only"
        ))
        ...
        self.assertNotIn(STANDING_NAMED_BY_CYCLE851, self.survey)
        self.assertEqual(self.provider.get_call_history(), [])

    def test_prior_locks_stay_locked(self):
        """Do not re-lock cycle 501 I-only or the cycle 832–851 boards."""
        ...
        self.assertEqual(STANDING_I_ONLY_CYCLE, 501)
        ...
        next18 = self.survey[CYCLE851_RESULT]
        self.assertEqual(next18["cycle"], STANDING_NEXT18_REFRAMED_CYCLE)
        self.assertEqual(next18["verdict"], CYCLE851_VERDICT)
        self.assertEqual(next18["verdict"], "incomplete-set LOSE")
        self.assertEqual(next18["N_hold"], CYCLE851_N_HOLD)
        self.assertEqual(next18["N_hold"], 0)
        self.assertEqual(next18["N_leak"], CYCLE851_N_LEAK)
        self.assertEqual(next18["N_leak"], 0)
        self.assertEqual(next18["hits_I"], CYCLE851_HITS_I)
        ...
        self.assertEqual(next18["next_cheap_lock"], STANDING_NAMED_BY_CYCLE851)
        self.assertEqual(next18["next_cheap_lock"], CYCLE851_NEXT_CHEAP_LOCK)
        self.assertNotIn(STANDING_NAMED_BY_CYCLE851, self.survey)
        ...
        self.assertTrue(STANDING_DO_NOT_RELOCK_CYCLES_832_851)
        self.assertEqual(self.provider.get_call_history(), [])
    ...
    def test_measured_table_is_incomplete_set_lose(self):
        ...
        self.assertEqual(tuple(row.hits[-1] for row in self.rows), CYCLE501_N_I_EACH)
        self.assertEqual(CYCLE501_N_I_EACH, (1,) * STANDING_N)
        ...
        self.assertTrue(STANDING_NEXT18_ALL_EXTEND)
        for n18, n19, site18, site19 in zip(
            CYCLE851_SEQUENCES, STANDING_SEQUENCES, CYCLE851_SITES, STANDING_SITES, strict=True
        ):
            self.assertEqual(n19[:18], n18)
            self.assertEqual(site19, site18)
        still = self.rows[STANDING_NEXT18_HAPAX_STILL_HAPAX[0]]
        self.assertEqual(
            still.tokens,
            (
                "604", "076", "071", "600", "999", "050", "076", "000",
                "002", "999", "076", "092", "535", "999", "208", "076", "532", "244", "999",
            ),
        )
        self.assertEqual(still.site, ("Ia", "Ia9", 9))
        self.assertEqual(still.as_map()["I"], 1)
        self.assertEqual(still.tokens[:18], CYCLE851_SEQUENCES[3])
        self.assertEqual(CYCLE851_SITES[3], ("Ia", "Ia9", 9))
        ...
        self.assertIn(("Ia", "Ia8", 165), STANDING_NO_NEXT19_SITES)
    ...
    def test_next_cheap_lock_is_the_locked_cycle_503_next20_list(self):
        ...
        self.assertTrue(all(len(tokens) == 20 for tokens in CYCLE503_SEQUENCES))
        ...
        prior = self.survey[CYCLE503_RESULT]
        all_i_only_key = (
            "i_leftover_n6_remaining_after_090_076_remaining_after_430_076_"
            "remaining_after_076_020_remaining_after_076_010_next_20grams_all_i_only"
        )
        self.assertEqual(prior["cycle"], 503)
        self.assertEqual(prior["N"], 18)
        self.assertEqual(prior["N_sequences"], 15)
        self.assertEqual(prior["N_6grams"], 18)
        self.assertTrue(prior["hypothesis_all_i_only"])
        self.assertFalse(prior[all_i_only_key])
        self.assertFalse(prior[CYCLE503_RESULT])
        self.assertEqual(prior["N_i_only"], 15)
        self.assertEqual(prior["N_leak"], 0)
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-852 incomplete-set LOSE table."""
        lock = self.survey[STANDING_RESULT]
        self.assertEqual(lock["cycle"], 852)
        ...
        self.assertEqual(
            tuple(tuple(site) for site in lock["no_next19_sites"]),
            STANDING_NO_NEXT19_SITES,
        )
        ...
        self.assertTrue(lock["do_not_relock_cycles_832_851"])
        ...
        self.assertEqual(lock["named_by_cycle851"], STANDING_NAMED_BY_CYCLE851)
        ...
        self.assertEqual(
            tuple(lock["next18_hapax_still_hapax"]),
            STANDING_NEXT18_HAPAX_STILL_HAPAX,
        )
        self.assertTrue(lock["next18_all_extend"])
        self.assertEqual(lock["next18_all_extend"], STANDING_NEXT18_ALL_EXTEND)
