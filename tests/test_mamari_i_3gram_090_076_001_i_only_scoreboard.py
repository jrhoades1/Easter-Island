"""I's cycle-231 leftover extra remaining-after-013 3-gram 090 076 001 off-I lock.

Cycle 232 text-search lock. Uses already-vendored A–V and the
cycle-231 leftover extra remaining-after-013 next stem G=001
(K=3 of N_remaining3=36). Does not retune that leftover extra
remaining-after-013 lock, leftover extra remaining-after-071
013, leftover extra remaining 071, leftover extra forward 070,
leftover extra sites, the leftover n=4 set, or the already-
closed leftover remaining family. Does not vendor a new
...
Same claim-shape as cycle 229 (090 076 013 was I-only 5/0),
cycle 195 (090 076 071 was I-only 6/0), cycle 174 (076 071 076
was I-only 6/0), and cycle 171 (076 071 was I-only 43/0).
Cycle 207 lost: 090 076 070 is not I-only (8/1 on T). Cycle
223 lost: 090 076 is not I-only (69/3 on T). This cycle is
the new leftover extra remaining-after-013 3-gram 090 076 001
only. 090 076 013, 090 076 070, 090 076 071, and 090 076
without 001 do not count as this 3-gram. Do not retune
leftover n=4, 076-cells, or any detector. Do not lock leftover
extra remaining after 001 this cycle. Off-I T sites of 090 076
are not this cycle except as off-I of 090 076 001 if they
match. Do not assume the I-only result.

Locks exact consecutive hits of 090 076 001 on tablet I and
on every other vendored tablet A–H and J–V. Claim that can
lose: i_3gram_090_076_001_i_only (I hits ≥ 1 and off-I hits
== 0). True only if N_off_I == 0. Measured: Ia is exactly 3
at Ia4[134]/Ia6[134]/Ia7[88]; Ib unpublished 0; every other
vendored tablet is exact-0. Those 3 equal the cycle-231
leftover extra remaining-after-013 001 cluster (next 4-grams
090 076 001 048 / 224 / 071). Extra I sites not in leftover
extra remaining-after-013 = 0 (leftover of leftover would be
a later cycle if any appeared). The claim is true. Not an
n≥8 island. Not the cycle-103 I 5-gram. Nested leftover extra
remaining-after-013 K=3 / G=001, cycle 229 5/0, cycle 228
5/013, cycle 227 6/071, cycle 226 8/070, cycle 223 69/3,
cycle 195 6/0, and cycle 171 43/0 stay.
...
from tests.test_mamari_i_leftover_extra_090_076_remaining_after_013_next_stem_scoreboard import (
    ...
    STANDING_N_DISTINCT_REMAINING3 as CYCLE231_N_DISTINCT_REMAINING3,
...
STANDING_IB_SITES = ()
STANDING_LEFTOVER_MATCHING_SITES = CYCLE231_MATCHING_SITES
STANDING_LEFTOVER_MATCHING_COUNT = 3
STANDING_LEFTOVER_MATCHING_NEXT_4GRAMS = CYCLE231_MATCHING_NEXT_4GRAMS
...
STANDING_CLAIM = "i_3gram_090_076_001_i_only"
STANDING_I_3GRAM_090_076_001_I_ONLY = True
STANDING_RESULT = "i_3gram_090_076_001_i_only"
...
def i_3gram_090_076_001_i_only(
    i_hits: int,
    off_i_hits: int,
) -> bool:
...
def leftover_extra_remaining_after_013_001_subset(
    ...
    """True iff leftover extra remaining-after-013 001 sites ⊆ I 090 076 001."""
...
def extra_i_sites(
    ...
    """I 090 076 001 sites that are not leftover extra remaining-after-013 001."""
...
class TestI3gram090076001IOnlyHelpers(unittest.TestCase):
    """Helpers on cycle-231 leftover extra remaining-after-013 3-gram. No CV, no LLM."""

    def test_counts_require_consecutive_tokens(self):
        """Adjacent 3-gram counts; a gap is not a hit. 090 076 013 / 070 / 071 are not."""
        ...
        overlap = [["090", "076", "001", "090", "076", "001"]]
    ...
    def test_i_only_requires_i_ge_1_and_zero_off_i(self):
        ...
        provider = MockProvider()
        self.assertTrue(i_3gram_090_076_001_i_only(1, 0))
        self.assertTrue(i_3gram_090_076_001_i_only(3, 0))
        self.assertFalse(i_3gram_090_076_001_i_only(3, 1))
        self.assertFalse(i_3gram_090_076_001_i_only(1, 1))
        self.assertFalse(i_3gram_090_076_001_i_only(0, 0))
        self.assertFalse(i_3gram_090_076_001_i_only(0, 1))
        self.assertEqual(STANDING_CLAIM, "i_3gram_090_076_001_i_only")
        self.assertTrue(STANDING_I_3GRAM_090_076_001_I_ONLY)
        self.assertTrue(HYPOTHESIS_I_ONLY)
        self.assertEqual(
            STANDING_I_3GRAM_090_076_001_I_ONLY,
            HYPOTHESIS_I_ONLY,
        )
        self.assertEqual(provider.get_call_history(), [])

    def test_leftover_matching_subset_and_extra_can_fail(self):
        """Leftover extra remaining-after-013 001 ⊆ I sites; extra can be nonempty."""
        provider = MockProvider()
        self.assertTrue(
    ...
    def test_3gram_is_cycle231_leftover_not_the_cycle_103_5gram(self):
        """3-gram is the cycle-231 leftover extra remaining-after-013 G, not priors."""
        provider = MockProvider()
        self.assertEqual(GRAM3, ("090", "076", "001"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], CYCLE231_G)
...
class TestMamariI3gram090076001IOnlyScoreboard(unittest.TestCase):
    """Cited-fixture leftover extra remaining-after-013 3-gram 090 076 001 off-I lock."""
    ...
    def setUp(self):
        ...
        self.claim_holds = i_3gram_090_076_001_i_only(
            self.i_hits,
            self.off_i_hits,
        )

    def test_tokens_are_cycle_231_leftover_not_retuned(self):
        """3-gram is the cycle-231 leftover extra remaining-after-013 G, not a new inventory."""
        self.assertEqual(GRAM3, ("090", "076", "001"))
        self.assertEqual(GRAM3, GRAM3_FORWARD)
        self.assertEqual(GRAM3[:2], GRAM2)
        self.assertEqual(GRAM3[2], "001")
        prior_231 = self.survey["i_leftover_extra_090_076_remaining_after_013_next_stem"]
        self.assertEqual(prior_231["cycle"], 231)
        self.assertEqual(tuple(prior_231["forward_3gram"]), GRAM3)
        self.assertEqual(prior_231["G"], "001")
        self.assertEqual(prior_231["K"], 3)
        self.assertEqual(prior_231["N_remaining3"], 36)
        self.assertEqual(prior_231["N_distinct_remaining3"], 27)
        self.assertEqual(CYCLE231_G, "001")
        self.assertEqual(CYCLE231_K, 3)
        self.assertEqual(CYCLE231_N_REMAINING3, 36)
        self.assertEqual(CYCLE231_N_DISTINCT_REMAINING3, 27)
        self.assertTrue(CYCLE231_CLAIM)
        self.assertTrue(prior_231["i_leftover_extra_090_076_remaining_after_013_exactly_K_share_G"])
        measured_matching = [list(site) for site in CYCLE231_MATCHING_SITES]
        self.assertEqual(
            [list(site) for site in prior_231["matching_leftover_extra_remaining_after_013_sites"]],
            measured_matching,
        )
        self.assertEqual(
            [list(gram) for gram in CYCLE231_MATCHING_NEXT_4GRAMS],
    ...
    def test_i_hits_are_three_on_ia_and_equal_leftover_extra_001(self):
        """3-gram is 3 on Ia; Ib 0. Those 3 equal leftover extra remaining-after-013 001."""
        ...
        if self.leftover_matching != self.i_sites:
            self.fail("leftover extra remaining-after-013 001 set drifted from I 090 076 001")
        if self.extra:
        ...
        for (side, line, index), prev4, nxt4 in zip(
            ...
            self.assertIn((side, line, index), CYCLE231_MATCHING_SITES)
        ...
        self.assertEqual(STANDING_I_NEXT_4GRAMS, CYCLE231_MATCHING_NEXT_4GRAMS)
    ...
    def test_3gram_is_zero_off_i_and_i_only(self):
        """3-gram is 0 on A–H and J–V. Ia has exactly 3. T 090 076 is not this 3-gram."""
        ...
        self.assertEqual(
            i_3gram_090_076_001_i_only(self.i_hits, self.off_i_hits),
            STANDING_I_3GRAM_090_076_001_I_ONLY,
        )
        self.assertEqual(
            self.claim_holds,
    ...
    def test_existing_231_229_228_227_226_223_195_and_171_still_compute(self):
        """Cycle 231 K=3/G=001, 229 5/0, 228 5/013, 227 6/071, 226 8/070, 223 69/3, 195 6/0, 171 43/0 stay."""
    ...
    def test_survey_matches_computed_lock(self):
        """CORPUS_SURVEY.json records the cycle-232 3-gram I-only lock."""
        lock = self.survey["i_3gram_090_076_001_i_only"]
        self.assertEqual(lock["cycle"], 232)
        ...
        self.assertEqual(
            tuple(tuple(row) for row in lock["leftover_extra_remaining_after_013_001_sites"]),
            STANDING_LEFTOVER_MATCHING_SITES,
        )
        self.assertEqual(
            lock["leftover_extra_remaining_after_013_001_count"],
            STANDING_LEFTOVER_MATCHING_COUNT,
        )
        self.assertEqual(lock["leftover_extra_remaining_after_013_001_count"], 3)
        self.assertTrue(lock["leftover_extra_remaining_after_013_001_subset_of_i_sites"])
        ...
        self.assertEqual(lock["nested_cycle231_N_distinct_remaining3"], 27)
        self.assertTrue(lock["known_distinct"])
        self.assertEqual(lock["known_distinct"], STANDING_KNOWN_DISTINCT)
        self.assertEqual(lock["claim"], STANDING_CLAIM)
        self.assertTrue(lock["i_3gram_090_076_001_i_only"])
        self.assertEqual(
            lock["i_3gram_090_076_001_i_only"],
            STANDING_I_3GRAM_090_076_001_I_ONLY,
        )
        ...
        self.assertEqual(self.survey["i_leftover_extra_090_076_remaining_after_071_next_stem"]["cycle"], 228)
        self.assertTrue(
...
class TestMamariI3gram090076001IOnlyImageSnapshot(unittest.TestCase):
    """Cycle 232 does not touch clustering. 83/62 / Hamming 6 stays."""
