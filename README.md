...
The stem-consistency scoreboard (`tests/test_mamari_stem_consistency_scoreboard.py`) lists the published Barthel stems under each G00n ID in the mixed hits. It is a positional alignment table, not a type map. Cycle 10 splits split-fragment types whose members fail unsigned Hu < 2.0 or column-ink r ≥ 0.85 (no Barthel in the splitter). Re-run the scoreboard after that split.
